"""
vpin_multi 사이클 203 — c200 엔트리 정밀화 결과 확장

배경:
- c200 avg_OOS=+28.694 (MOM=0.0005 rsiD=7 vpin=0.35 / trades=24).
- 승자가 그리드 모서리(rsiD=7, vpin∈{0.35, 0.40})에 걸려 있음 → 실제 최적은
  그리드 밖일 가능성. 모서리 근처를 세밀화 + 범위를 한 단계 확장해서 재탐색.

탐색 그리드 (3×4×4 = 48 combos):
  MOM_THRESH:    [0.0003, 0.0005, 0.0007]      — 0.0005 근처 finer
  RSI_DELTA_MIN: [6, 7, 8, 9]                  — 7 근처 상방 확장
  VPIN_LOW:      [0.32, 0.35, 0.38, 0.42]      — 0.35~0.40 세밀 + 0.42 탐침

고정: c192 출구 (regime 비활성), SYMBOLS = c200과 동일.
검증: 3-fold WF (c199 helper), slippage stress Top 3.
목표: avg_OOS Sharpe > 28.694 AND total_trades >= 24 (c200 동등 이상).
"""
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 203 — c200 엔트리 정밀화 확장 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  목표: OOS Sharpe > {C200_BASELINE_OOS:.3f} AND trades >= 24")
    print("그리드 확장: rsiD 상방(+8,9) / VPIN 세밀화(0.32~0.42) / MOM finer(0.0003)")
    print("=" * 80)
    ...
    # trades >= 24 제약 우선, 없으면 전체에서 OOS Sharpe 최대
    ...
    print(f"  c200 baseline avg_OOS=+{C200_BASELINE_OOS:.3f} → "
          f"c203 avg_OOS={best['avg_oos_sharpe']:+.3f} "
          f"(Δ {best['avg_oos_sharpe']-C200_BASELINE_OOS:+.3f})")
