"""
vpin_multi 사이클 192 — 시간감쇠 트레일 + ATR 레짐 TP 스케일링
- 기반: c190 OOS Sharpe +29.342, WR 66.7%, trades 26 PASS (최고)
  c191 진입완화 → Sharpe 22.881로 악화 (-6.5) — 진입완화 폐기, c190 유지
- 문제:
  1) trailing stop이 고정 ATR 배수 → 보유 후반부 수익 환원 빈번
  2) TP가 ATR 레짐 무관 → 저변동성에서 TP 미달, 고변동성에서 조기 이탈
- 가설:
  A) 시간감쇠 트레일: 보유 N바 이후 trail 배수 축소 → 수익 보호 강화
     → TRAIL_TIGHTEN_AFTER 바 경과 시 trail_dist *= (1/TRAIL_TIGHTEN_FACTOR)
  B) ATR 레짐 TP 스케일: ATR pctile >= 70이면 TP에 보너스 ATR 추가
     → 고변동성 추세 지속 시 더 큰 수익 확보
  C) A+B 동시 적용 → per-trade avg 개선 + 동일 trades 수 유지
- 탐색 그리드:
  TRAIL_TIGHTEN_AFTER: [6, 10, 14]   — 트레일 강화 시작 바
  TRAIL_TIGHTEN_FACTOR: [1.5, 2.0, 3.0] — 트레일 축소 비율 (trail /= factor)
  ATR_TP_SCALE_HI: [0.5, 1.0, 1.5]   — ATR pctile>=70시 추가 TP ATR 배수
  = 3×3×3 = 27 combos
- 목표: OOS Sharpe >= 30 AND trades >= 24
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- c192 탐색 그리드 --
TRAIL_TIGHTEN_AFTER_LIST = [6, 10, 14]
TRAIL_TIGHTEN_FACTOR_LIST = [1.5, 2.0, 3.0]
ATR_TP_SCALE_HI_LIST = [0.5, 1.0, 1.5]
...
def backtest(
    df: pd.DataFrame,
    trail_tighten_after: int,
    trail_tighten_factor: float,
    atr_tp_scale_hi: float,
    ...
    while i < n - 1:
        ...
        # 진입 조건: c165 최적 (고정) — c191 완화 폐기, c190 베이스
        vpin_ok = (
            vpin_val < VPIN_LOW
...
def build_combos() -> list[dict]:
    combos = []
    for tta, ttf, ats in product(
        TRAIL_TIGHTEN_AFTER_LIST,
        TRAIL_TIGHTEN_FACTOR_LIST,
        ATR_TP_SCALE_HI_LIST,
    ):
        combos.append({
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 192 — 시간감쇠 트레일 + ATR 레짐 TP 스케일링 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 30 AND trades >= 24")
    print("가설 A: 보유 N바 후 trail 축소 → 수익 보호 강화")
    print("가설 B: ATR pctile 고변동성 → TP 확대 → 추세 지속 포착")
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, combo["trail_tighten_after"],
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['trail_tighten_after']:>4} {r['trail_tighten_factor']:>5.1f} "
            f"{r['atr_tp_scale_hi']:>5.1f} | "
    ...
    for r in valid:
        key = (r["trail_tighten_after"], r["trail_tighten_factor"],
               r["atr_tp_scale_hi"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, tta, ttf, ats,
                             btc_c, btc_s, esp, vol_mom, atr_pct)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: ttA={tta} ttF={ttf:.1f} "
                  f"aTPS={ats:.1f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, tta, ttf, ats,
                             btc_c, btc_s, esp, vol_mom, atr_pct,
                             slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    tta = best["trail_tighten_after"]
    ttf = best["trail_tighten_factor"]
    ats = best["atr_tp_scale_hi"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: ttA={tta} "
          f"ttF={ttf:.1f} aTPS={ats:.1f}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c190 최적 (vLB=10 vMin=0.05 tpB=1.0): "
          f"avg_OOS=+29.342 n=26")
    print(f"  c192 최적 (ttA={best['trail_tighten_after']} "
          f"ttF={best['trail_tighten_factor']:.1f} "
          f"aTPS={best['atr_tp_scale_hi']:.1f}): "
          f"avg_OOS={best['avg_oos_sharpe']:+.3f} "
          f"n={best['total_oos_trades']}")
    delta_sh = best["avg_oos_sharpe"] - 29.342
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: TRAIL_TIGHTEN_AFTER={best['trail_tighten_after']} "
          f"TRAIL_TIGHTEN_FACTOR={best['trail_tighten_factor']:.1f} "
          f"ATR_TP_SCALE_HI={best['atr_tp_scale_hi']:.1f}")
    print(f"  (c190 고정: VOL_MOM_LB={VOL_MOM_LB} VOL_MOM_MIN={VOL_MOM_MIN} "
          f"TP_SLOPE_BONUS={TP_SLOPE_BONUS})")
    print(f"  (c186 고정: body={BODY_RATIO_MIN} rsiD={RSI_DELTA_MIN} "
          f"sLB={EMA_SLOPE_LB} sPth={EMA_SLOPE_PCTILE_TH})")
