...
- 기반: c202 OOS Sharpe +32.217 (ADX+OBV 진입 게이트) — ADX_TH=15 최적, OBV 무효
  c202 고정: ADX_TH=15 OBV_EMA_LB=5 OBV_SLOPE_MIN=0.00
  c199 고정: rTh=60 hiTP=1.0 hiTr=2.0 loSL=0.20
...
- 문제:
  1) c202에서 ADX_TH=15 최적 → 임계값 필터 자체는 효과 미미
  2) OBV_SLOPE_MIN=0.00 최적 → OBV 방향 게이트 무효
  3) ADX는 추세 강도만 측정 → 방향(DI+/DI-) 정보 미활용
- 가설:
  A) DI+ > DI- + DI_SPREAD_MIN → 롱 방향 추세 확인
     → 역추세 구간 진입 차단, WR 개선
  B) ADX slope > 0 (lookback 기반) → 추세 강화 중일 때만 진입
     → 추세 약화 구간 진입 차단
  C) ADX_TH + DI방향 + ADX상승 복합 → 삼중 추세 확인
- 탐색 그리드:
  ADX_TH: [10, 15, 20]              — ADX 최소 임계값 (넓은 범위)
  DI_SPREAD_MIN: [0, 3, 6]          — DI+ - DI- 최소 스프레드
  ADX_SLOPE_LB: [3, 5, 8]           — ADX 상승 확인 lookback
  = 3×3×3 = 27 combos
- 목표: OOS Sharpe >= 35 AND trades >= 18
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- c202 최적 고정 (OBV 무효 → 제거) --
ADX_PERIOD = 14

# -- c203 탐색 그리드 --
ADX_TH_LIST = [10, 15, 20]
DI_SPREAD_MIN_LIST = [0, 3, 6]
ADX_SLOPE_LB_LIST = [3, 5, 8]
...
def compute_adx_with_di(
    ...
    for i in range(period + 1, n):
        ...
        minus_di_smooth[i] = (minus_di_smooth[i - 1]
                              - minus_di_smooth[i - 1] / period + minus_dm[i])

    dx = np.full(n, np.nan)

    for i in range(period, n):
        if atr_w[i] > 0:
            plus_di_out[i] = 100.0 * plus_di_smooth[i] / atr_w[i]
            minus_di_out[i] = 100.0 * minus_di_smooth[i] / atr_w[i]
            di_sum = plus_di_out[i] + minus_di_out[i]
            if di_sum > 0:
                dx[i] = 100.0 * abs(plus_di_out[i] - minus_di_out[i]) / di_sum
...
def backtest(
    ...
    adx_arr: np.ndarray,
    plus_di_arr: np.ndarray,
    minus_di_arr: np.ndarray,
    ...
    atr_arr_local = compute_atr(h, lo, c, ATR_PERIOD)
    ...
    while i < n - 1:
        ...
        # === 신규 c203: DI+ > DI- 방향 게이트 ===
        di_plus = plus_di_arr[i]
        di_minus = minus_di_arr[i]
        di_ok = True
        if np.isnan(di_plus) or np.isnan(di_minus):
            di_ok = False
        else:
            di_ok = (di_plus - di_minus) >= di_spread_min

        # === 신규 c203: ADX 상승 게이트 ===
        adx_rising_ok = True
        if adx_slope_lb > 0:
            prev_idx = i - adx_slope_lb
            if prev_idx < 0 or np.isnan(adx_arr[prev_idx]) or np.isnan(adx_val):
                adx_rising_ok = False
            else:
                adx_rising_ok = adx_val > adx_arr[prev_idx]
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
                ...
                and adx_ok and di_ok and adx_rising_ok):
...
def build_combos() -> list[dict]:
    combos = []
    for adx_th, di_spread, adx_slb in product(
        ADX_TH_LIST, DI_SPREAD_MIN_LIST, ADX_SLOPE_LB_LIST,
    ):
        combos.append({
            "adx_th": adx_th,
            "di_spread_min": di_spread,
            "adx_slope_lb": adx_slb,
        })
    return combos
...
def precompute_indicators(
    ...
    """EMA slope pctile + vol momentum + ATR pctile + ADX+DI 사전 계산."""
    c = df["close"].values
    h = df["high"].values
    ...
    adx_arr, plus_di, minus_di = compute_adx_with_di(h, lo_arr, c, ADX_PERIOD)
    return ema_slope_pctile, vol_mom, atr_pctile, adx_arr, plus_di, minus_di


def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 203 — ADX 방향성(DI+>DI-) + ADX 상승 복합 게이트 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 35 AND trades >= 18")
    print("가설 A: DI+ > DI- + spread → 롱 방향 추세 확인 (역추세 차단)")
    print("가설 B: ADX 상승(slope_lb) → 추세 강화 중 진입")
    print("가설 C: ADX_TH + DI방향 + ADX상승 → 삼중 추세 확인")
    print(f"기준선: c202 OOS +32.217, c199 OOS +35.341, c192 OOS +47.314")
    ...
    for sym in sym_data_ok:
        ...
        if not df_tr.empty:
            ...
            esp, vol_mom, atr_pctile, adx_arr, plus_di, minus_di = (
                precompute_indicators(df_tr))
            sym_train_cache[sym] = (
                df_tr, btc_c, btc_s, esp, vol_mom, atr_pctile,
                adx_arr, plus_di, minus_di,
            )
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            if sym not in sym_train_cache:
                continue
            (df_tr, btc_c, btc_s, esp, vol_mom, atr_pctile,
             adx_a, plus_di, minus_di) = sym_train_cache[sym]
            r = backtest(df_tr, combo["adx_th"], combo["di_spread_min"],
                         combo["adx_slope_lb"],
                         btc_c, btc_s, esp, vol_mom, atr_pctile,
                         adx_a, plus_di, minus_di)
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['adx_th']:>4} {r['di_spread_min']:>4} "
            f"{r['adx_slope_lb']:>4} | "
    ...
    for r in valid:
        key = (r["adx_th"], r["di_spread_min"], r["adx_slope_lb"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        di_spread = params["di_spread_min"]
        adx_slb = params["adx_slope_lb"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                (esp, vol_mom, atr_pctile,
                 adx_a, plus_di, minus_di) = precompute_indicators(df_test)
                r = backtest(df_test, adx_th, di_spread, adx_slb,
                             btc_c, btc_s, esp, vol_mom, atr_pctile,
                             adx_a, plus_di, minus_di)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: aTh={adx_th} diS={di_spread} "
                  f"sLB={adx_slb} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                (esp, vol_mom, atr_pctile,
                 adx_a, plus_di, minus_di) = precompute_indicators(df_full)
                r = backtest(df_full, adx_th, di_spread, adx_slb,
                             btc_c, btc_s, esp, vol_mom, atr_pctile,
                             adx_a, plus_di, minus_di,
                             slippage=slip)
                sym_results.append(r)
    ...
    best = wf_sorted[0]
    adx_th = best["adx_th"]
    di_spread = best["di_spread_min"]
    adx_slb = best["adx_slope_lb"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: aTh={adx_th} "
          f"diS={di_spread} sLB={adx_slb}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"{'=' * 80}")
    print("=== 베이스라인 대비 비교 ===")
    print(f"  c199 최적 (rTh=60 hiTP=1.0 hiTr=2.0 loSL=0.20): "
          f"avg_OOS=+35.341 n=26")
    print(f"  c192 최적 (ttA=6 ttF=3.0 aTPS=0.5): "
          f"avg_OOS=+47.314 n=~20")
    print(f"  c202 최적 (aTh=15 oLB=5 oMin=0.00): "
          f"avg_OOS=+32.217 n=22")
    print(f"  c203 최적 (aTh={best['adx_th']} "
          f"diS={best['di_spread_min']} "
          f"sLB={best['adx_slope_lb']}): "
          f"avg_OOS={best['avg_oos_sharpe']:+.3f} "
          f"n={best['total_oos_trades']}")
    delta_sh_199 = best["avg_oos_sharpe"] - 35.341
    delta_sh_192 = best["avg_oos_sharpe"] - 47.314
    delta_sh_202 = best["avg_oos_sharpe"] - 32.217
    delta_n = best["total_oos_trades"] - 26
    print(f"  Δ vs c199: {delta_sh_199:+.3f} "
          f"({'개선' if delta_sh_199 > 0 else '악화'})")
    print(f"  Δ vs c192: {delta_sh_192:+.3f} "
          f"({'개선' if delta_sh_192 > 0 else '악화'})")
    print(f"  Δ vs c202: {delta_sh_202:+.3f} "
          f"({'개선' if delta_sh_202 > 0 else '악화'})")
    print(f"  Δ trades vs c199: {delta_n:+d}")

    # -- 최종 요약 --
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: ADX_TH={best['adx_th']} "
          f"DI_SPREAD_MIN={best['di_spread_min']} "
          f"ADX_SLOPE_LB={best['adx_slope_lb']}")
    print(f"  (c199 고정: rTh={REGIME_TH} hiTP={HI_TP_BONUS} "
          f"hiTr={HI_TRAIL_RELAX} loSL={LO_SL_TIGHTEN})")
    print(f"  (c192 고정: ttA={TRAIL_TIGHTEN_AFTER} ttF={TRAIL_TIGHTEN_FACTOR})")
    print(f"  (c190 고정: vMomLB={VOL_MOM_LB} vMomMin={VOL_MOM_MIN} "
