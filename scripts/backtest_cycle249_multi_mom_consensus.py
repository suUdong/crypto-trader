"""
사이클 249: c243 최적 고정 + 다중 기간 모멘텀 합의 (Multi-Period Momentum Consensus)

c243 결과:
  avg OOS Sharpe: +31.850 PASS  F2: +33.667 PASS  F3: +13.521 FAIL
  F3: Sharpe=+13.521  WR=60.0%  trades=5  avg=+1.24%  MDD=-2.75%

c241 참고: 모멘텀 필터 단일 LB=5, MIN=0.02 → Sharpe +35.11
c196 참고: 진입 필터 완화 → Sharpe +48.62 (더 많은 거래, 더 높은 Sharpe)

문제 분석:
  - c241은 단일 모멘텀 lookback(5봉)만 사용
  - 5봉 모멘텀이 양수여도 중기(10~20봉) 추세가 약하면 false breakout 가능
  - 가설: 단기(5봉) + 중기(10~20봉) 모멘텀 모두 양수여야 진짜 추세
    → 단기만 양수 = 노이즈 반등일 수 있음
    → 중기도 양수 = 구조적 상승 추세 내의 돌파
  - 추가 가설: 모멘텀 가속도 — 최근 모멘텀이 과거보다 강해지고 있는지
    → mom_accel = mom(5) - mom(10~20) > threshold
    → 가속하는 추세에서만 진입
  - 주의: 필터를 너무 강화하면 거래 0건 → 완화 옵션도 탐색
...
새 탐색 그리드 (c249):
  MOM_LB2:        [0, 10, 15, 20]    — 2차 모멘텀 lookback (0=OFF)
  MOM_MIN2:       [0.01, 0.02, 0.04] — 2차 모멘텀 최소값
  MOM_ACCEL_MIN:  [0.0, 0.005, 0.01] — 모멘텀 가속도 최소값 (0=OFF)
  = 4 × 3 × 3 = 36 combos
...
# ─── c249 탐색 그리드 ─────────────────────────────────────────
MOM_LB2_LIST = [0, 10, 15, 20]         # 2차 모멘텀 lookback (0=OFF)
MOM_MIN2_LIST = [0.01, 0.02, 0.04]     # 2차 모멘텀 최소값
MOM_ACCEL_MIN_LIST = [0.0, 0.005, 0.01]  # 모멘텀 가속도 최소값 (0=OFF)
...
def run_backtest(
    ...
    warmup = max(DC_UPPER_LB, DC_LOWER_LB, BTC_SMA_PERIOD, 60,
                 MOM_LB + 5, BTC_MOM_LB + 5, mom_lb2 + 5) + 10
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c249: 2차 모멘텀 (중기) ──────────────────────
            if mom_lb2 > 0 and i >= mom_lb2:
                mom2 = (c[i] / c[i - mom_lb2]) - 1.0
                if mom2 < mom_min2:
                    continue

                # ── c249: 모멘텀 가속도 ──────────────────────
                if (mom_accel_min > 0.0
                        and not np.isnan(mom1)):
                    # 가속도 = 단기 모멘텀 - 중기 모멘텀(봉당 정규화)
                    mom1_per_bar = mom1 / MOM_LB
                    mom2_per_bar = mom2 / mom_lb2
                    accel = mom1_per_bar - mom2_per_bar
                    if accel < mom_accel_min:
                        continue
...
def main() -> None:
    print("=" * 80)
    print("=== c249: c243최적 고정 + 다중 기간 모멘텀 합의 ===")
    ...
    print(f"탐색: MOM_LB2 × MOM_MIN2 × MOM_ACCEL_MIN")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        MOM_LB2_LIST, MOM_MIN2_LIST, MOM_ACCEL_MIN_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c243 최적, 2차 모멘텀 OFF)
    print("\n--- 베이스라인 (c243 최적, 2차모멘텀=OFF, 가속도=OFF) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for val in MOM_LB2_LIST:
        ...
        if sub:
            ...
            print(f"  mLB2={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")

    print("\n=== 2차 모멘텀 최소값 효과 ===")
    for val in MOM_MIN2_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  mMn2={val:.3f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")

    print("\n=== 모멘텀 가속도 효과 ===")
    for val in MOM_ACCEL_MIN_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  mAcc={val:.3f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  mLB2={p[0]} mMn2={p[1]:.3f} mAcc={p[2]:.3f} | "
    ...
    if valid:
        ...
        print(f"  c249 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: mLB2={p[0]} mMn2={p[1]:.3f} "
              f"mAcc={p[2]:.3f}")
        print(f"  (c243 고정: btcMLB={BTC_MOM_LB} btcMMin={BTC_MOM_MIN} "
              f"btcADX={BTC_ADX_GATE} boMgn={BREAKOUT_MARGIN})")
