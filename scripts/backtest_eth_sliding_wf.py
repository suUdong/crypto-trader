"""
momentum_eth 슬라이딩 윈도우 다중 OOS 검증 (사이클 103)
- 목적: 사이클 75 C0_base(lb=12, adx=25) W1+W2 2/3 통과 후 W3(2026) OOS 추가 검증
...
WINDOWS = [
    ...
    {"name": "W3", "is_start": "2024-01-01", "is_end": "2025-12-31", "oos_start": "2026-01-01", "oos_end": "2026-04-04"},
]

# daemon 파라미터 (사이클 75/76 확정) + 비교군
CANDIDATES = [
    {"lookback": 12, "adx": 25.0, "vol_mult": 2.0, "tp": 0.12, "sl": 0.04, "label": "C0_base (lb=12 adx=25) ★daemon"},
    {"lookback": 20, "adx": 25.0, "vol_mult": 2.0, "tp": 0.12, "sl": 0.04, "label": "C_old (lb=20 adx=25) 이전"},
...
def main() -> None:
    print("=" * 80)
    print("momentum_eth 슬라이딩 윈도우 다중 OOS 검증 (사이클 103)")
    print("목적: 사이클 75 W1+W2 2/3 통과 후 W3(2026) OOS 추가 확인")
    print("심볼: KRW-ETH | 4h봉 | lb=12, adx=25, TP=12%, SL=4%")
    print("=" * 80)
    ...
    for cand in CANDIDATES:
        ...
        print(f"{'윈도우':<6} | {'IS Sharpe':>10} {'IS WR':>7} {'IS T':>5} | {'OOS Sharpe':>10} {'OOS WR':>7} {'OOS T':>5} | {'판정':>6}")
        print("-" * 75)
        ...
        if pass_count == len(WINDOWS):
            ...
            print("◆◆ 2/3 통과 — 조건부 사용 가능 (BULL 전환 시 paper→live)")
        else:
