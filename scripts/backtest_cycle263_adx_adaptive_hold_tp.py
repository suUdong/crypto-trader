"""
사이클 263: c260 베이스라인 + ADX 강도 기반 적응형 보유기간/TP 스케일링

c260 최적 결과 (brMin=0.50 cpMin=0.70 bLB=1):
  avg OOS Sharpe: +60.998  F2: +33.412  F3: +113.278
  worst MDD: -4.98%  trades: 24  WR: 87.5%

문제:
  - 현재 MAX_HOLD는 고정(30봉) 또는 저변동성(25봉) 이분법
  - 강한 추세(ADX 40+)에서도 30봉에 강제 청산 → 수익 조기 실현
  - 약한 추세(ADX ~25, 최소 게이트 근처)에서 30봉 보유 → 불필요한 리스크 노출
  - c257(lowvol exit)이 성공적이었으므로, 추세 강도 차원을 추가하면 시너지 가능

가설:
  - ADX 값에 따라 MAX_HOLD와 TP2를 동적 조절
  - 강한 추세 (ADX > ADX_STRONG): MAX_HOLD 확대 + TP2 확대
  - 약한 추세 (ADX < ADX_WEAK): MAX_HOLD 축소 + TP2 축소 (조기 수익 실현)
  - 이미 c257의 ATR 레짐 분기와 겹치지 않게, ADX 기준만으로 추가 조절
  - 강한 추세에서 trail을 느슨하게 하여 수익 극대화

c260 최적 고정:
  brMin=0.50 cpMin=0.70 bLB=1
...
새 탐색 그리드 (c263):
  ADX_STRONG_THRESH:  [35, 40, 45]      — "강한 추세" 판정 기준
  STRONG_MH_BONUS:    [5, 10, 15]       — 강한 추세 시 MAX_HOLD 추가 봉수
  STRONG_TP2_BONUS:   [0.0, 0.5, 1.0]   — 강한 추세 시 TP2 추가 배수
  WEAK_MH_PENALTY:    [0, 3, 5]         — 약한 추세 시 MAX_HOLD 차감 봉수
  = 3 × 3 × 3 × 3 = 81 combos
...
# ─── c263 탐색 그리드 ─────────────────────────────────────────
ADX_STRONG_THRESH_LIST = [35, 40, 45]
STRONG_MH_BONUS_LIST = [5, 10, 15]
STRONG_TP2_BONUS_LIST = [0.0, 0.5, 1.0]
WEAK_MH_PENALTY_LIST = [0, 3, 5]

# "약한 추세" = ADX < ADX_THRESH + 5 (= 30)
ADX_WEAK_THRESH = 30
...
def run_backtest(
    ...
    sym: str,
    # c263 탐색 파라미터
    adx_strong_thresh: float,
    strong_mh_bonus: int,
    strong_tp2_bonus: float,
    weak_mh_penalty: int,
    oos_start: str,
    oos_end: str,
    index: pd.DatetimeIndex,
) -> list[dict]:
    """c260 최적 고정 + ADX 강도 기반 적응형 보유기간/TP."""
    n = len(c)
    trades: list[dict] = []
    position = None
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # c257 기본 MAX_HOLD
            base_mh = MH_LOW if is_exit_low_vol else BASE_MAX_HOLD

            # c263 핵심: ADX 강도 기반 MAX_HOLD 조절
            entry_adx = position.get("entry_adx", ADX_THRESH)
            if entry_adx >= adx_strong_thresh:
                eff_max_hold = base_mh + strong_mh_bonus
            elif entry_adx < ADX_WEAK_THRESH:
                eff_max_hold = max(10, base_mh - weak_mh_penalty)
            else:
                eff_max_hold = base_mh
            ...
            # c259 고정 + c263 신규: ATR 추세 + ADX 강도에 따른 TP2 조절
            eff_tp2 = BASE_TP2_MULT
            if atr_rising:
                eff_tp2 += TP2_BOOST
            # c263 핵심: 강한 추세 시 TP2 보너스
            if adx_val[i] >= adx_strong_thresh and strong_tp2_bonus > 0:
                eff_tp2 += strong_tp2_bonus
...
def main() -> None:
    print("=" * 80)
    print("=== c263: c260최적 고정 + ADX 강도 기반 적응형 보유기간/TP ===")
    ...
    print(f"탐색: ADX_STRONG × STRONG_MH_BONUS × STRONG_TP2 × WEAK_MH_PENALTY")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        ADX_STRONG_THRESH_LIST, STRONG_MH_BONUS_LIST,
        STRONG_TP2_BONUS_LIST, WEAK_MH_PENALTY_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c260 최적 = ADX 적응 비활성)
    print("\n--- 베이스라인 (c260 최적) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for window in WINDOWS:
        ...
        for sym in SYMBOLS:
            ...
            raw = run_backtest(
                ...
                99, 0, 0.0, 0,  # 비활성: strong_thresh=99 → 조건 불가
    ...
    print("=" * 80)
    hdr = (f"{'adxS':>5} {'mhB':>4} {'tp2B':>5} {'mhP':>4} | "
    ...
    for val in ADX_STRONG_THRESH_LIST:
        ...
        if sub:
            ...
            print(f"  adxS={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== STRONG_MH_BONUS 효과 ===")
    for val in STRONG_MH_BONUS_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  mhB={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== STRONG_TP2_BONUS 효과 ===")
    for val in STRONG_TP2_BONUS_LIST:
    ...
    print("\n=== WEAK_MH_PENALTY 효과 ===")
    for val in WEAK_MH_PENALTY_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  mhP={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  adxS={p[0]:.0f} mhB={p[1]:.0f} tp2B={p[2]:.1f} "
              f"mhP={p[3]:.0f} | "
    ...
    # 최종 요약
    print("\n" + "=" * 80)
    print("=== c260 베이스라인 대비 비교 ===")
    if valid:
        b = valid[0]
        print(f"  c260 기준: avg_OOS={base_avg:+.3f} F2={base_f2:+.3f} "
              f"F3={base_f3:+.3f} worst_MDD={base_worst_mdd:+.2f}%")
        print(f"  c263 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: adxS={p[0]:.0f} mhB={p[1]:.0f} "
              f"tp2B={p[2]:.1f} mhP={p[3]:.0f}")
        print(f"  (c260 고정: brMin={BODY_RATIO_MIN} cpMin={CLOSE_POS_MIN} "
              f"bLB={BODY_LOOKBACK})")
        print(f"  (c259 고정: atrLB={ATR_MOM_LB} atrTh={ATR_MOM_THRESH} "
              f"tp2B={TP2_BOOST} trTA={TRAIL_TIGHTEN_AMT})")
