"""
vpin_multi 사이클 191 — OBV+MACD 적응형 EXIT 부스터 (진입 게이트 → 출구 스케일링)
...
- c188 교훈:
  OBV+MACD를 AND 진입 게이트로 사용 → trades 60→27 감소, Sharpe 42→15 악화
  필터가 너무 엄격해서 좋은 신호까지 차단
- 가설: OBV+MACD를 진입이 아닌 EXIT 스케일링에 활용
  A) 진입 시 OBV slope>0 + MACD hist>0 "이중확인" → confirmed trade
     → TP 보너스 (더 넓은 TP), Hold 연장 (더 오래 보유)
  B) 하나만 확인 → 기본 c179 파라미터 유지
  C) 둘 다 미확인 → SL 타이트닝 (조기 손절)
  핵심: 모든 c179 진입 유지(trades ~60) + 확인된 신호에서 수익 극대화
- 탐색 그리드:
  CONFIRM_TP_BONUS: [0.5, 1.0, 1.5]    — 이중확인 시 TP 추가 ATR
  CONFIRM_HOLD_BONUS: [3, 5, 8]          — 이중확인 시 Hold 추가 봉수
  NOCONFIRM_SL_TIGHTEN: [0.0, 0.05, 0.10] — 미확인 시 SL 축소 비율
  OBV_EMA_PERIOD: [20, 30]               — c188 최적 범위 기반
  = 3×3×3×2 = 54 combos
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드: 적응형 EXIT 부스터 --
CONFIRM_TP_BONUS_LIST = [0.5, 1.0, 1.5]       # 이중확인 시 TP 추가 ATR
CONFIRM_HOLD_BONUS_LIST = [3, 5, 8]            # 이중확인 시 Hold 추가 봉수
NOCONFIRM_SL_TIGHTEN_LIST = [0.0, 0.05, 0.10]  # 미확인 시 SL 축소 비율
OBV_EMA_PERIOD_LIST = [20, 30]                  # c188 최적 범위
...
def backtest(
    df: pd.DataFrame,
    obv_ema_period: int,
    confirm_tp_bonus: float,
    confirm_hold_bonus: int,
    noconfirm_sl_tighten: float,
    ...
    while i < n - 1:
        ...
        # ★ 진입은 c179 동일 — OBV/MACD 게이트 없음
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
                and atr_pctile_ok and body_ok):

            # ★ OBV+MACD 확인 수준 판단 (exit 스케일링용)
            obv_slope_val = obv_slope_arr[i]
            macd_hist_val = macd_hist_arr[i]
            obv_confirmed = (
                not np.isnan(obv_slope_val) and obv_slope_val > 0
            )
            macd_confirmed = (
                not np.isnan(macd_hist_val) and macd_hist_val > 0
            )
            dual_confirmed = obv_confirmed and macd_confirmed
            no_confirm = not obv_confirmed and not macd_confirmed
            ...
            # ★ 적응형 EXIT: 확인 수준에 따라 TP/SL/Hold 조정
            effective_tp_mult = (TP_BASE_ATR + TP_BONUS_ATR * rsi_ratio) * tp_scale
            if dual_confirmed:
                # 이중확인: TP 확대 + Hold 연장
                effective_tp_mult += confirm_tp_bonus
            # (단일 확인: 기본값 유지)
            ...
            if no_confirm and noconfirm_sl_tighten > 0:
                # 미확인: SL 타이트닝 (조기 손절)
                effective_sl_mult *= (1.0 - noconfirm_sl_tighten)
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 191 — OBV+MACD 적응형 EXIT 부스터 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 43.0 (c179 +42.878 대비 개선)")
    print("가설: OBV+MACD를 진입 게이트 대신 EXIT 스케일링에 활용")
    print("  이중확인(OBV+MACD) → TP 확대 + Hold 연장")
    print("  미확인 → SL 타이트닝 (조기 손절)")
    print("  핵심: c179 진입 모두 유지 → trades ~60 보존")
    print(f"기준선: c179 OOS +42.878, WR ~45%, trades ~60")
    print(f"c188 교훈: AND 게이트 → trades 27, Sharpe 15 (악화)")
    ...
    print(f"MACD 고정: F={MACD_FAST} S={MACD_SLOW} Sg={MACD_SIGNAL}")
    print("=" * 80)
    ...
    combos = list(product(
        CONFIRM_TP_BONUS_LIST, CONFIRM_HOLD_BONUS_LIST,
        NOCONFIRM_SL_TIGHTEN_LIST, OBV_EMA_PERIOD_LIST,
    ))
    ...
    results: list[dict] = []
    for idx, (tp_bonus, hold_bonus, sl_tight, obv_ema) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, obv_ema, tp_bonus, hold_bonus, sl_tight,
                         btc_c, btc_s)
            sym_results.append(r)

        pooled = pool_results(sym_results)
        results.append({
            "confirm_tp_bonus": tp_bonus,
            "confirm_hold_bonus": hold_bonus,
            "noconfirm_sl_tighten": sl_tight,
            "obv_ema_period": obv_ema,
            **pooled,
        })
        if (idx + 1) % 18 == 0:
    ...
    hdr = (f"{'tpB':>5} {'hdB':>4} {'slT':>5} {'obvE':>5} | "
    ...
    for r in valid[:20]:
        ...
        print(
            f"{r['confirm_tp_bonus']:>5.1f} {r['confirm_hold_bonus']:>4} "
            f"{r['noconfirm_sl_tighten']:>5.2f} {r['obv_ema_period']:>5} | "
    ...
    for r in valid:
        key = (r["confirm_tp_bonus"], r["confirm_hold_bonus"],
               r["noconfirm_sl_tighten"], r["obv_ema_period"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        tp_bonus = params["confirm_tp_bonus"]
        hold_bonus = params["confirm_hold_bonus"]
        sl_tight = params["noconfirm_sl_tighten"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, obv_ema, tp_bonus, hold_bonus,
                             sl_tight, btc_c, btc_s)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: tpB={tp_bonus:.1f} hdB={hold_bonus} "
                  f"slT={sl_tight:.2f} obvE={obv_ema} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        sl_tight = params["noconfirm_sl_tighten"]
        obv_ema = params["obv_ema_period"]
        print(f"\n--- #{rank}: tpB={tp_bonus:.1f} hdB={hold_bonus} "
              f"slT={sl_tight:.2f} obvE={obv_ema} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, obv_ema, tp_bonus, hold_bonus,
                             sl_tight, btc_c, btc_s, slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
            sh = pooled["sharpe"] if not np.isnan(pooled["sharpe"]) else 0.0
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    tp_bonus = best["confirm_tp_bonus"]
    hold_bonus = best["confirm_hold_bonus"]
    sl_tight = best["noconfirm_sl_tighten"]
    obv_ema = best["obv_ema_period"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: tpB={tp_bonus:.1f} "
          f"hdB={hold_bonus} slT={sl_tight:.2f} obvE={obv_ema}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c191 최적 (tpB={best['confirm_tp_bonus']:.1f} "
          f"hdB={best['confirm_hold_bonus']} "
          f"slT={best['noconfirm_sl_tighten']:.2f} "
          f"obvE={best['obv_ema_period']}): "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: tpB={best['confirm_tp_bonus']:.1f} "
          f"hdB={best['confirm_hold_bonus']} "
          f"slT={best['noconfirm_sl_tighten']:.2f} "
          f"obvE={best['obv_ema_period']}")
    print(f"  MACD 고정: F={MACD_FAST} S={MACD_SLOW} Sg={MACD_SIGNAL}")
