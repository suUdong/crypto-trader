"""
미검증 L1 알트코인 momentum walk-forward 스크리닝 (사이클 84)
- 목적: BCH/DOGE/Layer2 탈락 후 미검증 L1 후보 풀 소진 탐색
- 심볼: KRW-ADA, KRW-AVAX, KRW-ATOM, KRW-DOT, KRW-TRX
...
SYMBOLS = ["KRW-ADA", "KRW-AVAX", "KRW-ATOM", "KRW-DOT", "KRW-TRX"]
...
def run_symbol(symbol: str) -> dict:
    ...
    best_result = {"symbol": symbol, "verdict": "탈락", "best_cand": None, "slide_pass": 0}
...
def main() -> None:
    print("=" * 70)
    print("미검증 L1 알트코인 momentum walk-forward 스크리닝 (사이클 84)")
    print("목적: BCH/DOGE/Layer2 탈락 후 미검증 L1 후보 풀 소진 탐색")
    print("심볼: ADA, AVAX, ATOM, DOT, TRX")
    ...
    for r in summary:
        ...
        if r.get("best_cand"):
            c = r["best_cand"]
            print(f"  {symbol:12s} → {verdict} | {c['label']} | slide={r['slide_pass']}/3 | OOS Sh={r.get('oos_sharpe', 0):+.2f} WR={r.get('oos_wr', 0):.1%}")
        else:
