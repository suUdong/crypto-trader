"""
vpin_multi 사이클 194 — 3단계 점진적 트레일 감쇠 + 모멘텀 지속시 홀드 연장
- 기반: c192 OOS Sharpe +30.947, WR 66.7%, trades 26 PASS
  c192 최적: ttA=6 ttF=3.0 aTPS=0.5
- 문제:
  1) c192 트레일 감쇠가 binary (6바 전/후) → 후반 보유에서 더 공격적 축소 여지
  2) 강한 추세에서 MAX_HOLD=20 도달 시 강제 청산 → 추세 꼬리 놓침
- 가설:
  A) 3단계 트레일: bars < 6(c192 고정) → base, 6~STAGE2 → base/3.0(c192 고정),
     >= STAGE2 → base/STAGE2_FACTOR (더 타이트)
     → 보유 후반부 수익 환원 최소화
  B) 홀드 연장: 미실현수익 > min_profit AND 최근 모멘텀 양수이면
     MAX_HOLD + HOLD_EXTEND_BARS까지 연장
     → 강한 추세의 꼬리 수익 포착, SOL/ETH 고변동 구간 활용
- 탐색 그리드:
  TRAIL_STAGE2_AFTER: [10, 12, 14] — 2단계→3단계 전환 바
  TRAIL_STAGE2_FACTOR: [4.0, 5.0, 6.0] — 3단계 트레일 축소 배수
  HOLD_EXTEND_BARS: [0, 4, 8] — 모멘텀 지속시 추가 홀드 바
  = 3×3×3 = 27 combos
- 목표: OOS Sharpe >= 31 AND trades >= 24
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- c194 탐색 그리드 --
TRAIL_STAGE2_AFTER_LIST = [10, 12, 14]
TRAIL_STAGE2_FACTOR_LIST = [4.0, 5.0, 6.0]
HOLD_EXTEND_BARS_LIST = [0, 4, 8]
...
def backtest(
    df: pd.DataFrame,
    trail_stage2_after: int,
    trail_stage2_factor: float,
    hold_extend_bars: int,
    ...
    while i < n - 1:
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
            ...
            # 기본 trail 파라미터
            base_trail_mult = TRAIL_BASE_ATR + TRAIL_BONUS_ATR * (1 - rsi_ratio)
            min_profit_dist = atr_at_entry * MIN_PROFIT_ATR

            # === 신규 c194: 홀드 연장 한도 ===
            effective_max_hold = MAX_HOLD + hold_extend_bars

            exit_ret = None
            for j in range(i + 2, min(i + 1 + effective_max_hold, n)):
                current_price = c[j]
                ...
                # === 신규 c194: 3단계 점진적 트레일 감쇠 ===
                # Stage 1: bars < 6 (c192 고정) → base
                # Stage 2: 6 <= bars < trail_stage2_after → base / 3.0 (c192)
                # Stage 3: bars >= trail_stage2_after → base / stage2_factor
                if bars_held >= trail_stage2_after:
                    effective_trail_mult = base_trail_mult / trail_stage2_factor
                elif bars_held >= TRAIL_TIGHTEN_AFTER:
                ...
                # === 신규 c194: 홀드 연장 조건 확인 ===
                # MAX_HOLD 이후 구간: 모멘텀 양수 + 미실현수익 > min_profit
                # 이어야 유지, 아니면 즉시 청산
                if bars_held >= MAX_HOLD and hold_extend_bars > 0:
                    cur_mom = mom_arr[j] if j < len(mom_arr) else np.nan
                    if (np.isnan(cur_mom) or cur_mom <= 0
                            or unrealized < min_profit_dist):
                        exit_ret = (current_price / buy - 1) - FEE - slippage
                        i = j
                        break

            if exit_ret is None:
                hold_end = min(i + effective_max_hold, n - 1)
...
def build_combos() -> list[dict]:
    combos = []
    for ts2a, ts2f, heb in product(
        TRAIL_STAGE2_AFTER_LIST,
        TRAIL_STAGE2_FACTOR_LIST,
        HOLD_EXTEND_BARS_LIST,
    ):
        combos.append({
            "trail_stage2_after": ts2a,
            "trail_stage2_factor": ts2f,
            "hold_extend_bars": heb,
        })
    return combos
...
def precompute_indicators(
    ...
    """EMA slope percentile + vol momentum + ATR percentile 사전 계산."""
    c = df["close"].values
    h = df["high"].values
    lo = df["low"].values
    v = df["volume"].values
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 194 — 3단계 점진적 트레일 감쇠 + 모멘텀 홀드 연장 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 31 AND trades >= 24")
    print("가설 A: 3단계 트레일 (base → /3.0 → /FACTOR2) → 후반부 수익 보호 강화")
    print("가설 B: 모멘텀 양수 + 수익 유지시 홀드 연장 → 추세 꼬리 포착")
    print(f"기준선: c192 OOS +30.947, WR 66.7%, trades 26")
    ...
    for sym in sym_data_ok:
        ...
        if not df_tr.empty:
            ...
            esp, vol_mom, atr_pct = precompute_indicators(df_tr)
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            if sym not in sym_train_cache:
                continue
            df_tr, btc_c, btc_s, esp, vol_mom, atr_pct = sym_train_cache[sym]
            r = backtest(df_tr, combo["trail_stage2_after"],
                         combo["trail_stage2_factor"],
                         combo["hold_extend_bars"],
                         btc_c, btc_s, esp, vol_mom, atr_pct)
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['trail_stage2_after']:>5} {r['trail_stage2_factor']:>5.1f} "
            f"{r['hold_extend_bars']:>4} | "
    ...
    for r in valid:
        key = (r["trail_stage2_after"], r["trail_stage2_factor"],
               r["hold_extend_bars"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ts2a = params["trail_stage2_after"]
        ts2f = params["trail_stage2_factor"]
        heb = params["hold_extend_bars"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                esp, vol_mom, atr_pct = precompute_indicators(df_test)
                r = backtest(df_test, ts2a, ts2f, heb,
                             btc_c, btc_s, esp, vol_mom, atr_pct)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: ts2A={ts2a} ts2F={ts2f:.1f} "
                  f"hEB={heb} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                esp, vol_mom, atr_pct = precompute_indicators(df_full)
                r = backtest(df_full, ts2a, ts2f, heb,
                             btc_c, btc_s, esp, vol_mom, atr_pct,
                             slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    ts2a = best["trail_stage2_after"]
    ts2f = best["trail_stage2_factor"]
    heb = best["hold_extend_bars"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: ts2A={ts2a} "
          f"ts2F={ts2f:.1f} hEB={heb}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c192 최적 (ttA=6 ttF=3.0 aTPS=0.5): "
          f"avg_OOS=+30.947 n=26")
    print(f"  c194 최적 (ts2A={best['trail_stage2_after']} "
          f"ts2F={best['trail_stage2_factor']:.1f} "
          f"hEB={best['hold_extend_bars']}): "
          f"avg_OOS={best['avg_oos_sharpe']:+.3f} "
          f"n={best['total_oos_trades']}")
    delta_sh = best["avg_oos_sharpe"] - 30.947
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: TRAIL_STAGE2_AFTER={best['trail_stage2_after']} "
          f"TRAIL_STAGE2_FACTOR={best['trail_stage2_factor']:.1f} "
          f"HOLD_EXTEND_BARS={best['hold_extend_bars']}")
    print(f"  (c192 고정: ttA={TRAIL_TIGHTEN_AFTER} ttF={TRAIL_TIGHTEN_FACTOR} "
          f"aTPS={ATR_TP_SCALE_HI})")
    print(f"  (c190 고정: VOL_MOM_LB={VOL_MOM_LB} VOL_MOM_MIN={VOL_MOM_MIN} "
          f"TP_SLOPE_BONUS={TP_SLOPE_BONUS})")
