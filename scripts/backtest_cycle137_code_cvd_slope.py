"""
사이클 137: stealth_3gate 코드 방식 CVD slope 임계값 탐색 (alt_cvd_slope_max)
- 사이클 136에서 발견: 백테스트(sign*vol 정규화) vs 코드((c-o)/(h-l)*vol 비정규화) 방식 불일치
- 이번: 코드와 동일한 방식(sign*vol 정규화)으로 통일 후 임계값 재탐색
- CVD slope = sum(sign(close-open)*volume) / (vol_mean * window) — 무차원 정규화
- 사이클 136에서 검증된 -0.1이 코드 방식에서도 유효한지 확인
- 탐색: alt cvd_slope_max ∈ {None(OFF), -0.05, -0.1, -0.15, -0.2, -0.3}
...
# 탐색 대상 alt cvd_slope 임계값 (None = OFF, 조건: cvd_slope < threshold)
# 코드 방식(sign 정규화) 기준으로 재탐색
CVD_CANDIDATES: list[float | None] = [None, -0.05, -0.1, -0.15, -0.2, -0.3]
...
def compute_cvd_slope_code(df: pd.DataFrame, window: int) -> pd.Series:
    """
    코드 방식(stealth_3gate.py 수정 후)과 동일한 CVD slope 계산.
    direction = sign(close - open)  {-1, 0, +1}
    CVD_raw per bar = direction * volume
    slope = rolling_sum(CVD_raw, window) / (vol_mean * window)  — 무차원
    """
    direction = np.sign(df["close"] - df["open"])
    cvd_raw = df["volume"] * direction
    cvd_sum = cvd_raw.rolling(window, min_periods=window).sum()
    vol_mean = df["volume"].rolling(window, min_periods=window).mean()
    slope = cvd_sum / (vol_mean.replace(0.0, np.nan) * window)
    return slope
...
def compute_alt_entry(
    ...
    Gate 5 (optional): alt cvd_slope < cvd_threshold (코드 방식 정규화)
    """
    ...
    if cvd_threshold is not None:
        # 코드 방식과 동일한 정규화 CVD slope
        cvd_slope = compute_cvd_slope_code(df_alt, W).reindex(df_alt.index)
...
def main() -> None:
    t0 = time.time()
    print("=== stealth_3gate 코드 방식 CVD slope 임계값 탐색 (사이클 137) ===")
    print(f"방식: sign(close-open)*vol / (vol_mean*W)  — 무차원 정규화 (코드 동일)")
    print(f"고정: W={W}, SMA={SMA_N}, RS=[{RS_LO},{RS_HI}), TP={TP*100:.0f}%, SL={SL*100:.1f}%, Gate4=OFF, acc>{ACC_MIN}")
    print(f"탐색: alt cvd_slope_max ∈ {CVD_CANDIDATES}")
    ...
    # CVD slope 분포 확인 (코드 방식, 참고용)
    sample_slopes = []
    for sym, df in list(alt_raw.items())[:20]:
        cvd_s = compute_cvd_slope_code(df, W)
        c = df["close"]
        v = df["volume"]
        c_ma = c.rolling(W, min_periods=W).mean()
        v_ma = v.rolling(W, min_periods=W).mean()
    ...
    if sample_slopes:
        ...
        print(f"\n  [참고] 코드 방식 CVD slope 분포 (알트 스텔스 시점, n={len(arr)}, 20심볼):")
    ...
    # Evaluate each cvd_slope_max threshold
    print("\n[3/3] CVD slope 임계값 탐색...")
    results = []
    ...
    for cvd_thresh in CVD_CANDIDATES:
        ...
        label_str = "None(OFF)" if cvd_thresh is None else f"< {cvd_thresh:.2f}"
        results.append({
            "label": label_str,
    ...
    # Summary table
    print("\n=== 결과 요약 ===")
    print(f"{'cvd_thresh':<16} {'alt_syms':>9} {'W1 Sharpe':>10} {'W1 n':>6} {'W2 Sharpe':>10} {'W2 n':>6} {'W2 WR':>7} {'통과':>5}")
    print("-" * 82)
    baseline_w2 = None
    best = None
    for r in results:
    ...
    if baseline_w2 is not None:
        if best is not None and best["cvd_thresh"] is not None:
            ...
            if delta > 0.3:
                print(f"✅ cvd_slope_max={best['cvd_thresh']} 채택 권장 (W2 Sharpe +{delta:.3f} vs 베이스라인 {baseline_w2:.3f})")
                print(f"   → daemon.toml에 alt_cvd_slope_max={best['cvd_thresh']} 추가")
            else:
                print(f"미미한 개선 (Δ{delta:+.3f}) — 현재 설정 유지 고려")
        else:
            print("통과 후보 없음 or 현재 설정(None)이 최선")
