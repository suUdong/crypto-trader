"""
vpin_multi 사이클 238: c236 최적 고정 + 심볼별 시간감쇠 trail tightening 3-fold WF

기반: c236 OOS avg=+35.424, WR=60.0%, trades=20
  최적: eTrail=0.8 sTrail=1.4 eMinP=1.2 sMinP=1.0
  ETH Sharpe=+27.875 (trades=12), SOL=+49.722 (trades=8), XRP=0 (trades=0)

문제:
  1) 시간감쇠 파라미터(ttA=6, ttF=3.0)가 전 심볼 동일 → 심볼 특성 미반영
  2) ETH는 타이트 trail(0.8)인데 빠르게 tighten하면 조기 청산 위험
  3) SOL은 넓은 trail(1.4)이므로 tighten 시점/강도 따로 최적화 가능
  4) c236에서 Sharpe가 c232 대비 -4.041 악화 → time-decay 최적화로 회복 시도
  5) XRP 0건 문제는 진입 필터 문제(별도 탐색 필요) → 여기서는 ETH/SOL 집중

가설:
  A) ETH: 빠른 수익보존 → 짧은 ttA(빠른 tighten 시작) + 강한 ttF(급격 축소)
  B) SOL: 큰 추세 캡처 → 긴 ttA(느린 tighten) + 약한 ttF(완만 축소)
  C) 심볼별 독립 탐색으로 최적 조합 탐색

탐색 그리드:
  E_TTA: [3, 5, 7]           — ETH bars before trail tighten
  E_TTF: [2.0, 3.0, 4.0]    — ETH tighten factor (높을수록 강하게 축소)
  S_TTA: [5, 8, 11]          — SOL bars before trail tighten
  S_TTF: [1.5, 2.0, 3.0]    — SOL tighten factor
  = 3×3×3×3 = 81 combos × 3-fold × 3 심볼

c236 고정 (전부):
  c236 최적: eTrail=0.8 sTrail=1.4 eMinP=1.2 sMinP=1.0
  c232 고정: eTP=0.70 sTP=1.00 eSL=0.80 sSL=1.00 xMOM=0.0003
...
# -- c236 최적: 심볼별 trail/minP 스케일 --
SYM_TRAIL_SCALE = {"KRW-ETH": 0.8, "KRW-SOL": 1.4, "KRW-XRP": 1.0}
SYM_MINP_SCALE = {"KRW-ETH": 1.2, "KRW-SOL": 1.0, "KRW-XRP": 1.0}

# -- c238 탐색 그리드: 심볼별 time-decay --
E_TTA_LIST = [3, 5, 7]       # ETH bars before trail tighten
E_TTF_LIST = [2.0, 3.0, 4.0]  # ETH tighten factor
S_TTA_LIST = [5, 8, 11]       # SOL bars before trail tighten
S_TTF_LIST = [1.5, 2.0, 3.0]  # SOL tighten factor
...
def backtest(
    ...
    while i < n - 1:
        ...
        for j in range(i + 2, min(i + 1 + max_hold, n)):
            ...
            # c238: 심볼별 시간감쇠 trail tightening
...
def get_sym_timedecay(
    symbol: str, combo: dict,
) -> tuple[int, float]:
    """심볼별 tta, ttf 반환."""
    if symbol == "KRW-ETH":
        return combo["e_tta"], combo["e_ttf"]
    elif symbol == "KRW-SOL":
        return combo["s_tta"], combo["s_ttf"]
    else:  # XRP — c192 기본값
        return TRAIL_TIGHTEN_AFTER, TRAIL_TIGHTEN_FACTOR


def build_combos() -> list[dict]:
    combos = []
    for et, ef, st, sf in product(
        E_TTA_LIST, E_TTF_LIST,
        S_TTA_LIST, S_TTF_LIST,
    ):
        combos.append({
            "e_tta": et,
            "e_ttf": ef,
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi c238 — 심볼별 시간감쇠 Trail Tightening ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 40 AND trades >= 18")
    print("가설: ETH 빠른 tighten(짧은 ttA, 강한 ttF) + "
          "SOL 느린 tighten(긴 ttA, 약한 ttF)")
    print(f"기준선: c236 OOS +35.424 / c232 OOS +39.465 / c199 OOS +51.425")
    print(f"c236 고정: eTrail={SYM_TRAIL_SCALE['KRW-ETH']} "
    ...
    print(f"탐색: eTTA={E_TTA_LIST}  eTTF={E_TTF_LIST}")
    print(f"      sTTA={S_TTA_LIST}  sTTF={S_TTF_LIST}")
    print("=" * 80)
    ...
    combos = build_combos()
    print(f"\n총 조합: {len(combos)}개 x {len(sym_data_ok)} 심볼")
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            tta, ttf = get_sym_timedecay(sym, combo)
            r = backtest(
                df_tr, sym,
                btc_c, btc_s, esp, vol_mom, atr_pctile, bb_wp, mom_pctile,
                sym_tta=tta, sym_ttf=ttf)
    ...
    hdr = (f"{'eTTA':>4} {'eTTF':>5} {'sTTA':>4} {'sTTF':>5} | "
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['e_tta']:>4} {r['e_ttf']:>5.1f} "
            f"{r['s_tta']:>4} {r['s_ttf']:>5.1f} | "
    ...
    for r in valid:
        key = (r["e_tta"], r["e_ttf"], r["s_tta"], r["s_ttf"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                tta, ttf = get_sym_timedecay(sym, params)
                r = backtest(
                    df_test, sym,
        ...
        if oos_sharpes:
            ...
            print(
                f"  #{rank}: eTTA={params['e_tta']} "
                f"eTTF={params['e_ttf']:.1f} "
                f"sTTA={params['s_tta']} "
                f"sTTF={params['s_ttf']:.1f} | "
    ...
    print(f"파라미터: eTTA={best['e_tta']} eTTF={best['e_ttf']:.1f} "
          f"sTTA={best['s_tta']} sTTF={best['s_ttf']:.1f}")
        ...
        for fold in WF_FOLDS:
            for sym in sym_data_ok:
                ...
                esp, vol_mom, atr_pctile, mom_pctile = precompute_base(
                    df_test)
                bb_wp = precompute_bb(df_test)
                tta, ttf = get_sym_timedecay(sym, best)
                r = backtest(
                    df_test, sym,
                    btc_c, btc_s, esp, vol_mom, atr_pctile, bb_wp,
                    mom_pctile,
    ...
    print(f"  c238 최적: avg_OOS={best['avg_oos']:+.3f}")
    delta199 = best["avg_oos"] - c199_baseline
    delta232 = best["avg_oos"] - c232_baseline
    delta236 = best["avg_oos"] - c236_baseline
    label199 = "개선" if delta199 > 0 else "악화"
    label232 = "개선" if delta232 > 0 else "악화"
    label236 = "개선" if delta236 > 0 else "악화"
    print(f"  Δ vs c199: {delta199:+.3f} ({label199})")
    print(f"  Δ vs c232: {delta232:+.3f} ({label232})")
    print(f"  Δ vs c236: {delta236:+.3f} ({label236})")

    # -- 최종 요약 --
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: eTTA={best['e_tta']} "
          f"eTTF={best['e_ttf']:.1f} "
          f"sTTA={best['s_tta']} "
          f"sTTF={best['s_ttf']:.1f}")
    print(f"  (c236 고정: eTrail={SYM_TRAIL_SCALE['KRW-ETH']} "
          f"sTrail={SYM_TRAIL_SCALE['KRW-SOL']} "
          f"eMinP={SYM_MINP_SCALE['KRW-ETH']} "
          f"sMinP={SYM_MINP_SCALE['KRW-SOL']})")
