"""
vpin_eth 사이클 161 — ATR 스케일링 + BTC 레짐 게이트 + 볼륨 확인
- 기반: c160 최적 ATR=20 hold=24 TP=4.0 SL=0.5 Tr=0.5 mP=1.5 cool=6
  c160 OOS: avg Sharpe +3.179, Fold1 +5.006 (WR=31.2% n=48), Fold2 +1.352 (WR=24.2% n=33)
- 문제: Fold2 불안정 (Sharpe +1.352). 약세장 진입이 수익 희석.
- 가설:
  1) BTC 레짐 게이트: BTC > SMA(N) 일 때만 진입 → 약세장 필터링 → Fold2 개선
     SMA 탐색: [100, 150, 200] (240m 기준)
  2) 볼륨 확인: 진입 시점 volume > vol_avg * vol_mult → 거래량 뒷받침 없는 허위 시그널 제거
     vol_avg window: [20, 40], vol_mult: [1.0, 1.2, 1.5]
  3) c160 ATR 파라미터 주변 미세 탐색: TP [3.5,4.0,4.5], SL [0.4,0.5,0.7]
...
# ── ATR 파라미터 (c160 최적 주변 미세 탐색) ────────────────────────────────
ATR_PERIOD = 20
MAX_HOLD = 24
COOLDOWN_BARS = 6

TP_ATR_MULT_LIST = [3.5, 4.0, 4.5]
SL_ATR_MULT_LIST = [0.4, 0.5, 0.7]
TRAIL_ATR_MULT = 0.5   # c160 최적 고정
MIN_PROFIT_ATR_MULT = 1.5  # c160 최적 고정

# ── 신규: BTC 레짐 게이트 ──────────────────────────────────────────────────
BTC_SMA_LIST = [100, 150, 200]

# ── 신규: 볼륨 확인 필터 ──────────────────────────────────────────────────
VOL_AVG_WINDOW_LIST = [20, 40]
VOL_MULT_LIST = [1.0, 1.2, 1.5]
...
def backtest(
    ...
    while i < n - 1:
        ...
        # 2) BTC 레짐 게이트: BTC > SMA(N)
        btc_ok = (
            not np.isnan(btc_close_aligned[i])
            and not np.isnan(btc_sma_aligned[i])
            and btc_close_aligned[i] > btc_sma_aligned[i]
        )
        # 3) 볼륨 확인: volume > vol_avg * vol_mult
        vol_ok = v[i] > va * vol_mult

        entry_ok = vpin_ok and btc_ok and vol_ok
        ...
        if entry_ok:
            ...
            tp_price = buy + atr_at_entry * tp_atr_mult
            sl_price = buy - atr_at_entry * sl_atr_mult
            trail_dist = atr_at_entry * TRAIL_ATR_MULT
            min_profit_dist = atr_at_entry * MIN_PROFIT_ATR_MULT
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_eth 사이클 161 — ATR 스케일링 + BTC 레짐 게이트 + 볼륨 확인 ===")
    print(f"심볼: {SYMBOL}  목표: OOS Sharpe ≥ 5.0, Fold2 안정화")
    print("가설: BTC>SMA(N) 게이트 → 약세장 진입 차단 → Fold2 Sharpe↑")
    print("      볼륨 확인 → 거래량 뒷받침 없는 허위 시그널 제거 → WR↑")
    print("기준선: c160 OOS avg +3.179 (Fold1=+5.006 WR=31.2%, Fold2=+1.352 WR=24.2%)")
    print("=" * 80)
    ...
    # c160 기준선 (BTC 게이트/볼륨 필터 없음)
    print("\n--- c160 기준선 (ATR=20 TP=4.0 SL=0.5 Tr=0.5 mP=1.5, 게이트 없음) ---")
    btc_c_tr, btc_sma_tr = align_btc_to_eth(df_train, df_btc_full, 200)
    # 볼륨 필터 off: vol_mult=0 → 항상 통과
    vol_avg_tr = vol_avg_calc(df_train["volume"].values, 20)
    base = backtest(df_train, 4.0, 0.5, btc_c_tr,
                    np.full(len(df_train), np.inf), vol_avg_tr, 0.0)
    ...
    combos = list(product(
        ...
        BTC_SMA_LIST,
        VOL_AVG_WINDOW_LIST, VOL_MULT_LIST,
    ))
    print(f"\n총 조합: {len(combos)}개")

    results: list[dict] = []
    for idx, (tp_m, sl_m, btc_sma_p, vol_w, vol_m) in enumerate(combos):
        btc_c_tr, btc_sma_tr = align_btc_to_eth(df_train, df_btc_full, btc_sma_p)
        va_tr = vol_avg_calc(df_train["volume"].values, vol_w)
        r = backtest(df_train, tp_m, sl_m, btc_c_tr, btc_sma_tr, va_tr, vol_m)
        results.append({
            "tp_atr_mult": tp_m, "sl_atr_mult": sl_m,
            "btc_sma": btc_sma_p, "vol_window": vol_w, "vol_mult": vol_m,
            **r,
        })
    ...
    for r in valid[:20]:
        ...
        print(
            ...
            f"{r['btc_sma']:>7} "
            f"{r['vol_window']:>5} {r['vol_mult']:>5.1f} | "
    ...
    for r in valid:
        key = (r["tp_atr_mult"], r["sl_atr_mult"],
               r["btc_sma"], r["vol_window"], r["vol_mult"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        vol_w = params["vol_window"]
        vol_m = params["vol_mult"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            va_t = vol_avg_calc(df_test["volume"].values, vol_w)
            r = backtest(df_test, tp_m, sl_m, btc_c_t, btc_sma_t, va_t, vol_m)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: TP={tp_m} SL={sl_m} BTC_SMA={btc_sma_p} "
                  f"vol_w={vol_w} vol_m={vol_m} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        va_f = vol_avg_calc(df_full["volume"].values, vol_w)
        print(f"\n--- #{rank}: TP={tp_m} SL={sl_m} BTC_SMA={btc_sma_p} "
              f"vol_w={vol_w} vol_m={vol_m} "
        ...
        for slip in SLIPPAGE_LEVELS:
            r = backtest(df_full, tp_m, sl_m, btc_c_f, btc_sma_f, va_f,
                         vol_m, slippage=slip)
            sh = r["sharpe"] if not np.isnan(r["sharpe"]) else 0.0
    ...
    # ── c160 기준선 vs c161 OOS 비교 ─────────────────────────────────────────
    print(f"\n{'=' * 80}")
    print("=== c160 기준선 vs c161 BTC게이트+볼륨 OOS 비교 ===")
    for fold_i, fold in enumerate(WF_FOLDS):
        df_test = load_historical(SYMBOL, "240m", fold["test"][0], fold["test"][1])
        if not df_test.empty:
            btc_c_t, _ = align_btc_to_eth(df_test, df_btc_full, 200)
            va_t = vol_avg_calc(df_test["volume"].values, 20)
            # c160 기준선 (게이트 없음: btc_sma=inf, vol_mult=0)
            r = backtest(df_test, 4.0, 0.5, btc_c_t,
                         np.full(len(df_test), np.inf), va_t, 0.0)
            sh = r["sharpe"] if not np.isnan(r["sharpe"]) else 0.0
            print(f"  [c160 기준] Fold {fold_i + 1}: Sharpe={sh:+.3f}  "
    ...
    for fold_i, fd in enumerate(best["fold_details"]):
        ...
        print(f"  [c161 최적] Fold {fold_i + 1}: Sharpe={sh:+.3f}  "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: TP={best['tp_atr_mult']} SL={best['sl_atr_mult']} "
          f"BTC_SMA={best['btc_sma']} "
          f"vol_w={best['vol_window']} vol_m={best['vol_mult']}")
    print(f"  (고정: ATR={ATR_PERIOD} hold={MAX_HOLD} "
          f"Tr={TRAIL_ATR_MULT} mP={MIN_PROFIT_ATR_MULT} cool={COOLDOWN_BARS})")
