"""
사이클 258: c257 최적 고정 + 브레이크아웃 품질 필터 (F2 Sharpe 개선)

c257 결과:
  avg OOS Sharpe: +35.593  F2: +25.408 FAIL  F3: +35.110
  worst MDD: -7.21%  trades: 31  WR: 80.6%

문제:
  - F2 (2025-03 ~ 2025-11) Sharpe +25.408 FAIL — 합격 기준 30 미달
  - F2 WR 72.7%, MDD -7.21% — 가짜 브레이크아웃 진입 후 되돌림 손실
  - c257 저변동성 출구 조정 효과 거의 0 (Δ avg +0.039) → 출구 문제 아님
  - 원인: 횡보장에서 돈치안 상단 돌파 후 즉시 되돌리는 false breakout

가설:
  - ADX가 상승 중일 때만 진입하면 추세 약화기 false breakout 필터링
  - 돌파 후 1-2봉 연속 DC upper 위 종가 확인하면 되돌림 제거
  - 진입봉 볼륨이 평균 대비 높을수록 진성 돌파 가능성 ↑

c257 최적 고정:
  slLow=0.8  trLow=1.2  mhLow=25  exSpl=40
...
새 탐색 그리드 (c258):
  ADX_SLOPE_LB:       [3, 5]            — ADX 상승 확인 lookback 봉수
  ADX_SLOPE_MIN:      [0.5, 1.0, 2.0]   — ADX 최소 상승폭
  CONFIRM_BARS:       [0, 1, 2]          — DC upper 위 연속 종가 확인 봉수
  VOL_SPIKE_MIN:      [1.5, 2.0, 2.5]   — 진입봉 볼륨/SMA 최소 배수
  = 2 × 3 × 3 × 3 = 54 combos
...
# ─── c258 탐색 그리드 ─────────────────────────────────────────
ADX_SLOPE_LB_LIST = [3, 5]
ADX_SLOPE_MIN_LIST = [0.5, 1.0, 2.0]
CONFIRM_BARS_LIST = [0, 1, 2]
VOL_SPIKE_MIN_LIST = [1.5, 2.0, 2.5]
...
def run_backtest(
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            eff_trail = TRAIL_LOW_MULT if is_exit_low_vol else BASE_TRAIL_MULT
            ...
            # ── c258 핵심: 브레이크아웃 확인 로직 ──
            is_above_dc = c[i] > dc_up[i]

            if not breakout_pending:
                # 새 브레이크아웃 감지
                if not is_above_dc:
                    continue
                ...
                if confirm_bars == 0:
                    ...
                    breakout_start_bar = i
                    breakout_confirm_count = 1
                    continue
            else:
                # 확인 중
                if not is_above_dc:
                    # DC upper 아래로 회귀 — false breakout
                    breakout_pending = False
                    continue

                breakout_confirm_count += 1
                if breakout_confirm_count < confirm_bars + 1:
                    continue
            ...
            # c258 신규: 볼륨 스파이크 (c250 VOL_RATIO_MIN 대체)
...
def main() -> None:
    print("=" * 80)
    print("=== c258: c257최적 고정 + 브레이크아웃 품질 필터 (F2 개선) ===")
    ...
    print(f"c255 고정: mLow={MOM_LOW} aSpl(진입)={ENTRY_ATR_SPLIT} "
          f"bLow={BTC_MOM_LOW}")
    print(f"c233 고정(고변동출구): trail={BASE_TRAIL_MULT} TP2={BASE_TP2_MULT} "
          f"SL={BASE_SL_MULT} MH={BASE_MAX_HOLD} TP1={BASE_TP1_MULT}")
    print(f"탐색: ADX_SLOPE_LB × ADX_SLOPE_MIN × CONFIRM_BARS × VOL_SPIKE_MIN")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        ADX_SLOPE_LB_LIST, ADX_SLOPE_MIN_LIST,
        CONFIRM_BARS_LIST, VOL_SPIKE_MIN_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")
    ...
    for gi, combo in enumerate(grid):
        adx_slb, adx_smin, conf_b, vol_sp = combo
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                ...
                raw = run_backtest(
                    ...
                    sp["atr_pctile"], sp["vol_sma"], sp["rsi"],
                    SYM_SL_SCALE[sym], sym,
                    adx_slb, adx_smin, conf_b, vol_sp,
                    window["oos_start"], window["oos_end"],
                    sp["index"],
                )
                agg = aggregate_trades(raw)
    ...
    print("=" * 80)
    hdr = (f"{'adxLB':>6} {'adxMn':>6} {'conf':>5} {'volSp':>6} | "
    ...
    for r in valid[:15]:
        p = r["params"]
        print(
            f"{p[0]:>6.0f} {p[1]:>6.1f} {p[2]:>5.0f} {p[3]:>6.1f} | "
    ...
    for val in ADX_SLOPE_LB_LIST:
        ...
        if sub:
            ...
            print(f"  adxLB={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== ADX_SLOPE_MIN 효과 ===")
    for val in ADX_SLOPE_MIN_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  adxMn={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== CONFIRM_BARS 효과 ===")
    for val in CONFIRM_BARS_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  conf={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== VOL_SPIKE_MIN 효과 ===")
    for val in VOL_SPIKE_MIN_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  volSp={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  adxLB={p[0]:.0f} adxMn={p[1]:.1f} conf={p[2]:.0f} "
              f"volSp={p[3]:.1f} | "
    ...
    # 최종 요약
    print("\n" + "=" * 80)
    print("=== c257 베이스라인 대비 비교 ===")
    if valid:
        b = valid[0]
        print(f"  c257 기준: avg_OOS={base_avg:+.3f} F2={base_f2:+.3f} "
              f"F3={base_f3:+.3f} worst_MDD={base_worst_mdd:+.2f}%")
        print(f"  c258 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: adxLB={p[0]:.0f} adxMn={p[1]:.1f} "
              f"conf={p[2]:.0f} volSp={p[3]:.1f}")
        print(f"  (c257 고정: slLow={SL_LOW_MULT} trLow={TRAIL_LOW_MULT} "
