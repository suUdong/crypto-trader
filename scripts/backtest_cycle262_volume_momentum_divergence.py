"""
사이클 262: c260 베이스라인 + 거래량-모멘텀 다이버전스 필터
...
문제:
  - 가격 breakout 시 거래량이 뒷받침되지 않는 경우 = 허위 돌파
  - 기존 VOL_SPIKE_MIN(c258)은 단순 "현재봉 거래량 > 평균" 체크
  - 가격 모멘텀은 상승인데 거래량 모멘텀은 하락 = 매수세 약화 시그널
  - 이런 "divergence" 상태의 breakout을 필터링하면 F2 약한 구간 개선 가능

가설:
  - 가격 N봉 모멘텀과 거래량 N봉 모멘텀을 비교
  - 가격 mom > 0 이면서 vol mom < 음수 임계값 = divergence → 진입 차단
  - 반대로, 가격+거래량 모두 상승 (convergence) 시 TP2 보너스 부여
  - 거래량 모멘텀: (vol_sma_short / vol_sma_long) - 1.0
...
새 탐색 그리드 (c262):
  VOL_MOM_SHORT:     [3, 5]             — 단기 거래량 SMA
  VOL_MOM_LONG:      [15, 20]           — 장기 거래량 SMA
  DIV_THRESH:        [-0.3, -0.2, -0.1, 0.0]  — divergence 차단 임계값 (0=비활성)
  CONV_TP2_BONUS:    [0.0, 0.5, 1.0]    — convergence 시 TP2 보너스
  = 2 × 2 × 4 × 3 = 48 combos
...
# ─── c262 탐색 그리드 ─────────────────────────────────────────
VOL_MOM_SHORT_LIST = [3, 5]
VOL_MOM_LONG_LIST = [15, 20]
DIV_THRESH_LIST = [-0.3, -0.2, -0.1, 0.0]
CONV_TP2_BONUS_LIST = [0.0, 0.5, 1.0]
...
def run_backtest(
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c262 핵심: 거래량-모멘텀 다이버전스 필터 ──
            is_converging = False
            if div_thresh < 0.0:
                vs = vol_sma_short_arr[i]
                vl = vol_sma_long_arr[i]
                if (not np.isnan(vs) and not np.isnan(vl)
                        and vl > 0):
                    vol_mom = (vs / vl) - 1.0
                    # 가격 mom > 0 (이미 통과) 이면서 vol_mom < 임계값 = divergence
                    if vol_mom < div_thresh:
                        continue  # divergence → 진입 차단
                    # convergence: 가격+거래량 모두 상승
            ...
            # c259 고정 + c262 신규: ATR 추세/convergence에 따른 TP2 조절
...
def main() -> None:
    print("=" * 80)
    print("=== c262: c260최적 고정 + 거래량-모멘텀 다이버전스 필터 ===")
    ...
    print(f"탐색: VOL_MOM_SHORT × VOL_MOM_LONG × DIV_THRESH × CONV_TP2_BONUS")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        VOL_MOM_SHORT_LIST, VOL_MOM_LONG_LIST,
        DIV_THRESH_LIST, CONV_TP2_BONUS_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c260 최적 = divergence 필터 비활성)
    print("\n--- 베이스라인 (c260 최적) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for window in WINDOWS:
        ...
        for sym in SYMBOLS:
            sp = sym_precomp[sym]
            # 비활성 파라미터: div_thresh=0.0
            vs_arr = sma_calc(sp["v"], 5)
            vl_arr = sma_calc(sp["v"], 20)
            raw = run_backtest(
                ...
                5, 20, 0.0, 0.0,  # divergence 필터 비활성
                vs_arr, vl_arr,
    ...
    for gi, combo in enumerate(grid):
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                sp = sym_precomp[sym]
                vs_arr = sma_calc(sp["v"], vm_s)
                vl_arr = sma_calc(sp["v"], vm_l)
                raw = run_backtest(
                    sp["c"], sp["o"], sp["h"], sp["lo"], sp["v"],
                    sp["dc_up"], sp["dc_lo"],
                    sp["atr"], sp["adx"],
    ...
    print("=" * 80)
    hdr = (f"{'vmS':>4} {'vmL':>4} {'dvTh':>6} {'ctpB':>5} | "
    ...
    for r in valid[:15]:
        p = r["params"]
        print(
            f"{p[0]:>4.0f} {p[1]:>4.0f} {p[2]:>6.2f} {p[3]:>5.1f} | "
    ...
    for val in DIV_THRESH_LIST:
        ...
        if sub:
            ...
            print(f"  dvTh={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== CONV_TP2_BONUS 효과 ===")
    for val in CONV_TP2_BONUS_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  ctpB={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== VOL_MOM_SHORT 효과 ===")
    for val in VOL_MOM_SHORT_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  vmS={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  vmS={p[0]:.0f} vmL={p[1]:.0f} dvTh={p[2]:.2f} "
              f"ctpB={p[3]:.1f} | "
    ...
    if valid:
        ...
        print(f"  c262 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: vmS={p[0]:.0f} vmL={p[1]:.0f} "
              f"dvTh={p[2]:.2f} ctpB={p[3]:.1f}")
        print(f"  (c260 고정: brMin={BODY_RATIO_MIN} cpMin={CLOSE_POS_MIN} "
              f"bLB={BODY_LOOKBACK})")
        print(f"  (c259 고정: atrLB={ATR_MOM_LB} atrTh={ATR_MOM_THRESH} "
              f"tp2B={TP2_BOOST} trTA={TRAIL_TIGHTEN_AMT})")
        ...
        for window in WINDOWS:
            for sym in SYMBOLS:
                sp = sym_precomp[sym]
                vs_arr = sma_calc(sp["v"], int(p[0]))
                vl_arr = sma_calc(sp["v"], int(p[1]))
                raw = run_backtest(
                    sp["c"], sp["o"], sp["h"], sp["lo"], sp["v"],
                    sp["dc_up"], sp["dc_lo"],
                    sp["atr"], sp["adx"],
