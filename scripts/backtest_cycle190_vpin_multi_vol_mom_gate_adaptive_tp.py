"""
vpin_multi 사이클 190 — 볼륨 모멘텀 게이트 + 추세강도 적응형 TP
- 기반: c186 OOS Sharpe +26.587, WR 61.5%, trades 33 PASS
  최적: body=0.50 rsiD=6 sLB=10 sPth=50
...
- 문제:
  1) c186이 trades 33으로 올렸지만 Sharpe -6.3 (c182 대비)
  2) 완화된 body/rsiD가 저품질 진입을 허용 → 볼륨 확인 부족
  3) SOL/XRP Fold1 0 trades — 약세장 필터링은 정상이나 품질 개선 여지
- 가설:
  A) 볼륨 모멘텀 게이트: 진입 시 거래량이 최근 LB 대비 증가 추세인지 확인
     → vol_mom = vol_ema[i] / vol_ema[i-LB] - 1 > threshold
     → 가격 상승 + 거래량 증가 = 건전한 모멘텀 확인
  B) EMA slope 강도 → TP 보너스: 추세가 강할수록 TP를 넓혀 수익 극대화
     → ema_slope_pctile > 70이면 TP에 추가 ATR 보너스
  C) 볼륨 모멘텀이 강할 때 body ratio 조건 완화 → 더 많은 진입 기회
- 탐색 그리드:
  VOL_MOM_LB: [5, 10, 15]          — 거래량 모멘텀 lookback
  VOL_MOM_MIN: [0.0, 0.05, 0.10]   — 최소 거래량 증가율 (0=비활성)
  TP_SLOPE_BONUS: [0.0, 1.0, 2.0]  — 강추세 TP 보너스 ATR 배수
  = 3×3×3 = 27 combos
- 목표: OOS Sharpe >= 30 AND trades >= 25 (c182 Sharpe 복구 + c186 trades 유지)
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드 --
VOL_MOM_LB_LIST = [5, 10, 15]
VOL_MOM_MIN_LIST = [0.0, 0.05, 0.10]
TP_SLOPE_BONUS_LIST = [0.0, 1.0, 2.0]
...
def build_combos() -> list[dict]:
    combos = []
    for vol_lb, vol_min, tp_bonus in product(
        VOL_MOM_LB_LIST, VOL_MOM_MIN_LIST, TP_SLOPE_BONUS_LIST,
    ):
        combos.append({
            "vol_mom_lb": vol_lb,
            "vol_mom_min": vol_min,
            "tp_slope_bonus": tp_bonus,
        })
    return combos
...
def precompute_indicators(
    ...
    """EMA slope percentile + 각 vol_mom_lb별 vol momentum 사전 계산."""
    c = df["close"].values
    v = df["volume"].values
    ema_arr = ema_calc(c, EMA_PERIOD)
    ema_slope_pctile = compute_ema_slope_percentile(ema_arr, EMA_SLOPE_LB)

    vol_mom_by_lb: dict[int, np.ndarray] = {}
    for lb in VOL_MOM_LB_LIST:
        vol_mom_by_lb[lb] = compute_vol_momentum(v, ema_period=lb)

    return ema_slope_pctile, vol_mom_by_lb


def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 190 — 볼륨 모멘텀 게이트 + 적응형 TP ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 30 AND trades >= 25")
    print("가설 A: 볼륨 모멘텀 게이트 → 저품질 진입 필터링")
    print("가설 B: EMA slope 강도 → TP 보너스 → 강추세 수익 극대화")
    print(f"기준선: c186 OOS +26.587, WR 61.5%, trades 33")
    ...
    for sym in sym_data_ok:
        ...
        if not df_tr.empty:
            ...
            esp, vol_mom_by_lb = precompute_indicators(df_tr)
            sym_train_cache[sym] = (df_tr, btc_c, btc_s, esp, vol_mom_by_lb)
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            if sym not in sym_train_cache:
                continue
            df_tr, btc_c, btc_s, esp, vol_mom_by_lb = sym_train_cache[sym]
            vol_mom_arr = vol_mom_by_lb[combo["vol_mom_lb"]]
            r = backtest(df_tr, combo["vol_mom_lb"], combo["vol_mom_min"],
                         combo["tp_slope_bonus"],
                         btc_c, btc_s, esp, vol_mom_arr)
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['vol_mom_lb']:>4} {r['vol_mom_min']:>6.2f} "
            f"{r['tp_slope_bonus']:>5.1f} | "
    ...
    for r in valid:
        key = (r["vol_mom_lb"], r["vol_mom_min"], r["tp_slope_bonus"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        vol_lb = params["vol_mom_lb"]
        vol_min = params["vol_mom_min"]
        tp_bonus = params["tp_slope_bonus"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                esp, vol_mom_by_lb = precompute_indicators(df_test)
                vol_mom_arr = vol_mom_by_lb[vol_lb]
                r = backtest(df_test, vol_lb, vol_min, tp_bonus,
                             btc_c, btc_s, esp, vol_mom_arr)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: vLB={vol_lb} vMin={vol_min:.2f} "
                  f"tpB={tp_bonus:.1f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        vol_min = params["vol_mom_min"]
        tp_bonus = params["tp_slope_bonus"]
        print(f"\n--- #{rank}: vLB={vol_lb} vMin={vol_min:.2f} "
              f"tpB={tp_bonus:.1f} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                esp, vol_mom_by_lb = precompute_indicators(df_full)
                vol_mom_arr = vol_mom_by_lb[vol_lb]
                r = backtest(df_full, vol_lb, vol_min, tp_bonus,
                             btc_c, btc_s, esp, vol_mom_arr,
                             slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    vol_lb = best["vol_mom_lb"]
    vol_min = best["vol_mom_min"]
    tp_bonus = best["tp_slope_bonus"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: vLB={vol_lb} "
          f"vMin={vol_min:.2f} tpB={tp_bonus:.1f}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    # -- c186 대비 비교 --
    print(f"{'=' * 80}")
    print("=== c186 베이스라인 대비 비교 ===")
    print(f"  c186 최적 (body=0.50 rsiD=6 sLB=10 sPth=50): "
          f"avg_OOS=+26.587 n=33")
    print(f"  c190 최적 (vLB={best['vol_mom_lb']} "
          f"vMin={best['vol_mom_min']:.2f} "
          f"tpB={best['tp_slope_bonus']:.1f}): "
          f"avg_OOS={best['avg_oos_sharpe']:+.3f} "
          f"n={best['total_oos_trades']}")
    delta_sh = best["avg_oos_sharpe"] - 26.587
    delta_n = best["total_oos_trades"] - 33
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: VOL_MOM_LB={best['vol_mom_lb']} "
          f"VOL_MOM_MIN={best['vol_mom_min']:.2f} "
          f"TP_SLOPE_BONUS={best['tp_slope_bonus']:.1f}")
    print(f"  (c186 고정: body={BODY_RATIO_MIN} rsiD={RSI_DELTA_MIN} "
          f"sLB={EMA_SLOPE_LB} sPth={EMA_SLOPE_PCTILE_TH})")
    print(f"  (c182 고정: vPth={VOL_PCTILE_TH} vPLB={VOL_PCTILE_LB})")
    print(f"  (c176 고정: atrLB={ATR_PCTILE_LB} atrTh={ATR_TH})")
