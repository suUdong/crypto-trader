...
# ─── c229: 직전 최적(adxB=10 volB=2.0 solTrail=1.5 xrpTrail=2.0) 고정 후
#         심볼별 trail 멀티 파인 그리드 + ETH trail도 함께 탐색 ────────────
# 직전 결과: avg_OOS=+29.478 F3=+44.032 SOL=+11.536 XRP=+70.785 worstMDD=-12.88%
#           목표: worst MDD 완화(-12%→-10%대) + avg/SOL Sharpe 유지·개선
SOL_ADX_BOOST_FIXED = 10               # SOL ADX>=35 (직전 최적)
SOL_VOL_BOOST_FIXED = 2.0              # SOL vol_ratio>=2.0 (직전 최적)
SOL_RSI_FLOOR_FIXED = 0                # 비활성 (직전 최적)
SOL_ATR_PCT_BOOST_FIXED = 0            # 비활성 (직전 최적)

# 심볼별 trail 그리드 (직전 최적 +- 파인 스텝)
ETH_TRAIL_LIST = [2.0, 2.5, 3.0]
SOL_TRAIL_LIST = [1.2, 1.4, 1.5, 1.7, 2.0]
XRP_TRAIL_LIST = [1.5, 1.8, 2.0, 2.2, 2.5]
...
def main() -> None:
    ...
    # 그리드 정의 (ETH/SOL/XRP 심볼별 trail 멀티)
    grid = list(product(ETH_TRAIL_LIST, SOL_TRAIL_LIST, XRP_TRAIL_LIST))
    print(f"\n총 trail 조합: {len(grid)} "
          f"(ETH×SOL×XRP = {len(ETH_TRAIL_LIST)}×"
          f"{len(SOL_TRAIL_LIST)}×{len(XRP_TRAIL_LIST)})")
    print(f"SOL 필터 고정: adxB={SOL_ADX_BOOST_FIXED} "
          f"volB={SOL_VOL_BOOST_FIXED} (c228-best)")

    # Walk-Forward grid search
    all_results: list[dict] = []

    for gi, combo in enumerate(grid):
        eth_trail, sol_trail, xrp_trail = combo
        sym_trail_map = {
            "KRW-ETH": eth_trail,
            "KRW-SOL": sol_trail,
            "KRW-XRP": xrp_trail,
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                ...
                # SOL만 고정 필터 강화, ETH/XRP 기본값
                if sym == "KRW-SOL":
                    adx_th_eff = ADX_THRESH + SOL_ADX_BOOST_FIXED
                    rsi_fl = SOL_RSI_FLOOR_FIXED
                    vol_min_eff = SOL_VOL_BOOST_FIXED
                    atr_pctile_th_eff = (
                        ATR_PCTILE_TH + SOL_ATR_PCT_BOOST_FIXED)
                else:
                    adx_th_eff = ADX_THRESH
                    rsi_fl = 0
                    vol_min_eff = VOL_RATIO_MIN
                    atr_pctile_th_eff = ATR_PCTILE_TH
                tm = sym_trail_map[sym]
    ...
    for i, r in enumerate(valid[:5]):
        p = r["params"]
        print(f"\n#{i+1}: ethTrail={p[0]:.1f} solTrail={p[1]:.1f} "
              f"xrpTrail={p[2]:.1f}")
    ...
    # 심볼별 trail 효과 분석
    print("\n" + "=" * 80)
    print("=== 심볼별 trail 멀티 효과 분석 ===")

    def _trail_stats(idx: int, values: list[float], label: str) -> None:
        print(f"\n--- {label} ---")
        for v in values:
            ...
            if subset:
                ...
                print(f"  trail={v:.1f}: mean avgSharpe={avg_sh:+.3f}  "
                      f"mean worstMDD={mdd_w:+.2f}%  n={len(subset)}")

    _trail_stats(0, ETH_TRAIL_LIST, "ETH trail")
    _trail_stats(1, SOL_TRAIL_LIST, "SOL trail")
    _trail_stats(2, XRP_TRAIL_LIST, "XRP trail")
    ...
    if valid:
        ...
        bp = best["params"]
        print("\n" + "=" * 80)
        print(f"=== 심볼별 OOS 성능 분해 (Top 1: ethT={bp[0]:.1f} "
              f"solT={bp[1]:.1f} xrpT={bp[2]:.1f}) ===")
        ...
        for sym in SYMBOLS:
            ...
            if sym == "KRW-SOL":
                ...
                rsi_fl = SOL_RSI_FLOOR_FIXED
                vol_min_eff = SOL_VOL_BOOST_FIXED
                atr_pctile_th_eff = ATR_PCTILE_TH + SOL_ATR_PCT_BOOST_FIXED
            else:
                adx_th_eff = ADX_THRESH
                rsi_fl = 0
                vol_min_eff = VOL_RATIO_MIN
                atr_pctile_th_eff = ATR_PCTILE_TH
            best_trail_map = {
                "KRW-ETH": bp[0], "KRW-SOL": bp[1], "KRW-XRP": bp[2],
            }
            tm_sym = best_trail_map[sym]
    ...
    if valid:
        b = valid[0]
        bp = b["params"]
        print(f"  c228 기준: avg_OOS=+29.478 F3=+44.032 "
              f"SOL_avg=+11.536 worst_MDD=-12.88%")
        print(f"  c229 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
              f"SOL_avg={b['sol_avg_sharpe']:+.3f} "
              f"worst_MDD={b['worst_mdd']:+.2f}%")
        delta = b["avg_sharpe"] - 29.478
        delta_f3 = b["f3_sharpe"] - 44.032
        delta_sol = b["sol_avg_sharpe"] - 11.536
        delta_mdd = b["worst_mdd"] - (-12.88)
    ...
    if valid:
        ...
        print(f"★ OOS 최적: ethTrail={p[0]:.1f} solTrail={p[1]:.1f} "
              f"xrpTrail={p[2]:.1f}")
        print(f"  SOL 필터 고정: ADX>={ADX_THRESH + SOL_ADX_BOOST_FIXED} "
              f"vol>={SOL_VOL_BOOST_FIXED}")
