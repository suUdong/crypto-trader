"""
사이클 245: c243 최적 고정 + RSI 동적 출구 + 레짐 적응형 TP
...
문제 분석:
  - F3 WR 60%: 진입은 OK지만 출구가 약함 — 이익을 지키지 못하고 반전
  - 기존 출구: TP1/TP2/SL/TRAIL/DC_LOW/MAX_HOLD — 전부 "가격 수준" 기반
  - 가설 1: RSI가 과열(>70) 후 하락 전환 시 조기 청산 → 수익 보호
    → "RSI 모멘텀 소진 출구" — 진입 후 RSI가 peak에서 N% 이상 하락하면 청산
  - 가설 2: 변동성 레짐(ATR pctile)에 따라 TP/SL 비율 동적 조정
    → 고변동성: TP 넓게 (큰 움직임 기대), 저변동성: TP 좁게 (보수적)
  - 진입 필터 변경 없음 — 출구 개선만으로 F3 WR/Sharpe 향상 목표
...
새 탐색 그리드 (c245):
  RSI_EXIT_TH:    [0, 70, 75]         — RSI 출구 활성화 임계값 (0=OFF)
  RSI_EXIT_DROP:  [5, 10, 15]         — RSI peak 대비 최소 하락폭
  REGIME_TP_ADJ:  [0.0, 0.3, 0.5, 0.8] — 레짐 TP 조정 강도 (0=OFF)
  REGIME_SL_ADJ:  [0.0, 0.2, 0.4]     — 레짐 SL 조정 강도 (0=OFF)
  = 3 × 3 × 4 × 3 = 108 combos
...
# ─── c245 탐색 그리드 ─────────────────────────────────────────
RSI_EXIT_TH_LIST = [0, 70, 75]         # RSI 출구 활성 임계 (0=OFF)
RSI_EXIT_DROP_LIST = [5, 10, 15]        # RSI peak 대비 하락폭
REGIME_TP_ADJ_LIST = [0.0, 0.3, 0.5, 0.8]  # 레짐 TP 조정 강도
REGIME_SL_ADJ_LIST = [0.0, 0.2, 0.4]   # 레짐 SL 조정 강도
...
def run_backtest(
    ...
    """c243 최적 고정 + RSI 동적 출구 + 레짐 적응형 TP/SL."""
    n = len(c)
    trades: list[dict] = []
    position = None
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # RSI peak 추적
            if rsi_exit_th > 0 and not np.isnan(rsi_arr[i]):
                if rsi_arr[i] > position.get("rsi_peak", 0):
                    position["rsi_peak"] = rsi_arr[i]
            ...
            if (exit_reason is None
                    ...
                    and cp > position["entry_price"]):  # 이익 상태에서만
                rsi_peak = position.get("rsi_peak", 0)
                if (rsi_peak >= rsi_exit_th
                        and rsi_arr[i] <= rsi_peak - rsi_exit_drop):
            ...
            # ── c245 레짐 적응형 TP/SL 조정 ──────────────────
            # ATR percentile 기반: 고변동성(>70) → TP 확대, 저변동성(<30) → TP 축소
            regime_tp_mult = BASE_TP2_MULT
            regime_sl_mult = BASE_SL_MULT * sl_scale
            regime_tp1_mult = BASE_TP1_MULT

            if regime_tp_adj > 0 and not np.isnan(atr_pctile[i]):
                # 정규화: (pctile - 50) / 50 → [-1, +1]
                regime_score = (atr_pctile[i] - 50.0) / 50.0
                # TP: 고변동 → 확대, 저변동 → 축소
                tp_factor = 1.0 + regime_score * regime_tp_adj
                tp_factor = max(0.5, min(2.0, tp_factor))  # clamp
                regime_tp_mult *= tp_factor
                regime_tp1_mult *= tp_factor

            if regime_sl_adj > 0 and not np.isnan(atr_pctile[i]):
                regime_score = (atr_pctile[i] - 50.0) / 50.0
                # SL: 고변동 → 확대 (whipsaw 방지), 저변동 → 축소 (손실 제한)
                sl_factor = 1.0 + regime_score * regime_sl_adj
                sl_factor = max(0.5, min(2.0, sl_factor))
                regime_sl_mult *= sl_factor
            ...
            tp_mult_final = regime_tp_mult + vol_tp_bonus

            tp2_pct = atr_now / c[i] * tp_mult_final
            tp1_pct = atr_now / c[i] * regime_tp1_mult
            sl_pct = atr_now / c[i] * regime_sl_mult
            ...
            position = {
                ...
                "tp1_hit": False,
                "remaining": 1.0,
                "rsi_peak": rsi_arr[i] if not np.isnan(rsi_arr[i]) else 0,
            }
...
def main() -> None:
    print("=" * 80)
    print("=== c245: c243최적 고정 + RSI동적출구 + 레짐적응형 TP/SL ===")
    ...
    print(f"탐색: RSI_EXIT_TH × RSI_EXIT_DROP × REGIME_TP_ADJ × REGIME_SL_ADJ")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        RSI_EXIT_TH_LIST, RSI_EXIT_DROP_LIST,
        REGIME_TP_ADJ_LIST, REGIME_SL_ADJ_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c243 최적, 새 출구 OFF)
    print("\n--- 베이스라인 (c243 최적, RSI출구=OFF, 레짐조정=OFF) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for gi, combo in enumerate(grid):
        rsi_eth, rsi_drp, rg_tp, rg_sl = combo
    ...
    print("=" * 80)
    hdr = (f"{'rETh':>5} {'rEDr':>5} {'rgTP':>5} {'rgSL':>5} | "
    ...
    for r in valid[:15]:
        p = r["params"]
        print(
            f"{p[0]:>5} {p[1]:>5} {p[2]:>5.1f} {p[3]:>5.1f} | "
    ...
    for val in RSI_EXIT_TH_LIST:
        ...
        if sub:
            ...
            print(f"  rsiETh={val}: top5 avgSh={avg_sh:+.3f}  F3={avg_f3:+.3f}")

    print("\n=== RSI 출구 하락폭 효과 ===")
    for val in RSI_EXIT_DROP_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  rsiEDr={val}: top5 avgSh={avg_sh:+.3f}  F3={avg_f3:+.3f}")

    print("\n=== 레짐 TP 조정 강도 효과 ===")
    for val in REGIME_TP_ADJ_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  rgTP={val:.1f}: top5 avgSh={avg_sh:+.3f}  F3={avg_f3:+.3f}")

    print("\n=== 레짐 SL 조정 강도 효과 ===")
    for val in REGIME_SL_ADJ_LIST:
        sub = [r for r in valid if r["params"][3] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  rgSL={val:.1f}: top5 avgSh={avg_sh:+.3f}  F3={avg_f3:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  rETh={p[0]} rEDr={p[1]} rgTP={p[2]:.1f} rgSL={p[3]:.1f} | "
    ...
    if valid:
        ...
        print(f"  c245 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: rETh={p[0]} rEDr={p[1]} "
              f"rgTP={p[2]:.1f} rgSL={p[3]:.1f}")
        print(f"  (c243 고정: btcMLB={BTC_MOM_LB} btcMMin={BTC_MOM_MIN} "
              f"btcADX={BTC_ADX_GATE} boMgn={BREAKOUT_MARGIN})")
