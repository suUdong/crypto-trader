"""
사이클 265: c262 최적 고정 + RSI 과매수 이탈 시 트레일 가속 출구
...
문제:
  - 기존 출구는 고정 trail / TP2 / max_hold 중심
  - breakout 후 RSI가 70+ 과매수 진입 → 다시 하락하면 모멘텀 소진 시그널
  - 이때 trail을 가속(tighten)하면 수익을 더 잘 보전할 수 있음
  - 반대로 RSI가 꾸준히 상승 중이면 추세 유지 = trail 유지/완화

가설:
  - 포지션 보유 중 RSI가 과매수(>OB) 도달 후 다시 OB 이하로 하락 시
    trail을 RSI_TRAIL_TIGHTEN만큼 추가 축소
  - RSI가 과매수에서 벗어나지 않으면 아무것도 안 함
  - OB 임계값과 tighten 강도를 탐색
  - 추가: RSI 과매도(<OS) 접근 시 조기 청산 (선택적)
...
새 탐색 그리드 (c265):
  RSI_OB_THRESH:     [65, 70, 75]        — 과매수 임계값
  RSI_TRAIL_TIGHTEN: [0.0, 0.3, 0.5, 0.8] — 과매수 이탈 시 trail 축소량
  RSI_OS_EXIT:       [0, 35, 40]          — 과매도 접근 시 조기 청산 (0=비활성)
  = 3 × 4 × 3 = 36 combos
"""
...
# ─── c265 탐색 그리드 ─────────────────────────────────────────
RSI_OB_THRESH_LIST = [65, 70, 75]
RSI_TRAIL_TIGHTEN_LIST = [0.0, 0.3, 0.5, 0.8]
RSI_OS_EXIT_LIST = [0, 35, 40]
...
def run_backtest(
    ...
    # c265 탐색 파라미터
    rsi_ob_thresh: int,
    rsi_trail_tighten: float,
    rsi_os_exit: int,
    oos_start: str,
    oos_end: str,
    index: pd.DatetimeIndex,
) -> list[dict]:
    """c262 최적 고정 + RSI 과매수 이탈 트레일 가속."""
    n = len(c)
    trades: list[dict] = []
    position = None
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c265 핵심: RSI 과매수 이탈 시 trail 가속 ──
            if rsi_trail_tighten > 0 and not np.isnan(rsi_arr[i]):
                rsi_now = rsi_arr[i]
                # 과매수 도달 기록
                if rsi_now >= rsi_ob_thresh:
                    position["rsi_was_overbought"] = True
                # 과매수 이탈 → trail 추가 축소
                if (position.get("rsi_was_overbought", False)
                        and rsi_now < rsi_ob_thresh):
                    eff_trail = max(0.3, eff_trail - rsi_trail_tighten)
            ...
            # c265: RSI 과매도 조기 청산
            if (rsi_os_exit > 0 and not np.isnan(rsi_arr[i])
                    and rsi_arr[i] <= rsi_os_exit
                    and bars_held >= 3):
                exit_reason = "RSI_OS"
...
def main() -> None:
    print("=" * 80)
    print("=== c265: c262최적 고정 + RSI 과매수 이탈 트레일 가속 출구 ===")
    ...
    print(f"탐색: RSI_OB_THRESH × RSI_TRAIL_TIGHTEN × RSI_OS_EXIT")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        RSI_OB_THRESH_LIST, RSI_TRAIL_TIGHTEN_LIST, RSI_OS_EXIT_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c262 최적 = RSI 출구 비활성)
    print("\n--- 베이스라인 (c262 최적) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for val in RSI_OB_THRESH_LIST:
        ...
        if sub:
            ...
            print(f"  rOB={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== RSI_TRAIL_TIGHTEN 효과 ===")
    for val in RSI_TRAIL_TIGHTEN_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  rTT={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== RSI_OS_EXIT 효과 ===")
    for val in RSI_OS_EXIT_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  rOS={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  rOB={p[0]:.0f} rTT={p[1]:.1f} rOS={p[2]:.0f} | "
    ...
    if valid:
        ...
        print(f"  c265 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: rOB={p[0]:.0f} rTT={p[1]:.1f} rOS={p[2]:.0f}")
        print(f"  (c262 고정: vmS={VOL_MOM_SHORT} vmL={VOL_MOM_LONG} "
              f"dvTh={DIV_THRESH} ctpB={CONV_TP2_BONUS})")
        print(f"  (c260 고정: brMin={BODY_RATIO_MIN} cpMin={CLOSE_POS_MIN} "
              f"bLB={BODY_LOOKBACK})")
