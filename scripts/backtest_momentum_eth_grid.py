"""
momentum_eth 4h 파라미터 그리드 탐색
- KRW-ETH 단일 심볼
- 기간: 2022~2026
- 베이스: lookback=12, adx=25, vol_mult=2.0, TP=0.12, SL=0.03 → Sharpe +13.59 (cycle 67)
- 확장 탐색: lookback × adx × vol_mult × TP × SL
"""
...
# ── 그리드 (베이스 lookback=12, adx=25, vol=2.0, tp=0.12, sl=0.03 중심 확장) ────
LOOKBACK_LIST  = [6, 8, 10, 12, 14, 16, 20, 24]
ADX_LIST       = [20.0, 22.0, 25.0, 28.0, 30.0]
...
def main() -> None:
    print(f"=== momentum_eth 4h 그리드 탐색 ===")
    print(f"심볼: {SYMBOL}  기간: {START} ~ {END}")
    print(f"베이스 파라미터 (cycle67): lookback=12 adx=25 vol=2.0 TP=0.12 SL=0.03 → Sharpe +13.59")
    ...
    print(f"\n베이스 (cycle67): lookback=12 adx=25.0 vol=2.0 TP=0.12 SL=0.03 → Sharpe +13.59")

    # ADX별 최고 Sharpe 분석
    print("\n=== ADX 임계값별 최고 Sharpe ===")
    for adx_v in sorted(set(r["adx"] for r in results)):
        sub = [r for r in results if r["adx"] == adx_v and not np.isnan(r["sharpe"])]
        if sub:
            b = sub[0]
            print(f"  adx≥{adx_v:.0f}: best Sharpe={b['sharpe']:+.3f}  lb={b['lookback']} "
                  f"vol={b['vol_mult']} TP={b['tp']} SL={b['sl']}")

    # lookback별 최고 Sharpe 분석
    print("\n=== lookback별 최고 Sharpe ===")
    for lb_v in sorted(set(r["lookback"] for r in results)):
        sub = [r for r in results if r["lookback"] == lb_v and not np.isnan(r["sharpe"])]
        if sub:
            b = sub[0]
            print(f"  lookback={lb_v:>2}: best Sharpe={b['sharpe']:+.3f}  adx={b['adx']:.0f} "
                  f"vol={b['vol_mult']} TP={b['tp']} SL={b['sl']}")
