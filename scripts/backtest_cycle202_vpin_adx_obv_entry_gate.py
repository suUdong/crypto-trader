"""
vpin_multi 사이클 202 — ADX 추세강도 + OBV 방향 진입 게이트
- 기반: c199 OOS Sharpe +35.341 (레짐 조건부 이중 출구) PASS
...
- 문제:
  1) c199 Sharpe가 c192 대비 -11.97 하락 → 진입 시그널 품질 문제
  2) XRP Fold 1,2 WR 0-33.3% → 약세장 가짜 시그널 진입
  3) SOL Fold 1 trades=0 → 필터가 너무 엄격하거나 타이밍 불일치
- 가설:
  A) ADX >= ADX_TH 추가 → 추세 부재 구간 진입 차단
     → 횡보장 가짜 돌파 필터, WR 개선
  B) OBV EMA slope > OBV_SLOPE_MIN → 거래량 방향이 가격과 일치
     → 거래량-가격 다이버전스 진입 차단
  C) ADX + OBV 복합 → 추세 강도 + 거래량 방향 이중 확인
- 탐색 그리드:
  ADX_TH: [15, 20, 25]           — 최소 ADX (추세 존재 확인)
  OBV_EMA_LB: [5, 10, 15]        — OBV EMA 기울기 lookback
  OBV_SLOPE_MIN: [0.0, 0.02, 0.05] — 최소 OBV 기울기 (정규화)
  = 3×3×3 = 27 combos
- 목표: OOS Sharpe >= 40 AND trades >= 15
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- c202 탐색 그리드 --
ADX_PERIOD = 14
ADX_TH_LIST = [15, 20, 25]
OBV_EMA_LB_LIST = [5, 10, 15]
OBV_SLOPE_MIN_LIST = [0.0, 0.02, 0.05]
...
def compute_adx(
    ...
    for i in range(period, n):
        if atr_w[i] > 0:
            plus_di[i] = 100.0 * plus_di_smooth[i] / atr_w[i]
            minus_di[i] = 100.0 * minus_di_smooth[i] / atr_w[i]
...
def compute_obv_slope(
    obv: np.ndarray, ema_lb: int,
) -> np.ndarray:
    """OBV EMA 기울기 (정규화: 현재 OBV 대비 비율)."""
    obv_ema = ema_calc(obv, ema_lb)
    n = len(obv)
    result = np.full(n, np.nan)
    for i in range(ema_lb + 1, n):
        if np.isnan(obv_ema[i]) or np.isnan(obv_ema[i - 1]):
            continue
        prev = obv_ema[i - 1]
        if abs(prev) < 1e-9:
            continue
        result[i] = (obv_ema[i] - prev) / abs(prev)
    return result
    ...
    while i < n - 1:
        ...
        if obv_slope_min > 0:
            if np.isnan(obv_slope_val):
                ...
                obv_ok = obv_slope_val >= obv_slope_min
...
def build_combos() -> list[dict]:
    combos = []
    for adx_th, obv_lb, obv_min in product(
        ADX_TH_LIST, OBV_EMA_LB_LIST, OBV_SLOPE_MIN_LIST,
    ):
        combos.append({
            "adx_th": adx_th,
            "obv_ema_lb": obv_lb,
            "obv_slope_min": obv_min,
        })
    return combos
...
def precompute_indicators(
    ...
    """EMA slope pctile + vol momentum + ATR pctile + ADX 사전 계산."""
    c = df["close"].values
    h = df["high"].values
    ...
    adx_arr = compute_adx(h, lo_arr, c, ADX_PERIOD)
    return ema_slope_pctile, vol_mom, atr_pctile, adx_arr


def precompute_obv_slopes(
    df: pd.DataFrame, obv_ema_lbs: list[int],
) -> dict[int, np.ndarray]:
    """OBV slope를 각 lookback별로 사전 계산."""
    c = df["close"].values
    v = df["volume"].values
    obv = compute_obv(c, v)
    result = {}
    for lb in obv_ema_lbs:
        result[lb] = compute_obv_slope(obv, lb)
    return result


def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 202 — ADX 추세강도 + OBV 방향 진입 게이트 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 40 AND trades >= 15")
    print("가설 A: ADX >= TH → 추세 부재 구간 진입 차단 (횡보장 필터)")
    print("가설 B: OBV slope >= MIN → 거래량-가격 다이버전스 차단")
    print("가설 C: ADX + OBV 복합 → 추세 강도 + 거래량 방향 이중 확인")
    print(f"기준선: c199 OOS +35.341, c192 OOS +47.314")
    ...
    for sym in sym_data_ok:
        ...
        if not df_tr.empty:
            ...
            esp, vol_mom, atr_pctile, adx_arr = precompute_indicators(df_tr)
            obv_slopes = precompute_obv_slopes(df_tr, OBV_EMA_LB_LIST)
            sym_train_cache[sym] = (
                df_tr, btc_c, btc_s, esp, vol_mom, atr_pctile, adx_arr, obv_slopes
            )
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            if sym not in sym_train_cache:
                continue
            df_tr, btc_c, btc_s, esp, vol_mom, atr_pctile, adx_a, obv_s = (
                sym_train_cache[sym])
            obv_slope = obv_s[combo["obv_ema_lb"]]
            r = backtest(df_tr, combo["adx_th"], combo["obv_ema_lb"],
                         combo["obv_slope_min"],
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['adx_th']:>4} {r['obv_ema_lb']:>4} "
            f"{r['obv_slope_min']:>5.2f} | "
    ...
    for r in valid:
        key = (r["adx_th"], r["obv_ema_lb"], r["obv_slope_min"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        obv_lb = params["obv_ema_lb"]
        obv_min = params["obv_slope_min"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                esp, vol_mom, atr_pctile, adx_a = precompute_indicators(df_test)
                obv_slopes = precompute_obv_slopes(df_test, [obv_lb])
                obv_slope = obv_slopes[obv_lb]
                r = backtest(df_test, adx_th, obv_lb, obv_min,
                             btc_c, btc_s, esp, vol_mom, atr_pctile,
                             adx_a, obv_slope)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: aTh={adx_th} oLB={obv_lb} "
                  f"oMin={obv_min:.2f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                esp, vol_mom, atr_pctile, adx_a = precompute_indicators(df_full)
                obv_slopes = precompute_obv_slopes(df_full, [obv_lb])
                obv_slope = obv_slopes[obv_lb]
                r = backtest(df_full, adx_th, obv_lb, obv_min,
                             btc_c, btc_s, esp, vol_mom, atr_pctile,
                             adx_a, obv_slope,
                             slippage=slip)
                sym_results.append(r)
    ...
    best = wf_sorted[0]
    adx_th = best["adx_th"]
    obv_lb = best["obv_ema_lb"]
    obv_min = best["obv_slope_min"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: aTh={adx_th} "
          f"oLB={obv_lb} oMin={obv_min:.2f}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c202 최적 (aTh={best['adx_th']} "
          f"oLB={best['obv_ema_lb']} "
          f"oMin={best['obv_slope_min']:.2f}): "
    ...
    print(f"★ OOS 최적: ADX_TH={best['adx_th']} "
          f"OBV_EMA_LB={best['obv_ema_lb']} "
          f"OBV_SLOPE_MIN={best['obv_slope_min']:.2f}")
    print(f"  (c199 고정: rTh={REGIME_TH} hiTP={HI_TP_BONUS} "
          f"hiTr={HI_TRAIL_RELAX} loSL={LO_SL_TIGHTEN})")
    print(f"  (c192 고정: ttA={TRAIL_TIGHTEN_AFTER} ttF={TRAIL_TIGHTEN_FACTOR})")
    print(f"  (c190 고정: vMomLB={VOL_MOM_LB} vMomMin={VOL_MOM_MIN} "
