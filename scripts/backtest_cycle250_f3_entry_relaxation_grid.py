"""
사이클 250: c243 최적 고정 + F3 진입 완화 3차원 그리드

c243 결과:
  avg OOS Sharpe: +31.850 PASS  F2: +33.667 PASS  F3: +13.521 FAIL
  F3: Sharpe=+13.521  WR=60.0%  trades=5  avg=+1.24%  MDD=-2.75%

c196 참고: 진입 필터 완화 → Sharpe +48.62 (유의미 엣지)
c244~c249: 필터 추가/강화 전부 실패 — 베이스라인 동일

문제 분석:
  - F3 구간(2025-12~2026-03)에서 거래 5건만 발생 → 필터가 너무 타이트
  - c244~c249은 모두 필터 추가/강화 방향 → 거래 수 줄거나 불변
  - c196은 필터 완화로 큰 개선 → 같은 방향을 F3 구간에 집중 적용
  - 핵심: breakout_margin + atr_pctile_min + vol_ratio_min 동시 완화
    → 이 3개가 F3에서 진입을 가장 많이 차단하는 게이트
  - 추가: BTC 모멘텀 최소값도 완화 시도 (BTC sideways면 진입 차단됨)
...
새 탐색 그리드 (c250):
  BREAKOUT_MARGIN: [0.0, 0.002, 0.005]     — 현재 0.005 → 0까지 완화
  ATR_PCTILE_MIN:  [15, 20, 30]             — 현재 30 → 15까지 완화
  VOL_RATIO_MIN:   [1.0, 1.2, 1.5]          — 현재 1.5 → 1.0까지 완화
  BTC_MOM_MIN:     [0.005, 0.01, 0.02]      — 현재 0.02 → 0.005까지 완화
  = 3 × 3 × 3 × 3 = 81 combos
...
# ─── c250 탐색 그리드 ─────────────────────────────────────────
BREAKOUT_MARGIN_LIST = [0.0, 0.002, 0.005]
ATR_PCTILE_MIN_LIST = [15, 20, 30]
VOL_RATIO_MIN_LIST = [1.0, 1.2, 1.5]
BTC_MOM_MIN_LIST = [0.005, 0.01, 0.02]
...
def main() -> None:
    print("=" * 80)
    print("=== c250: c243최적 고정 + F3 진입 완화 3차원 그리드 ===")
    ...
    print(f"탐색: BREAKOUT_MARGIN × ATR_PCTILE_MIN × VOL_RATIO_MIN × "
          f"BTC_MOM_MIN")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        BREAKOUT_MARGIN_LIST, ATR_PCTILE_MIN_LIST,
        VOL_RATIO_MIN_LIST, BTC_MOM_MIN_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")
    ...
    for gi, combo in enumerate(grid):
        bo_mgn, atr_pct_min, vol_rat_min, btc_mm = combo
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                ...
                raw = run_backtest(
                    ...
                    sp["atr_pctile"], sp["vol_sma"], sp["rsi"],
                    SYM_SL_SCALE[sym], sym,
                    bo_mgn, atr_pct_min, vol_rat_min, btc_mm,
                    window["oos_start"], window["oos_end"],
                    sp["index"],
                )
                agg = aggregate_trades(raw)
    ...
    print("=" * 80)
    hdr = (f"{'boMgn':>6} {'aPct':>5} {'vRat':>5} {'btcMM':>6} | "
    ...
    for r in valid[:15]:
        p = r["params"]
        print(
            f"{p[0]:>6.3f} {p[1]:>5} {p[2]:>5.1f} {p[3]:>6.3f} | "
    ...
    for val in BREAKOUT_MARGIN_LIST:
        ...
        if sub:
            ...
            print(f"  boMgn={val:.3f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")

    print("\n=== ATR Percentile Min 효과 ===")
    ...
    for val in BTC_MOM_MIN_LIST:
        ...
        if sub:
            ...
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  btcMM={val:.3f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  boMgn={p[0]:.3f} aPct={p[1]} vRat={p[2]:.1f} "
              f"btcMM={p[3]:.3f} | "
    ...
    if valid:
        ...
        print(f"  c250 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: boMgn={p[0]:.3f} aPct={p[1]} "
              f"vRat={p[2]:.1f} btcMM={p[3]:.3f}")
