"""
사이클 264: c262 최적 고정 + OBV(On-Balance Volume) 추세 진입 게이트

c262 최적 결과 (vmS=5 vmL=15 dvTh=-0.20 ctpB=1.0):
  avg OOS Sharpe: +62.227  F2: +34.013  F3: +120.208
  worst MDD: -4.98%  trades: 23  WR: 87.0%

문제:
  - 기존 진입 필터는 단순 거래량 스파이크(c258)와 다이버전스(c262)만 체크
  - 거래량이 순간적으로 급증해도 실제 매수세가 지속되지 않으면 허위 돌파
  - OBV는 거래량의 누적 방향성을 측정 → 돌파 전 구간에서 OBV가 상승 추세이면
    매수세가 축적되고 있다는 의미 = 더 신뢰할 수 있는 breakout

가설:
  - OBV N봉 기울기(선형회귀) > 양수 임계값이면 진입 허용
  - OBV 기울기가 음수면 매도 압력 = 진입 차단
  - OBV 추세가 강할수록 TP2 보너스를 줘서 수익 극대화
  - OBV 기울기 계산: (OBV[i] - OBV[i-N]) / N / price (정규화)

c262 최적 고정:
  vmS=5 vmL=15 dvTh=-0.20 ctpB=1.0
  (c260: brMin=0.50 cpMin=0.70 bLB=1)
  (c259: atrLB=15 atrTh=0.05 tp2B=1.0 trTA=0.5)
...
새 탐색 그리드 (c264):
  OBV_SLOPE_LB:      [5, 10, 15]          — OBV 기울기 lookback
  OBV_SLOPE_MIN:      [0.0, 0.001, 0.003]  — 최소 양의 기울기 (정규화)
  OBV_TP2_SCALE:      [0.0, 0.3, 0.5]      — 강한 OBV 추세 시 TP2 보너스
  = 3 × 3 × 3 = 27 combos
"""
...
# ─── c264 탐색 그리드 ─────────────────────────────────────────
OBV_SLOPE_LB_LIST = [5, 10, 15]
OBV_SLOPE_MIN_LIST = [0.0, 0.001, 0.003]
OBV_TP2_SCALE_LIST = [0.0, 0.3, 0.5]
...
def compute_obv_slope(
    obv: np.ndarray, closes: np.ndarray, lookback: int,
) -> np.ndarray:
    """OBV 기울기 (가격 정규화). (OBV[i] - OBV[i-lb]) / lb / price."""
    n = len(obv)
    result = np.full(n, np.nan)
    for i in range(lookback, n):
        if closes[i] > 0:
            result[i] = (obv[i] - obv[i - lookback]) / lookback / closes[i]
    return result
...
def run_backtest(
    ...
    sym: str,
    # c262 고정
    vol_sma_short_arr: np.ndarray,
    vol_sma_long_arr: np.ndarray,
    # c264 탐색 파라미터
    ...
    warmup = max(DC_UPPER_LB, DC_LOWER_LB, BTC_SMA_PERIOD, 60,
                 ...
                 ATR_MOM_LB + 5, VOL_MOM_LONG + 5) + 10
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            if BODY_LOOKBACK > 0:
                body_ok = True
                for bi in range(BODY_LOOKBACK):
                    idx = i - bi
                    if idx < 0:
                        body_ok = False
                        break
                    candle_range = h[idx] - lo[idx]
                    if candle_range <= 0:
                        body_ok = False
                        break
                    ...
                    if body_ratio < BODY_RATIO_MIN:
                        body_ok = False
                        break
                    close_pos = ((c[idx] - lo[idx]) / candle_range)
            ...
            # c262 고정: 거래량-모멘텀 다이버전스 필터
            is_converging = False
            if DIV_THRESH < 0.0:
                vs = vol_sma_short_arr[i]
                vl = vol_sma_long_arr[i]
                if (not np.isnan(vs) and not np.isnan(vl) and vl > 0):
                    vol_mom = (vs / vl) - 1.0
                    if vol_mom < DIV_THRESH:
                        continue
                    is_converging = vol_mom > 0.0

            # ── c264 핵심: OBV 추세 진입 게이트 ──
            obv_strong = False
            if obv_slope_min > 0.0:
                obs = obv_slope_arr[i]
                if np.isnan(obs):
                    continue
                if obs < obv_slope_min:
                    continue  # OBV 기울기 부족 → 진입 차단
                # 강한 OBV 추세 판정 (2배 이상)
                obv_strong = obs >= obv_slope_min * 2.0
            ...
            # TP2 조절: c259 + c262 + c264 신규
            eff_tp2 = BASE_TP2_MULT
            if atr_rising:
                eff_tp2 += TP2_BOOST
            if is_converging and CONV_TP2_BONUS > 0:
                eff_tp2 += CONV_TP2_BONUS
            if obv_strong and obv_tp2_scale > 0:
                eff_tp2 += obv_tp2_scale
...
def main() -> None:
    print("=" * 80)
    print("=== c264: c262최적 고정 + OBV 추세 진입 게이트 ===")
    print("=== 심볼: ETH/SOL/XRP | 240m | 슬리피지포함 | 다음봉시가진입 ===")
    print(f"c262 고정: vmS={VOL_MOM_SHORT} vmL={VOL_MOM_LONG} "
          f"dvTh={DIV_THRESH} ctpB={CONV_TP2_BONUS}")
    print(f"c260 고정: brMin={BODY_RATIO_MIN} cpMin={CLOSE_POS_MIN} "
          f"bLB={BODY_LOOKBACK}")
    print(f"c233 고정(고변동출구): trail={BASE_TRAIL_MULT} TP2={BASE_TP2_MULT} "
          f"SL={BASE_SL_MULT} MH={BASE_MAX_HOLD} TP1={BASE_TP1_MULT}")
    print(f"탐색: OBV_SLOPE_LB × OBV_SLOPE_MIN × OBV_TP2_SCALE")
    print("=" * 80)
    ...
    for sym in SYMBOLS:
        ...
        # c262 고정: 거래량 SMA
        vol_sma_short = sma_calc(v_arr, VOL_MOM_SHORT)
        vol_sma_long = sma_calc(v_arr, VOL_MOM_LONG)

        # c264 신규: OBV 계산
        obv_arr = compute_obv(c_arr, v_arr)
        ...
        sym_precomp[sym] = {
            ...
            "index": df.index,
            "vol_sma_short": vol_sma_short,
            "vol_sma_long": vol_sma_long,
            "obv": obv_arr,
        }

    # 그리드
    grid = list(product(
        OBV_SLOPE_LB_LIST, OBV_SLOPE_MIN_LIST, OBV_TP2_SCALE_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c262 최적 = OBV 필터 비활성)
    print("\n--- 베이스라인 (c262 최적) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for window in WINDOWS:
        ...
        for sym in SYMBOLS:
            sp = sym_precomp[sym]
            obv_slope_disabled = np.full(len(sp["c"]), np.nan)
            raw = run_backtest(
                ...
                sp["vol_sma_short"], sp["vol_sma_long"],
                obv_slope_disabled, 0.0, 0.0,
    ...
    for gi, combo in enumerate(grid):
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                sp = sym_precomp[sym]
                obv_slope = compute_obv_slope(sp["obv"], sp["c"], obv_lb)
                raw = run_backtest(
                    sp["c"], sp["o"], sp["h"], sp["lo"], sp["v"],
                    sp["dc_up"], sp["dc_lo"],
                    sp["atr"], sp["adx"],
    ...
    for val in OBV_SLOPE_LB_LIST:
        ...
        if sub:
            ...
            print(f"  obvLB={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== OBV_SLOPE_MIN 효과 ===")
    for val in OBV_SLOPE_MIN_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  obvMn={val:.3f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  obvLB={p[0]:.0f} obvMn={p[1]:.3f} obvTP={p[2]:.1f} | "
    ...
    # 최종 요약
    print("\n" + "=" * 80)
    print("=== c262 베이스라인 대비 비교 ===")
    if valid:
        b = valid[0]
        print(f"  c262 기준: avg_OOS={base_avg:+.3f} F2={base_f2:+.3f} "
              f"F3={base_f3:+.3f} worst_MDD={base_worst_mdd:+.2f}%")
        print(f"  c264 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: obvLB={p[0]:.0f} obvMn={p[1]:.3f} "
              f"obvTP={p[2]:.1f}")
        print(f"  (c262 고정: vmS={VOL_MOM_SHORT} vmL={VOL_MOM_LONG} "
              f"dvTh={DIV_THRESH} ctpB={CONV_TP2_BONUS})")
        print(f"  (c260 고정: brMin={BODY_RATIO_MIN} cpMin={CLOSE_POS_MIN} "
