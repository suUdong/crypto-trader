"""
사이클 255: c250 최적 고정 + ATR 레짐 적응형 모멘텀 임계값
...
문제:
  - F2 (2025-03 ~ 2025-11) 구간이 Sharpe +27.352로 목표 30 미달
  - 이 구간은 변동성이 낮은 횡보장 → 고정 MOM_MIN=0.02가 과도하게 필터링
  - F2에서 유효 신호가 차단되어 거래수 부족 or 진입 타이밍 지연

가설:
  - ATR percentile이 낮을 때(저변동성) 모멘텀 임계값을 낮추면 F2 거래 포착률 향상
  - ATR percentile이 높을 때(고변동성)는 기존 임계값 유지 → F1/F3 성과 보존
  - 추가로 BTC 모멘텀 임계값도 레짐 적응 가능 — 저변동성 시장에서 BTC도 움직임이 작음
...
새 탐색 그리드 (c255):
  MOM_LOW:      [0.005, 0.010, 0.015]  — 저변동성 시 모멘텀 임계값
  ATR_SPLIT:    [30, 40, 50]           — 저/고변동성 분기 ATR percentile
  BTC_MOM_LOW:  [0.005, 0.010, 0.015]  — 저변동성 시 BTC 모멘텀 임계값
  = 3 × 3 × 3 = 27 combos
...
ATR_PCTILE_MIN = 15
VOL_RATIO_MIN = 1.5
BTC_MOM_MIN_HIGH = 0.020  # 고변동성 시 기존 값

# ─── c255 탐색 그리드 ─────────────────────────────────────────
MOM_LOW_LIST = [0.005, 0.010, 0.015]
ATR_SPLIT_LIST = [30, 40, 50]
BTC_MOM_LOW_LIST = [0.005, 0.010, 0.015]
...
def run_backtest(
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c255 핵심: ATR 레짐 적응형 모멘텀 임계값 ──────
            is_low_vol = (not np.isnan(atr_pctile[i])
                          and atr_pctile[i] < atr_split)
            effective_mom_min = mom_low if is_low_vol else MOM_MIN_HIGH
            effective_btc_mom_min = btc_mom_low if is_low_vol else BTC_MOM_MIN_HIGH

            # 1차 모멘텀 (c241 고정 LB, 적응형 임계값)
            if i >= MOM_LB:
...
def main() -> None:
    ...
    # 그리드
    grid = list(product(
        MOM_LOW_LIST, ATR_SPLIT_LIST, BTC_MOM_LOW_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c250 최적 — 적응 없이 고정)
    print("\n--- 베이스라인 (c250 최적, 고정 임계값) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for window in WINDOWS:
        ...
        for sym in SYMBOLS:
            ...
            raw = run_backtest(
                ...
                MOM_MIN_HIGH, 0, BTC_MOM_MIN_HIGH,  # 고정 = split=0 (항상 고)
    ...
    for val in MOM_LOW_LIST:
        ...
        if sub:
            ...
            print(f"  mLow={val:.3f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")

    print("\n=== ATR_SPLIT 효과 ===")
    for val in ATR_SPLIT_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  aSpl={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")

    print("\n=== BTC_MOM_LOW 효과 ===")
    for val in BTC_MOM_LOW_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  bLow={val:.3f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  mLow={p[0]:.3f} aSpl={p[1]:.0f} bLow={p[2]:.3f} | "
    ...
    if valid:
        ...
        print(f"  c255 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: mLow={p[0]:.3f} aSpl={p[1]:.0f} "
              f"bLow={p[2]:.3f}")
        print(f"  (c250 고정: boMgn={BREAKOUT_MARGIN} aPct={ATR_PCTILE_MIN} "
