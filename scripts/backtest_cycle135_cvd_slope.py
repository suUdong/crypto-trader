"""
사이클 135: stealth_3gate cvd_slope 게이트 탐색
- 현재: cvd_slope 필터 없음 (미활용)
- 탐색: alt cvd_slope_threshold ∈ {None(OFF), 0.001, 0.005, 0.01}
- cvd_slope 정의: (close>open ? +vol : -vol).cumsum().diff(12) / vol_ma / 12
  → 양수 = 순매수 압력, 값이 클수록 강한 매수세
...
- 가설: 스텔스 진입 시 순매수 압력 확인이 신호 품질을 강화할 수 있음
"""
...
# 탐색 대상 cvd_slope 임계값 (None = OFF)
CVD_CANDIDATES: list[float | None] = [None, 0.001, 0.005, 0.01]
...
def compute_alt_entry(
    ...
    Gate 5 (optional): alt cvd_slope > cvd_threshold
    """
...
def main() -> None:
    t0 = time.time()
    print("=== stealth_3gate cvd_slope 게이트 탐색 (사이클 135) ===")
    ...
    print(f"탐색: cvd_slope_threshold ∈ {[str(x) if x is not None else 'None(OFF)' for x in CVD_CANDIDATES]}")
    ...
    print("\n[3/3] cvd_slope 임계값 탐색...")
    results = []

    for cvd_thresh in CVD_CANDIDATES:
        label_str = "None(OFF) 📌 (현재)" if cvd_thresh is None else f"{cvd_thresh:.3f}"
    ...
    for r in results:
        ...
        lbl = "None(OFF)" if r["cvd_thresh"] is None else f"{r['cvd_thresh']:.3f}"
        print(
            f"{lbl:<14} {r['alt_syms']:>9d} "
    ...
    # Recommendation
    print("\n=== 권고 ===")
    if baseline_w2 is not None and best is not None:
        delta = best["w2_sharpe"] - baseline_w2
        if best["cvd_thresh"] is None:
            print("현재 설정(cvd_slope 필터 없음) 유지 — cvd_slope 추가가 Sharpe를 개선하지 못함")
        elif delta > 0.3:
            print(f"cvd_slope > {best['cvd_thresh']:.3f} 추가 권장 (W2 Sharpe +{delta:.3f})")
            print(f"  daemon.toml: cvd_slope_threshold 추가 고려")
        else:
            print(f"cvd_slope > {best['cvd_thresh']:.3f} 미미하게 우세 (델타 {delta:+.3f}) — 현재 설정 유지 권장")
    else:
        print("통과 후보 없음 — 현재 설정(cvd_slope 미사용) 유지")
