"""
vpin_multi 사이클 192 — Entry Quality Score 기반 적응형 TP/Hold/SL
...
- c191 교훈:
  OBV+MACD 이진 확인 → TP/SL 변경 시 Sharpe 42→17 악화
  이진 분류(확인/미확인)가 너무 거칠어서 노이즈 유입
- 가설: 기존 지표(VPIN/RSI/MOM/VOL)의 연속형 품질 스코어 → TP/Hold/SL 그라데이션
  Q = w1*(VPIN_headroom) + w2*(RSI_headroom) + w3*(MOM_strength) + w4*(VOL_surge)
  각 항목 0~1 정규화, Q는 0~1 범위
  Q 높을수록: TP 확대, Hold 연장
  Q 낮을수록: SL 타이트닝
  핵심: c179 진입 모두 유지(trades ~60) + 연속 스케일링으로 정밀 조정
- 탐색 그리드:
  VPIN_WEIGHT: [0.3, 0.5]         — VPIN headroom 가중치
  RSI_WEIGHT:  [0.2, 0.3]         — RSI headroom 가중치
  MOM_WEIGHT:  [0.2, 0.3]         — MOM 강도 가중치
  VOL_WEIGHT:  [0.1, 0.2]         — VOL surge 가중치
  TP_SCALE_RANGE: [0.5, 1.0, 1.5] — Q=1일때 TP bonus ATR
  HOLD_SCALE_MAX: [3, 5]          — Q=1일때 Hold bonus 봉수
  SL_TIGHTEN_MAX: [0.05, 0.10]    — Q=0일때 SL 축소 비율
  = 2×2×2×2 × 3×2×2 = 192 combos
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드: Entry Quality Score 가중치 --
VPIN_WEIGHT_LIST = [0.3, 0.5]
RSI_WEIGHT_LIST = [0.2, 0.3]
MOM_WEIGHT_LIST = [0.2, 0.3]
VOL_WEIGHT_LIST = [0.1, 0.2]
TP_SCALE_RANGE_LIST = [0.5, 1.0, 1.5]   # Q=1일때 TP bonus ATR
HOLD_SCALE_MAX_LIST = [3, 5]             # Q=1일때 Hold bonus 봉수
SL_TIGHTEN_MAX_LIST = [0.05, 0.10]       # Q=0일때 SL 축소 비율

# -- 정규화 기준 (headroom 계산용) --
MOM_MAX = 0.005   # momentum 정규화 상한 (0.5%)
VOL_SURGE_MAX = 3.0  # volume/vol_sma 비율 상한
...
def compute_entry_quality(
    vpin_val: float, rsi_val: float, mom_val: float,
    vol_ratio: float,
    w_vpin: float, w_rsi: float, w_mom: float, w_vol: float,
) -> float:
    """진입 시점의 품질 스코어 (0~1). 기존 지표의 headroom 기반."""
    # VPIN headroom: VPIN이 낮을수록 좋음 (0.35 threshold)
    # vpin_val < 0.35 이므로, headroom = (0.35 - vpin_val) / 0.35
    vpin_score = min(1.0, max(0.0, (VPIN_LOW - vpin_val) / VPIN_LOW))

    # RSI headroom: RSI가 RSI_FLOOR에 가까울수록 상승 여지 큼
    rsi_score = min(1.0, max(0.0,
        (RSI_CEILING - rsi_val) / (RSI_CEILING - RSI_FLOOR)))

    # Momentum strength: 높을수록 좋음
    mom_score = min(1.0, max(0.0, mom_val / MOM_MAX))

    # Volume surge: vol/vol_sma 비율
    vol_score = min(1.0, max(0.0, (vol_ratio - 1.0) / (VOL_SURGE_MAX - 1.0)))

    # 가중 합산 (가중치 합 = 1로 정규화)
    w_total = w_vpin + w_rsi + w_mom + w_vol
    if w_total <= 0:
        return 0.5
    q = (w_vpin * vpin_score + w_rsi * rsi_score
         + w_mom * mom_score + w_vol * vol_score) / w_total
...
def backtest(
    df: pd.DataFrame,
    w_vpin: float, w_rsi: float, w_mom: float, w_vol: float,
    tp_scale_range: float,
    hold_scale_max: int,
    sl_tighten_max: float,
    ...
    while i < n - 1:
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
            ...
            # ★ Entry Quality Score 계산
            vol_ratio = v[i] / vol_sma_val if vol_sma_val > 0 else 1.0
            q_score = compute_entry_quality(
                vpin_val, rsi_val, mom_val, vol_ratio,
                w_vpin, w_rsi, w_mom, w_vol,
            )
            ...
            # ★ Q-score 기반 적응형 TP/SL/Hold
            # Q가 높을수록 TP 확대 + Hold 연장
            # Q가 낮을수록 SL 타이트닝
            effective_tp_mult = (
                (TP_BASE_ATR + TP_BONUS_ATR * rsi_ratio) * tp_scale
                + tp_scale_range * q_score  # Q에 비례하여 TP 보너스
            )

            tp_price = buy + atr_at_entry * effective_tp_mult

            # SL: Q가 낮을수록 타이트닝 (1-Q 비례)
            ...
            sl_tighten = sl_tighten_max * (1.0 - q_score)
            if sl_tighten > 0:
                effective_sl_mult *= (1.0 - sl_tighten)
                effective_sl_mult = max(0.1, effective_sl_mult)
            ...
            max_hold += int(hold_scale_max * q_score)
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 192 — Entry Quality Score 적응형 TP/Hold/SL ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 43.0 (c179 +42.878 대비 개선)")
    print("가설: 기존 지표의 연속형 Q-score → TP/Hold/SL 그라데이션 스케일링")
    print("  Q = w*(VPIN_headroom) + w*(RSI_headroom) + w*(MOM_strength) + w*(VOL_surge)")
    print("  Q 높음 → TP 확대 + Hold 연장 / Q 낮음 → SL 타이트닝")
    print("  핵심: c179 진입 모두 유지 + 연속 스케일링으로 정밀 조정")
    print(f"기준선: c179 OOS +42.878, WR ~45%, trades ~60")
    print(f"c191 교훈: 이진 OBV/MACD 확인 → Sharpe 42→17 악화")
    ...
    combos = list(product(
        VPIN_WEIGHT_LIST, RSI_WEIGHT_LIST, MOM_WEIGHT_LIST, VOL_WEIGHT_LIST,
        TP_SCALE_RANGE_LIST, HOLD_SCALE_MAX_LIST, SL_TIGHTEN_MAX_LIST,
    ))
    ...
    results: list[dict] = []
    for idx, (w_vpin, w_rsi, w_mom, w_vol,
              tp_sr, hd_sm, sl_tm) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, w_vpin, w_rsi, w_mom, w_vol,
                         tp_sr, hd_sm, sl_tm, btc_c, btc_s)
            sym_results.append(r)
        pooled = pool_results(sym_results)
        results.append({
            "w_vpin": w_vpin, "w_rsi": w_rsi, "w_mom": w_mom, "w_vol": w_vol,
            "tp_sr": tp_sr, "hd_sm": hd_sm, "sl_tm": sl_tm,
            **pooled,
        })
        if (idx + 1) % 48 == 0 or idx == len(combos) - 1:
    ...
    for rank, r in enumerate(top5, 1):
        print(f"  #{rank} wV={r['w_vpin']} wR={r['w_rsi']} wM={r['w_mom']} "
              f"wVol={r['w_vol']} tpSR={r['tp_sr']} hdSM={r['hd_sm']} "
              f"slTM={r['sl_tm']} → Sharpe={r['sharpe']:+.3f} "
    ...
    for rank, combo in enumerate(top5, 1):
        ...
        print(f"\n--- #{rank}: wV={w_vpin} wR={w_rsi} wM={w_mom} wVol={w_vol} "
              f"tpSR={tp_sr} hdSM={hd_sm} slTM={sl_tm} ---")
        ...
        for fi, fold in enumerate(WF_FOLDS, 1):
            ...
            for sym in sym_data_ok:
                ...
                if df_te.empty:
                    continue
                btc_c, btc_s = align_btc_to_symbol(df_te, df_btc_full, BTC_SMA_PERIOD)
                r = backtest(df_te, w_vpin, w_rsi, w_mom, w_vol,
                             tp_sr, hd_sm, sl_tm, btc_c, btc_s)
    ...
    w_vpin = best_combo["w_vpin"]
    w_rsi = best_combo["w_rsi"]
    w_mom = best_combo["w_mom"]
    w_vol = best_combo["w_vol"]
    tp_sr = best_combo["tp_sr"]
    hd_sm = best_combo["hd_sm"]
    sl_tm = best_combo["sl_tm"]

    print(f"최적: wV={w_vpin} wR={w_rsi} wM={w_mom} wVol={w_vol} "
          f"tpSR={tp_sr} hdSM={hd_sm} slTM={sl_tm}")
    ...
    for slip in SLIPPAGE_LEVELS:
        ...
        for sym in sym_data_ok:
            for fold in WF_FOLDS:
                ...
                r = backtest(df_te, w_vpin, w_rsi, w_mom, w_vol,
                             tp_sr, hd_sm, sl_tm, btc_c, btc_s, slip)
    ...
    # -- 심볼별 OOS 분해 --
    print("\n" + "=" * 80)
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: wV={w_vpin} wR={w_rsi} "
          f"wM={w_mom} wVol={w_vol} tpSR={tp_sr} hdSM={hd_sm} slTM={sl_tm}) ===")
    ...
    print(f"  c192 최적 (wV={w_vpin} wR={w_rsi} wM={w_mom} wVol={w_vol} "
          f"tpSR={tp_sr} hdSM={hd_sm} slTM={sl_tm}): "
    ...
    print(f"★ OOS 최적: wV={w_vpin} wR={w_rsi} wM={w_mom} wVol={w_vol} "
          f"tpSR={tp_sr} hdSM={hd_sm} slTM={sl_tm}")
    print(f"  (c179 고정: volTh={VOL_REGIME_THRESH} tpSc={HIGH_VOL_TP_SCALE} "
          f"trSc={HIGH_VOL_TRAIL_SCALE} hdSc={HIGH_VOL_HOLD_SCALE})")
    print(f"  (c177 고정: atrTh={ATR_PCTILE_THRESH} body={BODY_RATIO_MIN} "
