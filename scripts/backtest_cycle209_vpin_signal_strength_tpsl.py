"""
vpin_multi 사이클 209 — 시그널 강도 기반 적응형 TP/SL 스케일링
- 기반: c179 OOS Sharpe +42.878 (vol regime adaptive) — 현 최적
- 실패 분석:
  1. c200~c203 진입필터(CMF/OBV/RSI div) → 좋은 진입까지 필터링 → 악화
  2. c207 청산최적화(mom반전/시간감쇠/vol반전) → fat-tail 수익 파괴 → 악화
  3. 공통 원인: 진입을 줄이거나 이익을 일찍 끊으면 fat-tail 수익 손실
- 가설:
  진입은 그대로 두되, 시그널 품질에 따라 TP/SL 배수를 동적 조정
  → 강한 시그널(낮은 VPIN, 높은 모멘텀) = 더 넓은 TP (fat tail 수확)
  → 약한 시그널(높은 VPIN, 낮은 모멘텀) = 타이트 TP/SL (작은 이익+빠른 손절)
  이 방식은 진입을 막지 않으므로 거래수 유지, 이익 분포만 최적화
- 설계:
  진입: c179 100% 동일
  TP/SL 스케일링:
    signal_strength = w_vpin * (1 - vpin/vpin_low) + w_mom * (mom/mom_max)
    signal_strength = clamp(signal_strength, 0.3, 1.5)
    effective_tp = base_tp * (1 + tp_boost * signal_strength)
    effective_sl = base_sl * (1 - sl_tighten * (1 - signal_strength))
  → 강한 시그널: TP 최대 1+tp_boost*1.5 배, SL은 넓게 (수익 극대화)
  → 약한 시그널: TP 좁게, SL 타이트 (빠른 탈출)
- 탐색 그리드:
  W_VPIN: [0.3, 0.5, 0.7]         — VPIN 가중치
  W_MOM: [0.3, 0.5, 0.7]          — 모멘텀 가중치
  TP_BOOST: [0.0, 0.3, 0.5, 0.8]  — TP 확대 계수 (0=OFF)
  SL_TIGHTEN: [0.0, 0.2, 0.4]     — SL 축소 계수 (0=OFF)
  = 3×3×4×3 = 108조합
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
★슬리피지포함 | 🔄다음봉시가진입
...
# -- 탐색 그리드: 시그널 강도 TP/SL 스케일링 --
W_VPIN_LIST = [0.3, 0.5, 0.7]
W_MOM_LIST = [0.3, 0.5, 0.7]
TP_BOOST_LIST = [0.0, 0.3, 0.5, 0.8]     # 0=OFF (c179 baseline)
SL_TIGHTEN_LIST = [0.0, 0.2, 0.4]        # 0=OFF
# 총 조합: 3×3×4×3 = 108
...
def backtest(
    ...
    mom_p95 = float(np.percentile(valid_mom, 95)) if len(valid_mom) > 50 else 0.05
    ...
    while i < n - 1:
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
            ...
            # ★ 시그널 강도 계산
            # VPIN 강도: VPIN이 낮을수록 강함 (0~1)
            vpin_strength = max(0.0, 1.0 - vpin_val / VPIN_LOW)
            # 모멘텀 강도: 모멘텀이 클수록 강함 (0~1)
            mom_strength = min(1.0, max(0.0, mom_val / mom_p95)) if mom_p95 > 0 else 0.5
            # 가중 결합
            raw_signal = w_vpin * vpin_strength + w_mom * mom_strength
            # 정규화: w_vpin + w_mom 합으로 나누어 [0, 1] 범위
            signal_strength = raw_signal / (w_vpin + w_mom + 1e-9)
            signal_strength = max(0.2, min(1.5, signal_strength))

            # ★ 시그널 강도 기반 TP 스케일링
            tp_signal_scale = 1.0 + tp_boost * signal_strength
            # ★ 시그널 강도 기반 SL 스케일링 (약한 시그널 → 타이트)
            sl_signal_scale = 1.0 - sl_tighten * (1.0 - signal_strength)
            sl_signal_scale = max(0.5, sl_signal_scale)  # SL 최소 50%

            # TP (시그널 강도 적용)
            effective_tp_mult = (
                (TP_BASE_ATR + TP_BONUS_ATR * rsi_ratio) * tp_scale * tp_signal_scale
            )
            ...
            effective_sl_mult *= sl_signal_scale
            if is_high_vol:
            ...
            # Trail (시그널 강도에 비례해서 조정)
            trail_signal_scale = 1.0 + tp_boost * signal_strength * 0.5
            effective_trail_mult = (
                (TRAIL_BASE_ATR + TRAIL_BONUS_ATR * (1.0 - rsi_ratio))
                * trail_scale * trail_signal_scale
            )
            min_profit_dist = (
                atr_at_entry * MIN_PROFIT_ATR * trail_scale * trail_signal_scale
            )
            ...
            for j in range(i + 2, min(i + 1 + max_hold, n)):
                ...
                if unrealized >= min_profit_dist:
                    if peak_price - current_price >= atr_at_entry * effective_trail_mult:
                        exit_ret = (
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 209 — 시그널 강도 기반 적응형 TP/SL 스케일링 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe > 42.878 (c179 baseline)")
    print("가설: 진입 건드리지 않고, 시그널 품질로 TP/SL 배수 동적 조정")
    print("  강한 시그널(낮은 VPIN+높은 MOM) → TP 확대 (fat tail 수확)")
    print("  약한 시그널(높은 VPIN+낮은 MOM) → 타이트 TP/SL (빠른 탈출)")
    ...
    combos = list(product(
        W_VPIN_LIST, W_MOM_LIST, TP_BOOST_LIST, SL_TIGHTEN_LIST,
    ))
    ...
    results: list[dict] = []
    for idx, (wv, wm, tp_b, sl_t) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, wv, wm, tp_b, sl_t, btc_c, btc_s)
            sym_results.append(r)
        pooled = pool_results(sym_results)
        results.append({
            "wv": wv, "wm": wm, "tp_b": tp_b, "sl_t": sl_t,
            **pooled,
        })
        if (idx + 1) % 20 == 0 or idx == len(combos) - 1:
    ...
    for rank, r in enumerate(top5, 1):
        print(f"  #{rank} wVPIN={r['wv']} wMOM={r['wm']} "
              f"tpBoost={r['tp_b']} slTighten={r['sl_t']} → "
    ...
    for rank, combo in enumerate(top5, 1):
        ...
        print(f"\n--- #{rank}: wVPIN={wv} wMOM={wm} "
              f"tpBoost={tp_b} slTighten={sl_t} ---")
        ...
        for fi, fold in enumerate(WF_FOLDS, 1):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_te, wv, wm, tp_b, sl_t,
    ...
    sl_t = best_combo["sl_t"]

    print(f"최적: wVPIN={wv} wMOM={wm} tpBoost={tp_b} slTighten={sl_t}")
    ...
    for slip in SLIPPAGE_LEVELS:
        ...
        for sym in sym_data_ok:
            for fold in WF_FOLDS:
                ...
                btc_c, btc_s = align_btc_to_symbol(
                    df_te, df_btc_full, BTC_SMA_PERIOD)
                r = backtest(df_te, wv, wm, tp_b, sl_t, btc_c, btc_s, slip)
    ...
    # -- 심볼별 OOS 분해 --
    print("\n" + "=" * 80)
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: wVPIN={wv} wMOM={wm} "
          f"tpBoost={tp_b} slTighten={sl_t}) ===")
    ...
    print(f"  c209 최적 (wVPIN={wv} wMOM={wm} tpBoost={tp_b} "
          f"slTighten={sl_t}): avg_OOS={best_oos_sharpe:+.3f} n={total_n}")
    ...
    print(f"★ OOS 최적: wVPIN={wv} wMOM={wm} tpBoost={tp_b} "
          f"slTighten={sl_t}")
    print(f"  (c179 고정: volTh={VOL_REGIME_THRESH} tpSc={HIGH_VOL_TP_SCALE} "
          f"trSc={HIGH_VOL_TRAIL_SCALE} hdSc={HIGH_VOL_HOLD_SCALE})")
    print(f"  (c177 고정: atrTh={ATR_PCTILE_THRESH} body={BODY_RATIO_MIN} "
