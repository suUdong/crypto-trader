"""
vpin_multi 사이클 185 — 모멘텀 가속도 진입 + ATR 적응형 exit 스케일링
- 기반: c180/c176 OOS Sharpe +16.345, WR 41.1%, trades 58
  최적: vsMul=1.0 cBars=2 cWin=4 cpMin=0.0 (필터 전부 비활성 = c176 동일)
  고정: VPIN=0.35 MOM=0.0007 Hold=20 CD=4
  c176: atrLB=60 atrTh=30 body=0.7
  c164: dLB=3 dMin=0.0 SL=0.4-0.2 vMul=0.8
  TP/Trail: TP=4.0+2.0 Trail=0.3+0.2 minP=1.5 BTC_SMA=200
- c180 교훈:
  1) 거래량 서지/양봉 연속/캔들 위치 필터 전부 비활성이 최적 → 폐기
  2) SOL Fold2 Sharpe=-5.6 (WR 28.6%, avg -0.21%) 가 전체 드래그
  3) XRP 평균 +25.052 vs SOL +7.544 → 심볼간 exit 최적점 상이
- 가설:
  A) 모멘텀 가속도: mom[i] - mom[i-accelLB] > 임계값
     → 단순 모멘텀(1차 미분)이 아닌 가속(2차 미분) = 추세 강화 구간에서만 진입
     → SOL Fold2 같은 약한 추세 진입 회피
  B) ATR 적응형 exit 스케일링: 현재 ATR / 최근 N봉 평균 ATR 비율로
     TP/SL 배수 동적 조정 → 변동성 높은 구간은 SL 확대+TP 확대,
     낮은 구간은 SL 축소+TP 축소 → 심볼별 자동 적응
  C) A+B 조합
- 탐색 그리드:
  ACCEL_LB: [3, 5, 8]         — 가속도 lookback
  ACCEL_MIN: [0.0, 0.0003, 0.0006]  — 최소 가속도 (0=비활성)
  ATR_ADAPT: [0, 1]           — ATR 적응형 exit 활성화
  ATR_ADAPT_LB: [40, 60]      — ATR 평균 lookback (ATR_ADAPT=1일 때만)
  = 3×3×(1 + 1×2) = 27 combos
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드 --
ACCEL_LB_LIST = [3, 5, 8]             # 모멘텀 가속도 lookback
ACCEL_MIN_LIST = [0.0, 0.0003, 0.0006]  # 최소 가속도 임계값 (0=비활성)
ATR_ADAPT_LIST = [0, 1]               # ATR 적응형 exit 활성화
ATR_ADAPT_LB_LIST = [40, 60]          # ATR 평균 lookback
...
def backtest(
    ...
    warmup = max(BUCKET_COUNT, EMA_PERIOD, RSI_PERIOD + 1,
                 ...
                 ATR_PCTILE_LB, accel_lb + MOM_LOOKBACK, 50) + 5
    if atr_adapt == 1:
        warmup = max(warmup, atr_adapt_lb + ATR_PERIOD + 5)
    i = warmup
    ...
    while i < n - 1:
        ...
        accel_ok = True
        if accel_min > 0:
            mom_prev_idx = i - accel_lb
            if mom_prev_idx < MOM_LOOKBACK or np.isnan(mom_arr[mom_prev_idx]):
                accel_ok = False
            else:
                accel = mom_val - mom_arr[mom_prev_idx]
                accel_ok = accel >= accel_min

        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
                and atr_pctile_ok and body_ok and accel_ok):
            ...
            # === 신규 B: ATR 적응형 exit 스케일링 ===
            atr_scale = 1.0
            if atr_adapt == 1:
                ar = atr_ratio_arr[i]
                if not np.isnan(ar):
                    # 변동성 높으면 exit 배수 확대, 낮으면 축소
                    # 0.7~1.5 범위로 클램프
...
def build_combos() -> list[dict]:
    """탐색 조합 생성. ATR_ADAPT=0이면 ATR_ADAPT_LB는 의미 없으므로 40 고정."""
    combos = []
    for accel_lb, accel_min in product(ACCEL_LB_LIST, ACCEL_MIN_LIST):
        # ATR adapt OFF
        combos.append({
            "accel_lb": accel_lb, "accel_min": accel_min,
            "atr_adapt": 0, "atr_adapt_lb": 40,
        })
        # ATR adapt ON with each lookback
        for atr_lb in ATR_ADAPT_LB_LIST:
            combos.append({
                "accel_lb": accel_lb, "accel_min": accel_min,
                "atr_adapt": 1, "atr_adapt_lb": atr_lb,
            })
    return combos
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 185 — 모멘텀 가속도 + ATR 적응형 exit ===")
    print(f"심볼: {', '.join(SYMBOLS)}  목표: OOS Sharpe >= 17.0 (c176 +16.3 초과)")
    print("가설 A: 모멘텀 가속도 — mom 증가 속도로 추세 강화 구간 선별")
    print("가설 B: ATR 적응형 exit — ATR/avg 비율로 TP/SL 동적 스케일링")
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, combo["accel_lb"], combo["accel_min"],
                         combo["atr_adapt"], combo["atr_adapt_lb"],
                         btc_c, btc_s)
    ...
    hdr = (f"{'accLB':>5} {'accMin':>7} {'adapt':>5} {'adLB':>5} | "
    ...
    for r in valid[:16]:
        ...
        ad_lb = r["atr_adapt_lb"] if r["atr_adapt"] == 1 else "-"
        print(
            f"{r['accel_lb']:>5} {r['accel_min']:>7.4f} "
            f"{r['atr_adapt']:>5} {str(ad_lb):>5} | "
    ...
    for r in valid:
        key = (r["accel_lb"], r["accel_min"], r["atr_adapt"], r["atr_adapt_lb"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        atr_adapt = params["atr_adapt"]
        atr_adapt_lb = params["atr_adapt_lb"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, accel_lb, accel_min,
                             atr_adapt, atr_adapt_lb,
                             btc_c, btc_s)
        ...
        if oos_sharpes:
            ...
            ad_lb_str = str(atr_adapt_lb) if atr_adapt == 1 else "-"
            print(f"  #{rank}: accLB={accel_lb} accMin={accel_min:.4f} "
                  f"adapt={atr_adapt} adLB={ad_lb_str} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        atr_adapt_lb = params["atr_adapt_lb"]
        ad_lb_str = str(atr_adapt_lb) if atr_adapt == 1 else "-"
        print(f"\n--- #{rank}: accLB={accel_lb} accMin={accel_min:.4f} "
              f"adapt={atr_adapt} adLB={ad_lb_str} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, accel_lb, accel_min,
                             atr_adapt, atr_adapt_lb,
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    accel_lb = best["accel_lb"]
    accel_min = best["accel_min"]
    atr_adapt = best["atr_adapt"]
    atr_adapt_lb = best["atr_adapt_lb"]

    print(f"\n{'=' * 80}")
    ad_lb_str = str(atr_adapt_lb) if atr_adapt == 1 else "-"
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: accLB={accel_lb} "
          f"accMin={accel_min:.4f} adapt={atr_adapt} adLB={ad_lb_str}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    ad_lb_str = str(best["atr_adapt_lb"]) if best["atr_adapt"] == 1 else "-"
    print(f"  c185 최적 (accLB={best['accel_lb']} "
          f"accMin={best['accel_min']:.4f} "
          f"adapt={best['atr_adapt']} adLB={ad_lb_str}): "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: ACCEL_LB={best['accel_lb']} "
          f"ACCEL_MIN={best['accel_min']:.4f} "
          f"ATR_ADAPT={best['atr_adapt']} "
          f"ATR_ADAPT_LB={ad_lb_str}")
