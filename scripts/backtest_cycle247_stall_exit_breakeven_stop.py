"""
사이클 247: c243 최적 고정 + 모멘텀 정체 조기 청산 + 브레이크이븐 스탑
...
c245 교훈:
  - RSI exit로 F3 +15.8 개선했으나 F2 -2.9 악화 — 지표 기반 출구는 트레이드오프 큼
  - 더 단순한 출구 개선이 필요

문제 분석:
  - F3 WR 60%: 진입 후 모멘텀이 이어지지 않고 반전하는 케이스
  - 가설 1: "정체 감지" — 진입 후 N봉 내 새 고점 못 만들면 이익 보호 청산
    → 고점 갱신 없음 = 모멘텀 소진 → 반전 전 청산
    → RSI와 달리 순수 가격 기반 — 더 로버스트
  - 가설 2: "브레이크이븐 스탑" — X% 수익 도달 후 SL을 진입가로 이동
    → 수익 거래가 손실로 반전되는 것을 방지
    → TP1 분할익절과 독립적으로 작동
  - 진입 필터 변경 없음 — 출구 로직만 추가
...
새 탐색 그리드 (c247):
  STALL_BARS:         [0, 5, 8, 12]   — 고점 미갱신 허용 봉수 (0=OFF)
  STALL_MIN_PROFIT:   [0.5, 1.0, 2.0] — 정체 청산 최소 수익률(%) (이익 상태에서만)
  BE_TRIGGER_PCT:     [0.0, 1.0, 2.0, 3.0] — 브레이크이븐 활성화 수익률(%) (0=OFF)
  = 4 × 3 × 4 = 48 combos
...
# ─── c247 탐색 그리드 ─────────────────────────────────────────
STALL_BARS_LIST = [0, 5, 8, 12]           # 고점 미갱신 허용 봉수 (0=OFF)
STALL_MIN_PROFIT_LIST = [0.5, 1.0, 2.0]   # 정체 청산 최소 수익률(%)
BE_TRIGGER_PCT_LIST = [0.0, 1.0, 2.0, 3.0]  # 브레이크이븐 활성화 수익률(%)
...
def run_backtest(
    ...
    # c247 새 파라미터
    stall_bars: int,
    stall_min_profit: float,
    be_trigger_pct: float,
    oos_start: str,
    oos_end: str,
    index: pd.DatetimeIndex,
) -> list[dict]:
    """c243 최적 고정 + 모멘텀 정체 출구 + 브레이크이븐 스탑."""
    n = len(c)
    trades: list[dict] = []
    position = None
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            if cp > position["peak"]:
                position["peak"] = cp
                position["last_high_bar"] = i  # 마지막 고점 봉
            ...
            if be_trigger_pct > 0.0:
                current_ret_pct = (cp / position["entry_price"] - 1) * 100
                if (current_ret_pct >= be_trigger_pct
                        and not position.get("be_active", False)):
                    # SL을 진입가로 이동 (수수료 고려)
                    be_price = position["entry_price"] * (1 + FEE * 2)
                    if be_price > position["sl_price"]:
            ...
            if (exit_reason is None
                    and stall_bars > 0
                    and bars_held >= stall_bars):
                bars_since_high = i - position.get(
                    "last_high_bar", position["entry_bar"])
                current_ret_pct = (cp / position["entry_price"] - 1) * 100
                if (bars_since_high >= stall_bars
                        and current_ret_pct >= stall_min_profit):
                    exit_reason = "STALL"
...
def main() -> None:
    print("=" * 80)
    print("=== c247: c243최적 고정 + 모멘텀 정체 출구 + 브레이크이븐 스탑 ===")
    ...
    print(f"탐색: STALL_BARS × STALL_MIN_PROFIT × BE_TRIGGER_PCT")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        STALL_BARS_LIST, STALL_MIN_PROFIT_LIST, BE_TRIGGER_PCT_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c243 최적, 새 출구 OFF)
    print("\n--- 베이스라인 (c243 최적, 정체출구=OFF, BE=OFF) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for gi, combo in enumerate(grid):
        st_bars, st_min_pft, be_pct = combo
    ...
    # 결과 정렬
    valid = [r for r in all_results if r["total_n"] >= 15]
    valid.sort(
        key=lambda x: (
            x["f3_sharpe"] >= 20,
            x["avg_sharpe"] >= 28,
    ...
    print(f"\n유효 조합 (n>=15): {len(valid)}/{len(all_results)}")

    print("\n" + "=" * 80)
    print("=== Top 15 결과 (F3개선 우선) ===")
    print("=" * 80)
    hdr = (f"{'stBr':>5} {'stPf':>5} {'bePc':>5} | "
    ...
    for val in STALL_BARS_LIST:
        ...
        if sub:
            ...
            print(f"  stBr={val}: top5 avgSh={avg_sh:+.3f}  F3={avg_f3:+.3f}")

    print("\n=== 정체 청산 최소 수익률 효과 ===")
    for val in STALL_MIN_PROFIT_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  stPf={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")

    print("\n=== 브레이크이븐 활성화 수익률 효과 ===")
    for val in BE_TRIGGER_PCT_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  bePc={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  stBr={p[0]} stPf={p[1]:.1f} bePc={p[2]:.1f} | "
    ...
    if valid:
        ...
        print(f"  c247 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: stBr={p[0]} stPf={p[1]:.1f} "
              f"bePc={p[2]:.1f}")
        print(f"  (c243 고정: btcMLB={BTC_MOM_LB} btcMMin={BTC_MOM_MIN} "
              f"btcADX={BTC_ADX_GATE} boMgn={BREAKOUT_MARGIN})")
