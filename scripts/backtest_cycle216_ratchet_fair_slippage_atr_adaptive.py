#!/usr/bin/env python3
"""
사이클 216: c212 래칫 스탑 — slippage=0.0005 공정비교 + ATR-adaptive 트리거 + 심볼별 래칫 모드
- 기반: c212 래칫 스탑 avg OOS Sharpe +17.408
  c212 핵심 발견: slippage=0.001 이중적용이 주요 하락원인 (c179는 0.0005)
  XRP F3 Sharpe +43.381 → 래칫이 XRP에 극도 유효
  SOL F2 Sharpe -1.485 → 래칫이 SOL에 역효과
- 문제:
  1) c212는 slippage=0.001(이중)로 c179(0.0005)와 불공정 비교
  2) 고정 ATR 배수 트리거 → 변동성 높은/낮은 심볼에 동일 기준 부적절
  3) 래칫이 모든 심볼에 동일 적용 → XRP에만 유효할 가능성
- 가설:
  A) slippage=0.0005로 통일 → 순수 래칫 효과 측정
  B) ATR percentile 기반 adaptive 트리거: 고변동 시 더 먼 트리거, 저변동 시 가까운 트리거
     → beTr/lkTr에 vol_scale 곱: high_vol=wider, low_vol=tighter
  C) 심볼별 래칫 모드: all(전체) / eth_xrp(SOL 제외) / xrp_only
     → SOL은 래칫 OFF가 최적일 수 있음
  D) lockPct 세밀 탐색: [0.5, 0.6, 0.7, 0.8, 0.9]
...
- 탐색 그리드:
  BE_TRIGGER_ATR: [0, 0.5, 1.0, 1.5]         — breakeven trigger (0=OFF)
  LOCK_TRIGGER_ATR: [0, 1.5, 2.0, 2.5, 3.0]  — profit lock trigger (0=OFF)
  LOCK_PCT: [0.5, 0.7, 0.9]                   — profit lock %
  VOL_ADAPT: [0, 1]                            — 0=고정, 1=ATR pctile 스케일
  SYM_MODE: [all, no_sol, xrp_only]            — 래칫 적용 범위
  = 4×5×3×2×3 = 360 조합 (be=0 & lk=0 중복 제거 → ~200 유효)
- 목표: OOS Sharpe >= 43 (c179 돌파) AND trades >= 40
- 3-fold WF + 슬리피지 스트레스
- slippage=0.0005 (c179 동일 기준)
"""
...
FEE = 0.0005
SLIPPAGE = 0.0005  # c179 동일 기준 (c212의 0.001 이중적용 수정)
...
# -- c216 탐색 그리드 --
BE_TRIGGER_LIST = [0.0, 0.5, 1.0, 1.5]
LOCK_TRIGGER_LIST = [0.0, 1.5, 2.0, 2.5, 3.0]
LOCK_PCT_LIST = [0.5, 0.7, 0.9]
VOL_ADAPT_LIST = [0, 1]
SYM_MODE_LIST = ["all", "no_sol", "xrp_only"]
...
def backtest(
    ...
    래칫 로직:
    - L1 (breakeven): 수익 >= be_trigger_atr * ATR → SL을 진입가(breakeven)로 이동
    - L2 (profit lock): 수익 >= lock_trigger_atr * ATR → SL을 진입가 + 수익*lock_pct로
    - vol_adapt=1이면: 고변동 시 트리거를 1.3x, 저변동 시 0.8x 스케일
    """
    c = df["close"].values
    o = df["open"].values
    h = df["high"].values
    ...
    while i < n - 1:
        ...
        # === c216: 래칫 스탑 설정 (vol-adaptive) ===
        if ratchet_enabled and (be_trigger_atr > 0 or lock_trigger_atr > 0):
            if vol_adapt == 1:
                # 고변동: 트리거 넓게(1.3x), 저변동: 좁게(0.8x)
                vol_scale = 1.3 if is_high_vol else 0.8
            else:
                vol_scale = 1.0
            eff_be_trigger = be_trigger_atr * vol_scale * atr_at_entry
            eff_lock_trigger = lock_trigger_atr * vol_scale * atr_at_entry
            ratchet_active = True
        else:
            eff_be_trigger = 0.0
        ...
        for j in range(i + 2, min(i + 1 + max_hold, n)):
            ...
            if ratchet_active:
                ...
                if (not be_locked and eff_be_trigger > 0
                    ...
                    sl_price = max(sl_price, buy)  # SL을 breakeven으로
                    be_locked = True
...
def build_combos() -> list[dict]:
    """Build parameter grid, deduplicating ratchet-OFF combos."""
    combos = []
    seen_off: set[tuple] = set()
    for be, lk, pct, va, sm in product(
        BE_TRIGGER_LIST, LOCK_TRIGGER_LIST, LOCK_PCT_LIST,
        VOL_ADAPT_LIST, SYM_MODE_LIST,
    ):
        # If both triggers are 0, ratchet is OFF → only need one combo per sym_mode
        if be == 0.0 and lk == 0.0:
            key = (sm,)
            if key in seen_off:
                continue
            seen_off.add(key)
            combos.append({
                "be_trigger": 0.0, "lock_trigger": 0.0,
                "lock_pct": 0.0, "vol_adapt": 0, "sym_mode": sm,
            })
            continue
        # If only be > 0 and lk = 0, pct is irrelevant
        if lk == 0.0:
            key = (be, lk, va, sm)
            if key in seen_off:
                continue
            seen_off.add(key)
            combos.append({
                "be_trigger": be, "lock_trigger": 0.0,
                "lock_pct": 0.0, "vol_adapt": va, "sym_mode": sm,
            })
            continue
        combos.append({
            "be_trigger": be, "lock_trigger": lk,
            "lock_pct": pct, "vol_adapt": va, "sym_mode": sm,
        })
    return combos


def sym_ratchet_enabled(sym: str, sym_mode: str) -> bool:
    """Check if ratchet is enabled for a symbol given the mode."""
    if sym_mode == "all":
        return True
    if sym_mode == "no_sol":
        return sym != "KRW-SOL"
    if sym_mode == "xrp_only":
        return sym == "KRW-XRP"
    return True
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi c216 — 래칫 스탑 공정비교(slip=0.0005) + "
          "ATR-adaptive + 심볼별 모드 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 43 (c179 돌파) AND trades >= 40")
    print("가설: c212 slip=0.001 이중적용 수정 → 래칫 순수 효과 측정, "
          "XRP 래칫 극도 유효(F3 +43.381)")
    ...
    print(f"슬리피지: {SLIPPAGE} (c179 동일 — c212의 0.001 이중적용 수정)")
    print("=" * 80)
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            ratchet_on = sym_ratchet_enabled(sym, combo["sym_mode"])
            r = backtest(
                df_tr,
                combo["be_trigger"], combo["lock_trigger"],
                combo["lock_pct"], combo["vol_adapt"],
                ratchet_on,
    ...
    hdr = (f"{'beTr':>5} {'lkTr':>5} {'lkPct':>6} {'vAdp':>5} {'sMode':>9} | "
    ...
    for r in valid[:20]:
        ...
        print(
            f"{r['be_trigger']:>5.1f} {r['lock_trigger']:>5.1f} "
            f"{r['lock_pct']:>6.2f} {r['vol_adapt']:>5} "
            f"{r['sym_mode']:>9} | "
    ...
    for r in valid:
        key = (r["be_trigger"], r["lock_trigger"], r["lock_pct"],
               r["vol_adapt"], r["sym_mode"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                ratchet_on = sym_ratchet_enabled(sym, params["sym_mode"])
                r = backtest(
                    df_test,
                    params["be_trigger"], params["lock_trigger"],
                    params["lock_pct"], params["vol_adapt"],
                    ratchet_on,
                    btc_c, btc_s, SLIPPAGE)
        ...
        if oos_sharpes:
            ...
            print(
                f"\n[{rank}] beTr={params['be_trigger']:.1f} "
                f"lkTr={params['lock_trigger']:.1f} "
                f"lkPct={params['lock_pct']:.2f} "
                f"vAdp={params['vol_adapt']} "
                f"sMode={params['sym_mode']}  "
    ...
    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: beTr={best['be_trigger']:.1f} "
          f"lkTr={best['lock_trigger']:.1f} lkPct={best['lock_pct']:.2f} "
          f"vAdp={best['vol_adapt']} sMode={best['sym_mode']}) ===")
    ...
    print(f"  Δ trades: {best['total_n'] - 60}")
    ...
    for slip_level in SLIPPAGE_LEVELS:
        ...
        for fold in WF_FOLDS:
            ...
            for sym in sym_data_ok:
                ...
                btc_c, btc_s = align_btc_to_symbol(
                    df_test, df_btc_full, BTC_SMA_PERIOD)
                ratchet_on = sym_ratchet_enabled(sym, best["sym_mode"])
                r = backtest(
                    df_test,
                    best["be_trigger"], best["lock_trigger"],
                    best["lock_pct"], best["vol_adapt"],
                    ratchet_on,
    ...
    # -- 래칫 OFF (순수 c179) 대조군 --
    print(f"\n{'=' * 80}")
    print("=== 래칫 OFF (순수 c179 재현, slip=0.0005) 대조군 ===")
    ...
    print(f"  c179 재현 (slip=0.0005): avg Sharpe={ctrl_avg:+.3f} n={ctrl_trades}")
    for line in ctrl_fold_details:
        print(line)
    print(f"  c216 최적 vs c179 재현: Δ = {best['avg_oos'] - ctrl_avg:+.3f}")

    # -- 최종 요약 --
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    best_fd = best["fold_details"]
    print(f"★ OOS 최적: beTr={best['be_trigger']:.1f} "
          f"lkTr={best['lock_trigger']:.1f} lkPct={best['lock_pct']:.2f} "
          f"vAdp={best['vol_adapt']} sMode={best['sym_mode']}")
