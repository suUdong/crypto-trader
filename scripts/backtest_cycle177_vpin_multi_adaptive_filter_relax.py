"""
vpin_multi 사이클 177 — c176 최적 기반 신호강도 적응적 필터 완화
- 기반: c176 OOS Sharpe +16.345, WR 41.1%, trades 58 (ETH+SOL+XRP, 3-fold)
...
- c176 약점:
  1) trades 58 (c165 319 대비 82% 감소) → 필터가 과도하게 거래 차단
  2) SOL Fold 2 Sharpe=-5.596 → 특정 시장 상황에서 필터가 역효과
  3) XRP 총 11거래 → 통계적 유의성 부족
- 가설:
  A) 신호강도 적응적 완화: VPIN이 매우 낮을 때(강한 독성) body/ATR 필터 완화
     → 강한 신호에서 거래 회복, 약한 신호에서만 엄격 필터 유지
  B) ATR pctile 정밀 그리드: 20~40 범위 세분화 → 최적 임계값 정밀 탐색
  C) body ratio 정밀 그리드: 0.4~0.8 세분화 → trade count 회복점 탐색
  D) VPIN 강도 임계값 그리드: 언제 완화할지 탐색
- 탐색 그리드: 4 atrTh × 4 body × 3 vpin_relax_th × 2 relax_scale = 96 combos
  atrLB=60 고정 (c176 최적), c165 파라미터 전부 고정
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드 --
ATR_PCTILE_THRESH_LIST = [20, 25, 30, 40]       # c176 최적 30 주변 정밀 탐색
BODY_RATIO_MIN_LIST = [0.4, 0.5, 0.6, 0.7]      # c176 최적 0.7 주변 + 완화
VPIN_RELAX_THRESH_LIST = [0.20, 0.25, 0.30]      # VPIN < 이 값이면 필터 완화
RELAX_SCALE_LIST = [0.5, 0.7]                     # 완화 비율 (0.5=절반, 0.7=30%만 감소)
...
def backtest(
    ...
    while i < n - 1:
        ...
        # 신호강도 적응적 필터 완화
        # VPIN이 relax_thresh 미만이면(강한 신호) → 필터 임계값을 relax_scale만큼 축소
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 177 — 신호강도 적응적 필터 완화 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 16.0 (c176 이상), trades >= 80")
    print("가설: VPIN < relax_thresh (강한 신호) → body/ATR 필터 완화 "
          "→ 거래 수 회복 + Sharpe 유지")
    ...
    combos = list(product(ATR_PCTILE_THRESH_LIST, BODY_RATIO_MIN_LIST,
                          VPIN_RELAX_THRESH_LIST, RELAX_SCALE_LIST))
    ...
    results: list[dict] = []
    for idx, (atr_th, body_min, vpin_rx, rx_scale) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, atr_th, body_min, vpin_rx, rx_scale, btc_c, btc_s)
            sym_results.append(r)

        pooled = pool_results(sym_results)
        results.append({
            "atr_pctile_thresh": atr_th, "body_ratio_min": body_min,
            "vpin_relax_thresh": vpin_rx, "relax_scale": rx_scale,
            **pooled,
        })
        if (idx + 1) % 24 == 0:
    ...
    hdr = (f"{'atrTh':>5} {'body':>5} {'vpRx':>5} {'rxSc':>5} | "
    ...
    for r in valid[:20]:
        ...
        print(
            f"{r['atr_pctile_thresh']:>5} {r['body_ratio_min']:>5.1f} "
            f"{r['vpin_relax_thresh']:>5.2f} {r['relax_scale']:>5.1f} | "
    ...
    for r in valid:
        key = (r["atr_pctile_thresh"], r["body_ratio_min"],
               r["vpin_relax_thresh"], r["relax_scale"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        vpin_rx = params["vpin_relax_thresh"]
        rx_scale = params["relax_scale"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, atr_th, body_min, vpin_rx, rx_scale,
                             btc_c, btc_s)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: atrTh={atr_th} body={body_min:.1f} "
                  f"vpRx={vpin_rx:.2f} rxSc={rx_scale:.1f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        vpin_rx = params["vpin_relax_thresh"]
        rx_scale = params["relax_scale"]
        print(f"\n--- #{rank}: atrTh={atr_th} body={body_min:.1f} "
              f"vpRx={vpin_rx:.2f} rxSc={rx_scale:.1f} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, atr_th, body_min, vpin_rx, rx_scale,
    ...
    vpin_rx = best["vpin_relax_thresh"]
    rx_scale = best["relax_scale"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: atrTh={atr_th} body={body_min:.1f} "
          f"vpRx={vpin_rx:.2f} rxSc={rx_scale:.1f}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
    ...
    print(f"  c176 기준 (atrTh=30 body=0.7 고정): "
          f"avg_OOS=+16.345 n=58")
    print(f"  c177 최적 (atrTh={best['atr_pctile_thresh']} "
          f"body={best['body_ratio_min']:.1f} "
          f"vpRx={best['vpin_relax_thresh']:.2f} "
          f"rxSc={best['relax_scale']:.1f}): "
          f"avg_OOS={best['avg_oos_sharpe']:+.3f} "
          f"n={best['total_oos_trades']}")
    delta = best["avg_oos_sharpe"] - 16.345
    delta_n = best["total_oos_trades"] - 58
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: ATR_PCTILE_THRESH={best['atr_pctile_thresh']} "
          f"BODY_RATIO_MIN={best['body_ratio_min']:.1f} "
          f"VPIN_RELAX_THRESH={best['vpin_relax_thresh']:.2f} "
          f"RELAX_SCALE={best['relax_scale']:.1f}")
    print(f"  (c176 고정: atrLB={ATR_PCTILE_LB})")
    print(f"  (c165 고정: VPIN={VPIN_LOW} MOM={MOM_THRESH} Hold={MAX_HOLD} "
          f"CD={COOLDOWN_BARS})")
