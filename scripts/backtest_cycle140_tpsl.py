"""
사이클 140: stealth_3gate TP/SL 조합 탐색
- 사이클 139 확정: btc_stealth_acc_min=1.2 → W2 Sharpe 10.160
- 탐색: TP ∈ {0.08, 0.10, 0.12, 0.15} × SL ∈ {0.008, 0.010, 0.015, 0.020} (16조합)
- 고정: W=36, SMA=10, RS=[0.4,0.9), BTC_ACC_MIN=1.2, alt_acc>1.0, MAX_HOLD=24, Gate4=OFF
...
# Fixed params (사이클 129-139 최적값)
W = 36           # stealth_window
SMA_N = 10       # BTC SMA period
RS_LO = 0.4      # RS 하한
RS_HI = 0.9      # RS 상한
BTC_ACC_MIN = 1.2  # BTC stealth acc threshold (사이클 139 확정)
...
BASELINE_TP = 0.10
BASELINE_SL = 0.010
BASELINE_W2_SHARPE = 10.160

# 탐색 대상 TP/SL 조합
TP_CANDIDATES: list[float] = [0.08, 0.10, 0.12, 0.15]
SL_CANDIDATES: list[float] = [0.008, 0.010, 0.015, 0.020]
...
def main() -> None:
    t0 = time.time()
    print("=== stealth_3gate TP/SL 조합 탐색 (사이클 140) ===")
    print(f"고정: W={W}, SMA={SMA_N}, RS=[{RS_LO},{RS_HI}), BTC_ACC_MIN={BTC_ACC_MIN}, alt_acc>{ALT_ACC_MIN}, MAX_HOLD={MAX_HOLD}")
    print(f"탐색: TP ∈ {[f'{v*100:.0f}%' for v in TP_CANDIDATES]} × SL ∈ {[f'{v*100:.1f}%' for v in SL_CANDIDATES]} ({len(TP_CANDIDATES)*len(SL_CANDIDATES)}조합)")
    ...
    # Slice per window (shared across TP/SL combos)
    ...
    for tp in TP_CANDIDATES:
        for sl in SL_CANDIDATES:
            idx_combo += 1
            is_baseline = (tp == BASELINE_TP and sl == BASELINE_SL)
            ...
            print(
                ...
                f"  W1={w1_sh:.3f}(n={w1.get('n',0)}) W2={w2_sh:.3f}(n={w2.get('n',0)},WR={w2.get('wr',float('nan')):.3f}){delta_str}"
            )

    # Summary table
    print("\n=== 결과 요약 (W2 Sharpe 내림차순) ===")
    print(f"{'TP':>6} {'SL':>6} {'W1 Sh':>8} {'W1 n':>6} {'W2 Sh':>8} {'W2 n':>6} {'W2 WR':>7} {'ΔW2':>8} {'통과':>5}")
    print("-" * 75)
    sorted_results = sorted(results, key=lambda r: r["w2_sharpe"] if not np.isnan(r["w2_sharpe"]) else -999, reverse=True)
    for r in sorted_results:
        flag = "✅" if r["passed"] else "  "
        base = "*" if r["is_baseline"] else " "
        delta_str = f"{r['delta_w2']:+.3f}" if not np.isnan(r["delta_w2"]) else "  nan"
        print(
            f"{base}TP={r['tp']*100:.0f}%  SL={r['sl']*100:.1f}%  "
            f"{r['w1_sharpe']:>8.3f} {r['w1_n']:>6d}  "
            f"{r['w2_sharpe']:>8.3f} {r['w2_n']:>6d}  "
            f"{r['w2_wr']:>7.3f}  {delta_str:>8} "
            f"{flag:>5}"
        )

    # Best
    passed_results = [r for r in results if r["passed"]]
    best = max(passed_results, key=lambda r: r["w2_sharpe"]) if passed_results else None

    print("\n=== 권고 ===")
    if best and not best["is_baseline"]:
        delta = best["delta_w2"]
        if delta > 0.1:
            print(f"✅ TP={best['tp']*100:.0f}%/SL={best['sl']*100:.1f}% 채택 권장")
            print(f"   W2 Sharpe: {BASELINE_W2_SHARPE} → {best['w2_sharpe']:.3f} ({delta:+.3f})")
            print(f"   daemon.toml: stop_loss_pct = {best['sl']:.3f}, take_profit_pct = {best['tp']:.2f}")
        else:
            print(f"미미한 개선 (Δ{delta:+.3f}) — 현재 TP={BASELINE_TP*100:.0f}%/SL={BASELINE_SL*100:.1f}% 유지 고려")
    elif best and best["is_baseline"]:
        print(f"현재 TP={BASELINE_TP*100:.0f}%/SL={BASELINE_SL*100:.1f}% 최적 — 변경 불필요")
    else:
        print("통과 후보 없음")
