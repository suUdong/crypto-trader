"""
사이클 257: c255 최적 고정 + 저변동성 레짐 적응형 출구 파라미터

c255 결과:
  avg OOS Sharpe: +35.555  F2: +25.408 FAIL  F3: +35.110
  worst MDD: -7.21%  trades: 31  WR: 80.6%

문제:
  - F2 (2025-03 ~ 2025-11) MDD -7.21%로 c250 대비 -2.23%p 악화
  - F2 WR 72.7%로 F1(86.7%) 대비 열위 — 저변동성 횡보장에서 손실 거래가 과대
  - 원인: ATR 기반 SL/Trail이 저변동성에서도 동일 배수 적용 → 가격 움직임 대비 과도한 stop

가설:
  - 저변동성(ATR pctile < split)일 때 SL 배수/Trail 배수를 축소하면 MDD 개선
  - 저변동성에서 max_hold도 단축하면 횡보 포지션 조기 청산 → 기회비용 절감
  - 고변동성 구간(F1/F3)은 기존 출구 유지 → 성과 보존

c255 최적 고정:
  mLow=0.005  aSpl=40  bLow=0.010
...
새 탐색 그리드 (c257):
  SL_LOW_MULT:     [0.8, 1.0, 1.2]    — 저변동성 SL 배수 (고변동: 1.5 고정)
  TRAIL_LOW_MULT:  [1.2, 1.5, 1.8]    — 저변동성 Trail 배수 (고변동: 2.0 고정)
  MH_LOW:          [15, 20, 25]        — 저변동성 max hold (고변동: 30 고정)
  EXIT_ATR_SPLIT:  [30, 40, 50]        — 출구 레짐 분기 ATR percentile
  = 3 × 3 × 3 × 3 = 81 combos
...
# ─── c257 탐색 그리드 ─────────────────────────────────────────
SL_LOW_MULT_LIST = [0.8, 1.0, 1.2]
TRAIL_LOW_MULT_LIST = [1.2, 1.5, 1.8]
MH_LOW_LIST = [15, 20, 25]
EXIT_ATR_SPLIT_LIST = [30, 40, 50]
...
def run_backtest(
    ...
    # c257 탐색 파라미터
    sl_low_mult: float,
    trail_low_mult: float,
    mh_low: int,
    exit_atr_split: float,
    oos_start: str,
    oos_end: str,
    index: pd.DatetimeIndex,
) -> list[dict]:
    """c255 최적 고정 + 저변동성 레짐 적응형 출구."""
    n = len(c)
    trades: list[dict] = []
    position = None
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c257 핵심: 진입 시 변동성 레짐에 따른 출구 파라미터 ──
...
def main() -> None:
    print("=" * 80)
    print("=== c257: c255최적 고정 + 저변동성 레짐 적응형 출구 ===")
    ...
    print(f"탐색: SL_LOW × TRAIL_LOW × MH_LOW × EXIT_ATR_SPLIT")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        SL_LOW_MULT_LIST, TRAIL_LOW_MULT_LIST, MH_LOW_LIST,
        EXIT_ATR_SPLIT_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c255 최적 — 적응형 출구 없이)
    print("\n--- 베이스라인 (c255 최적, 고정 출구) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for window in WINDOWS:
        ...
        for sym in SYMBOLS:
            ...
            raw = run_backtest(
                ...
                BASE_SL_MULT, BASE_TRAIL_MULT, BASE_MAX_HOLD, 0,
    ...
    print("=" * 80)
    hdr = (f"{'slLow':>6} {'trLow':>6} {'mhLow':>6} {'exSpl':>6} | "
    ...
    for r in valid[:15]:
        p = r["params"]
        print(
            f"{p[0]:>6.1f} {p[1]:>6.1f} {p[2]:>6.0f} {p[3]:>6.0f} | "
    ...
    for val in SL_LOW_MULT_LIST:
        ...
        if sub:
            ...
            print(f"  slLow={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== TRAIL_LOW_MULT 효과 ===")
    for val in TRAIL_LOW_MULT_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  trLow={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== MH_LOW 효과 ===")
    for val in MH_LOW_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  mhLow={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== EXIT_ATR_SPLIT 효과 ===")
    for val in EXIT_ATR_SPLIT_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  exSpl={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  slLow={p[0]:.1f} trLow={p[1]:.1f} mhLow={p[2]:.0f} "
              f"exSpl={p[3]:.0f} | "
    ...
    # 최종 요약
    print("\n" + "=" * 80)
    print("=== c255 베이스라인 대비 비교 ===")
    if valid:
        b = valid[0]
        print(f"  c255 기준: avg_OOS={base_avg:+.3f} F2={base_f2:+.3f} "
              f"F3={base_f3:+.3f} worst_MDD={base_worst_mdd:+.2f}%")
        print(f"  c257 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: slLow={p[0]:.1f} trLow={p[1]:.1f} "
              f"mhLow={p[2]:.0f} exSpl={p[3]:.0f}")
