"""
사이클 224: c219 후속 — 심볼별 분할익절 파라미터 refinement
- 기반: c219 OOS +23.575 F3 +29.650 SOL_avg +7.658 worst_MDD -12.12%
  최적: tp1M=2.5 pRat=0.7 hiTP=0.0 loSL=0.70
- 가설: SOL은 변동성이 달라 공통 tp1M/pRat가 부적합 → 심볼별 독립 탐색
- hiTPBonus=0.0 고정 (c219서 dominant). loSLScale=0.70 고정
- 목표: avg_OOS>=20 AND F3>=25 AND SOL_avg>=10 AND worst_MDD>=-12%
원본 c219 기반
...
# ─── c228 탐색 그리드 ────────────────────────────────────────
# c224 winner 고정: tp1M=2.5 pRat=0.7 solTp1=2.0 solPr=0.5
# 가설: SOL_avg<10 실패 원인 = 저품질 SOL 진입 → SOL 전용 엔트리 게이트 강화
#  - SOL ADX 문턱 상향 (기본 25 → 30/35)
#  - SOL 거래량 비율 상향 (기본 1.0 → 1.3/1.6)
#  - SOL ATR 백분위 문턱 상향 (기본 30 → 40/50)
# ETH/XRP는 baseline 유지
TP1_MULT_FIXED = 2.5
PART_RATIO_FIXED = 0.7
SOL_TP1_MULT_FIXED = 2.0
SOL_PART_RATIO_FIXED = 0.5
SOL_ADX_MIN_LIST = [25, 30, 35]
SOL_VOL_RATIO_MIN_LIST = [1.0, 1.3, 1.6]
SOL_ATR_PCTILE_TH_LIST = [30, 40, 50]
HI_TP_BONUS_FIXED = 0.0
LO_SL_SCALE_FIXED = 0.70
...
def run_backtest(
    ...
    hi_tp_bonus: float,
    lo_sl_scale: float,
    # c228 심볼별 엔트리 게이트 오버라이드
    adx_min_override: float,
    vol_ratio_min_override: float,
    atr_pctile_th_override: float,
    oos_start: str,
    oos_end: str,
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # c205 고정 조건 (ADX는 심볼별 오버라이드)
            donchian_ok = c[i] > dc_up[i]
            adx_ok = adx_val[i] >= adx_min_override
            btc_ok = btc_close[i] > btc_sma[i]

            # c207: ATR 백분위 변동성 레짐 필터 (심볼별 오버라이드)
            atr_pctile_ok = True
            if atr_pctile_th_override > 0:
                if np.isnan(atr_pctile[i]):
                    atr_pctile_ok = False
                else:
                    atr_pctile_ok = atr_pctile[i] >= atr_pctile_th_override

            # c207: 거래량 확인 (심볼별 오버라이드)
            vol_ok = True
            if vol_ratio_min_override > 0:
                if np.isnan(vol_sma[i]) or vol_sma[i] <= 0:
                    vol_ok = False
                else:
                    vol_ok = v[i] / vol_sma[i] >= vol_ratio_min_override
...
def main() -> None:
    ...
    # 그리드 — (SOL_ADX, SOL_VOL_RATIO, SOL_ATR_PCTILE_TH)
    grid = list(product(
        SOL_ADX_MIN_LIST, SOL_VOL_RATIO_MIN_LIST, SOL_ATR_PCTILE_TH_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")
    ...
    for gi, combo in enumerate(grid):
        sol_adx, sol_vol, sol_atr_pth = combo
        tp1_m = TP1_MULT_FIXED
        part_r = PART_RATIO_FIXED
        sol_tp1_m = SOL_TP1_MULT_FIXED
        sol_part_r = SOL_PART_RATIO_FIXED
        hi_bonus = HI_TP_BONUS_FIXED
        lo_sl = LO_SL_SCALE_FIXED
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                ...
                # 심볼별 tp1/partRatio + 엔트리 게이트 주입
                if sym == "KRW-SOL":
                    use_tp1, use_pr = sol_tp1_m, sol_part_r
                    use_adx = sol_adx
                    use_vol = sol_vol
                    use_atr_pth = sol_atr_pth
                else:
                    use_tp1, use_pr = tp1_m, part_r
                    use_adx = float(ADX_THRESH)
                    use_vol = float(VOL_RATIO_MIN)
                    use_atr_pth = float(ATR_PCTILE_TH)
                ...
                raw_trades = run_backtest(
                    ...
                    sl_sc,
                    use_tp1, use_pr, hi_bonus, lo_sl,
                    use_adx, use_vol, use_atr_pth,
    ...
    for i, r in enumerate(valid[:5]):
        p = r["params"]
        print(f"\n#{i+1}: solADX={p[0]} solVol={p[1]} solAtrPth={p[2]}")
    ...
    if valid:
        ...
        bp = best["params"]
        print("\n" + "=" * 80)
        print(f"=== 심볼별 OOS 성능 분해 (Top 1: solADX={bp[0]} "
              f"solVol={bp[1]} solAtrPth={bp[2]}) ===")
        ...
        for sym in SYMBOLS:
            ...
            sym_total_n = 0
            if sym == "KRW-SOL":
                use_tp1, use_pr = SOL_TP1_MULT_FIXED, SOL_PART_RATIO_FIXED
                use_adx, use_vol, use_atr_pth = bp[0], bp[1], bp[2]
            else:
                use_tp1, use_pr = TP1_MULT_FIXED, PART_RATIO_FIXED
                use_adx = float(ADX_THRESH)
                use_vol = float(VOL_RATIO_MIN)
                use_atr_pth = float(ATR_PCTILE_TH)
            ...
            for window in WINDOWS:
                raw_trades = run_backtest(
                    ...
                    sl_sc,
                    use_tp1, use_pr,
                    HI_TP_BONUS_FIXED, LO_SL_SCALE_FIXED,
                    use_adx, use_vol, use_atr_pth,
                    window["oos_start"], window["oos_end"],
                    sp["index"],
                )
    ...
    # SOL 게이트 효과 분석
    print("\n" + "=" * 80)
    print("=== SOL 엔트리 게이트 효과 분석 ===")
    for adx_v in SOL_ADX_MIN_LIST:
        subset = [r for r in valid if r["params"][0] == adx_v]
        if subset:
            avg_sol = np.mean([r["sol_avg_sharpe"] for r in subset])
            avg_sh = np.mean([r["avg_sharpe"] for r in subset])
            print(f"  solADX={adx_v}: SOL_avg={avg_sol:+.3f}  "
                  f"avg={avg_sh:+.3f}")
    print()
    for vr in SOL_VOL_RATIO_MIN_LIST:
        subset = [r for r in valid if r["params"][1] == vr]
        if subset:
            avg_sol = np.mean([r["sol_avg_sharpe"] for r in subset])
            print(f"  solVol={vr}: SOL_avg={avg_sol:+.3f}")
    print()
    for ap in SOL_ATR_PCTILE_TH_LIST:
        subset = [r for r in valid if r["params"][2] == ap]
        if subset:
            avg_sol = np.mean([r["sol_avg_sharpe"] for r in subset])
            print(f"  solAtrPth={ap}: SOL_avg={avg_sol:+.3f}")
    ...
    if valid:
        ...
        print(f"★ OOS 최적: solADX={p[0]} solVol={p[1]} solAtrPth={p[2]}")
        print(f"  (공통 고정: tp1M={TP1_MULT_FIXED} pRat={PART_RATIO_FIXED} "
              f"solTp1={SOL_TP1_MULT_FIXED} solPr={SOL_PART_RATIO_FIXED})")
