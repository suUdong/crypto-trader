"""
vpin_eth RSI velocity + volume surge 확인 필터 — 사이클 170
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
배경: 사이클 168 vpin_eth trailing stop + 레짐 hold
      최적: hvH=24 lvH=14 trA=2.0 trSL=0.5, avg OOS Sharpe=+9.897
      문제:
        1) Fold 2 (2025-07~2026-04 BEAR) WR=38.1%, n=21 — 약한 진입
        2) VPIN+모멘텀만으로 BEAR 구간 진입 품질 부족
        3) 추가 확인 필터 없이 노이즈 진입 다수

가설:
  1) RSI velocity (RSI 변화율): RSI가 상승 추세일 때만 진입
     → BEAR 구간의 false breakout 필터링
     rsi_vel = RSI[i] - RSI[i-rsi_vel_lb], rsi_vel > rsi_vel_thresh
  2) Volume surge 확인: 단순 vol > vol_sma*1.5 대신
     volume_ratio 크기별 진입 품질 차별화
     vol_ratio_min ∈ {1.5, 2.0, 2.5} — 높을수록 신호 품질 ↑, n ↓
  3) c168 최적 hold/trail 고정, 필터만 그리드 탐색

그리드:
  - rsi_vel_lb: [3, 5, 7]    — RSI velocity lookback (3)
  - rsi_vel_thresh: [1, 3, 5] — RSI 변화량 최소값 (3)
  - vol_ratio_min: [1.5, 2.0, 2.5] — 볼륨 서지 배수 (3)
  = 3×3×3 = 27 조합
  + 베이스라인 (c168 최적, 필터 없음) = 총 28 비교
"""
...
RSI_VEL_THRESH_LIST = [1, 3, 5]
VOL_RATIO_MIN_LIST = [1.5, 2.0, 2.5]
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_eth RSI velocity + volume surge 확인 필터 (사이클 170) ===")
    print(f"심볼: {SYMBOL}")
    print(f"기반: c168 최적 (hvH={HV_HOLD} lvH={LV_HOLD} "
          f"trA={TRAIL_ACTIVATE_MULT} trSL={TRAIL_SL_MULT})")
    print(f"가설: RSI velocity + volume ratio 필터로 BEAR fold 진입 품질 개선")
    print(f"그리드: rsi_vel_lb={RSI_VEL_LB_LIST} rsi_vel_thresh={RSI_VEL_THRESH_LIST} "
          f"vol_ratio={VOL_RATIO_MIN_LIST}")
    print("=" * 80)
    ...
    base = backtest(df_eth, df_btc, rsi_vel_lb=0, rsi_vel_thresh=0, vol_ratio_min=1.5)
    ...
    # WR 개선 하이라이트
    base_wr = base["wr"]
    wr_improved = [r for r in valid if r["wr"] > base_wr]
    print(f"WR 개선 (>{base_wr:.1%}): {len(wr_improved)}개")
    ...
    print(f"\n  vs c168 베이스라인 (no RSI vel filter): "
          f"Sharpe={fmt_sh(base['sharpe'])}  WR={base['wr']:.1%}  "
          f"MDD={base['max_dd'] * 100:+.2f}%  n={base['trades']}")

    # c168 WF 비교
    print(f"  vs c168 WF 최적: avg OOS Sharpe=+9.897  WR=41.4%  n=59")
