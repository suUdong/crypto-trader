"""
사이클 243: c241 최적 고정 + BTC 품질 게이트 + 브레이크아웃 마진 필터

c241 결과:
  avg OOS Sharpe: +28.368 PASS  F2: +19.876 FAIL (목표 >=22)
  F1: Sharpe=+30.117  WR=80.0%  trades=20  avg=+5.92%  MDD=-4.98%
  F2: Sharpe=+19.876  WR=63.2%  trades=19  avg=+2.16%  MDD=-8.40%  ← 병목
  F3: Sharpe=+35.110  WR=80.0%  trades=5   avg=+2.64%  MDD=-2.21%
  SOL avg: +18.895 PASS  worst MDD: -8.40% PASS

문제 분석:
  - F2 WR 63.2% = 약 7/19 손실 — c241 진입 필터로도 해결 안됨
  - F2 기간(2025-03~11): 조정/횡보장 혼재 — 이전 필터는 단순 ATR/RSI/볼륨
  - c241 필터는 "alt 자체" 상태 체크 → "BTC 모멘텀 품질"은 미검증
  - 가설: BTC도 단기 상승 모멘텀이 있을 때만 진입 → alt 위 노이즈 진입 줄임
  - 가설: DC_UPPER 브레이크아웃이 근소할 때(경계선 신호) → 마진 필터로 품질 확보

c241 고정 최적값:
  rCeil=80, rFlr=30, mLB=5, mMin=0.02, aPct=30, vRat=1.5

새 탐색 그리드 (c243):
  BTC_MOM_LB:      [5, 10, 20]           — BTC N-봉 모멘텀 lookback
  BTC_MOM_MIN:     [0.0, 0.01, 0.02, 0.03] — BTC 최소 모멘텀 (0=OFF)
  BTC_ADX_GATE:    [0, 20, 25]           — BTC ADX 하한 (0=OFF)
  BREAKOUT_MARGIN: [0.0, 0.003, 0.005, 0.008] — close > DC_UPPER * (1+margin) (0=OFF)
  = 3 × 4 × 3 × 4 = 144 combos
...
# ─── c243 탐색 그리드 ─────────────────────────────────────────
BTC_MOM_LB_LIST = [5, 10, 20]                    # BTC 모멘텀 lookback
BTC_MOM_MIN_LIST = [0.0, 0.01, 0.02, 0.03]       # BTC 최소 수익률 (0=OFF)
BTC_ADX_GATE_LIST = [0, 20, 25]                  # BTC ADX 최소 (0=OFF)
BREAKOUT_MARGIN_LIST = [0.0, 0.003, 0.005, 0.008]  # 브레이크아웃 마진 (0=OFF)
...
def run_backtest(
    ...
    """c241 최적 고정 + BTC 품질 게이트 + 브레이크아웃 마진."""
    n = len(c)
    trades: list[dict] = []
    position = None
    ...
    warmup = max(DC_UPPER_LB, DC_LOWER_LB, BTC_SMA_PERIOD, 60, MOM_LB + 5,
                 btc_mom_lb + 5) + 10
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # 2) BTC ADX 게이트
            if btc_adx_gate > 0:
                if np.isnan(btc_adx[i]) or btc_adx[i] < btc_adx_gate:
                    continue

            # 3) 브레이크아웃 마진 필터 (노이즈 진입 차단)
...
def main() -> None:
    print("=" * 80)
    print("=== c243: c241최적 고정 + BTC품질 게이트 + 브레이크아웃 마진 ===")
    ...
    print(f"탐색: BTC_MOM_LB × BTC_MOM_MIN × BTC_ADX_GATE × BREAKOUT_MARGIN")
    print("=" * 80)

    # 데이터 로드
    btc_df = load_historical("KRW-BTC", "240m", "2022-01-01", "2026-04-10")
    print(f"BTC 데이터: {len(btc_df)} rows")

    sym_data: dict[str, pd.DataFrame] = {}
    for sym in SYMBOLS:
        df = load_historical(sym, "240m", "2022-01-01", "2026-04-10")
        sym_data[sym] = df
        print(f"{sym} 데이터: {len(df)} rows")

    btc_close_full = btc_df["close"].values
    btc_sma_full = sma_calc(btc_close_full, BTC_SMA_PERIOD)
    btc_adx_full = compute_adx(
        btc_df["high"].values, btc_df["low"].values, btc_close_full,
        BTC_ADX_PERIOD,
    )
    ...
    for sym in SYMBOLS:
        ...
        sym_precomp[sym] = {
            ...
            "btc_adx": btc_adx_aligned,
            "index": df.index,
        }

    # 그리드
    grid = list(product(
        BTC_MOM_LB_LIST, BTC_MOM_MIN_LIST,
        BTC_ADX_GATE_LIST, BREAKOUT_MARGIN_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c241 최적, 새 필터 OFF)
    print("\n--- 베이스라인 (c241 최적, BTC게이트=OFF, 마진=0) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for gi, combo in enumerate(grid):
        btc_mlb, btc_mmin, btc_adx_g, bo_margin = combo
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                ...
                raw = run_backtest(
                    ...
                    sp["atr_pctile"], sp["vol_sma"], sp["rsi"],
                    SYM_SL_SCALE[sym], sym,
                    btc_mlb, btc_mmin, btc_adx_g, bo_margin,
                    window["oos_start"], window["oos_end"],
                    sp["index"],
                )
                agg = aggregate_trades(raw)
    ...
    print("=" * 80)
    hdr = (f"{'bMLB':>5} {'bMMin':>6} {'bADX':>5} {'boMgn':>6} | "
    ...
    for r in valid[:20]:
        p = r["params"]
        print(
            f"{p[0]:>5} {p[1]:>6.2f} {p[2]:>5} {p[3]:>6.3f} | "
    ...
    for val in BTC_MOM_LB_LIST:
        ...
        if sub:
            ...
            print(f"  btcMLB={val}: top5 avgSh={avg_sh:+.3f}  F2={avg_f2:+.3f}")

    print("\n=== BTC 모멘텀 최소값 효과 ===")
    for val in BTC_MOM_MIN_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f2_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  btcMMin={val:.2f}: top5 avgSh={avg_sh:+.3f}  F2={avg_f2:+.3f}")

    print("\n=== BTC ADX 게이트 효과 ===")
    for val in BTC_ADX_GATE_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f2_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  btcADX={val}: top5 avgSh={avg_sh:+.3f}  F2={avg_f2:+.3f}")

    print("\n=== 브레이크아웃 마진 효과 ===")
    for val in BREAKOUT_MARGIN_LIST:
        sub = [r for r in valid if r["params"][3] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f2_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  boMgn={val:.3f}: top5 avgSh={avg_sh:+.3f}  F2={avg_f2:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  btcMLB={p[0]} btcMMin={p[1]:.2f} btcADX={p[2]} "
              f"boMgn={p[3]:.3f} | "
    ...
    # 최종 요약
    print("\n" + "=" * 80)
    print("=== c241 베이스라인 대비 비교 ===")
    if valid:
        b = valid[0]
        print(f"  c241 기준: avg_OOS=+28.368 F2=+19.876 "
              f"F3=+35.110 SOL=+18.895 worst_MDD=-8.40%")
        print(f"  c243 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
              f"SOL_avg={b['sol_avg_sharpe']:+.3f} "
              f"worst_MDD={b['worst_mdd']:+.2f}%")
        d_avg = b["avg_sharpe"] - 28.368
        d_f2 = b["f2_sharpe"] - 19.876
        d_f3 = b["f3_sharpe"] - 35.110
        d_sol = b["sol_avg_sharpe"] - 18.895
        d_mdd = b["worst_mdd"] - (-8.40)
    ...
    if valid:
        ...
        avg_pass = b["avg_sharpe"] >= 27.0
        print(f"★ OOS 최적: btcMLB={p[0]} btcMMin={p[1]:.2f} "
              f"btcADX={p[2]} boMgn={p[3]:.3f}")
        print(f"  (c241 고정: rCeil={RSI_CEIL} rFlr={RSI_FLOOR} "
              f"mLB={MOM_LB} mMin={MOM_MIN} aPct={ATR_PCTILE_MIN} vRat={VOL_RATIO_MIN})")
        print(f"  (c233 고정: trail={BASE_TRAIL_MULT} TP2={BASE_TP2_MULT} "
              f"SL={BASE_SL_MULT} MH={BASE_MAX_HOLD} TP1={BASE_TP1_MULT})")
