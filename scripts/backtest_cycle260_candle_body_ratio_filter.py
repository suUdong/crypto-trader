"""
사이클 260: c259 베이스라인 + 캔들 바디 비율 필터 (F2 Sharpe 개선)
...
문제:
  - F2 (2025-03 ~ 2025-11) Sharpe +27.548 — 30 미달 8사이클 연속
  - F2 구간 avg return +2.79% — 진입 품질이 아니라 진입 선택에 문제
  - 기존 필터(ADX, RSI, 볼륨 등)는 추세 강도만 측정 — 캔들 형태 무시
  - Donchian breakout 시 캔들 바디가 작고 윗꼬리 긴 경우 = 약한 breakout
  - 바디 비율(body/range) 높은 캔들 = 매수 conviction 강함

가설:
  - breakout 캔들의 body ratio = (close - open) / (high - low)로 conviction 측정
  - body ratio가 높을수록 (close가 high에 가까울수록) 강한 breakout
  - 낮은 body ratio breakout 필터링 → 약한 breakout에서 F2 손실 감소
  - 추가: close_position = (close - low) / (high - low) — close가 range 상단이면 강한 종가
...
새 탐색 그리드 (c260):
  BODY_RATIO_MIN:    [0.0, 0.3, 0.5, 0.7]  — 최소 body ratio (0=비활성)
  CLOSE_POS_MIN:     [0.0, 0.5, 0.7]        — 최소 close position (0=비활성)
  BODY_LOOK_BACK:    [1, 2]                  — 최근 N봉 평균 body ratio 사용
  = 4 × 3 × 2 = 24 combos
...
# ─── c260 탐색 그리드 ─────────────────────────────────────────
BODY_RATIO_MIN_LIST = [0.0, 0.3, 0.5, 0.7]
CLOSE_POS_MIN_LIST = [0.0, 0.5, 0.7]
BODY_LOOK_BACK_LIST = [1, 2]
...
def run_backtest(
    ...
    warmup = max(DC_UPPER_LB, DC_LOWER_LB, BTC_SMA_PERIOD, 60,
                 ...
                 ATR_MOM_LB + 5, body_look_back + 5) + 10
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c260 핵심: 캔들 바디 비율 필터 ──
            if body_ratio_min > 0.0 or close_pos_min > 0.0:
                # body_look_back 봉 평균
                avg_body_ratio = 0.0
                avg_close_pos = 0.0
                valid_bars = 0

                for lb in range(body_look_back):
                    idx = i - lb
                    if idx < 0:
                        continue
                    ...
                    if candle_range <= 0:
                        continue
                    body = c[idx] - o[idx]  # 양봉이면 양수
                    body_ratio = body / candle_range
                    close_pos = (c[idx] - lo[idx]) / candle_range

                    avg_body_ratio += body_ratio
                    avg_close_pos += close_pos
                    valid_bars += 1

                if valid_bars > 0:
                    avg_body_ratio /= valid_bars
                    avg_close_pos /= valid_bars

                    if body_ratio_min > 0.0 and avg_body_ratio < body_ratio_min:
                        continue
                    if close_pos_min > 0.0 and avg_close_pos < close_pos_min:
                        continue
...
def main() -> None:
    print("=" * 80)
    print("=== c260: c259최적 고정 + 캔들 바디 비율 필터 (F2 진입 선택 개선) ===")
    ...
    print(f"탐색: BODY_RATIO_MIN × CLOSE_POS_MIN × BODY_LOOK_BACK")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        BODY_RATIO_MIN_LIST, CLOSE_POS_MIN_LIST,
        BODY_LOOK_BACK_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c259 최적 = body filter 비활성)
    print("\n--- 베이스라인 (c259 최적) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    # 효과 분석
    print("\n=== BODY_RATIO_MIN 효과 ===")
    for val in BODY_RATIO_MIN_LIST:
        ...
        if sub:
            ...
            print(f"  brMin={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== CLOSE_POS_MIN 효과 ===")
    for val in CLOSE_POS_MIN_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  cpMin={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")

    print("\n=== BODY_LOOK_BACK 효과 ===")
    for val in BODY_LOOK_BACK_LIST:
        ...
        if sub:
            ...
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            avg_mdd = np.mean([r["worst_mdd"] for r in top5])
            print(f"  bLB={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}  MDD={avg_mdd:+.2f}%")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  brMin={p[0]:.2f} cpMin={p[1]:.2f} bLB={p[2]:.0f} | "
    ...
    if valid:
        ...
        print(f"  c260 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: brMin={p[0]:.2f} cpMin={p[1]:.2f} "
              f"bLB={p[2]:.0f}")
        print(f"  (c259 고정: atrLB={ATR_MOM_LB} atrTh={ATR_MOM_THRESH} "
              f"tp2B={TP2_BOOST} trTA={TRAIL_TIGHTEN_AMT})")
        print(f"  (c258 고정: adxLB={ADX_SLOPE_LB} adxMn={ADX_SLOPE_MIN} "
