"""
vpin_multi 사이클 190 — c186 최적 + ADX 추세강도 필터 정밀 탐색
- 기반: c186 OOS Sharpe +26.587 PASS, WR 61.5%, trades 33
  최적: body=0.50 rsiD=6 sLB=10 sPth=50
- 문제: SOL/XRP Fold1=0 — 약추세 구간 거짓 신호로 차단됨
- 가설:
  A) ADX(14) 추세강도 필터 추가 → 횡보장 제거, 추세장만 진입
  B) c186 최적 인근 좁은 그리드 (body 0.45/0.50, rsiD 5/6/7, sPth 40/50/60)
- 그리드: ADX_TH[0,15,20,25] × body[0.45,0.50] × rsiD[5,6,7] × sPth[40,50,60]
  = 4×2×3×3 = 72 combos (sLB=10 고정)
- 목표: OOS Sharpe>=30 AND trades>=25
...
# -- c186 최적 인근 좁은 그리드 + ADX 신규 차원 --
BODY_RATIO_LIST = [0.45, 0.50]
RSI_DELTA_MIN_LIST = [5, 6, 7]
EMA_SLOPE_LB_LIST = [10]              # c186 최적 고정
EMA_SLOPE_PCTILE_TH_LIST = [40, 50, 60]
ADX_PERIOD = 14
ADX_TH_LIST = [0, 15, 20, 25]         # 0=비활성
...
def compute_adx(
    ...
    plus_s[period] = plus_dm[1:period + 1].sum()
    minus_s[period] = minus_dm[1:period + 1].sum()
    ...
    for i in range(period, n):
        ...
        if s <= 0:
            continue
        dx[i] = 100.0 * abs(plus_di - minus_di) / s
    start = period * 2
    if start >= n:
        return adx
...
def backtest(
    ...
    adx_arr = compute_adx(h, lo, c, ADX_PERIOD) if adx_th > 0 else np.full(n, np.nan)
...
def build_combos() -> list[dict]:
    combos = []
    for body, rsi_d, slope_lb, slope_th, adx_th in product(
        BODY_RATIO_LIST, RSI_DELTA_MIN_LIST,
        EMA_SLOPE_LB_LIST, EMA_SLOPE_PCTILE_TH_LIST, ADX_TH_LIST,
    ):
        combos.append({
            "body_ratio": body,
            "rsi_delta_min": rsi_d,
...
def main() -> None:
    ...
    for r in valid:
        key = (r["body_ratio"], r["rsi_delta_min"],
               r["ema_slope_lb"], r["ema_slope_pctile_th"], r["adx_th"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        print(f"\n--- #{rank}: body={body_ratio:.2f} rsiD={rsi_d} "
              f"sLB={slope_lb} sPth={slope_th_str} adx={int(adx_th)} "
    ...
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: body={body_ratio:.2f} "
          f"rsiD={rsi_d} sLB={slope_lb} sPth={slope_th_str} adx={int(adx_th)}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
