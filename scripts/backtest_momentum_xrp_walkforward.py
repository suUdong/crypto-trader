"""
momentum_xrp walk-forward 검증 (사이클 78)
- 슬라이딩 2/3 통과 후보: C8(lb=8, adx=25, vol=2.0, TP=12%, SL=4%)
  W1(2024 OOS): Sh+5.5, WR=47.4%, T=19 ✅
  W2(2025 OOS): Sh+10.7, WR=47.1%, T=17 ✅
...
# 후보 파라미터 (사이클 77 슬라이딩 결과 기반)
CANDIDATES = [
    {"lookback":  8, "adx": 25.0, "vol_mult": 2.0, "tp": 0.12, "sl": 0.04, "label": "C8 (슬라이딩2/3 통과)"},
    {"lookback":  6, "adx": 25.0, "vol_mult": 2.0, "tp": 0.12, "sl": 0.04, "label": "C_lb6"},
    {"lookback": 10, "adx": 25.0, "vol_mult": 2.0, "tp": 0.12, "sl": 0.04, "label": "C_lb10"},
    {"lookback":  8, "adx": 20.0, "vol_mult": 2.0, "tp": 0.12, "sl": 0.04, "label": "C_adx20"},
    {"lookback":  8, "adx": 30.0, "vol_mult": 2.0, "tp": 0.12, "sl": 0.04, "label": "C_adx30"},
    {"lookback":  8, "adx": 25.0, "vol_mult": 2.0, "tp": 0.10, "sl": 0.03, "label": "C_tp10_sl3"},
    {"lookback":  8, "adx": 25.0, "vol_mult": 1.5, "tp": 0.12, "sl": 0.04, "label": "C_vol1.5"},
    {"lookback": 12, "adx": 25.0, "vol_mult": 2.0, "tp": 0.12, "sl": 0.04, "label": "C_lb12 (SOL/ETH 동일)"},
]
...
def main() -> None:
    print("=" * 70)
    print("momentum_xrp walk-forward 검증 (사이클 78)")
    ...
    print(f"{'파라미터':<48} | {'IS Sharpe':>10} {'IS WR':>7} {'IS T':>5} | {'OOS Sharpe':>10} {'OOS WR':>7} {'OOS T':>5} | {'판정':>6}")
    print("-" * 120)
    ...
    for p in CANDIDATES:
        ...
        label = p['label']
        print(
            f"lb={lb} adx={adx_t:.0f} vol={vm} TP={tp} SL={sl} ({label:<22}) | "
    ...
    if passed:
        ...
        print("   → XRP 특성 재탐색 또는 슬라이딩 2/3 통과한 C8 단기 lookback이 2025 전체 OOS에서 약화된 것")
