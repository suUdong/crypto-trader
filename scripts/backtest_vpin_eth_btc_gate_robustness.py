"""
vpin_eth 3-fold WF Sharpe +8.055 후속 — BTC 트렌드 게이트 + 5-fold OOS 강건성 검증.

직전 최적: vh=0.50 vm=0.0005 hold=18 TP=0.05 SL=0.006 → Sharpe +8.055 (3-fold mean).
다음 단계: 동일 ETH 진입 규칙에 BTC 트렌드 게이트(BTC 10봉 수익률 > 0)를 더해
거래수 감소 대비 Sharpe/WR 개선 여부 확인하고, 5-fold walk-forward로 강건성을 재평가.
필터 추가가 거래를 0으로 만들면 즉시 거부 (실전 진입 0건 함정 방지).
"""
...
# 직전 최적 근방 좁은 범위 — 추가 필터의 영향만 격리해서 본다
VPIN_HIGH = 0.50
VPIN_MOM  = 0.0005
MAX_HOLD_LIST = [14, 18, 22]
TP_LIST       = [0.045, 0.050, 0.060]
SL_LIST       = [0.006, 0.008]

# BTC 트렌드 게이트 lookback (None = 게이트 미적용 baseline)
BTC_LOOKBACK_LIST: list[int | None] = [None, 6, 10, 14]
...
def backtest(
    ...
    btc_mom  = compute_mom(btc_close, btc_lb) if (btc_lb and btc_close is not None) else None

    returns: list[float] = []
    warmup = max(BUCKET_COUNT, EMA_PERIOD, RSI_PERIOD + 1, MOM_LOOKBACK, btc_lb or 0) + 5
    i = warmup
    while i < n - 1:
        vpin_val = vpin_arr[i]
        mom_val  = mom_arr[i]
        ...
        if entry_ok and btc_mom is not None:
            bm = btc_mom[i] if i < len(btc_mom) else np.nan
...
def wf_eval(
    df: pd.DataFrame, btc_close: np.ndarray | None,
    vh: float, vm: float, mh: int, tp: float, sl: float,
    btc_lb: int | None,
) -> dict:
    ...
    for k in range(N_FOLDS):
        ...
        sub_btc = btc_close[s:e] if btc_close is not None else None
        folds.append(backtest(df.iloc[s:e], sub_btc, vh, vm, mh, tp, sl, btc_lb))
...
def main() -> None:
    print("=== vpin_eth + BTC 트렌드 게이트 강건성 검증 (5-fold WF) ===")
    ...
    if df.empty:
        print("데이터 없음.")
        return
    btc_df = load_historical(BTC, "240m", START, END)
    btc_close = btc_df["close"].values if not btc_df.empty else None
    if btc_close is not None and len(btc_close) != len(df):
        # 길이 미스매치 시 안전하게 잘라 맞춤
        m = min(len(btc_close), len(df))
        df = df.iloc[:m]
        btc_close = btc_close[:m]
    print(f"데이터: {len(df)}행  BTC: {'있음' if btc_close is not None else '없음'}")

    combos = list(product(MAX_HOLD_LIST, TP_LIST, SL_LIST, BTC_LOOKBACK_LIST))
    print(f"총 조합: {len(combos)}개\n")

    results: list[dict] = []
    for idx, (mh, tp, sl, btc_lb) in enumerate(combos, 1):
        r = wf_eval(df, btc_close, VPIN_HIGH, VPIN_MOM, mh, tp, sl, btc_lb)
        results.append({"hold": mh, "tp": tp, "sl": sl, "btc_lb": btc_lb, **r})
        if idx % 10 == 0:
    ...
    hdr = (f"{'hold':>5} {'TP':>6} {'SL':>6} {'BTClb':>6} | "
    ...
    for r in results[:15]:
        ...
        btc_lb_str = "OFF" if r["btc_lb"] is None else str(r["btc_lb"])
        print(
            f"{r['hold']:>5} {r['tp']:>6.3f} {r['sl']:>6.3f} {btc_lb_str:>6} | "
            f"{shm:>7} {shn:>7} {r['wr']:>5.1%} {r['trades']:>7}"
        )

    best = results[0]
    btc_lb_str = "OFF" if best["btc_lb"] is None else str(best["btc_lb"])
    print(
        f"\n★ 최적 (min-fold 기준): hold={best['hold']} TP={best['tp']} "
        f"SL={best['sl']} BTC_lb={btc_lb_str}"
    )
    ...
    # baseline(BTC OFF) 대비 BTC ON 최적 비교 — 필터 효과 정량화
    baseline = next((r for r in results if r["btc_lb"] is None
                     and r["hold"] == best["hold"] and r["tp"] == best["tp"]
                     and r["sl"] == best["sl"]), None)
    if baseline and best["btc_lb"] is not None:
        delta = best["sharpe_min"] - baseline["sharpe_min"]
        print(
            f"\n📊 BTC 게이트 효과: ShMin {baseline['sharpe_min']:+.3f} → "
            f"{best['sharpe_min']:+.3f} (Δ {delta:+.3f}), "
            f"trades {baseline['trades']} → {best['trades']}"
        )

    gate = best["sharpe_min"] if not np.isnan(best["sharpe_min"]) else -99
    if gate >= 3.0 and best["trades"] >= 100:
        print("\n✅ 모든 폴드 Sharpe ≥ 3.0 & 거래수 충분 — paper 데이터 30건 수집 후보")
    elif best["trades"] < 100:
        print(f"\n⚠️ 거래수 {best['trades']} < 100 — 필터가 너무 엄격, 과최적화 의심")
    else:
