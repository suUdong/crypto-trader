"""
vpin_multi 사이클 209 — MACD histogram slope 진입게이트 + 적응적 홀드/조기청산
- 기반: c179 OOS Sharpe +42.878 (vol regime adaptive) — 현 최적
- c208 결과: MACD histogram slope gradient mode Sharpe +14.364, XRP +21.375
  → 단독 교체는 악화, but slope 정보를 보조 필터로 활용하면 alpha 보존+noise 필터 가능
- 핵심 변경:
  1. c179 진입조건 유지 + MACD histogram slope > gate 추가 필터
  2. slope magnitude → 홀드 기간 보너스 (가속 강할수록 오래 보유)
  3. slope 반전(감속) 감지 시 trailing stop 조기 활성화
  4. c208 최적 mcF=8 mcS=26 hsLB=5 기반 확장 탐색
- 그리드:
  mcF: [8, 12]
  mcS: [21, 26]
  hsLB: [3, 5]
  hsGate: [0.0, 0.3, 0.6] (ATR-정규화 최소 slope)
  holdBonusMul: [0.0, 0.5, 1.0] (slope→홀드 확장 배수)
  decelExit: [0, 1] (감속 시 조기 trailing 활성화)
  = 2×2×2×3×3×2 = 144조합
- 3-fold WF + 슬리피지 스트레스
★슬리피지포함 | 🔄다음봉시가진입
"""
...
FEE = 0.0005
ANNUALIZE_FACTOR = np.sqrt(252 * 6)  # 240m = 6 bars/day
...
# -- c201 OBV 고정 (사실상 비활성: obvSB=0, obvVS=0) --
OBV_PERIOD = 20
OBV_SLOPE_BONUS = 0.0
OBV_VOL_SCALE = 0.0
...
MC_SLOW_LIST = [21, 26]
HS_LB_LIST = [3, 5]
HS_GATE_LIST = [0.0, 0.3, 0.6]   # ATR-정규화된 histogram slope 진입 최소값
HOLD_BONUS_LIST = [0.0, 0.5, 1.0]  # slope magnitude → hold bonus multiplier
DECEL_EXIT_LIST = [0, 1]            # slope 반전 시 조기 trailing 활성화
# 총: 2×2×2×3×3×2 = 144
...
def compute_macd_histogram(
    ...
    """MACD histogram = EMA(fast) - EMA(slow)."""
    ema_fast = ema_calc(closes, fast_period)
    ema_slow = ema_calc(closes, slow_period)
    return ema_fast - ema_slow


def compute_histogram_slope(
    histogram: np.ndarray, atr: np.ndarray, lookback: int,
) -> np.ndarray:
    """ATR-정규화된 histogram slope (기울기).
    slope = (hist[i] - hist[i-lookback]) / (lookback * ATR[i])
    """
    n = len(histogram)
    result = np.full(n, np.nan)
    for i in range(lookback, n):
        h_cur = histogram[i]
        h_prev = histogram[i - lookback]
        a = atr[i]
        if np.isnan(h_cur) or np.isnan(h_prev) or np.isnan(a) or a <= 0:
            continue
        result[i] = (h_cur - h_prev) / (lookback * a)
    return result
...
def backtest(
    ...
    macd_hist = compute_macd_histogram(c, mc_fast, mc_slow)
    hist_slope = compute_histogram_slope(macd_hist, atr_arr, hs_lb)

    returns: list[float] = []
    warmup = max(BUCKET_COUNT, EMA_PERIOD, RSI_PERIOD + 1,
                 MOM_LOOKBACK, ATR_PERIOD, VOL_SMA_PERIOD,
                 ATR_PCTILE_LB, mc_slow + hs_lb, 50) + 5
    i = warmup
    ...
    while i < n - 1:
        ...
        # ★ NEW: MACD histogram slope gate
        slope_ok = slope_val >= hs_gate

        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
                and atr_pctile_ok and body_ok and slope_ok):

            buy = o[i + 1] * (1 + FEE + slippage)
            peak_price = buy
            atr_at_entry = atr_val
            slope_at_entry = slope_val
            ...
            # ★ NEW: slope magnitude → hold bonus
            # slope_at_entry가 클수록 모멘텀 가속 → 보유 기간 확장
            slope_hold_bonus = 0
            if hold_bonus_mul > 0 and slope_at_entry > 0:
                # slope는 ATR-정규화됨, 보통 0~2 범위
                # 보너스: min(slope * mul, 1.0) * MAX_HOLD 만큼 추가
                bonus_frac = min(slope_at_entry * hold_bonus_mul, 1.0)
                slope_hold_bonus = int(bonus_frac * MAX_HOLD * 0.5)

            eff_max_hold = max(5, int((MAX_HOLD + slope_hold_bonus) * hold_scale))
            ...
            for j in range(i + 2, min(i + 1 + eff_max_hold, n)):
                ...
                # ★ NEW: decel exit — slope 반전 감지 시 trailing 조기 활성화
                decel_triggered = False
                if decel_exit == 1 and j < n:
                    cur_slope = hist_slope[j] if j < len(hist_slope) else np.nan
                    if not np.isnan(cur_slope) and cur_slope < 0:
                        # 감속 중: min_profit 요건 절반으로 완화
                        decel_triggered = True

                unrealized = peak_price - buy
                if decel_triggered:
                    # 감속 시 조기 trailing: 수익 요건 50% 완화
                    eff_min_profit = min_profit_dist * 0.5
                else:
                    eff_min_profit = min_profit_dist

                if unrealized >= eff_min_profit:
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 209 — MACD histogram slope 진입게이트 "
          "+ 적응적 홀드/조기청산 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe > c179(+42.878)")
    print("가설: c179 진입조건 위에 MACD slope gate 추가 → noise 필터, "
          "slope→hold bonus + decel exit → 수익 극대화")
    ...
    # -- BTC 데이터 --
    df_btc_full = load_historical("KRW-BTC", "240m", "2022-01-01", "2026-12-31")
    if df_btc_full.empty:
        print("BTC 데이터 없음.")
        return

    print(f"BTC 240m: {len(df_btc_full)}행")

    # -- 심볼별 데이터 확인 --
    print("\n--- 심볼별 데이터 확인 ---")
    sym_data_ok = []
    for sym in SYMBOLS:
        df_check = load_historical(sym, "240m", "2022-05-01", "2026-03-31")
    ...
    combos = list(product(
        MC_FAST_LIST, MC_SLOW_LIST,
        HS_LB_LIST, HS_GATE_LIST,
        HOLD_BONUS_LIST, DECEL_EXIT_LIST,
    ))
    ...
    btc_train_c: dict[str, np.ndarray] = {}
    btc_train_s: dict[str, np.ndarray] = {}
    for sym in sym_data_ok:
        if sym in sym_train_data:
            bc, bs = align_btc_to_symbol(
                sym_train_data[sym], df_btc_full, BTC_SMA_PERIOD)
            btc_train_c[sym] = bc
            btc_train_s[sym] = bs

    results: list[dict] = []
    for idx, (mcf, mcs, hslb, hsg, hbm, dex) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(
                sym_train_data[sym],
                btc_train_c[sym], btc_train_s[sym],
                mcf, mcs, hslb, hsg, hbm, dex,
            )
            sym_results.append(r)
        pooled = pool_results(sym_results)
        results.append({
            "mcf": mcf, "mcs": mcs, "hslb": hslb,
            "hsg": hsg, "hbm": hbm, "dex": dex,
            **pooled,
        })
    ...
    if not valid:
        print("유효 결과 없음 (n<10 or all NaN).")
        ...
        if any_result:
            ...
            for r in any_result[:5]:
                print(f"  mcF={r['mcf']} mcS={r['mcs']} hsLB={r['hslb']} "
                      f"hsG={r['hsg']} hbM={r['hbm']} dex={r['dex']} → "
    ...
    for rank, r in enumerate(top5, 1):
        print(f"  #{rank} mcF={r['mcf']} mcS={r['mcs']} hsLB={r['hslb']} "
              f"hsG={r['hsg']} hbM={r['hbm']} dex={r['dex']} → "
    ...
    for rank, combo in enumerate(top5, 1):
        ...
        print(f"\n--- #{rank}: mcF={mcf} mcS={mcs} hsLB={hslb} "
              f"hsG={hsg} hbM={hbm} dex={dex} ---")
    ...
    mcf = best_combo["mcf"]
    mcs = best_combo["mcs"]
    hslb = best_combo["hslb"]
    hsg = best_combo["hsg"]
    hbm = best_combo["hbm"]
    dex = best_combo["dex"]

    print(f"최적: mcF={mcf} mcS={mcs} hsLB={hslb} "
          f"hsG={hsg} hbM={hbm} dex={dex}")
    ...
    # -- 심볼별 OOS 분해 --
    print("\n" + "=" * 80)
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: mcF={mcf} mcS={mcs} "
          f"hsLB={hslb} hsG={hsg} hbM={hbm} dex={dex}) ===")
    ...
    print(f"  c179 기준 (vol regime adaptive): "
          f"avg_OOS=+{c179_baseline} n=~60")
    print(f"  c209 최적 (mcF={mcf} mcS={mcs} hsLB={hslb} "
          f"hsG={hsg} hbM={hbm} dex={dex}): "
          f"avg_OOS={best_oos_sharpe:+.3f} n={total_n}")
    print(f"  Δ Sharpe: {delta:+.3f} ({status_vs_179})")
    print(f"  Δ trades: {total_n - 60} (vs c179 ~60)")

    # -- 최종 요약 --
    print("\n" + "=" * 80)
    print("=== 최종 요약 ===")
    overall_status = ("PASS" if best_oos_sharpe > 5.0 and total_n >= 10
                      else "FAIL")
    print(f"★ OOS 최적: mcF={mcf} mcS={mcs} hsLB={hslb} "
          f"hsG={hsg} hbM={hbm} dex={dex}")
    print(f"  (c201 고정: obvP={OBV_PERIOD} obvSB={OBV_SLOPE_BONUS} "
          f"obvVS={OBV_VOL_SCALE})")
