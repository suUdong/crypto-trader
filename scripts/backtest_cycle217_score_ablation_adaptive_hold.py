"""
vpin_multi 사이클 217 — Score Component Ablation + Score-Adaptive Hold Duration
- 기반: c213 OOS Sharpe +37.362 (Entry Quality Score 7컴포넌트)
  c213 결과: tpSM=0, trSM=0, slSM=0.10 → 스코어 기반 TP/Trail 무용
  c199 기준: avg_OOS=+51.425 (regime dual exit) BEST
- 문제:
  1) 7개 컴포넌트 동일 가중치 → 약한 컴포넌트가 강한 것을 희석
  2) 스코어→TP/SL/Trail 매핑은 효과 없었지만 hold 기간 미탐색
  3) 어떤 컴포넌트가 실제 예측력 있는지 ablation 미수행
- 가설:
  A) 7개 중 일부 컴포넌트는 노이즈 — 제거 시 Sharpe 개선
  B) 고확신(고점수) 진입 → 더 오래 홀드(fat tail 포착)
  C) 저확신 진입 → 짧게 홀드(빠른 손절)
  D) 스코어→hold 매핑이 TP/SL 조정보다 자연스러움
- 탐색 그리드:
  ABLATION_MODE: 8가지 (all, -vpin, -mom, -rsi_vel, -vol, -regime, -body, -squeeze)
  HOLD_BONUS_PER_SCORE: [0, 2, 4]    — 스코어 초과분당 추가 홀드 바
  SL_SCORE_MULT: [0.0, 0.10]         — c213 검증: slSM=0.10 소폭 도움
  SCORE_MIN: [1.0, 1.5, 2.0]         — 더 넓은 범위
  = 8×3×2×3 = 144 combos
...
# -- c217 탐색 그리드 --
# ablation: 0=all, 1~7=drop that component
ABLATION_MODES = [
    "all",         # 0: 7컴포넌트 전부 (c213 베이스라인)
    "-vpin",       # 1: score_vpin 제거
    "-mom",        # 2: score_mom 제거
    "-rsi_vel",    # 3: score_rsi_vel 제거
    "-vol",        # 4: score_volume 제거
    "-regime",     # 5: regime_score 제거
    "-body",       # 6: score_body 제거
    "-squeeze",    # 7: squeeze_bonus 제거
]
HOLD_BONUS_LIST = [0, 2, 4]        # 스코어 초과분당 추가 홀드 바
SL_SCORE_MULT_LIST = [0.0, 0.10]   # c213 검증
SCORE_MIN_LIST = [1.0, 1.5, 2.0]
...
def compute_entry_score(
    ...
    vol_ratio: float, atr_pctile_val: float, body_val: float,
    bb_width_pctile_arr: np.ndarray, idx: int,
    ablation: str,
) -> float:
    """Entry Quality Score (0~6.5). ablation으로 특정 컴포넌트 제거."""
    components: dict[str, float] = {}

    # score_vpin
    components["vpin"] = min(1.0, max(0.0,
        (VPIN_LOW - vpin_val) / VPIN_LOW))

    # score_mom
    components["mom"] = min(1.0, max(0.0,
        (mom_val - MOM_THRESH) / (MOM_THRESH + 1e-9)))

    # score_rsi_vel
    components["rsi_vel"] = min(1.0, max(0.0,
        (rsi_delta - RSI_DELTA_MIN) / (RSI_DELTA_MIN + 1e-9)))

    # score_volume
    components["vol"] = min(1.0, max(0.0,
        (vol_ratio - VOL_MULT) / (VOL_MULT + 1e-9)))
    ...
    # score_body
    components["body"] = min(1.0, max(0.0,
        (body_val - BODY_RATIO_MIN) / (BODY_RATIO_MIN + 1e-9)))

    # squeeze_bonus
    squeeze_bonus = 0.0
    if not np.isnan(bb_width_pctile_arr[idx]):
        for eb in range(0, EXPAND_BARS + 1):
            ...
            if idx_back >= 0 and not np.isnan(bb_width_pctile_arr[idx_back]):
                if bb_width_pctile_arr[idx_back] <= SQUEEZE_PCTILE_TH:
                    squeeze_bonus = 0.5 * (
                        1.0 - bb_width_pctile_arr[idx_back] / 100.0)
                    break
    components["squeeze"] = squeeze_bonus

    # ablation: 특정 컴포넌트 제거
    drop_map = {
        "-vpin": "vpin", "-mom": "mom", "-rsi_vel": "rsi_vel",
        "-vol": "vol", "-regime": "regime", "-body": "body",
        "-squeeze": "squeeze",
    }
    if ablation in drop_map:
        components[drop_map[ablation]] = 0.0

    return sum(components.values()), components.get("regime", 0.0)
...
def backtest(
    ...
    warmup = max(BUCKET_COUNT, EMA_PERIOD, RSI_PERIOD + 1,
                 ...
                 BB_PERIOD + SQUEEZE_LB + EXPAND_BARS, 50) + 5
    i = warmup
    ...
    while i < n - 1:
        ...
        # === c217: Entry Quality Score (ablation 지원) ===
        total_score, regime_score = compute_entry_score(
            vpin_val, mom_val, rsi_delta, vol_ratio,
            atr_pctile_val, body_val,
            bb_width_pctile_arr, i, ablation)
        ...
        score_excess = total_score - score_min
        ...
        # c217: score-based SL tightening (c213 검증: slSM=0.10 소폭 도움)
        score_sl_tighten = sl_score_mult * score_excess
        effective_sl_mult = (SL_BASE_ATR - SL_BONUS_ATR * rsi_ratio
                             - sl_tighten - score_sl_tighten)
        ...
        # === c217 핵심: score-adaptive hold duration ===
        hold_extra = int(hold_bonus * score_excess)
        max_hold = MAX_HOLD_BASE + int(6 * regime_score) + hold_extra
...
def build_combos() -> list[dict]:
    combos = []
    for abl, hb, sl_sm, sc_min in product(
        ABLATION_MODES, HOLD_BONUS_LIST,
        SL_SCORE_MULT_LIST, SCORE_MIN_LIST,
    ):
        combos.append({
            "ablation": abl,
            "hold_bonus": hb,
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi c217 — Score Component Ablation + Adaptive Hold ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 52 AND trades >= 12")
    print("가설: 약한 컴포넌트 제거→Sharpe 개선 + 고확신=장기홀드")
    print(f"기준선: c199 OOS +51.425 / c213 OOS +37.362")
    ...
    print(f"Ablation modes: {', '.join(ABLATION_MODES)}")
    print(f"Hold bonus: {HOLD_BONUS_LIST}  SL mult: {SL_SCORE_MULT_LIST}  "
          f"Score min: {SCORE_MIN_LIST}")
    print("=" * 80)
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            r = backtest(
                df_tr,
                combo["score_min"], combo["hold_bonus"],
                combo["sl_score_mult"], combo["ablation"],
    ...
    # -- Ablation 분석: 각 모드별 최고 train Sharpe --
    print(f"\n{'=' * 80}")
    print("=== Ablation 분석 (모드별 최고 train Sharpe) ===")
    for abl in ABLATION_MODES:
        abl_results = [r for r in valid if r["ablation"] == abl]
        if abl_results:
            best_abl = max(abl_results, key=lambda x: x["sharpe"])
            print(f"  {abl:>10}: Sharpe={best_abl['sharpe']:+.3f}  "
                  f"WR={best_abl['wr']:.1%}  n={best_abl['trades']}  "
                  f"hb={best_abl['hold_bonus']} slSM={best_abl['sl_score_mult']:.2f} "
                  f"scMin={best_abl['score_min']:.1f}")
        else:
            print(f"  {abl:>10}: 유효 결과 없음")

    print(f"\n=== Train Top 16 (pooled Sharpe 기준) ===")
    hdr = (f"{'ablation':>10} {'scMin':>6} {'hb':>3} {'slSM':>5} | "
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['ablation']:>10} {r['score_min']:>6.1f} "
            f"{r['hold_bonus']:>3} "
            f"{r['sl_score_mult']:>5.2f} | "
    ...
    for r in valid:
        key = (r["ablation"], r["score_min"],
               r["hold_bonus"], r["sl_score_mult"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(
                    df_test,
                    params["score_min"], params["hold_bonus"],
                    params["sl_score_mult"], params["ablation"],
                    btc_c, btc_s, esp, vol_mom, atr_pctile, bb_wp)
        ...
        if oos_sharpes:
            ...
            print(
                f"  #{rank}: {params['ablation']:>10} "
                f"scMin={params['score_min']:.1f} "
                f"hb={params['hold_bonus']} "
                f"slSM={params['sl_score_mult']:.2f} | "
    ...
    print(f"파라미터: {best['ablation']} scMin={best['score_min']:.1f} "
          f"hb={best['hold_bonus']} slSM={best['sl_score_mult']:.2f}")
    ...
    for slip in SLIPPAGE_LEVELS:
        ...
        for fold in WF_FOLDS:
            for sym in sym_data_ok:
                ...
                r = backtest(
                    df_test,
                    best["score_min"], best["hold_bonus"],
                    best["sl_score_mult"], best["ablation"],
                    btc_c, btc_s, esp, vol_mom, atr_pctile, bb_wp,
                    slippage=slip)
                if r["trades"] > 0 and not np.isnan(r["sharpe"]):
    ...
    c213_baseline = 37.362
    print(f"  c199 기준 (regime dual exit): avg_OOS={c199_baseline:+.3f}")
    print(f"  c213 기준 (entry quality score): avg_OOS={c213_baseline:+.3f}")
    print(f"  c217 최적: avg_OOS={best['avg_oos']:+.3f}")
    delta199 = best["avg_oos"] - c199_baseline
    delta213 = best["avg_oos"] - c213_baseline
    label199 = "개선" if delta199 > 0 else "악화"
    label213 = "개선" if delta213 > 0 else "악화"
    print(f"  Δ vs c199: {delta199:+.3f} ({label199})")
    print(f"  Δ vs c213: {delta213:+.3f} ({label213})")

    # -- 최종 요약 --
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: ablation={best['ablation']} "
          f"scMin={best['score_min']:.1f} "
          f"hb={best['hold_bonus']} "
          f"slSM={best['sl_score_mult']:.2f}")
    print(f"  (c199 고정: rTh={REGIME_TH} hiTP={HI_TP_BONUS} "
          f"hiTr={HI_TRAIL_RELAX} loSL={LO_SL_TIGHTEN})")
    print(f"  (c192 고정: ttA={TRAIL_TIGHTEN_AFTER} ttF={TRAIL_TIGHTEN_FACTOR})")
    print(f"  (c190 고정: vMomLB={VOL_MOM_LB} vMomMin={VOL_MOM_MIN} "
