...
# ─── c224 탐색 그리드 (c219 winner 주변 refinement) ──────────
# 공통 tp1M/pRat (ETH/XRP 적용)
TP1_MULT_LIST = [2.0, 2.5, 3.0, 3.5]
PART_RATIO_LIST = [0.5, 0.7]
# SOL 전용 — 독립 탐색
SOL_TP1_MULT_LIST = [1.5, 2.0, 2.5]
SOL_PART_RATIO_LIST = [0.5, 0.7]
# c219서 확정된 고정값
...
def main() -> None:
    ...
    # 그리드 정의 — (공통 tp1M, 공통 pRat, SOL tp1M, SOL pRat)
    grid = list(product(
        TP1_MULT_LIST, PART_RATIO_LIST,
        SOL_TP1_MULT_LIST, SOL_PART_RATIO_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")
    ...
    for gi, combo in enumerate(grid):
        tp1_m, part_r, sol_tp1_m, sol_part_r = combo
    ...
    print("=== Top 15 결과 ===")
    print("=" * 80)
    hdr = (f"{'tp1M':>5} {'pRat':>5} {'sTp1':>5} {'sPr':>5} | "
    ...
    if valid:
        ...
        print(f"=== 심볼별 OOS 성능 분해 (Top 1: tp1M={bp[0]:.1f} "
              f"pRat={bp[1]:.1f} solTp1={bp[2]:.1f} solPr={bp[3]:.1f}) ===")
        ...
        for sym in SYMBOLS:
            ...
            sym_total_n = 0
            if sym == "KRW-SOL":
                use_tp1, use_pr = bp[2], bp[3]
            else:
                use_tp1, use_pr = bp[0], bp[1]
    ...
    print("\n=== SOL 전용 파라미터 효과 분석 ===")
    for stp in SOL_TP1_MULT_LIST:
        subset = [r for r in valid if r["params"][2] == stp]
        if subset:
            avg_sol = np.mean([r["sol_avg_sharpe"] for r in subset[:10]])
            avg_sh = np.mean([r["avg_sharpe"] for r in subset[:10]])
            print(f"  solTp1={stp:.1f}: top10 SOL_avg={avg_sol:+.3f}  "
                  f"avg={avg_sh:+.3f}")
    print()
    for spr in SOL_PART_RATIO_LIST:
        subset = [r for r in valid if r["params"][3] == spr]
        if subset:
            avg_sol = np.mean([r["sol_avg_sharpe"] for r in subset[:10]])
            print(f"  solPr={spr:.1f}: top10 SOL_avg={avg_sol:+.3f}")
