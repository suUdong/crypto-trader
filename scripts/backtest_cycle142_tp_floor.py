"""
사이클 142: stealth_3gate TP 하한 탐색 (TP=3%/4%)
- 사이클 141 확정: TP=5%/SL=1.0% → W2 Sharpe 12.126
- 패턴: TP 낮을수록 W2 Sharpe 단조증가 (5%>6%>7%>8%)
- 질문: TP=3%/4%에서도 개선이 지속되는가? 거래비용(왕복 ~0.2%) 감안 시 하한은?
- 탐색: TP ∈ {0.03, 0.04, 0.05} × SL ∈ {0.008, 0.010} (6조합, 0.05는 베이스라인)
...
BASELINE_TP = 0.05
BASELINE_SL = 0.010
BASELINE_W2_SHARPE = 12.126

# 탐색 대상: TP 하한 탐색 + 베이스라인 포함
TP_CANDIDATES: list[float] = [0.03, 0.04, 0.05]
...
def main() -> None:
    t0 = time.time()
    print("=== stealth_3gate TP 하한 탐색 (사이클 142) ===")
    ...
    print(f"거래비용: {FEE*100:.2f}% 왕복 (TP=3% 실수익 상한 ~2.8%)")
    ...
    for r in results:
        ...
        base_mark = " ← 현재" if r["is_baseline"] else ""
        print(
            f"{r['TP']:>6} {r['SL']:>6} | "
            f"{r['W1_sh']:>8.3f} {r['W1_n']:>6} | "
            f"{r['W2_sh']:>8.3f} {r['W2_n']:>6} {wr_str:>7} | "
            f"{delta_str} | {mark}{base_mark}"
        )
    ...
    # CSV 저장
    out_csv = _root / "scripts" / "cycle142_tp_floor_results.csv"
