"""
사이클 246: c243 최적 고정 + 캔들 품질 진입 필터
...
문제 분석:
  - F3 WR 60%: 최근 시장에서 false breakout 진입이 많음
  - 기존 진입: Donchian 돌파 + ADX + BTC SMA + 모멘텀 + 거래량 등 "수치" 기반
  - 가설: 캔들 자체의 "품질"(body ratio, close position)로 진입 신뢰도 개선 가능
    → 몸통이 큰 양봉(body_ratio 높음) = 매수세 강한 돌파
    → close가 high 근처(close_pos 높음) = 종가까지 매수세 유지
  - 추가: breakout bar의 거래량이 직전 N봉 대비 극단적으로 높은지 확인
  - 진입 필터 강화만으로 F3 WR/Sharpe 향상 목표 (출구 변경 없음)
...
새 탐색 그리드 (c246):
  BODY_RATIO_MIN:    [0.0, 0.3, 0.5, 0.7]  — 캔들 몸통 비율 최소값 (0=OFF)
  CLOSE_POS_MIN:     [0.0, 0.5, 0.7]        — 종가 위치 최소값 (0=OFF)
  VOL_SPIKE_MULT:    [0.0, 2.0, 3.0]        — 직전 N봉 대비 거래량 스파이크 (0=OFF)
  = 4 × 3 × 3 = 36 combos
...
# ─── c246 탐색 그리드 ─────────────────────────────────────────
BODY_RATIO_MIN_LIST = [0.0, 0.3, 0.5, 0.7]   # 캔들 몸통 비율 (0=OFF)
CLOSE_POS_MIN_LIST = [0.0, 0.5, 0.7]          # 종가 위치 (0=OFF)
VOL_SPIKE_MULT_LIST = [0.0, 2.0, 3.0]         # 거래량 스파이크 배수 (0=OFF)
...
def run_backtest(
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # 종가 위치: (close - low) / (high - low) — 1.0이면 high에서 마감
            if close_pos_min > 0.0:
                close_pos = (c[i] - lo[i]) / candle_range
                if close_pos < close_pos_min:
                    continue

            # 거래량 스파이크: 직전 N봉 대비 vol_spike_mult 배 이상
            if vol_spike_mult > 0.0 and i >= VOL_SMA_PERIOD:
                vol_avg_prev = np.mean(v[i - VOL_SMA_PERIOD:i])
                if vol_avg_prev > 0 and v[i] / vol_avg_prev < vol_spike_mult:
                    continue
...
def main() -> None:
    print("=" * 80)
    print("=== c246: c243최적 고정 + 캔들 품질 진입 필터 ===")
    ...
    print(f"탐색: BODY_RATIO_MIN × CLOSE_POS_MIN × VOL_SPIKE_MULT")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        BODY_RATIO_MIN_LIST, CLOSE_POS_MIN_LIST, VOL_SPIKE_MULT_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c243 최적, 캔들 필터 OFF)
    print("\n--- 베이스라인 (c243 최적, 캔들필터=OFF) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    print("\n=== 거래량 스파이크 효과 ===")
    for val in VOL_SPIKE_MULT_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  vsMlt={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  brMin={p[0]:.1f} cpMin={p[1]:.1f} vsMlt={p[2]:.1f} | "
    ...
    if valid:
        ...
        print(f"  c246 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: brMin={p[0]:.1f} cpMin={p[1]:.1f} "
              f"vsMlt={p[2]:.1f}")
        print(f"  (c243 고정: btcMLB={BTC_MOM_LB} btcMMin={BTC_MOM_MIN} "
              f"btcADX={BTC_ADX_GATE} boMgn={BREAKOUT_MARGIN})")
