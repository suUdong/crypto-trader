"""
vpin_multi 사이클 173 — c165 최적 기반 ADX 추세 필터 + BTC SMA 기간 그리드
...
- c165 약점:
  1) Fold 3 Sharpe 하락 (ETH 8.3 SOL 9.5 XRP 8.3) → 횡보/약세 구간 false signal
  2) BTC_SMA=200 고정 — 최적 기간 미검증
  3) 추세 강도 필터 없음 → choppy 구간 진입 허용
- 가설:
  A) ADX(14) > threshold → 비추세 구간 진입 차단, Fold 3 Sharpe 개선
  B) BTC SMA 기간 [100, 150, 200, 250] 그리드 → 레짐 게이트 최적화
  C) ADX 없이(=0) + BTC SMA 변경만으로도 개선 가능성 확인 (ablation)
- 탐색 그리드: 4 ADX_THRESH × 4 BTC_SMA = 16 combos × 3 symbols
  c165 최적 파라미터 고정 (VPIN=0.35 MOM=0.0007 Hold=20 CD=4)
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드 --
ADX_THRESH_LIST = [0, 15, 20, 25]  # 0 = ADX 필터 OFF (c165 베이스라인)
BTC_SMA_LIST = [100, 150, 200, 250]  # 200 = c165 베이스라인
...
def backtest(
    ...
    while i < n - 1:
        ...
        if vpin_ok and btc_ok and rsi_velocity_ok and vol_ok and adx_ok:
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 173 — ADX 추세 필터 + BTC SMA 기간 그리드 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  목표: OOS Sharpe >= 11.0 (c165 이상), trades >= 200")
    print("가설 A: ADX >= 15~25 → 비추세 구간 false signal 차단, Fold 3 개선")
    print("가설 B: BTC SMA [100,150,200,250] → 레짐 게이트 최적화")
    print("가설 C: ADX=0(OFF) + BTC SMA 변경만으로 개선 가능 (ablation)")
    ...
    results: list[dict] = []
    for idx, (adx_th, btc_sma) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            if sym not in sym_train_data:
                continue
            df_tr = sym_train_data[sym]
            btc_c, btc_s = align_btc_to_symbol(df_tr, df_btc_full, btc_sma)
            r = backtest(df_tr, adx_th, btc_c, btc_s)
            sym_results.append(r)

        pooled = pool_results(sym_results)
        results.append({
            "adx_thresh": adx_th, "btc_sma": btc_sma,
            **pooled,
        })
    ...
    for rank, params in enumerate(unique_top, 1):
        adx_th = params["adx_thresh"]
        btc_sma = params["btc_sma"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                btc_c, btc_s = align_btc_to_symbol(
                    df_test, df_btc_full, btc_sma)
                r = backtest(df_test, adx_th, btc_c, btc_s)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: ADX={adx_th} BTC_SMA={btc_sma} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        adx_th = params["adx_thresh"]
        btc_sma = params["btc_sma"]
        print(f"\n--- #{rank}: ADX={adx_th} BTC_SMA={btc_sma} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                if df_full.empty:
                    continue
                btc_c, btc_s = align_btc_to_symbol(
                    df_full, df_btc_full, btc_sma)
                r = backtest(df_full, adx_th, btc_c, btc_s,
                             slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
            sh = pooled["sharpe"] if not np.isnan(pooled["sharpe"]) else 0.0
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    adx_th = best["adx_thresh"]
    btc_sma = best["btc_sma"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: ADX={adx_th} BTC_SMA={btc_sma}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    # ADX=0 BTC_SMA=200 = c165 베이스라인
    baseline = None
    for wr in wf_results:
        if wr["adx_thresh"] == 0 and wr["btc_sma"] == 200:
            baseline = wr
            break
    if baseline:
        print(f"  c165 재현 (ADX=0 SMA=200): avg_OOS={baseline['avg_oos_sharpe']:+.3f} "
              f"n={baseline['total_oos_trades']}")
    print(f"  c173 최적 (ADX={best['adx_thresh']} SMA={best['btc_sma']}): "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: ADX_THRESH={best['adx_thresh']} BTC_SMA={best['btc_sma']}")
    print(f"  (c165 고정: VPIN={VPIN_LOW} MOM={MOM_THRESH} Hold={MAX_HOLD} CD={COOLDOWN_BARS})")
