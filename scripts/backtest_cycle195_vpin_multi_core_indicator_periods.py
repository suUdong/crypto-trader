"""
vpin_multi 사이클 195 — 핵심 지표 주기 탐색 (RSI/EMA/MOM)
- 기반: c192 최적 전체 복원 (c194 BTC 필터 탐색 악화 → c192 베이스라인 복귀)
  c192: VPIN=0.4 MOM=0.0005 bSMA=200 rCeil=65 rFlr=20 avg_OOS=+20.999 n=36
  c194: bSMA=100 rCeil=60 rFlr=30 avg_OOS=+15.660 n=14 (악화, 폐기)
- 문제:
  1) RSI_PERIOD=14 — 표준값이나 최적 보장 없음. 단축→민감, 연장→안정
  2) EMA_PERIOD=20 — 추세 기준선 속도. 단축→빠른 반응, 연장→노이즈 필터
  3) MOM_LOOKBACK=8 — 모멘텀 계산 윈도우. 짧으면 민감, 길면 안정
  ★ 이 3개는 c164 이후 한 번도 탐색되지 않은 고정 디폴트
- 가설:
  A) RSI 주기 단축 → 더 빠른 과매도/과매수 감지 → 진입 타이밍 개선
  B) EMA 주기 조정 → 추세 확인 속도 최적화
  C) MOM 윈도우 조정 → 모멘텀 신호 품질 개선
  ★ 이 3개는 진입 조건의 "센서 해상도" — 필터 추가 없이 기존 신호 품질 개선
- 탐색 그리드:
  RSI_PERIOD:   [10, 12, 14, 16]     — 4수준
  EMA_PERIOD:   [15, 20, 25, 30]     — 4수준
  MOM_LOOKBACK: [6, 8, 10, 12]       — 4수준
  = 4×4×4 = 64 combos
...
# -- c192 최적 고정 (c194 결과 폐기, c192 복원) --
VPIN_LOW = 0.4
MOM_THRESH = 0.0005
MAX_HOLD = 20
COOLDOWN_BARS = 4

BTC_SMA_PERIOD = 200   # c192 최적 (c194 100 악화)
RSI_CEILING = 65        # c192 최적 (c194 60 악화)
RSI_FLOOR = 20          # c192 최적 (c194 30 악화)
...
# -- 탐색 그리드: 핵심 지표 주기 --
RSI_PERIOD_LIST = [10, 12, 14, 16]
EMA_PERIOD_LIST = [15, 20, 25, 30]
MOM_LOOKBACK_LIST = [6, 8, 10, 12]
...
def build_combos() -> list[dict]:
    combos = []
    for rsi_p, ema_p, mom_lb in product(
        RSI_PERIOD_LIST, EMA_PERIOD_LIST, MOM_LOOKBACK_LIST,
    ):
        combos.append({
            "rsi_period": rsi_p,
            "ema_period": ema_p,
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 195 — 핵심 지표 주기 탐색 (RSI/EMA/MOM) ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 22 AND trades >= 30")
    print("가설: 미탐색 디폴트 지표 주기 최적화 → 센서 해상도 개선 → 신호 품질↑")
    print(f"기준선: c192 OOS +20.999, WR 54.4%, trades 36")
    print(f"  c194 결과 폐기 (악화): bSMA=100 rCeil=60 rFlr=30 → +15.660 n=14")
    print(f"c192 복원: VPIN={VPIN_LOW} MOM={MOM_THRESH} "
    ...
    print(f"  탐색: RSI_P={RSI_PERIOD_LIST} EMA_P={EMA_PERIOD_LIST} "
          f"MOM_LB={MOM_LOOKBACK_LIST}")
    print("=" * 80)
    ...
    for idx, combo in enumerate(combos):
        rsi_p = combo["rsi_period"]
        ema_p = combo["ema_period"]
        mom_lb = combo["mom_lookback"]
        ...
        for sym in sym_data_ok:
            ...
            btc_s_aligned = btc_sma_s.reindex(
                df_tr.index, method="ffill").values
            r = backtest(df_tr, rsi_p, ema_p, mom_lb,
                         btc_c_aligned, btc_s_aligned)
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['rsi_period']:>4} {r['ema_period']:>4} "
            f"{r['mom_lookback']:>5} | "
    ...
    for r in valid:
        key = (r["rsi_period"], r["ema_period"], r["mom_lookback"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                btc_s_aligned = btc_sma_s.reindex(
                    df_test.index, method="ffill").values
                r = backtest(df_test, rsi_p, ema_p, mom_lb,
                             btc_c_aligned, btc_s_aligned)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: rsiP={rsi_p} emaP={ema_p} "
                  f"momLB={mom_lb} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                btc_s_aligned = btc_sma_s.reindex(
                    df_full.index, method="ffill").values
                r = backtest(df_full, rsi_p, ema_p, mom_lb,
                             btc_c_aligned, btc_s_aligned,
                             slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    rsi_p = best["rsi_period"]
    ema_p = best["ema_period"]
    mom_lb = best["mom_lookback"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: rsiP={rsi_p} "
          f"emaP={ema_p} momLB={mom_lb}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c192 최적 (rsiP=14 emaP=20 momLB=8): "
          f"avg_OOS=+20.999 n=36")
    print(f"  c195 최적 (rsiP={best['rsi_period']} "
          f"emaP={best['ema_period']} "
          f"momLB={best['mom_lookback']}): "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: RSI_PERIOD={best['rsi_period']} "
          f"EMA_PERIOD={best['ema_period']} "
          f"MOM_LOOKBACK={best['mom_lookback']}")
