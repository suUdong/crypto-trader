"""
사이클 256: c250 최적 고정 + ADX 강도 비례 RSI 상한 완화 + 적응형 breakout margin
...
문제:
  - F2 (2025-03 ~ 2025-11) Sharpe +27.352 → 목표 30 미달
  - RSI_CEIL=80 고정이 강한 추세에서 유효 진입을 차단 (추세 강할수록 RSI 높음)
  - BREAKOUT_MARGIN=0.002 고정이 저변동성 구간에서 과도 필터링

가설:
  - ADX가 높을수록(강추세) RSI 상한을 올림 → RSI 80~90 구간의 유효 모멘텀 진입 포착
  - breakout margin을 ATR 기반으로 적응 → 저변동 시 작은 margin, 고변동 시 큰 margin
  - 이 두 조합이 F2 구간의 횡보→추세 전환 초기를 더 잘 포착할 수 있음
...
새 탐색 그리드 (c256):
  RSI_CEIL_BASE:  [75, 80]         — 기본 RSI 상한
  RSI_ADX_BOOST:  [0, 0.3, 0.5]   — ADX 30+ 초과분 * boost = RSI 상한 추가분
  BO_MGN_ATR_MULT: [0.0, 0.3, 0.5, 0.7]  — breakout margin = ATR*mult/price
                                            (0=기존 고정 0.002 사용)
  = 2 × 3 × 4 = 24 combos

RSI 상한 공식: rsi_ceil_effective = RSI_CEIL_BASE + max(0, ADX - 30) * RSI_ADX_BOOST
  - ADX=25: rsi_ceil = 75 or 80 (기본)
  - ADX=40, boost=0.5: rsi_ceil = 75 + 10*0.5 = 80 or 80 + 10*0.5 = 85
  - cap at 95

breakout margin 공식: bo_margin = max(0.001, ATR/price * BO_MGN_ATR_MULT)
  - 저변동(ATR 작음) → margin 자동 축소 → 진입 쉬움
  - 고변동(ATR 큼) → margin 자동 확대 → 노이즈 필터링
...
# ─── c250 최적 고정 (진입 완화) ───────────────────────────────
FIXED_BREAKOUT_MARGIN = 0.002
ATR_PCTILE_MIN = 15
VOL_RATIO_MIN = 1.5
BTC_MOM_MIN = 0.020

# ─── c256 탐색 그리드 ─────────────────────────────────────────
RSI_CEIL_BASE_LIST = [75, 80]
RSI_ADX_BOOST_LIST = [0.0, 0.3, 0.5]
BO_MGN_ATR_MULT_LIST = [0.0, 0.3, 0.5, 0.7]
...
def run_backtest(
    ...
    rsi_ceil_base: float,
    rsi_adx_boost: float,
    bo_mgn_atr_mult: float,
    oos_start: str,
    oos_end: str,
    index: pd.DatetimeIndex,
) -> list[dict]:
    """c250 최적 고정 + ADX 비례 RSI 상한 + 적응형 breakout margin."""
    n = len(c)
    trades: list[dict] = []
    position = None
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c256 핵심 1: ADX 비례 RSI 상한 완화 ───────────
            effective_rsi_ceil = rsi_ceil_base
            if rsi_adx_boost > 0 and adx_val[i] > 30:
                effective_rsi_ceil = min(
                    95, rsi_ceil_base + (adx_val[i] - 30) * rsi_adx_boost)

            if not np.isnan(rsi_arr[i]):
                if rsi_arr[i] > effective_rsi_ceil or rsi_arr[i] < RSI_FLOOR:
                    continue
            ...
            # ── c256 핵심 2: 적응형 breakout margin ───────────
            if bo_mgn_atr_mult > 0.0:
                # ATR 기반 동적 margin
                adaptive_margin = max(
                    0.001, (atr_val[i] / c[i]) * bo_mgn_atr_mult)
                if dc_up[i] > 0 and c[i] < dc_up[i] * (1.0 + adaptive_margin):
                    continue
            else:
                # 기존 고정 margin
                if FIXED_BREAKOUT_MARGIN > 0.0 and dc_up[i] > 0:
                    if c[i] < dc_up[i] * (1.0 + FIXED_BREAKOUT_MARGIN):
                        continue
...
def main() -> None:
    print("=" * 80)
    print("=== c256: c250최적 고정 + ADX비례 RSI상한 완화 + 적응형 breakout margin ===")
    print("=== 심볼: ETH/SOL/XRP | 240m | 슬리피지포함 | 다음봉시가진입 ===")
    print(f"c250 고정: boMgn(고정)={FIXED_BREAKOUT_MARGIN} "
          f"aPct={ATR_PCTILE_MIN} vRat={VOL_RATIO_MIN} btcMM={BTC_MOM_MIN}")
    print(f"c241 고정: rFlr={RSI_FLOOR} mLB={MOM_LB} mMin={MOM_MIN}")
    print(f"c233 고정: trail={BASE_TRAIL_MULT} TP2={BASE_TP2_MULT} "
          f"SL={BASE_SL_MULT} MH={BASE_MAX_HOLD} TP1={BASE_TP1_MULT}")
    print(f"탐색: RSI_CEIL_BASE × RSI_ADX_BOOST × BO_MGN_ATR_MULT")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        RSI_CEIL_BASE_LIST, RSI_ADX_BOOST_LIST, BO_MGN_ATR_MULT_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c250 최적 — RSI_CEIL=80 고정, boMgn=0.002 고정)
    print("\n--- 베이스라인 (c250 최적, 고정 RSI=80/boMgn=0.002) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for gi, combo in enumerate(grid):
        rsi_base, rsi_boost, bo_atr_mult = combo
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                ...
                raw = run_backtest(
                    ...
                    sp["atr_pctile"], sp["vol_sma"], sp["rsi"],
                    SYM_SL_SCALE[sym], sym,
                    rsi_base, rsi_boost, bo_atr_mult,
                    window["oos_start"], window["oos_end"],
                    sp["index"],
                )
                agg = aggregate_trades(raw)
    ...
    for val in RSI_CEIL_BASE_LIST:
        ...
        if sub:
            ...
            print(f"  rBase={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")

    print("\n=== RSI_ADX_BOOST 효과 ===")
    for val in RSI_ADX_BOOST_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  rBst={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")

    print("\n=== BO_MGN_ATR_MULT 효과 ===")
    for val in BO_MGN_ATR_MULT_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  boAM={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  rBase={p[0]:.0f} rBst={p[1]:.1f} boAM={p[2]:.1f} | "
    ...
    if valid:
        ...
        print(f"  c256 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: rBase={p[0]:.0f} rBst={p[1]:.1f} "
              f"boAM={p[2]:.1f}")
        print(f"  (c250 고정: boMgn(fallback)={FIXED_BREAKOUT_MARGIN} "
              f"aPct={ATR_PCTILE_MIN} vRat={VOL_RATIO_MIN} "
              f"btcMM={BTC_MOM_MIN})")
        print(f"  (c241 고정: rFlr={RSI_FLOOR} mLB={MOM_LB} mMin={MOM_MIN})")
        print(f"  (c233 고정: trail={BASE_TRAIL_MULT} TP2={BASE_TP2_MULT} "
              f"SL={BASE_SL_MULT} MH={BASE_MAX_HOLD} TP1={BASE_TP1_MULT})")
