"""
vpin_multi 사이클 197 — 핵심 진입 게이트(VPIN/MOM/RSI범위) 재탐색
- 기반: c196 최적 (body=0.50 volP=40 emaSP=50 volMm=0.05) avg_OOS=+27.895 n=14
- 문제:
  1) c196에서 진입 필터 완화해도 거래수/Sharpe 불변 → 병목이 필터가 아님
  2) XRP 전 폴드 0건 → 핵심 게이트(VPIN < 0.4, MOM >= 0.0005)가 XRP 진입 차단
  3) 거래수 14건 = 통계적 불충분 → 핵심 게이트 조정으로 25+ 복원 필요
  ★ VPIN_LOW, MOM_THRESH, RSI_CEILING, RSI_FLOOR는 c192에서 고정 후
    c195 지표주기 변경 이후 한 번도 재탐색 안 됨
- 가설:
  A) VPIN_LOW ↑ → VPIN 문턱 상향 → 더 많은 봉이 진입 허용 (VPIN < threshold)
  B) MOM_THRESH ↓ → 모멘텀 요구 완화 → 초기 추세/약추세 구간 포착
  C) RSI_CEILING ↑ → 과매수 존 상한 올림 → 강한 추세 구간에서도 진입 가능
  D) RSI_FLOOR ↓ → 과매도 존 하한 내림 → 반등 초기 포착
  = 3×3×3×3 = 81 combos
- 목표: OOS Sharpe >= 20 AND trades >= 20 (Sharpe 유지 + 거래수 증가)
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드: 핵심 진입 게이트 --
VPIN_LOW_LIST = [0.35, 0.40, 0.50]          # 현 0.40 (VPIN < threshold)
MOM_THRESH_LIST = [0.0002, 0.0005, 0.0010]  # 현 0.0005
RSI_CEILING_LIST = [60, 65, 75]             # 현 65
RSI_FLOOR_LIST = [15, 20, 25]               # 현 20
...
def backtest(
    ...
    while i < n - 1:
        ...
        # 진입 조건 (탐색 대상: vpin_low, mom_thresh, rsi_ceiling, rsi_floor)
        vpin_ok = (
            vpin_val < vpin_low
...
def build_combos() -> list[dict]:
    combos = []
    for vpin, mom, rsi_c, rsi_f in product(
        VPIN_LOW_LIST, MOM_THRESH_LIST,
        RSI_CEILING_LIST, RSI_FLOOR_LIST,
    ):
        if rsi_f >= rsi_c - 20:
            continue
        combos.append({
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 197 — 핵심 진입 게이트 재탐색 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 20 AND trades >= 20")
    print("가설: VPIN/MOM/RSI 범위 재탐색 → XRP 진입 + 거래수 복원")
    print(f"기준선: c196 OOS +27.895, WR 72.2%, trades 14")
    ...
    print(f"  탐색: VPIN_LOW={VPIN_LOW_LIST} MOM_TH={MOM_THRESH_LIST} "
          f"RSI_CEIL={RSI_CEILING_LIST} RSI_FLR={RSI_FLOOR_LIST}")
    print("=" * 80)
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, btc_c_aligned, btc_s_aligned,
                         combo["vpin_low"], combo["mom_thresh"],
                         combo["rsi_ceiling"], combo["rsi_floor"])
    ...
    hdr = (f"{'VPIN':>5} {'MOM':>7} {'rCeil':>5} {'rFlr':>5} | "
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['vpin_low']:>5.2f} {r['mom_thresh']:>7.4f} "
            f"{r['rsi_ceiling']:>5.0f} {r['rsi_floor']:>5.0f} | "
    ...
    for r in valid:
        key = (r["vpin_low"], r["mom_thresh"],
               r["rsi_ceiling"], r["rsi_floor"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        vpin_l = params["vpin_low"]
        mom_t = params["mom_thresh"]
        rsi_c = params["rsi_ceiling"]
        rsi_f = params["rsi_floor"]
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: VPIN={vpin_l:.2f} MOM={mom_t:.4f} "
                  f"rCeil={rsi_c:.0f} rFlr={rsi_f:.0f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        rsi_c = params["rsi_ceiling"]
        rsi_f = params["rsi_floor"]
        print(f"\n--- #{rank}: VPIN={vpin_l:.2f} MOM={mom_t:.4f} "
              f"rCeil={rsi_c:.0f} rFlr={rsi_f:.0f} "
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    vpin_l = best["vpin_low"]
    mom_t = best["mom_thresh"]
    rsi_c = best["rsi_ceiling"]
    rsi_f = best["rsi_floor"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: VPIN={vpin_l:.2f} "
          f"MOM={mom_t:.4f} rCeil={rsi_c:.0f} rFlr={rsi_f:.0f}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    # -- c196 대비 비교 --
    print(f"{'=' * 80}")
    print("=== c196 베이스라인 대비 비교 ===")
    print(f"  c196 최적 (VPIN=0.40 MOM=0.0005 rCeil=65 rFlr=20): "
          f"avg_OOS=+27.895 n=14")
    print(f"  c197 최적 (VPIN={best['vpin_low']:.2f} "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: VPIN_LOW={best['vpin_low']:.2f} "
