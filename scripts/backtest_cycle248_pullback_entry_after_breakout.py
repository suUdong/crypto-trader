"""
사이클 248: c243 최적 고정 + 풀백 진입 (돌파 후 눌림목 대기)
...
문제 분석:
  - F3 WR 60%: Donchian 돌파 즉시 진입 → false breakout에 취약
  - c246(캔들품질), c247(정체출구)은 후처리 방식 — 근본적 해결이 아님
  - 가설: 돌파 발생 시 즉시 진입 대신, 소폭 풀백을 확인한 후 진입
    → 풀백 없이 그냥 올라가면 → 놓침 (기회비용)
    → 풀백 후 재상승 → 진짜 돌파 확인 + 더 좋은 진입가
    → 풀백이 너무 깊으면 → 진입 취소 (false breakout 회피)
  - 구현: 돌파 감지 → "대기 모드" 진입 → N봉 내 pullback_depth ATR만큼 내려온 후
    다시 DC_upper 근처로 올라오면 진입
  - 기존 모든 필터(c241/c243)는 돌파 시점에서 체크
...
새 탐색 그리드 (c248):
  PULLBACK_ATR_MULT:   [0.0, 0.3, 0.5, 0.8]  — 풀백 깊이 (ATR배수, 0=OFF 즉시진입)
  PULLBACK_MAX_BARS:   [3, 5, 8]              — 풀백 대기 최대 봉수
  REENTRY_CONFIRM:     [0, 1, 2]              — 재진입 확인: 0=풀백만, 1=종가>DC, 2=종가>풀백고점
  = 4 × 3 × 3 = 36 combos
...
# ─── c248 탐색 그리드 ─────────────────────────────────────────
PULLBACK_ATR_MULT_LIST = [0.0, 0.3, 0.5, 0.8]  # 풀백 깊이 ATR배수 (0=OFF)
PULLBACK_MAX_BARS_LIST = [3, 5, 8]              # 풀백 대기 최대 봉수
REENTRY_CONFIRM_LIST = [0, 1, 2]                # 0=풀백만, 1=종가>DC, 2=종가>풀백고점
...
def run_backtest(
    ...
    # 풀백 대기 상태
    pending_breakout = None  # {"bar": i, "dc_up": val, "atr": val, "peak": val}
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── 풀백 대기 중인 경우 확인 ─────────────────────
            if pending_breakout is not None and pb_atr_mult > 0:
                pb = pending_breakout
                bars_waited = i - pb["bar"]

                # 대기 시간 초과 → 취소
                if bars_waited > pb_max_bars:
                    pending_breakout = None
                else:
                    # 풀백 깊이 체크: 돌파 후 최저점이 충분히 내려왔는지
                    low_since = np.min(lo[pb["bar"]:i + 1])
                    pullback_depth = pb["peak"] - low_since
                    required_depth = pb["atr"] * pb_atr_mult

                    if pullback_depth >= required_depth:
                        # 풀백 발생! 이제 재진입 조건 확인
                        do_enter = False
                        if reentry_confirm == 0:
                            # 풀백만 확인하면 진입
                            do_enter = True
                        elif reentry_confirm == 1:
                            # 종가 > DC upper
                            if not np.isnan(dc_up[i]) and c[i] > dc_up[i]:
                                do_enter = True
                        elif reentry_confirm == 2:
                            # 종가 > 풀백 구간 고점 (재돌파)
                            high_since_pb = np.max(
                                h[pb["bar"] + 1:i]) if i > pb["bar"] + 1 else 0
                            if c[i] > high_since_pb:
                                do_enter = True
                        ...
                        if do_enter:
                            ...
                            vol_tp_bonus = 0.0
                            if (TP_VOL_SCALE > 0
                                    and not np.isnan(atr_pctile[i])):
                                vol_score = max(
                                    0, atr_pctile[i] - 50) / 50.0
...
def main() -> None:
    print("=" * 80)
    print("=== c248: c243최적 고정 + 풀백 진입 (돌파 후 눌림목 대기) ===")
    ...
    print(f"탐색: PULLBACK_ATR × MAX_BARS × REENTRY_CONFIRM")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        PULLBACK_ATR_MULT_LIST, PULLBACK_MAX_BARS_LIST, REENTRY_CONFIRM_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c243 최적, 풀백 OFF)
    print("\n--- 베이스라인 (c243 최적, 풀백=OFF) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    # 효과 분석
    print("\n=== 풀백 깊이 효과 ===")
    for val in PULLBACK_ATR_MULT_LIST:
        ...
        if sub:
            ...
            print(f"  pbATR={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")

    print("\n=== 풀백 대기 봉수 효과 ===")
    for val in PULLBACK_MAX_BARS_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  pbBar={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")

    print("\n=== 재진입 확인 효과 ===")
    for val in REENTRY_CONFIRM_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  reCnf={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  pbATR={p[0]:.1f} pbBar={p[1]} reCnf={p[2]} | "
    ...
    if valid:
        ...
        print(f"  c248 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: pbATR={p[0]:.1f} pbBar={p[1]} "
              f"reCnf={p[2]}")
        print(f"  (c243 고정: btcMLB={BTC_MOM_LB} btcMMin={BTC_MOM_MIN} "
              f"btcADX={BTC_ADX_GATE} boMgn={BREAKOUT_MARGIN})")
