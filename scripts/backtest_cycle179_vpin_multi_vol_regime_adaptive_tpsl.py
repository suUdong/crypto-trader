"""
vpin_multi 사이클 179 — 변동성 레짐 적응형 TP/SL/Hold
- 기반: c177 OOS Sharpe +16.353, WR 42.5%, trades 64 (ETH+SOL+XRP, 3-fold)
  최적: atrTh=30 body=0.7 vpRx=0.25 rxSc=0.5
  c176 고정: atrLB=60
...
- c177 약점:
  1) SOL Fold 2 Sharpe=-7.540 → 고변동 구간에서 TP/SL 고정값이 역효과
  2) 전체 Sharpe 개선 미미(+0.008) → 진입 최적화 한계, 출구 최적화 필요
  3) 고변동기 trail이 너무 넓어 수익 환수, 저변동기 TP가 너무 높아 미체결
- 가설:
  A) ATR percentile로 변동성 레짐 분류 (highVol/lowVol 경계 탐색)
  B) highVol: TP 축소 + trail 타이트 + hold 단축 → 빠른 수익 실현
  C) lowVol: TP 확대 + trail 넓게 + hold 연장 → 추세 수익 극대화
  D) 레짐별 TP/SL 스케일 팩터 그리드 탐색
- 탐색 그리드: 3 vol_thresh × 3 high_tp_scale × 3 high_trail_scale × 2 hold_scale = 54 combos
...
# -- 탐색 그리드: 변동성 레짐 적응 --
# vol_regime_thresh: ATR percentile 경계 (이 값 이상 = highVol)
VOL_REGIME_THRESH_LIST = [50, 60, 70]
# highVol일 때 TP 스케일 (기준 대비 배율, <1 = 축소)
HIGH_VOL_TP_SCALE_LIST = [0.5, 0.65, 0.8]
# highVol일 때 trail 스케일 (기준 대비 배율, <1 = 타이트)
HIGH_VOL_TRAIL_SCALE_LIST = [0.5, 0.7, 0.9]
# highVol일 때 hold 스케일
HIGH_VOL_HOLD_SCALE_LIST = [0.6, 0.8]
...
def backtest(
    df: pd.DataFrame,
    vol_regime_thresh: float,
    high_vol_tp_scale: float,
    high_vol_trail_scale: float,
    high_vol_hold_scale: float,
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 179 — 변동성 레짐 적응형 TP/SL/Hold ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 17.0 (c177 +16.353 대비 개선)")
    print("가설: ATR percentile로 변동성 레짐 분류 → "
          "highVol시 TP/trail 축소+hold 단축, lowVol시 확대")
    print(f"기준선: c177 OOS +16.353, WR 42.5%, trades 64")
    ...
    combos = list(product(
        VOL_REGIME_THRESH_LIST, HIGH_VOL_TP_SCALE_LIST,
        HIGH_VOL_TRAIL_SCALE_LIST, HIGH_VOL_HOLD_SCALE_LIST,
    ))
    ...
    results: list[dict] = []
    for idx, (vol_th, tp_sc, trail_sc, hold_sc) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, vol_th, tp_sc, trail_sc, hold_sc, btc_c, btc_s)
            sym_results.append(r)

        pooled = pool_results(sym_results)
        results.append({
            "vol_regime_thresh": vol_th, "high_vol_tp_scale": tp_sc,
            "high_vol_trail_scale": trail_sc, "high_vol_hold_scale": hold_sc,
            **pooled,
        })
        if (idx + 1) % 18 == 0:
    ...
    hdr = (f"{'volTh':>5} {'tpSc':>5} {'trSc':>5} {'hdSc':>5} | "
    ...
    for r in valid[:20]:
        ...
        print(
            f"{r['vol_regime_thresh']:>5} {r['high_vol_tp_scale']:>5.2f} "
            f"{r['high_vol_trail_scale']:>5.2f} {r['high_vol_hold_scale']:>5.1f} | "
    ...
    for r in valid:
        key = (r["vol_regime_thresh"], r["high_vol_tp_scale"],
               r["high_vol_trail_scale"], r["high_vol_hold_scale"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        vol_th = params["vol_regime_thresh"]
        tp_sc = params["high_vol_tp_scale"]
        trail_sc = params["high_vol_trail_scale"]
        hold_sc = params["high_vol_hold_scale"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, vol_th, tp_sc, trail_sc, hold_sc,
                             btc_c, btc_s)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: volTh={vol_th} tpSc={tp_sc:.2f} "
                  f"trSc={trail_sc:.2f} hdSc={hold_sc:.1f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        trail_sc = params["high_vol_trail_scale"]
        hold_sc = params["high_vol_hold_scale"]
        print(f"\n--- #{rank}: volTh={vol_th} tpSc={tp_sc:.2f} "
              f"trSc={trail_sc:.2f} hdSc={hold_sc:.1f} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, vol_th, tp_sc, trail_sc, hold_sc,
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    vol_th = best["vol_regime_thresh"]
    tp_sc = best["high_vol_tp_scale"]
    trail_sc = best["high_vol_trail_scale"]
    hold_sc = best["high_vol_hold_scale"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: volTh={vol_th} tpSc={tp_sc:.2f} "
          f"trSc={trail_sc:.2f} hdSc={hold_sc:.1f}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
    ...
    print(f"  c177 기준 (고정 TP/SL/Hold): "
          f"avg_OOS=+16.353 n=64")
    print(f"  c179 최적 (volTh={best['vol_regime_thresh']} "
          f"tpSc={best['high_vol_tp_scale']:.2f} "
          f"trSc={best['high_vol_trail_scale']:.2f} "
          f"hdSc={best['high_vol_hold_scale']:.1f}): "
          f"avg_OOS={best['avg_oos_sharpe']:+.3f} "
          f"n={best['total_oos_trades']}")
    delta = best["avg_oos_sharpe"] - 16.353
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: VOL_REGIME_THRESH={best['vol_regime_thresh']} "
          f"HIGH_VOL_TP_SCALE={best['high_vol_tp_scale']:.2f} "
          f"HIGH_VOL_TRAIL_SCALE={best['high_vol_trail_scale']:.2f} "
          f"HIGH_VOL_HOLD_SCALE={best['high_vol_hold_scale']:.1f}")
    print(f"  (c177 고정: atrTh={ATR_PCTILE_THRESH} body={BODY_RATIO_MIN} "
          f"vpRx={VPIN_RELAX_THRESH} rxSc={RELAX_SCALE})")
    print(f"  (c176 고정: atrLB={ATR_PCTILE_LB})")
    print(f"  (c165 고정: VPIN={VPIN_LOW} MOM={MOM_THRESH} Hold={MAX_HOLD_BASE} "
