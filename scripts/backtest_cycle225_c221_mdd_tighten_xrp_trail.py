...
# ─── c225 탐색 그리드: c221 최적 근방 + 심볼별 trail 타이트닝 (MDD 개선) ──
# 베이스: c221 최적 adxB=10 rsiF=0 volB=1.5 atrB=20 (worst MDD=-17.45% FAIL)
# 가설: F2 SOL MDD=-9.67%, F1 XRP MDD=-12.12% → trail 짧게 조여서 MDD 컷
SOL_ADX_BOOST_LIST = [10, 15]          # 최적 근방 확장 (>=35, >=40)
SOL_VOL_BOOST_LIST = [1.5, 2.0]        # 최적 근방
SOL_TRAIL_LIST = [2.5, 2.0, 1.5]       # SOL 전용 trail 멀티
XRP_TRAIL_LIST = [2.5, 2.0, 1.5]       # XRP 전용 trail 멀티
# 고정: rsiF=0, SOL atrB=20
SOL_RSI_FLOOR_FIXED = 0
SOL_ATR_BOOST_FIXED = 20
...
def run_backtest(
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # trailing stop 업데이트
            if trail_mult_eff > 0 and current_price > position["peak"]:
                position["peak"] = current_price
                atr_now = atr_val[i] if not np.isnan(atr_val[i]) else 0
                trail_stop = current_price - atr_now * trail_mult_eff
...
def main() -> None:
    ...
    print(f"c210 고정: trail={trail_mult_eff} tpM={ATR_TP_MULT_BASE} "
    ...
    # 그리드 정의
    grid = list(product(
        SOL_ADX_BOOST_LIST, SOL_VOL_BOOST_LIST,
        SOL_TRAIL_LIST, XRP_TRAIL_LIST,
    ))
    ...
    for gi, combo in enumerate(grid):
        sol_adx_b, sol_vol_b, sol_trail, xrp_trail = combo
        sol_rsi_f = SOL_RSI_FLOOR_FIXED
        sol_atr_b = SOL_ATR_BOOST_FIXED
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                ...
                raw_trades = run_backtest(
                    ...
                    sl_sc,
                    adx_th_eff, rsi_fl, vol_min_eff, atr_pctile_th_eff, trail_eff,
    ...
    print("=== Top 15 결과 ===")
    print("=" * 80)
    hdr = (f"{'adxB':>5} {'volB':>5} {'sTrl':>5} {'xTrl':>5} | "
    ...
    for r in valid[:15]:
        p = r["params"]
        print(
            f"{p[0]:>5} {p[1]:>5.1f} {p[2]:>5.1f} {p[3]:>5.1f} | "
    ...
    for i, r in enumerate(valid[:5]):
        p = r["params"]
        print(f"\n#{i+1}: adxB={p[0]} volB={p[1]:.1f} "
              f"solTrail={p[2]:.1f} xrpTrail={p[3]:.1f}")
        print(f"  → SOL 실효: ADX>={ADX_THRESH + p[0]} "
              f"vol>={p[1]:.1f} trail={p[2]:.1f} | XRP trail={p[3]:.1f}")
    ...
    for vb in SOL_VOL_BOOST_LIST:
        subset = [r for r in valid if r["params"][1] == vb]
        if subset:
            avg_sh = np.mean([r["avg_sharpe"] for r in subset[:10]])
            avg_sol = np.mean([r["sol_avg_sharpe"] for r in subset[:10]])
            print(f"  vol>={vb:.1f}: avgSh={avg_sh:+.3f}  SOL={avg_sol:+.3f}")

    print("\n--- SOL Trail 효과 ---")
    for st in SOL_TRAIL_LIST:
        subset = [r for r in valid if r["params"][2] == st]
        if subset:
            avg_sh = np.mean([r["avg_sharpe"] for r in subset[:10]])
            avg_sol = np.mean([r["sol_avg_sharpe"] for r in subset[:10]])
            avg_mdd = np.mean([r["worst_mdd"] for r in subset[:10]])
            print(f"  solTrail={st:.1f}: avgSh={avg_sh:+.3f}  "
                  f"SOL={avg_sol:+.3f}  wMDD={avg_mdd:+.2f}%")

    print("\n--- XRP Trail 효과 ---")
    for xt in XRP_TRAIL_LIST:
        subset = [r for r in valid if r["params"][3] == xt]
        if subset:
            avg_sh = np.mean([r["avg_sharpe"] for r in subset[:10]])
            avg_xrp = np.mean([r["xrp_avg_sharpe"] for r in subset[:10]])
            avg_mdd = np.mean([r["worst_mdd"] for r in subset[:10]])
            print(f"  xrpTrail={xt:.1f}: avgSh={avg_sh:+.3f}  "
                  f"XRP={avg_xrp:+.3f}  wMDD={avg_mdd:+.2f}%")
    ...
    if valid:
        ...
        bp = best["params"]
        print("\n" + "=" * 80)
        print(f"=== 심볼별 OOS 성능 분해 (Top 1: adxB={bp[0]} volB={bp[1]:.1f} "
              f"solTrail={bp[2]:.1f} xrpTrail={bp[3]:.1f}) ===")
        ...
        for sym in SYMBOLS:
            ...
            if sym == "KRW-SOL":
                ...
                vol_min_eff = bp[1]
                atr_pctile_th_eff = ATR_PCTILE_TH + SOL_ATR_BOOST_FIXED
                trail_eff = bp[2]
            elif sym == "KRW-XRP":
                adx_th_eff = ADX_THRESH
                rsi_fl = 0
    ...
    if valid:
        ...
        print(f"★ OOS 최적: adxB={p[0]} volB={p[1]:.1f} "
              f"solTrail={p[2]:.1f} xrpTrail={p[3]:.1f}")
        print(f"  → SOL 실효: ADX>={ADX_THRESH + p[0]} "
              f"vol>={p[1]:.1f} trail={p[2]:.1f} | XRP trail={p[3]:.1f}")
