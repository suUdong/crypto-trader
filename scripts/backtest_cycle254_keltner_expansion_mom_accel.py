"""
사이클 254: c250 최적 고정 + Keltner 채널 확장 + 모멘텀 가속도 그리드

c250 결과:
  avg OOS Sharpe: +36.648  F2: +27.352  F3: +35.110
  worst MDD: -4.98%  trades: 28  WR: 82.1%

가설:
  - Donchian 브레이크아웃은 "가격이 N봉 최고가를 넘었는가"만 확인
  - Keltner 채널(EMA ± ATR×K)은 변동성 기반 밴드 → 추세 강도를 측정하는 다른 축
  - KC 밴드 폭이 확장 중 = 변동성 증가 = 진정한 추세 시작 구간
  - 모멘텀 "가속도": 단기 모멘텀 > 장기 모멘텀이면 추세가 가속 중
  - 이 두 조건 조합으로 "가속 중인 진정한 추세 돌파"만 필터링
  - c249(multi_mom_consensus)은 엣지 없음으로 실패했지만, 이는 3개 모멘텀 일치 요구.
    여기서는 가속도(차이)만 확인하므로 다른 접근.
...
새 탐색 그리드 (c254):
  KC_PERIOD:       [14, 20]        — Keltner EMA 기간
  KC_ATR_MULT:     [1.5, 2.0, 2.5] — KC 밴드 폭 배수
  KC_EXPAND_LB:    [0, 3, 5]       — KC 폭 확장 확인 룩백 (0=비활성)
  MOM_ACCEL_LB:    [0, 10, 20]     — 장기 모멘텀 룩백 (0=비활성, 단기는 c241 MOM_LB=5 고정)
  = 2 × 3 × 3 × 3 = 54 combos
...
# ─── c254 탐색 그리드 ─────────────────────────────────────────
KC_PERIOD_LIST = [14, 20]
KC_ATR_MULT_LIST = [1.5, 2.0, 2.5]
KC_EXPAND_LB_LIST = [0, 3, 5]
MOM_ACCEL_LB_LIST = [0, 10, 20]
...
def compute_keltner(
    ...
    ema_period: int, atr_mult: float, atr_period: int = 14,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Keltner Channel: (upper, lower, width)."""
    ema = ema_calc(closes, ema_period)
    ...
    for i in range(n):
        if not np.isnan(ema[i]) and not np.isnan(atr[i]):
            upper[i] = ema[i] + atr[i] * atr_mult
            lower[i] = ema[i] - atr[i] * atr_mult
            if ema[i] > 0:
                width[i] = (upper[i] - lower[i]) / ema[i] * 100.0
...
def run_backtest(
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c254 탐색: Keltner 채널 확장 확인 ──────────────
            # 가격이 KC upper 위에 있어야 (추세 강도 이중 확인)
            if not np.isnan(kc_upper[i]) and c[i] < kc_upper[i]:
                continue

            # KC 밴드 폭 확장 확인
            if (kc_expand_lb > 0 and i >= kc_expand_lb
                    and not np.isnan(kc_width[i])
                    and not np.isnan(kc_width[i - kc_expand_lb])):
                if kc_width[i] <= kc_width[i - kc_expand_lb]:
                    continue

            # ── c254 탐색: 모멘텀 가속도 ──────────────────────
            # 단기 모멘텀(5봉) > 장기 모멘텀(N봉) = 가속 중
            if mom_accel_lb > 0 and i >= mom_accel_lb:
                mom_short = (c[i] / c[i - MOM_LB]) - 1.0
                mom_long = (c[i] / c[i - mom_accel_lb]) - 1.0
                # 장기 기간으로 정규화하여 비교
                mom_long_norm = mom_long * MOM_LB / mom_accel_lb
                if mom_short <= mom_long_norm:
                    continue
...
def main() -> None:
    print("=" * 80)
    print("=== c254: c250최적 고정 + Keltner 채널 확장 + 모멘텀 가속도 그리드 ===")
    ...
    print(f"탐색: KC_PERIOD × KC_ATR_MULT × KC_EXPAND_LB × MOM_ACCEL_LB")
    print("=" * 80)
    ...
    # Keltner 채널 사전 계산 (period × mult 조합마다)
    ...
    for sym in SYMBOLS:
        ...
        # KC 사전 계산 (period × mult)
        kc_cache: dict[tuple[int, float], tuple[np.ndarray, np.ndarray]] = {}
        for kc_p in KC_PERIOD_LIST:
            for kc_m in KC_ATR_MULT_LIST:
                kc_up, _, kc_w = compute_keltner(
                    c_arr, h_arr, lo_arr, kc_p, kc_m)
                kc_cache[(kc_p, kc_m)] = (kc_up, kc_w)
    ...
    # 그리드
    grid = list(product(
        KC_PERIOD_LIST, KC_ATR_MULT_LIST,
        KC_EXPAND_LB_LIST, MOM_ACCEL_LB_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c250 최적 — KC/가속도 비활성)
    print("\n--- 베이스라인 (c250 최적, KC/가속도 OFF) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for window in WINDOWS:
        ...
        for sym in SYMBOLS:
            sp = sym_precomp[sym]
            dummy_kc_up = np.full(len(sp["c"]), np.nan)
            dummy_kc_w = np.full(len(sp["c"]), np.nan)
            raw = run_backtest(
    ...
    for gi, combo in enumerate(grid):
        kc_p, kc_m, kc_elb, mom_alb = combo
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                sp = sym_precomp[sym]
                kc_up, kc_w = sp["kc_cache"][(kc_p, kc_m)]
                raw = run_backtest(
                    sp["c"], sp["o"], sp["h"], sp["lo"], sp["v"],
                    sp["dc_up"], sp["dc_lo"],
                    sp["atr"], sp["adx"],
    ...
    print("\n" + "=" * 80)
    print("=== Top 15 결과 (F2+avg 우선) ===")
    print("=" * 80)
    hdr = (f"{'kcP':>5} {'kcM':>5} {'kcE':>5} {'mAcl':>5} | "
    ...
    for val in KC_PERIOD_LIST:
        ...
        if sub:
            ...
            print(f"  kcP={val}: top5 avgSh={avg_sh:+.3f}  F2={avg_f2:+.3f}")

    print("\n=== KC ATR Mult 효과 ===")
    for val in KC_ATR_MULT_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  kcM={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")

    print("\n=== KC Expand Lookback 효과 ===")
    for val in KC_EXPAND_LB_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  kcE={val}: top5 avgSh={avg_sh:+.3f}  F2={avg_f2:+.3f}")

    print("\n=== Momentum Accel LB 효과 ===")
    for val in MOM_ACCEL_LB_LIST:
        sub = [r for r in valid if r["params"][3] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  mAcl={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  kcP={p[0]} kcM={p[1]:.1f} kcE={p[2]} mAcl={p[3]} | "
    ...
    if valid:
        ...
        print(f"  c254 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: kcP={p[0]} kcM={p[1]:.1f} "
              f"kcE={p[2]} mAcl={p[3]}")
        print(f"  (c250 고정: boMgn={BREAKOUT_MARGIN} aPct={ATR_PCTILE_MIN} "
              f"vRat={VOL_RATIO_MIN} btcMM={BTC_MOM_MIN})")
        ...
        for window in WINDOWS:
            for sym in SYMBOLS:
                sp = sym_precomp[sym]
                kc_up, kc_w = sp["kc_cache"][(p[0], p[1])]
                raw = run_backtest(
                    sp["c"], sp["o"], sp["h"], sp["lo"], sp["v"],
                    sp["dc_up"], sp["dc_lo"],
                    sp["atr"], sp["adx"],
