...
# -- 탐색 그리드 --
# c192 최적 고정 (slB=0.1 rTh=0.40 rF=0.9, OOS +31.139 n=28)
SL_SLOPE_BONUS_LIST = [0.10]
RELAX_VOL_MOM_TH_LIST = [0.40]
RELAX_FACTOR_LIST = [0.90]
# 신규 탐색: VPIN 임계 및 MOM 임계 미세 그리드
VPIN_LOW_LIST = [0.30, 0.33, 0.35, 0.37, 0.40]
MOM_THRESH_LIST = [0.0005, 0.0007, 0.0010, 0.0013]
# = 1*1*1*5*4 = 20 combos
...
def build_combos() -> list[dict]:
    combos = []
    for sl_bonus, relax_th, relax_f, vpin_low, mom_th in product(
...
def _apply_entry_params(vpin_low: float, mom_thresh: float) -> None:
    """Override module-level VPIN/MOM constants per combo."""
    global VPIN_LOW, MOM_THRESH
    VPIN_LOW = vpin_low
...
def main() -> None:
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            _apply_entry_params(combo["vpin_low"], combo["mom_thresh"])
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        _apply_entry_params(params["vpin_low"], params["mom_thresh"])
    ...
    _apply_entry_params(best["vpin_low"], best["mom_thresh"])
