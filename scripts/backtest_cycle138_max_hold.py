"""
사이클 138: stealth_3gate MAX_HOLD 최적화
- 현재 24봉 고정 (4h 캔들 기준 = 4일) — 탐색 근거 없음
- 탐색: MAX_HOLD ∈ {12, 18, 24, 36}봉
- 고정: W=36, SMA=10, RS=[0.4,0.9), TP=10%, SL=1.0%, Gate4=OFF, acc>1.0, cvd_slope=OFF
...
# 탐색 대상 MAX_HOLD 값 (봉 단위, 4h 캔들)
MAX_HOLD_CANDIDATES: list[int] = [12, 18, 24, 36]
# 12봉 = 2일, 18봉 = 3일, 24봉 = 4일(현재), 36봉 = 6일
...
def main() -> None:
    t0 = time.time()
    print("=== stealth_3gate MAX_HOLD 최적화 (사이클 138) ===")
    print(f"고정: W={W}, SMA={SMA_N}, RS=[{RS_LO},{RS_HI}), TP={TP*100:.0f}%, SL={SL*100:.1f}%, Gate4=OFF, acc>{ACC_MIN}, cvd_slope=OFF")
    print(f"탐색: MAX_HOLD ∈ {MAX_HOLD_CANDIDATES} (4h 캔들 봉수)")
    ...
    # Pre-compute entry signals (MAX_HOLD-independent)
    print("[2.5] 알트 진입 신호 사전 계산...")
    ...
    print(f"  {len(alt_signals)}개 심볼에 유효 신호 있음 ({time.time()-t0:.1f}s)")

    # Evaluate each MAX_HOLD
    print("\n[3/3] MAX_HOLD 탐색...")
    results = []

    for max_hold in MAX_HOLD_CANDIDATES:
        hold_days = max_hold * 4 / 24  # 4h 캔들 → 일수
        ...
        for wname, wstart, wend in WINDOWS:
            ...
            for sym, (closes, entry, dates) in alt_signals.items():
                ...
                if mask.sum() < W * 2:
                    continue
                rets = run_symbol(closes[mask], entry[mask], max_hold)
        ...
        label = f"{max_hold}봉({hold_days:.0f}일)"
        results.append({
            "label": label,
            "max_hold": max_hold,
        ...
        print(
            f"\n{flag} MAX_HOLD={max_hold}봉 ({hold_days:.0f}일){current}"
    ...
    # Summary table
    print("\n=== 결과 요약 ===")
    print(f"{'MAX_HOLD':<14} {'W1 Sharpe':>10} {'W1 n':>6} {'W1 WR':>7} {'W1 avg%':>8} {'W2 Sharpe':>10} {'W2 n':>6} {'W2 WR':>7} {'W2 avg%':>8} {'통과':>5}")
    print("-" * 95)
    baseline_w2 = None
    best = None
    for r in results:
        flag = "✅" if r["passed"] else "  "
        current = "*" if r["max_hold"] == 24 else " "
        if r["max_hold"] == 24:
            baseline_w2 = r["w2_sharpe"]
        print(
            f"{current}{r['label']:<13} "
    ...
    if baseline_w2 is not None:
        if best is not None and best["max_hold"] != 24:
            ...
            if delta > 0.1:
                print(f"✅ MAX_HOLD={best['max_hold']} 채택 권장 (W2 Sharpe +{delta:.3f} vs 베이스라인 {baseline_w2:.3f})")
                print(f"   → daemon.toml: stealth_3gate max_holding_bars={best['max_hold']}")
            else:
                print(f"미미한 개선 (Δ{delta:+.3f}) — 현재 MAX_HOLD=24 유지 고려")
        elif best is not None and best["max_hold"] == 24:
            print(f"현재 MAX_HOLD=24 최적 — 변경 불필요")
        else:
            print("통과 후보 없음 — 추가 분석 필요")
