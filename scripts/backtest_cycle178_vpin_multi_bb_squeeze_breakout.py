"""
vpin_multi 사이클 178 — c176 최적 기반 볼린저밴드 스퀴즈 해제 진입 타이밍
...
- c176 약점:
  1) ATR 백분위 30 필터가 저변동 구간 차단하지만, 변동성 "확장 시점" 미포착
  2) 횡보→상승 전환 초기 시그널 놓침 (ATR은 후행)
- 가설:
  A) BB bandwidth percentile < squeeze_th → 스퀴즈 상태 탐지 (변동성 압축)
  B) 스퀴즈 후 BB width 확장 확인 (현재 width > N봉전 width) → 브레이크아웃 시작
  C) close > upper_bb * ratio → 밴드 상단 돌파 확인
  D) ATR pctile 필터 대체: BB 기반 변동성 필터가 더 정밀한지 비교
     (ATR pctile은 절대 변동성, BB squeeze는 상대 변동성 → 다른 관점)
- 탐색 그리드:
  2 BB_PERIOD × 3 SQUEEZE_PCTILE_TH × 3 EXPANSION_LB × 2 UPPER_RATIO = 36 combos
  c176 바디 필터 고정 (body=0.7), ATR pctile 고정 (atrLB=60 atrTh=30)
  → ATR pctile OFF 옵션도 포함하여 대체 가능성 검증
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드 --
BB_PERIOD_LIST = [20, 30]                    # 볼린저밴드 기간
SQUEEZE_PCTILE_TH_LIST = [20, 35, 50]       # 스퀴즈 탐지 백분위 (bandwidth < this%)
EXPANSION_LB_LIST = [2, 4, 6]               # 확장 확인 룩백 (봉)
UPPER_RATIO_LIST = [0.98, 1.00]             # close >= upper * ratio → 돌파 확인
# ATR pctile ON/OFF 테스트
ATR_PCTILE_MODE_LIST = [True, False]         # True=c176 고정, False=OFF
...
def backtest(
    ...
    while i < n - 1:
        ...
        if (np.isnan(bw_pctile_val) or np.isnan(bb_bw_val)
            ...
            # 최근 expansion_lb 봉 내에 스퀴즈 상태가 있었는지 확인
            had_squeeze = False
            for k in range(max(0, i - expansion_lb - 10), i - expansion_lb + 1):
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 178 — BB 스퀴즈 해제 브레이크아웃 진입 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  목표: OOS Sharpe >= 16.0 (c176 이상)")
    print("가설 A: BB bandwidth pctile < th → 스퀴즈 탐지 (변동성 압축)")
    print("가설 B: 스퀴즈 후 bandwidth 확장 + close > upper*ratio → 브레이크아웃")
    print("가설 C: ATR pctile 필터 대체 가능성 검증 (ON/OFF)")
    ...
    combos = list(product(
        BB_PERIOD_LIST, SQUEEZE_PCTILE_TH_LIST,
        EXPANSION_LB_LIST, UPPER_RATIO_LIST, ATR_PCTILE_MODE_LIST,
    ))
    ...
    results: list[dict] = []
    for idx, (bb_p, sq_th, exp_lb, up_r, atr_on) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, bb_p, sq_th, exp_lb, up_r, atr_on,
                         btc_c, btc_s)
            sym_results.append(r)

        pooled = pool_results(sym_results)
        results.append({
            "bb_period": bb_p, "squeeze_th": sq_th, "expansion_lb": exp_lb,
            "upper_ratio": up_r, "atr_on": atr_on, **pooled,
        })
    ...
    hdr = (f"{'bbP':>4} {'sqTh':>5} {'expLB':>5} {'upR':>5} {'atr':>4} | "
           f"{'Sharpe':>7} {'WR':>6} {'avg%':>7} {'MDD':>7} {'MCL':>4} {'n':>5}")
    print(hdr)
    print("-" * len(hdr))
    for r in valid[:16]:
        sh = f"{r['sharpe']:+.3f}" if not np.isnan(r["sharpe"]) else "  nan"
        atr_flag = "ON" if r["atr_on"] else "OFF"
        print(
            f"{r['bb_period']:>4} {r['squeeze_th']:>5} "
            f"{r['expansion_lb']:>5} {r['upper_ratio']:>5.2f} {atr_flag:>4} | "
    ...
    for r in valid:
        key = (r["bb_period"], r["squeeze_th"], r["expansion_lb"],
               r["upper_ratio"], r["atr_on"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        exp_lb = params["expansion_lb"]
        up_r = params["upper_ratio"]
        atr_on = params["atr_on"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, bb_p, sq_th, exp_lb, up_r, atr_on,
                             btc_c, btc_s)
        ...
        if oos_sharpes:
            ...
            atr_flag = "ON" if atr_on else "OFF"
            print(f"  #{rank}: bbP={bb_p} sqTh={sq_th} expLB={exp_lb} "
                  f"upR={up_r:.2f} atr={atr_flag} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        atr_on = params["atr_on"]
        atr_flag = "ON" if atr_on else "OFF"
        print(f"\n--- #{rank}: bbP={bb_p} sqTh={sq_th} expLB={exp_lb} "
              f"upR={up_r:.2f} atr={atr_flag} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, bb_p, sq_th, exp_lb, up_r, atr_on,
    ...
    exp_lb = best["expansion_lb"]
    up_r = best["upper_ratio"]
    atr_on = best["atr_on"]
    atr_flag = "ON" if atr_on else "OFF"

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: bbP={bb_p} sqTh={sq_th} "
          f"expLB={exp_lb} upR={up_r:.2f} atr={atr_flag}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c178 최적 (bbP={best['bb_period']} sqTh={best['squeeze_th']} "
          f"expLB={best['expansion_lb']} upR={best['upper_ratio']:.2f} "
          f"atr={atr_flag}): "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: BB_PERIOD={best['bb_period']} "
          f"SQUEEZE_PCTILE_TH={best['squeeze_th']} "
          f"EXPANSION_LB={best['expansion_lb']} "
          f"UPPER_RATIO={best['upper_ratio']:.2f} "
          f"ATR_PCTILE={'ON' if best['atr_on'] else 'OFF'}")
    print(f"  (c176 고정: body={BODY_RATIO_MIN})")
    print(f"  (c165 고정: VPIN={VPIN_LOW} MOM={MOM_THRESH} Hold={MAX_HOLD} "
          f"CD={COOLDOWN_BARS})")
