"""
사이클 121: APT vpin 전략 탐색
- 목적: ONDO vpin 슬리피지 민감도 실패(W2 구조적 취약) 이후 새 심볼 탐색
        → APT에서 W1+W2 동시 통과 + 슬리피지 내성 확인
- 배경:
    사이클 119: ONDO vpin 2/2창 통과 (W1=+9.132 / W2=+5.296)
    사이클 120: ONDO vpin 슬리피지 민감도 — 0.05% 에서 W2 즉시 탈락, 배포 보류
    가설: APT는 2024-01부터 데이터 존재 → 더 긴 IS 확보 → W2 엣지 안정성 기대
          ONDO 실패 패턴 참고해 W2 기준 강화 (슬리피지 0.10% 내성 검증 포함)
- 데이터: KRW-APT 240m, 2024-01~2026-03 (27개월)
- WF 창:
    W1: IS=2024-01~2024-12, OOS=2025-01~2025-09 (BULL 기간, 9개월 OOS)
    W2: IS=2024-01~2025-06, OOS=2025-07~2026-04 (혼재, 10개월 OOS)
- 판정 기준: OOS Sharpe > 5.0 && WR > 25% && trades >= 5 (2/2창)
- 추가 검증: 2/2창 통과 조합은 슬리피지 0.10% 내성 즉시 확인
...
# Walk-forward windows (APT 데이터: 2024-01 시작, ONDO와 동일 OOS 구간)
WINDOWS = [
    {
        "name": "W1",
        "is_start":  "2024-01-01", "is_end":  "2024-12-31",
        "oos_start": "2025-01-01", "oos_end": "2025-09-30",
    },
    {
        "name": "W2",
        "is_start":  "2024-01-01", "is_end":  "2025-06-30",
        "oos_start": "2025-07-01", "oos_end": "2026-04-04",
    },
]

# 기준선: ONDO 최적 파라미터 (사이클 119 확정)
BASELINE = dict(vh=0.55, vm=0.0005, hold=18, tp=0.10, sl=0.015)
...
# 슬리피지 내성 기준 (0.10%에서도 통과해야 daemon 후보)
SLIP_TEST = 0.0010  # 0.10% 편도 슬리피지
...
def backtest(
    ...
    while i < n - 1:
        ...
        if (gate1_ok
            ...
            buy = c[i + 1] * (1.0 + total_fee)
            exited = False
            for j in range(i + 2, min(i + 1 + hold, n)):
                ret = c[j] / buy - 1
                if ret >= tp:
                    returns.append(tp - total_fee)
                    i = j
                    exited = True
                    break
                if ret <= -sl:
                    returns.append(-sl - total_fee)
                    i = j
                    exited = True
                    break
            ...
            if not exited:
                ...
                returns.append(c[hold_end] / buy - 1 - total_fee)
                i = hold_end
        else:
            i += 1
...
def run_window(w: dict, use_gate1: bool, params: dict, extra_fee: float = 0.0) -> dict:
    ...
    is_r  = backtest(df_sym_is,  df_btc_is,  **params, use_gate1=use_gate1, extra_fee=extra_fee)
    oos_r = backtest(df_sym_oos, df_btc_oos, **params, use_gate1=use_gate1, extra_fee=extra_fee)
...
def run_combo(use_gate1: bool, params: dict, extra_fee: float = 0.0) -> dict:
    results    = [run_window(w, use_gate1, params, extra_fee) for w in WINDOWS]
...
def main() -> None:
    ...
    print("배경: ONDO vpin W2 슬리피지 0.05% 즉시 탈락 → APT로 전환, W2 내성 중점 확인")
    print("추가: 2/2창 통과 조합은 슬리피지 0.10% 내성 즉시 검증")
    print("=" * 75)

    # ── 1단계: ONDO 최적 파라미터 기준선 테스트 ──────────────────────────────
    print("\n▶ 1단계: ONDO 확정 파라미터 기준선 (hold=18/TP=10%/SL=1.5%/Gate1=Y)")
    ...
    # ── 3단계: 2/2창 통과 조합 슬리피지 내성 즉시 검증 ───────────────────────
    if two_pass:
        print(f"\n▶ 3단계: 슬리피지 내성 검증 (0.10% 추가 슬리피지, 상위 5개)")
        print("  배경: ONDO는 0.05%에서 W2 즉시 탈락 → 0.10% 내성이 daemon 기준")
        print(f"  {'조합':>40} | {'W1 OOS':>8} {'WR':>5} | {'W2 OOS':>8} {'WR':>5} | {'avg':>7} | 통과")
        print("  " + "-" * 95)

        slip_pass_list = []
        for combo in two_pass[:5]:
            p = combo["params"]
            use_gate1 = combo["gate1"]
            slip_combo = run_combo(use_gate1, p, extra_fee=SLIP_TEST)

            r1 = slip_combo["results"][0]["oos"]
            r2 = slip_combo["results"][1]["oos"]
            sh1 = f"{r1['sharpe']:+.3f}" if not np.isnan(r1["sharpe"]) else "  nan"
            sh2 = f"{r2['sharpe']:+.3f}" if not np.isnan(r2["sharpe"]) else "  nan"
            avg_slip = slip_combo["avg_sharpe"]
            ok  = f"{slip_combo['pass_count']}/2"
            flag = " ★" if slip_combo["pass_count"] == 2 else ""
            g = "Gate1" if use_gate1 else "NoGate"

            label = f"{g} vh={p['vh']:.2f} vm={p['vm']:.4f} h={p['hold']} TP={p['tp']*100:.0f}% SL={p['sl']*100:.1f}%"
            print(
                f"  {label:>40} | "
                f"{sh1:>8} {r1['wr']:>4.0%} | "
                f"{sh2:>8} {r2['wr']:>4.0%} | "
                f"{avg_slip:>7.3f} | {ok}{flag}"
            )
            if slip_combo["pass_count"] == 2:
                slip_pass_list.append((combo, slip_combo))

        if slip_pass_list:
            print(f"\n  슬리피지 0.10% 내성 통과: {len(slip_pass_list)}개 → daemon 후보!")
        else:
            print("\n  슬리피지 0.10% 내성 통과 없음 — APT도 슬리피지 민감도 존재")
    ...
    if two_pass:
        ...
        print(f"  ✅ APT vpin 전략 유효! 2/2창 통과 {len(two_pass)}개")
        print(f"  최적: {g2} vh={p2['vh']:.2f} vm={p2['vm']:.4f} hold={p2['hold']} "
              f"TP={p2['tp']*100:.0f}% SL={p2['sl']*100:.1f}% (avg={best2['avg_sharpe']:+.3f})")
        if two_pass:
            slip_ok = any(run_combo(c["gate1"], c["params"], SLIP_TEST)["pass_count"] == 2
                         for c in two_pass[:3])
            if slip_ok:
                print("  슬리피지 내성 통과 → daemon 반영 검토 가능")
            else:
                print("  슬리피지 민감도 주의 → ONDO와 동일 패턴, daemon 반영 보류 검토")
    elif one_pass:
        best1 = one_pass[0]
        print(f"  ◆ APT vpin 1/2창 통과 — 추가 파라미터 조정 필요")
        print(f"  최고 avg Sharpe: {best1['avg_sharpe']:+.3f}")
    else:
        print("  ❌ APT vpin 기준 미달 — 다른 전략/심볼 탐색 필요")
