"""
vpin_multi 사이클 200 — CMF (Chaikin Money Flow) + RSI Bullish Divergence 진입
...
- 가설:
  1. CMF (Chaikin Money Flow): 가격 위치 기반 매수/매도 압력 측정
     VPIN은 BVC로 volume imbalance를 보지만, CMF는 close의 high-low 범위 내
     위치로 accumulation/distribution을 판단 → 상호 보완적 관점
  2. RSI Bullish Divergence: 가격은 lower low인데 RSI는 higher low
     → 하락 모멘텀 소진, 반전 가능성 포착
  3. CMF 양수 + RSI 다이버전스 = 자금 유입 + 반전 시그널 동시 확인
- 설계:
  진입 = c179 기본 조건 유지 + CMF > CMF_MIN (자금 유입 확인)
  또는 c179 기본 조건 + RSI bullish divergence (가격LL + RSI HL)
  CMF/Divergence는 BONUS 조건: 충족시 c179 진입 + TP 보너스
- 탐색 그리드:
  CMF_PERIOD: [10, 20]           — CMF 계산 기간
  CMF_MIN: [0.0, 0.05, 0.10]    — CMF 최소 임계값 (0=비활성)
  DIV_LB: [8, 14, 20]           — 다이버전스 탐색 룩백
  DIV_PRICE_DROP: [0.0, -0.005, -0.01] — 가격 하락 최소폭
...
# -- 탐색 그리드: CMF + RSI Divergence --
CMF_PERIOD_LIST = [10, 20]
CMF_MIN_LIST = [0.0, 0.05, 0.10]
DIV_LB_LIST = [8, 14, 20]
DIV_PRICE_DROP_LIST = [0.0, -0.005, -0.01]
TP_BONUS_ON_CONFIRM_LIST = [0.0, 0.5, 1.0]
...
def detect_rsi_bullish_divergence(
    ...
    lookback: int, price_drop_min: float,
) -> bool:
    """RSI Bullish Divergence: 가격은 lower low, RSI는 higher low.
    현재 바 기준 lookback 내에서 가격의 이전 저점 찾고 비교.
    """
    if idx < lookback + 1:
        return False

    current_price = closes[idx]
    current_rsi = rsi_arr[idx]
    if np.isnan(current_rsi):
        return False

    # lookback 기간 내 가격 최저점 위치 찾기
    window_start = idx - lookback
    price_window = closes[window_start:idx]
    rsi_window = rsi_arr[window_start:idx]

    # 유효한 RSI가 있는 로컬 저점 찾기
    best_div = False
    for k in range(1, len(price_window) - 1):
        abs_k = window_start + k
        if np.isnan(rsi_window[k]):
            continue
        # 로컬 저점: 양쪽보다 낮음
        if price_window[k] <= price_window[k - 1] and price_window[k] <= price_window[k + 1]:
            price_change = (current_price - price_window[k]) / price_window[k]
            # 현재 가격이 이전 저점보다 낮거나 비슷 (price_drop_min 이하)
            if price_change <= price_drop_min:
                # RSI는 현재가 이전 저점보다 높아야 함 (bullish divergence)
                if current_rsi > rsi_window[k] + 1.0:  # 최소 1포인트 차이
                    best_div = True
                    break
    return best_div
...
def backtest(
    df: pd.DataFrame,
    cmf_period: int, cmf_min: float,
    div_lb: int, div_price_drop: float,
    ...
    warmup = max(BUCKET_COUNT, EMA_PERIOD, RSI_PERIOD + 1,
                 ...
                 ATR_PCTILE_LB, cmf_period, div_lb + 2, 50) + 5
    i = warmup
    ...
    while i < n - 1:
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
            ...
            # ★ CMF + RSI Divergence 확인 (보너스 조건)
            cmf_confirmed = cmf_val >= cmf_min if cmf_min > 0 else False
            div_confirmed = detect_rsi_bullish_divergence(
                c, rsi_arr, i, div_lb, div_price_drop
            )
            has_bonus = cmf_confirmed or div_confirmed
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 200 — CMF + RSI Bullish Divergence 진입 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 43.0 (c179 +42.878 대비 개선)")
    print("가설: CMF(자금유입) + RSI Divergence(반전신호) 이중확인 → TP 보너스")
    print("  CMF > 0: 매수 압력 우세 확인 / RSI Divergence: 모멘텀 소진 반전")
    print("  두 조건은 OR (하나라도 충족 시 보너스)")
    ...
    combos = list(product(
        CMF_PERIOD_LIST, CMF_MIN_LIST, DIV_LB_LIST,
        DIV_PRICE_DROP_LIST, TP_BONUS_ON_CONFIRM_LIST,
    ))
    ...
    results: list[dict] = []
    for idx, (cmf_p, cmf_m, div_l, div_d, tp_b) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, cmf_p, cmf_m, div_l, div_d, tp_b,
                         btc_c, btc_s)
            sym_results.append(r)
        pooled = pool_results(sym_results)
        results.append({
            "cmf_p": cmf_p, "cmf_m": cmf_m, "div_l": div_l,
            "div_d": div_d, "tp_b": tp_b,
            **pooled,
        })
    ...
    for rank, r in enumerate(top5, 1):
        print(f"  #{rank} cmfP={r['cmf_p']} cmfM={r['cmf_m']} divL={r['div_l']} "
              f"divD={r['div_d']} tpB={r['tp_b']} → Sharpe={r['sharpe']:+.3f} "
    ...
    for rank, combo in enumerate(top5, 1):
        ...
        print(f"\n--- #{rank}: cmfP={cmf_p} cmfM={cmf_m} divL={div_l} "
              f"divD={div_d} tpB={tp_b} ---")
        ...
        for fi, fold in enumerate(WF_FOLDS, 1):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_te, cmf_p, cmf_m, div_l, div_d, tp_b,
                             btc_c, btc_s)
    ...
    cmf_p = best_combo["cmf_p"]
    cmf_m = best_combo["cmf_m"]
    div_l = best_combo["div_l"]
    div_d = best_combo["div_d"]
    tp_b = best_combo["tp_b"]

    print(f"최적: cmfP={cmf_p} cmfM={cmf_m} divL={div_l} "
          f"divD={div_d} tpB={tp_b}")
    ...
    # -- 심볼별 OOS 분해 --
    print("\n" + "=" * 80)
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: cmfP={cmf_p} cmfM={cmf_m} "
          f"divL={div_l} divD={div_d} tpB={tp_b}) ===")
    ...
    print(f"  c200 최적 (cmfP={cmf_p} cmfM={cmf_m} divL={div_l} "
          f"divD={div_d} tpB={tp_b}): "
    ...
    print(f"★ OOS 최적: cmfP={cmf_p} cmfM={cmf_m} divL={div_l} "
          f"divD={div_d} tpB={tp_b}")
    print(f"  (c179 고정: volTh={VOL_REGIME_THRESH} tpSc={HIGH_VOL_TP_SCALE} "
          f"trSc={HIGH_VOL_TRAIL_SCALE} hdSc={HIGH_VOL_HOLD_SCALE})")
    print(f"  (c177 고정: atrTh={ATR_PCTILE_THRESH} body={BODY_RATIO_MIN} "
