"""
vpin_multi 사이클 200 — Profit-Level Adaptive Trail + ATR Regime SL Scaling
- 기반: c192 OOS Sharpe +47.314 (c198 필터 추가 실패 → 출구 개선으로 전환)
  c198 결론: VPIN RoC/RSI divergence 최적=비활성, 진입 필터 포화 확인
...
- 문제:
  1) c192 trail tightening은 시간 기반(6봉 후 3x 축소) → 이익이 적어도 시간만 지나면 축소
     → 수익 거래가 조기 청산, 손실 거래는 개선 없음
  2) SL이 ATR 레짐 무관하게 고정 → high-vol 구간에서 SL이 상대적으로 타이트하여 조기 청산
- 가설:
  A) Profit-Level Adaptive Trail: unrealized profit이 특정 ATR 배수 이상일 때만 trail 타이트닝
     → 이익이 충분한 거래만 보호, 작은 이익은 여유 부여
  B) ATR Regime SL Scaling: ATR percentile 높을 때 SL 배수 확대
     → high-vol 노이즈에서 불필요한 SL 트리거 감소
- 탐색 그리드:
  PROFIT_TRAIL_TH: [1.5, 2.0, 3.0]   — 이익이 X*ATR 이상일 때 trail 타이트닝 활성화
  PROFIT_TRAIL_F: [2.0, 3.0, 4.0]     — 타이트닝 배율 (높을수록 빡빡)
  SL_REGIME_BONUS: [0.0, 0.1, 0.2]    — ATR pctile>70일 때 SL 추가 여유
  SL_REGIME_TH: [60, 70]              — ATR pctile 기준
  = 3×3×3×2 = 54 combos
...
# -- c192 최적 고정 (시간기반 trail은 c200에서 교체) --
ATR_TP_SCALE_HI = 0.5

# -- c200 탐색 그리드 --
PROFIT_TRAIL_TH_LIST = [1.5, 2.0, 3.0]     # 이익 X*ATR 이상시 trail 활성
PROFIT_TRAIL_F_LIST = [2.0, 3.0, 4.0]       # trail 타이트닝 배율
SL_REGIME_BONUS_LIST = [0.0, 0.1, 0.2]      # high-vol SL 추가 여유
SL_REGIME_TH_LIST = [60, 70]                 # ATR pctile 기준
...
def backtest(
    df: pd.DataFrame,
    profit_trail_th: float,
    profit_trail_f: float,
    sl_regime_bonus: float,
    sl_regime_th: int,
    ...
    while i < n - 1:
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
            ...
            # === c200-B: ATR Regime SL Scaling ===
            sl_extra = 0.0
            if sl_regime_bonus > 0 and not np.isnan(atr_pctile_val):
                if atr_pctile_val >= sl_regime_th:
                    sl_extra = sl_regime_bonus

            effective_sl_mult = SL_BASE_ATR - SL_BONUS_ATR * rsi_ratio + sl_extra
            ...
            for j in range(i + 2, min(i + 1 + MAX_HOLD, n)):
                ...
                # === c200-A: Profit-Level Adaptive Trail ===
                unrealized_atr = (peak_price - buy) / atr_at_entry
                if unrealized_atr >= profit_trail_th:
                    effective_trail_mult = base_trail_mult / profit_trail_f
                else:
...
def build_combos() -> list[dict]:
    combos = []
    for pt_th, pt_f, sl_bonus, sl_th in product(
        PROFIT_TRAIL_TH_LIST, PROFIT_TRAIL_F_LIST,
        SL_REGIME_BONUS_LIST, SL_REGIME_TH_LIST,
    ):
        combos.append({
            "profit_trail_th": pt_th,
            "profit_trail_f": pt_f,
            "sl_regime_bonus": sl_bonus,
            "sl_regime_th": sl_th,
        })
    return combos
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 200 — Profit-Level Trail + ATR Regime SL ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 35 AND trades >= 20")
    print("가설 A: Profit-Level Adaptive Trail → "
          "이익이 X*ATR 이상일 때만 trail 타이트닝")
    print("가설 B: ATR Regime SL Scaling → "
          "high-vol 구간 SL 여유 확대로 노이즈 청산 감소")
    print(f"기준선: c192 OOS +47.314 (c198 RoC/div 필터 FAIL→+30.947)")
    print(f"c192 고정: aTPS={ATR_TP_SCALE_HI}")
    ...
    print("  ★ c192 시간기반 trail(ttA=6 ttF=3.0) → profit-level trail로 교체")
    print("=" * 80)
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            df_tr, btc_c, btc_s, esp, vol_mom, atr_pctile, vpin = (
                sym_train_cache[sym])
            r = backtest(df_tr, combo["profit_trail_th"], combo["profit_trail_f"],
                         combo["sl_regime_bonus"], combo["sl_regime_th"],
    ...
    hdr = (f"{'ptTh':>5} {'ptF':>5} {'slB':>5} {'slTh':>5} | "
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['profit_trail_th']:>5.1f} {r['profit_trail_f']:>5.1f} "
            f"{r['sl_regime_bonus']:>5.2f} {r['sl_regime_th']:>5} | "
    ...
    for r in valid:
        key = (r["profit_trail_th"], r["profit_trail_f"],
               r["sl_regime_bonus"], r["sl_regime_th"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        pt_th = params["profit_trail_th"]
        pt_f = params["profit_trail_f"]
        sl_b = params["sl_regime_bonus"]
        sl_th = params["sl_regime_th"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, pt_th, pt_f, sl_b, sl_th,
                             btc_c, btc_s, esp, vol_mom, atr_pctile, vpin)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: ptTh={pt_th:.1f} ptF={pt_f:.1f} "
                  f"slB={sl_b:.2f} slTh={sl_th} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        sl_b = params["sl_regime_bonus"]
        sl_th = params["sl_regime_th"]
        print(f"\n--- #{rank}: ptTh={pt_th:.1f} ptF={pt_f:.1f} "
              f"slB={sl_b:.2f} slTh={sl_th} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, pt_th, pt_f, sl_b, sl_th,
                             btc_c, btc_s, esp, vol_mom, atr_pctile, vpin,
                             slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    pt_th = best["profit_trail_th"]
    pt_f = best["profit_trail_f"]
    sl_b = best["sl_regime_bonus"]
    sl_th = best["sl_regime_th"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: ptTh={pt_th:.1f} "
          f"ptF={pt_f:.1f} slB={sl_b:.2f} slTh={sl_th}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c200 최적 (ptTh={best['profit_trail_th']:.1f} "
          f"ptF={best['profit_trail_f']:.1f} "
          f"slB={best['sl_regime_bonus']:.2f} "
          f"slTh={best['sl_regime_th']}): "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: PROFIT_TRAIL_TH={best['profit_trail_th']:.1f} "
          f"PROFIT_TRAIL_F={best['profit_trail_f']:.1f} "
          f"SL_REGIME_BONUS={best['sl_regime_bonus']:.2f} "
          f"SL_REGIME_TH={best['sl_regime_th']}")
    print(f"  (c192 고정: aTPS={ATR_TP_SCALE_HI})")
    print(f"  (c190 고정: vMomLB={VOL_MOM_LB} vMomMin={VOL_MOM_MIN} "
          f"tpBonus={TP_SLOPE_BONUS})")
    print(f"  (c186 고정: body={BODY_RATIO_MIN} rsiD={RSI_DELTA_MIN} "
    ...
    print(f"  ★ c192 시간기반 trail(ttA=6 ttF=3.0) → profit-level trail로 교체")
