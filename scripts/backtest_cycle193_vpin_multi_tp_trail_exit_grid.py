"""
vpin_multi 사이클 193 — TP/Trail 출구 파라미터 미세조정
- 기반: c192 최적 VPIN=0.4 MOM=0.0005 위에 c191 최적 slB=0.1 rTh=0.40 rF=0.9
  avg OOS Sharpe: +20.999, WR 54.4%, trades 36
- 문제:
  1) c191→c192 Sharpe -10 하락 (31→21) — 출구 최적화 미실시 상태
  2) avg_ret +1.37% 수준 — TP/Trail 조정으로 수익 확대 여지
  3) TP/Trail 파라미터 초기 설정(TP=4.0+2.0 Trail=0.3+0.2 minP=1.5) 이후 미조정
- 가설:
  A) TP_BASE_ATR 확대 → 수익거래 평균 수익 증가 (단, 보유기간 증가 리스크)
  B) TRAIL_BASE_ATR 축소 → 수익 보존 강화 (단, 조기 청산 리스크)
  C) MIN_PROFIT_ATR 조정 → 트레일 활성화 시점 최적화
  D) 3변수 동시 탐색으로 최적 출구 조합 탐색
- 탐색 그리드:
  TP_BASE_ATR: [3.0, 3.5, 4.0, 4.5, 5.0]     — 5수준
  TRAIL_BASE_ATR: [0.15, 0.25, 0.35, 0.45]     — 4수준
  MIN_PROFIT_ATR: [1.0, 1.5, 2.0, 2.5]         — 4수준
  = 5×4×4 = 80 combos
- 목표: OOS Sharpe >= 25 AND trades >= 30
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드 --
TP_BASE_ATR_LIST = [3.0, 3.5, 4.0, 4.5, 5.0]
TRAIL_BASE_ATR_LIST = [0.15, 0.25, 0.35, 0.45]
MIN_PROFIT_ATR_LIST = [1.0, 1.5, 2.0, 2.5]

# -- 3-fold Walkforward --
WF_FOLDS = [
    {"train": ("2022-01-01", "2024-03-31"), "test": ("2024-04-01", "2025-01-31")},
    {"train": ("2022-07-01", "2024-09-30"), "test": ("2024-10-01", "2025-07-31")},
    {"train": ("2023-01-01", "2025-03-31"), "test": ("2025-04-01", "2026-04-09")},
]
...
def build_combos() -> list[dict]:
    combos = []
    for tp_base, trail_base, min_p in product(
        TP_BASE_ATR_LIST, TRAIL_BASE_ATR_LIST, MIN_PROFIT_ATR_LIST,
    ):
        combos.append({
            "tp_base_atr": tp_base,
            "trail_base_atr": trail_base,
            "min_profit_atr": min_p,
        })
    return combos
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 193 — TP/Trail 출구 파라미터 미세조정 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 25 AND trades >= 30")
    print("가설: TP/Trail/MinProfit ATR 배수 최적화 → 수익 확대 + Sharpe 회복")
    ...
    print(f"  탐색: TP_BASE=[3.0..5.0] Trail_BASE=[0.15..0.45] "
          f"MinP=[1.0..2.5]")
    print(f"  고정: TP_BONUS={TP_BONUS_ATR} Trail_BONUS={TRAIL_BONUS_ATR} "
    ...
    sym_data_ok = []
    for sym in SYMBOLS:
        df_check = load_historical(sym, "240m", "2022-01-01", "2026-04-09")
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, combo["tp_base_atr"],
                         combo["trail_base_atr"], combo["min_profit_atr"],
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['tp_base_atr']:>4.1f} {r['trail_base_atr']:>5.2f} "
    ...
    for r in valid:
        key = (r["tp_base_atr"], r["trail_base_atr"], r["min_profit_atr"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        tp_base = params["tp_base_atr"]
        trail_base = params["trail_base_atr"]
        min_p = params["min_profit_atr"]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, tp_base, trail_base, min_p,
                             btc_c, btc_s, esp, vol_mom)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: tpB={tp_base:.1f} trB={trail_base:.2f} "
                  f"mnP={min_p:.1f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        trail_base = params["trail_base_atr"]
        min_p = params["min_profit_atr"]
        print(f"\n--- #{rank}: tpB={tp_base:.1f} trB={trail_base:.2f} "
              f"mnP={min_p:.1f} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, tp_base, trail_base, min_p,
                             btc_c, btc_s, esp, vol_mom,
                             slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    tp_base = best["tp_base_atr"]
    trail_base = best["trail_base_atr"]
    min_p = best["min_profit_atr"]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: tpB={tp_base:.1f} "
          f"trB={trail_base:.2f} mnP={min_p:.1f}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c192 최적 (tpB=4.0 trB=0.30 mnP=1.5): "
          f"avg_OOS=+20.999 n=36")
    print(f"  c193 최적 (tpB={best['tp_base_atr']:.1f} "
          f"trB={best['trail_base_atr']:.2f} "
          f"mnP={best['min_profit_atr']:.1f}): "
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: TP_BASE_ATR={best['tp_base_atr']:.1f} "
          f"TRAIL_BASE_ATR={best['trail_base_atr']:.2f} "
          f"MIN_PROFIT_ATR={best['min_profit_atr']:.1f}")
    print(f"  (c192 고정: VPIN={VPIN_LOW} MOM={MOM_THRESH})")
    print(f"  (c191 고정: slB={SL_SLOPE_BONUS} rTh={RELAX_VOL_MOM_TH} "
          f"rF={RELAX_FACTOR})")
    print(f"  (c190 고정: VOL_MOM_LB={VOL_MOM_LB} VOL_MOM_MIN={VOL_MOM_MIN} "
    ...
    print(f"  (TP/Trail 탐색: TP_BASE+{TP_BONUS_ATR} "
          f"Trail_BASE+{TRAIL_BONUS_ATR} BTC_SMA={BTC_SMA_PERIOD})")
