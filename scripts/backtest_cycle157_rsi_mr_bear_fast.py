"""
사이클 157: RSI Mean-Reversion Bear/Sideways 전용 (최적화 버전)
- 목적: BEAR 레짐 전용 전략 확보 (현재 0개 — 포트폴리오 리스크 집중 해소)
- c154/c155 BB bounce → 전부 실패, c156 RSI MR → 미실행(성능 문제)
- 최적화: 지표 사전계산 + 벡터화 신호 탐색 → 그리드 10x 가속
- 진입 조건:
  1) BTC < SMA(period) → BEAR/Sideways 레짐
  2) RSI < oversold & RSI 반등(RSI_t > RSI_{t-1})
  3) ADX < ceiling (과도한 추세 필터)
  4) 볼륨 > avg × mult (캐피튤레이션 감지, optional)
  5) 다음 봉 시가 진입 (look-ahead bias 제거)
- 청산: TP | SL | RSI >= exit_level | max_hold
- WF: 2-fold
...
class PrecomputedData:
    ...
    def __init__(self, df: pd.DataFrame, btc_closes: np.ndarray | None = None):
        ...
        # BTC SMA 레짐 (여러 SMA period)
        self.btc_sma: dict[int, np.ndarray] = {}
...
def main():
    print("=" * 80)
    print("사이클 157: RSI Mean-Reversion Bear/Sideways 전용 (최적화)")
    print("  진입: BTC<SMA + RSI 과매도 반등 + ADX<ceil + 볼륨")
    print("  60m 타임프레임, 슬리피지 0.10% 포함")
    print("=" * 80)

    # ── 데이터 로드 + 지표 사전계산 ──
    all_data: dict[str, pd.DataFrame] = {}
    all_pre: dict[str, dict[str, PrecomputedData]] = {}  # symbol -> {period_key: pre}
    ...
    btc_closes = btc_full["close"].values.astype(float)
    ...
    RSI_EXIT = [50, 55, 65, 75]
    ADX_CEIL = [20, 25, 30, 35]
    ...
    grid = list(product(RSI_OVERSOLD, RSI_EXIT, ADX_CEIL, VOL_MULT,
    ...
    for sym in SYMBOLS:
        ...
        for rsi_os, rsi_ex, adx_c, vol_m, tp, sl, mh, sma_p in grid:
            r = backtest_fast(
            ...
            results.append({
                ...
                "adx_c": adx_c, "vol_m": vol_m, "tp": tp, "sl": sl,
                "mh": mh, "sma_p": sma_p, **r,
            })
            done += 1
            if done % 1000 == 0:
    ...
    if not safe:
        ...
        for p in partial[:5]:
            print(f"    {p['symbol']} rsi_os={p['rsi_os']:.0f}"
                  f" rsi_ex={p['rsi_ex']:.0f}"
                  f" → avg OOS={p['avg_oos']:+.3f}")
            for fd in p["fold_details"]:
                print(f"      {fd['window']}: Sharpe={fd['sharpe']:+.3f}"
                      f" WR={fd['wr']:.1%} n={fd['n']}")

        print(f"\nSharpe: {partial[0]['avg_oos']:+.3f}" if partial else "\nSharpe: -999")
        print(f"WR: {partial[0]['fold_details'][0]['wr']:.1%}" if partial else "WR: 0.0%")
        n_total = sum(fd['n'] for fd in partial[0]['fold_details']) if partial else 0
