"""
vpin_multi 사이클 194 — BTC SMA 주기 + RSI 상한/하한 진입 필터 탐색
- 기반: c192 최적 (VPIN=0.4 MOM=0.0005) + c193 결과 출구 파라미터 개선 실패
  → c192 전체 파라미터 고정, 미탐색 진입 핵심 변수 탐색
- 문제:
  1) BTC_SMA_PERIOD=200 고정 — 표준값이지만 최적 보장 없음
  2) RSI_CEILING=65 RSI_FLOOR=20 — 초기 설정 이후 미조정
  3) 이 3개 파라미터가 진입 필터링 폭을 직접 결정 → 거래 수/품질 영향 큼
- 가설:
  A) BTC SMA 단축 → 추세 반응 빨라짐 → 더 많은 진입 (단, 노이즈 리스크)
  B) RSI_CEILING 상향 → 강한 추세에서도 진입 허용 (단, 고점 매수 리스크)
  C) RSI_FLOOR 상향 → 과매도 구간 진입 차단 → 품질 향상 (단, 기회 감소)
- 탐색 그리드:
  BTC_SMA_PERIOD: [100, 150, 200, 250]     — 4수준
  RSI_CEILING:    [60, 65, 70, 75]          — 4수준
  RSI_FLOOR:      [15, 20, 25, 30]          — 4수준
  = 4×4×4 = 64 combos
- 목표: OOS Sharpe >= 22 AND trades >= 30 (c192 +20.999 대비 개선)
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드 --
BTC_SMA_LIST = [100, 150, 200, 250]
RSI_CEILING_LIST = [60, 65, 70, 75]
RSI_FLOOR_LIST = [15, 20, 25, 30]
...
def build_combos() -> list[dict]:
    combos = []
    for btc_sma, rsi_ceil, rsi_fl in product(
        BTC_SMA_LIST, RSI_CEILING_LIST, RSI_FLOOR_LIST,
    ):
        combos.append({
            "btc_sma": btc_sma,
            "rsi_ceiling": rsi_ceil,
            "rsi_floor": rsi_fl,
        })
    return combos
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 194 — BTC SMA + RSI 상한/하한 진입 필터 탐색 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 22 AND trades >= 30")
    print("가설: BTC 추세 감도 + RSI 진입 윈도우 최적화 → 진입 품질 개선")
    print(f"기준선: c192 OOS +20.999, WR 54.4%, trades 36")
    print(f"c192 고정: VPIN={VPIN_LOW} MOM={MOM_THRESH}")
    print(f"c192 출구 유지: tpB={TP_BASE_ATR} trB={TRAIL_BASE_ATR} "
    ...
    print(f"  탐색: BTC_SMA={BTC_SMA_LIST} RSI_CEIL={RSI_CEILING_LIST} "
          f"RSI_FLOOR={RSI_FLOOR_LIST}")
    print("=" * 80)
    ...
    # BTC SMA별로 캐시 (SMA 주기가 다르면 재계산 필요)
    btc_sma_cache: dict[int, tuple[pd.Series, pd.Series]] = {}
    btc_close = df_btc_full["close"].values
    for sma_p in BTC_SMA_LIST:
        btc_sma_arr = sma_calc(btc_close, sma_p)
        btc_sma_cache[sma_p] = (
            pd.Series(btc_close, index=df_btc_full.index),
            pd.Series(btc_sma_arr, index=df_btc_full.index),
        )
    ...
    for sym in sym_data_ok:
        ...
        if not df_tr.empty:
            ...
            sym_train_cache[sym] = (df_tr, esp, vol_mom)
            print(f"  {sym} train: {len(df_tr)}행")

    results: list[dict] = []
    for idx, combo in enumerate(combos):
        btc_sma_p = combo["btc_sma"]
        btc_c_s, btc_s_s = btc_sma_cache[btc_sma_p]

        sym_results = []
        for sym in sym_data_ok:
            if sym not in sym_train_cache:
                continue
            df_tr, esp, vol_mom = sym_train_cache[sym]
            btc_c_aligned = btc_c_s.reindex(df_tr.index, method="ffill").values
            btc_s_aligned = btc_s_s.reindex(df_tr.index, method="ffill").values
            r = backtest(df_tr, combo["rsi_ceiling"], combo["rsi_floor"],
                         btc_c_aligned, btc_s_aligned, esp, vol_mom)
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['btc_sma']:>4} {r['rsi_ceiling']:>5.0f} "
            f"{r['rsi_floor']:>4.0f} | "
    ...
    for r in valid:
        key = (r["btc_sma"], r["rsi_ceiling"], r["rsi_floor"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        rsi_ceil = params["rsi_ceiling"]
        rsi_fl = params["rsi_floor"]
        btc_c_s, btc_s_s = btc_sma_cache[btc_sma_p]
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                if df_test.empty:
                    continue
                btc_c_aligned = btc_c_s.reindex(
                    df_test.index, method="ffill").values
                btc_s_aligned = btc_s_s.reindex(
                    df_test.index, method="ffill").values
                esp, vol_mom = precompute_indicators(df_test)
                r = backtest(df_test, rsi_ceil, rsi_fl,
                             btc_c_aligned, btc_s_aligned, esp, vol_mom)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: bSMA={btc_sma_p} rCeil={rsi_ceil:.0f} "
                  f"rFlr={rsi_fl:.0f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        rsi_fl = params["rsi_floor"]
        btc_c_s, btc_s_s = btc_sma_cache[btc_sma_p]
        print(f"\n--- #{rank}: bSMA={btc_sma_p} rCeil={rsi_ceil:.0f} "
              f"rFlr={rsi_fl:.0f} "
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, rsi_ceil, rsi_fl,
                             btc_c_aligned, btc_s_aligned, esp, vol_mom,
                             slippage=slip)
                sym_results.append(r)
            pooled = pool_results(sym_results)
    ...
    # -- 심볼별 성능 분해 (Top 1) --
    best = wf_sorted[0]
    btc_sma_p = best["btc_sma"]
    rsi_ceil = best["rsi_ceiling"]
    rsi_fl = best["rsi_floor"]
    btc_c_s, btc_s_s = btc_sma_cache[btc_sma_p]

    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: bSMA={btc_sma_p} "
          f"rCeil={rsi_ceil:.0f} rFlr={rsi_fl:.0f}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
        for fold_i, fold in enumerate(WF_FOLDS):
    ...
    print(f"  c192 최적 (bSMA=200 rCeil=65 rFlr=20): "
          f"avg_OOS=+20.999 n=36")
    print(f"  c194 최적 (bSMA={best['btc_sma']} "
          f"rCeil={best['rsi_ceiling']:.0f} "
          f"rFlr={best['rsi_floor']:.0f}): "
          f"avg_OOS={best['avg_oos_sharpe']:+.3f} "
          f"n={best['total_oos_trades']}")
    delta_sh = best["avg_oos_sharpe"] - 20.999
    delta_n = best["total_oos_trades"] - 36
    ...
    print(f"★ OOS 최적: BTC_SMA={best['btc_sma']} "
          f"RSI_CEILING={best['rsi_ceiling']:.0f} "
          f"RSI_FLOOR={best['rsi_floor']:.0f}")
    print(f"  (c192 고정: VPIN={VPIN_LOW} MOM={MOM_THRESH})")
