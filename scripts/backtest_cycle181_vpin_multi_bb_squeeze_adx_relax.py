"""
vpin_multi 사이클 181 — c178 최적 기반 BB스퀴즈 진입 완화 + ADX 트렌드 강도 필터
...
- c178 약점:
  1) n=19 극히 낮음 — 통계적 유의성 부족, XRP 0건
  2) squeeze_pctile_th=50 + expansion_lb=4 + upper_ratio=0.98 → 과도한 필터 조합
  3) BB스퀴즈 해제는 좋지만 "스퀴즈 history 윈도우"가 좁아 많은 유효 진입 놓침
- 가설:
  A) 스퀴즈 히스토리 윈도우 확장 (squeeze_lookback): 10봉→20/30봉으로 늘려
     더 넓은 스퀴즈 탐지 → 진입 기회 증가
  B) upper_ratio 완화: 0.95/0.96 추가하여 밴드 근접 진입 허용
  C) ADX 필터 추가: ADX > threshold → 트렌드 확인 시에만 진입
     (BB스퀴즈 조건 완화의 안전장치 역할)
  D) body_ratio 완화: 0.5/0.6 추가 — c176의 0.7은 양봉 필터 과격
- 탐색 그리드:
  2 SQUEEZE_LB_WINDOW × 3 UPPER_RATIO × 3 BODY_RATIO × 2 ADX_TH = 36 combos
  BB_PERIOD=20, SQUEEZE_PCTILE_TH=50, EXPANSION_LB=4, ATR_PCTILE=ON 고정 (c178 최적)
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드 --
SQUEEZE_LB_WINDOW_LIST = [20, 30]         # 스퀴즈 탐지 히스토리 윈도우 (c178=10)
UPPER_RATIO_LIST = [0.95, 0.96, 0.98]     # close >= upper*ratio (c178=0.98)
BODY_RATIO_LIST = [0.5, 0.6, 0.7]         # 양봉 바디 비율 (c176=0.7)
ADX_TH_LIST = [20, 25]                     # ADX 트렌드 강도 임계값
...
def backtest(
    ...
    warmup = max(BUCKET_COUNT, EMA_PERIOD, RSI_PERIOD + 1,
                 ...
                 ATR_PCTILE_LB, BB_PERIOD, 120, 50, 30) + 5
    i = warmup
    ...
    while i < n - 1:
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
                and body_ok and atr_pctile_ok and bb_squeeze_ok and adx_ok):
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 181 — BB스퀴즈 진입완화 + ADX 트렌드 필터 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  목표: Sharpe >= 20.0 & trades >= 30")
    print("가설 A: 스퀴즈 히스토리 윈도우 확장 (10→20/30봉) → 진입 기회 증가")
    print("가설 B: upper_ratio 완화 (0.98→0.95/0.96) → 밴드 근접 진입 허용")
    print("가설 C: body_ratio 완화 (0.7→0.5/0.6) → 양봉 필터 완화")
    print("가설 D: ADX >= threshold → 트렌드 확인 (완화 안전장치)")
    ...
    combos = list(product(
        SQUEEZE_LB_WINDOW_LIST, UPPER_RATIO_LIST,
        BODY_RATIO_LIST, ADX_TH_LIST,
    ))
    ...
    results: list[dict] = []
    for idx, (sq_lb, up_r, body_r, adx_th) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, sq_lb, up_r, body_r, adx_th, btc_c, btc_s)
            sym_results.append(r)

        pooled = pool_results(sym_results)
        results.append({
            "squeeze_lb": sq_lb, "upper_ratio": up_r,
            "body_ratio": body_r, "adx_th": adx_th, **pooled,
        })

    valid = [r for r in results if r["trades"] >= 5 and not np.isnan(r["sharpe"])]
    valid.sort(key=lambda x: x["sharpe"], reverse=True)

    print(f"\n유효 조합 (n>=5): {len(valid)}/{len(results)}")
    print(f"\n=== Train Top 16 (pooled Sharpe 기준) ===")
    hdr = (f"{'sqLB':>5} {'upR':>5} {'body':>5} {'adx':>4} | "
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['squeeze_lb']:>5} {r['upper_ratio']:>5.2f} "
            f"{r['body_ratio']:>5.2f} {r['adx_th']:>4} | "
    ...
    for r in valid:
        key = (r["squeeze_lb"], r["upper_ratio"], r["body_ratio"], r["adx_th"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_test, sq_lb, up_r, body_r, adx_th,
                             btc_c, btc_s)
        ...
        if oos_sharpes:
            ...
            print(f"  #{rank}: sqLB={sq_lb} upR={up_r:.2f} "
                  f"body={body_r:.2f} adx={adx_th} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(df_full, sq_lb, up_r, body_r, adx_th,
    ...
    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: sqLB={sq_lb} upR={up_r:.2f} "
          f"body={body_r:.2f} adx={adx_th}) ===")
    for sym in sym_data_ok:
        sym_oos_sharpes = []
        sym_oos_trades = 0
    ...
    print(f"  c181 최적 (sqLB={best['squeeze_lb']} upR={best['upper_ratio']:.2f} "
          f"body={best['body_ratio']:.2f} adx={best['adx_th']}): "
    ...
    delta_n = best["total_oos_trades"] - 19
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: SQUEEZE_LB_WINDOW={best['squeeze_lb']} "
