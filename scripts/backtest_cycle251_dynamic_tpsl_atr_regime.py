"""
사이클 251: c243 최적 고정 + ATR 레짐 기반 동적 TP/SL 스케일링

c243 결과:
  avg OOS Sharpe: +31.850 PASS  F2: +33.667 PASS  F3: +13.521 FAIL
  F3: Sharpe=+13.521  WR=60.0%  trades=5  avg=+1.24%  MDD=-2.75%

c232 참고: tight trail + ATR SL 스케일 → Sharpe +30.08 (유의미 엣지)
c196 참고: 진입 필터 완화 → Sharpe +48.62

문제 분석:
  - F3 구간(2025-12~2026-03) 5건 거래 중 avg +1.24% → TP가 너무 넓거나 SL이 비효율
  - 고정 TP/SL 배수(TP1=2.5, TP2=3.0, SL=1.5, trail=2.0)는 모든 레짐에 동일
  - 저변동성 구간: ATR 자체가 작아 TP/SL 절대폭이 좁아지지만,
    레짐 전환기에는 ATR percentile이 중간대(30~60)로 떨어짐
  - 가설: ATR percentile에 따라 TP/SL 배수를 동적 조절
    → 저vol 레짐(pctile<50): 타이트한 TP/SL로 빠른 익절/손절
    → 고vol 레짐(pctile>50): 넓은 TP/SL로 큰 무브 포착
  - 구현: multiplier = base * (lo_scale + (hi_scale-lo_scale) * pctile/100)
    → pctile=0 → base*lo_scale, pctile=100 → base*hi_scale
...
새 탐색 그리드 (c251):
  TP_LO_SCALE:    [0.6, 0.7, 0.8, 1.0]    — 저vol TP 축소율 (1.0=변화없음)
  TP_HI_SCALE:    [1.0, 1.2, 1.4]          — 고vol TP 확대율
  SL_LO_SCALE:    [0.6, 0.7, 0.8, 1.0]    — 저vol SL 축소율
  = 4 × 3 × 4 = 48 combos
  (trail도 TP와 같은 스케일 적용)
...
# ─── c251 탐색 그리드 ─────────────────────────────────────────
TP_LO_SCALE_LIST = [0.6, 0.7, 0.8, 1.0]   # 저vol TP 배수 스케일
TP_HI_SCALE_LIST = [1.0, 1.2, 1.4]         # 고vol TP 배수 스케일
SL_LO_SCALE_LIST = [0.6, 0.7, 0.8, 1.0]   # 저vol SL 배수 스케일
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            if cp > position["peak"]:
                ...
                # 동적 trail 스케일 (TP와 동일 스케일 적용)
                pctile_now = atr_pctile[i] if not np.isnan(atr_pctile[i]) else 50
                regime_scale = tp_lo_scale + (tp_hi_scale - tp_lo_scale) * (
                    pctile_now / 100.0)
                effective_trail = BASE_TRAIL_MULT * regime_scale
            ...
            # ── c251: ATR percentile 기반 동적 TP/SL ────────
            pctile_now = atr_pctile[i] if not np.isnan(atr_pctile[i]) else 50
            # TP 스케일: pctile=0 → tp_lo_scale, pctile=100 → tp_hi_scale
            tp_regime_scale = tp_lo_scale + (tp_hi_scale - tp_lo_scale) * (
                pctile_now / 100.0)
            # SL 스케일: pctile=0 → sl_lo_scale, pctile=100 → 1.0
            sl_regime_scale = sl_lo_scale + (1.0 - sl_lo_scale) * (
                pctile_now / 100.0)
            ...
            # 동적 배수 적용
            tp2_mult = (BASE_TP2_MULT + vol_tp_bonus) * tp_regime_scale
            tp1_mult = BASE_TP1_MULT * tp_regime_scale
            sl_mult = BASE_SL_MULT * sl_regime_scale

            tp2_pct = atr_now / c[i] * tp2_mult
            tp1_pct = atr_now / c[i] * tp1_mult
            sl_pct = atr_now / c[i] * sl_mult * sl_scale
...
def main() -> None:
    print("=" * 80)
    print("=== c251: c243최적 고정 + ATR 레짐 기반 동적 TP/SL 스케일링 ===")
    ...
    print(f"탐색: TP_LO_SCALE × TP_HI_SCALE × SL_LO_SCALE")
    print("=" * 80)
    ...
    # 그리드
    grid = list(product(
        TP_LO_SCALE_LIST, TP_HI_SCALE_LIST, SL_LO_SCALE_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c243 최적 = 고정 TP/SL, 스케일 모두 1.0)
    print("\n--- 베이스라인 (c243 최적, 고정 TP/SL) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for window in WINDOWS:
        ...
        for sym in SYMBOLS:
            ...
            raw = run_backtest(
                ...
                1.0, 1.0, 1.0,  # 스케일 모두 1.0 = c243 동일
    ...
    for val in TP_LO_SCALE_LIST:
        ...
        if sub:
            ...
            print(f"  tpLo={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")

    print("\n=== TP 고vol 스케일 효과 ===")
    for val in TP_HI_SCALE_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  tpHi={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")

    print("\n=== SL 저vol 스케일 효과 ===")
    for val in SL_LO_SCALE_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["f3_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f3 = np.mean([r["f3_sharpe"] for r in top5])
            print(f"  slLo={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F3={avg_f3:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  tpLo={p[0]:.1f} tpHi={p[1]:.1f} slLo={p[2]:.1f} | "
    ...
    if valid:
        ...
        print(f"  c251 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: tpLo={p[0]:.1f} tpHi={p[1]:.1f} "
              f"slLo={p[2]:.1f}")
        print(f"  (c243 고정: btcMLB={BTC_MOM_LB} btcMMin={BTC_MOM_MIN} "
              f"btcADX={BTC_ADX_GATE} boMgn={BREAKOUT_MARGIN})")
