"""
vpin_multi 사이클 193 — Q-score 진입 게이트 (exit 미수정)
- 기반: c179 OOS Sharpe +42.878, vol regime adaptive TP/SL
- c192 교훈: Q-score로 TP/SL/Hold 수정 → Sharpe 42→18 악화
  exit 메커니즘은 c179가 이미 최적. 문제는 저품질 진입.
- 가설: Q-score를 exit 수정이 아닌 진입 필터로 사용
  Q = w*(VPIN_headroom) + w*(RSI_headroom) + w*(MOM_strength) + w*(VOL_surge)
  Q >= Q_MIN 일 때만 진입 허용 → 저품질 진입 제거
  c179 exit (TP/SL/Hold/Trail) 100% 유지
  심볼별 적응형 Q 임계값: SOL에 추가 Q 보너스 요구
- 탐색 그리드:
  VPIN_WEIGHT: [0.3, 0.5]
  RSI_WEIGHT:  [0.2, 0.3]
  MOM_WEIGHT:  [0.2, 0.3]
  VOL_WEIGHT:  [0.1, 0.2]
  Q_MIN:       [0.20, 0.30, 0.40, 0.50]  — 진입 최소 품질
  Q_SOL_BONUS: [0.0, 0.05, 0.10]         — SOL 추가 요구
  = 2×2×2×2 × 4×3 = 192 combos
...
# -- 탐색 그리드: Q-score 진입 게이트 --
VPIN_WEIGHT_LIST = [0.3, 0.5]
RSI_WEIGHT_LIST = [0.2, 0.3]
MOM_WEIGHT_LIST = [0.2, 0.3]
VOL_WEIGHT_LIST = [0.1, 0.2]
Q_MIN_LIST = [0.20, 0.30, 0.40, 0.50]     # 진입 최소 품질
Q_SOL_BONUS_LIST = [0.0, 0.05, 0.10]      # SOL 추가 임계값
...
def backtest(
    ...
    if symbol == "KRW-SOL":
        effective_q_min = q_min + q_sol_bonus
    ...
    while i < n - 1:
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
            ...
            if q_score < effective_q_min:
                i += 1
                continue

            # ★ c179 exit 메커니즘 100% 유지 (Q-score 미수정)
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi 사이클 193 — Q-score 진입 게이트 (exit 미수정) ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 43.0 (c179 +42.878 대비 개선)")
    print("가설: Q-score를 exit 수정 아닌 진입 필터로 사용")
    print("  Q = w*(VPIN_headroom) + w*(RSI_headroom) + w*(MOM_strength)"
          " + w*(VOL_surge)")
    print("  Q >= Q_MIN → 진입 / Q < Q_MIN → 스킵")
    print("  c179 exit (TP/SL/Hold/Trail) 100% 유지")
    print("  SOL 추가 Q 임계값 (Q_SOL_BONUS) — 약한 심볼 필터링")
    print(f"c192 교훈: Q-score→exit수정 시 Sharpe 42→18 악화")
    ...
    for idx, (w_vpin, w_rsi, w_mom, w_vol,
              q_min, q_sol_b) in enumerate(combos):
        sym_results = []
        for sym in sym_data_ok:
            ...
            r = backtest(df_tr, w_vpin, w_rsi, w_mom, w_vol,
                         q_min, q_sol_b, sym, btc_c, btc_s)
            sym_results.append(r)
        pooled = pool_results(sym_results)
        results.append({
            "w_vpin": w_vpin, "w_rsi": w_rsi, "w_mom": w_mom, "w_vol": w_vol,
            "q_min": q_min, "q_sol_b": q_sol_b,
            **pooled,
        })
        if (idx + 1) % 48 == 0 or idx == len(combos) - 1:
            print(f"  [{idx + 1}/{len(combos)}] 완료")

    # 최소 trades 기준: Q 필터가 너무 많이 제거하면 안 됨
    valid = [r for r in results if not np.isnan(r["sharpe"]) and r["trades"] >= 20]
    if not valid:
        print("유효 결과 없음.")
        print("\nSharpe: nan")
        print("WR: 0.0%")
    ...
    for rank, r in enumerate(top5, 1):
        print(f"  #{rank} wV={r['w_vpin']} wR={r['w_rsi']} wM={r['w_mom']} "
              f"wVol={r['w_vol']} qMin={r['q_min']} qSolB={r['q_sol_b']} "
    ...
    for rank, combo in enumerate(top5, 1):
        ...
        w_vol = combo["w_vol"]
        q_min = combo["q_min"]
        q_sol_b = combo["q_sol_b"]

        print(f"\n--- #{rank}: wV={w_vpin} wR={w_rsi} wM={w_mom} wVol={w_vol} "
              f"qMin={q_min} qSolB={q_sol_b} ---")
    ...
    q_min = best_combo["q_min"]
    q_sol_b = best_combo["q_sol_b"]
        ...
        for sym in sym_data_ok:
            for fold in WF_FOLDS:
                ...
                btc_c, btc_s = align_btc_to_symbol(
                    df_te, df_btc_full, BTC_SMA_PERIOD)
                r = backtest(df_te, w_vpin, w_rsi, w_mom, w_vol,
                             q_min, q_sol_b, sym, btc_c, btc_s, slip)
    ...
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: wV={w_vpin} wR={w_rsi} "
          f"wM={w_mom} wVol={w_vol} qMin={q_min} qSolB={q_sol_b}) ===")
    ...
    print(f"  c193 최적 (wV={w_vpin} wR={w_rsi} wM={w_mom} wVol={w_vol} "
          f"qMin={q_min} qSolB={q_sol_b}): "
