"""
vpin_multi 사이클 182 — 시간경과 SL 강화 + 모멘텀 악화 조기 청산
...
- c180 결론: 진입 적응(VPIN/MOM 스케일링 + ADX) → FAIL. 진입 건드리지 않음.
- c179 약점 (출구 관점):
  1) WR 43.8% — 패배 거래가 hold 끝까지 끌려감 (SL 고정)
  2) XRP Fold 1 WR=16.7% n=6 — 진입 후 모멘텀 악화에도 계속 보유
  3) 고정 SL: 시간 경과에도 동일 → 초기 stop 넓음이 후반까지 유지
- 가설:
  A) breakeven SL: N바 경과 후 SL을 entry price로 이동 → 수수료 제외 손실 차단
  B) 시간경과 SL 강화: 바 경과당 SL을 ATR 비율만큼 올림 → 느린 하락 방지
  C) 모멘텀 악화 청산: hold 중 momentum < threshold → 추세 약화 시 즉시 청산
  D) A+B+C 복합
- 탐색 그리드: 4 breakeven_bars × 3 sl_tighten_rate × 3 mom_exit_mode = 36 combos
  c179 진입/출구 기본값 전부 고정, 출구 보조 로직만 탐색
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
# -- 탐색 그리드: 출구 보조 로직 --
# breakeven_bars: N바 후 SL을 entry price로 이동 (0=비활성화)
BREAKEVEN_BARS_LIST = [0, 5, 8, 12]
# sl_tighten_rate: 바 경과당 SL을 올리는 ATR 비율 (0=비활성화)
SL_TIGHTEN_RATE_LIST = [0.0, 0.015, 0.03]
# mom_exit_mode: hold 중 모멘텀 기반 청산 (0=비활성, 1=mom<-0.001, 2=mom<0.0)
MOM_EXIT_MODE_LIST = [0, 1, 2]
MOM_EXIT_THRESHOLDS = {0: None, 1: -0.001, 2: 0.0}
MOM_EXIT_LOOKBACK = 4  # hold 중 모멘텀 계산 lookback
...
def backtest(
    ...
    mom_exit_thresh = MOM_EXIT_THRESHOLDS[mom_exit_mode]
    ...
    while i < n - 1:
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
            ...
            base_sl_price = buy - atr_at_entry * effective_sl_mult
            ...
            for j in range(i + 2, min(i + 1 + max_hold, n)):
                ...
                # ★ c182: 시간경과 SL 강화
                dynamic_sl = base_sl_price
                if sl_tighten_rate > 0 and bars_held > 0:
                    sl_raise = atr_at_entry * sl_tighten_rate * bars_held
                    dynamic_sl = base_sl_price + sl_raise

                # ★ c182: breakeven SL
                if breakeven_bars > 0 and bars_held >= breakeven_bars:
                    breakeven_price = buy + atr_at_entry * 0.05  # 수수료 커버
                    dynamic_sl = max(dynamic_sl, breakeven_price)

                # SL은 entry 위로 올라갈 수 있지만 TP보다는 낮아야 함
                dynamic_sl = min(dynamic_sl, tp_price - atr_at_entry * 0.1)
                ...
                if current_price <= dynamic_sl:
                    exit_ret = (dynamic_sl / buy - 1) - FEE - slippage
                    i = j
                    break
                ...
                # ★ c182: 모멘텀 악화 조기 청산
                if mom_exit_thresh is not None and bars_held >= MOM_EXIT_LOOKBACK:
                    in_trade_mom = c[j] / c[j - MOM_EXIT_LOOKBACK] - 1
                    if in_trade_mom < mom_exit_thresh:
                        exit_ret = (current_price / buy - 1) - FEE - slippage
                        i = j
                        break
...
def main() -> None:
    print("=" * 80)
    print("c182: 시간경과 SL 강화 + 모멘텀 악화 조기 청산")
    print("기반: c179 OOS Sharpe +17.938 (진입 고정, 출구만 탐색)")
    print("=" * 80)
    ...
    for ci, (be_bars, sl_rate, mom_mode) in enumerate(combos):
        mom_th_str = (
            f"{MOM_EXIT_THRESHOLDS[mom_mode]}"
            if MOM_EXIT_THRESHOLDS[mom_mode] is not None else "off"
        )
        label = f"beBars={be_bars} slRate={sl_rate:.3f} momExit={mom_th_str}"
        ...
        for fi, fold in enumerate(WF_FOLDS):
            ...
            # Train: 최적 파라미터가 이미 c179에서 고정이므로 검증만
            train_results = []
            for sym in SYMBOLS:
                if sym not in cache:
                    continue
                df_sym = cache[sym]
                mask_train = (df_sym.index >= train_start) & (df_sym.index <= train_end)
                df_train = df_sym[mask_train]
                if len(df_train) < 100:
                    continue
                btc_c, btc_s = align_btc_to_symbol(df_train, df_btc, BTC_SMA_PERIOD)
                r = backtest(df_train, be_bars, sl_rate, mom_mode, btc_c, btc_s)
            ...
            for sym in SYMBOLS:
                if sym not in cache:
                    continue
                df_sym = cache[sym]
                mask_test = (df_sym.index >= test_start) & (df_sym.index <= test_end)
                df_test = df_sym[mask_test]
                if len(df_test) < 50:
                    continue
                btc_c, btc_s = align_btc_to_symbol(df_test, df_btc, BTC_SMA_PERIOD)
                r = backtest(df_test, be_bars, sl_rate, mom_mode, btc_c, btc_s)
        ...
        avg_oos_sharpe = float(np.nanmean(fold_oos_sharpes)) if fold_oos_sharpes else float("nan")
        ...
        avg_oos_ret = float(np.nanmean([
            fr["oos"]["avg_ret"] for fr in fold_oos_results if fr["oos"]["trades"] > 0
        ])) if fold_oos_results else 0.0
        avg_oos_mdd = float(np.nanmean([
            fr["oos"]["max_dd"] for fr in fold_oos_results if fr["oos"]["trades"] > 0
        ])) if fold_oos_results else 0.0
    ...
    if valid_results:
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in SYMBOLS:
                ...
                r = backtest(
                    df_sym, best["be_bars"], best["sl_rate"],
                    best["mom_mode"], btc_c, btc_s, slippage=slip,
                )
    ...
    # c179 대비 비교
    C179_BASELINE_SHARPE = 17.938
    C179_BASELINE_TRADES = 64
    if valid_results:
        best = valid_results[0]
        print("\n" + "=" * 80)
        print("=== c179 베이스라인 대비 비교 ===")
        print(f"  c179 기준 (고정 SL): avg_OOS={C179_BASELINE_SHARPE:+.3f} n={C179_BASELINE_TRADES}")
        print(f"  c182 최적 ({best['label']}): "
    ...
    if valid_results:
        best = valid_results[0]
        tag = "PASS" if best["avg_oos_sharpe"] > C179_BASELINE_SHARPE else "FAIL"
        print(f"★ OOS 최적: BREAKEVEN_BARS={best['be_bars']} "
              f"SL_TIGHTEN_RATE={best['sl_rate']:.3f} "
              f"MOM_EXIT_MODE={best['mom_mode']}")
