"""
vpin_multi 사이클 206 — Bollinger Band Squeeze → Expansion 진입 타이밍 최적화
...
- 문제:
  1) c202/c203 추가 필터(ADX/OBV/CMF)는 거래 수 감소 → Sharpe 악화
  2) 기존 진입은 "좋은 시점"이 아니라 "조건 충족 시점" — 타이밍 최적화 여지
  3) 횡보(BB squeeze) 후 확장(expansion) 구간에서 진입하면 추세 초기 포착 가능
- 가설:
  A) BB width percentile이 낮은(squeeze) 상태에서 확장으로 전환 시 진입
     → BB_WIDTH = (upper - lower) / middle
     → BB_WIDTH_PCTILE < SQUEEZE_TH (최근 N봉 대비 좁음) AND
        BB_WIDTH > BB_WIDTH[prev] (확장 시작)
  B) 기존 VPIN 진입 조건과 AND — 추가 필터가 아닌 타이밍 보정
  C) squeeze 강도에 따른 TP 보너스 — 압축이 강할수록 확장 후 움직임 큼
- 탐색 그리드:
  BB_PERIOD: [20, 30]                  — BB 기간
  BB_STD: [2.0, 2.5]                   — BB 표준편차 배수
  SQUEEZE_TH: [20, 30, 40]             — BB width percentile 임계값 (squeeze 판정)
  SQUEEZE_LB: [30, 60]                 — width percentile lookback
  EXPAND_BARS: [1, 2, 3]               — 확장 확인 봉수
  TP_SQUEEZE_BONUS: [0.0, 0.5, 1.0]    — squeeze 강도 → TP 보너스
  = 2×2×3×2×3×3 = 216 combos
- 목표: OOS Sharpe >= 45 AND trades >= 15
- 3-fold WF + 슬리피지 스트레스
- 진입: next_bar open
"""
...
BB_STD_LIST = [2.0, 2.5]
SQUEEZE_TH_LIST = [20, 30, 40]
SQUEEZE_LB_LIST = [30, 60]
EXPAND_BARS_LIST = [1, 2, 3]
TP_SQUEEZE_BONUS_LIST = [0.0, 0.5, 1.0]
...
def backtest(
    ...
    while i < n - 1:
        ...
        # === 신규 c206: BB Squeeze → Expansion 진입 타이밍 ===
        bb_squeeze_ok = False
        squeeze_intensity = 0.0
        if not np.isnan(bb_width_pctile_arr[i]):
            # 최근 expand_bars 봉 내에 squeeze 상태였고 현재 확장 중인지 확인
            was_squeezed = False
            for eb in range(1, expand_bars + 1):
                idx_back = i - eb
                if idx_back >= 0 and not np.isnan(bb_width_pctile_arr[idx_back]):
                    if bb_width_pctile_arr[idx_back] <= squeeze_th:
                        was_squeezed = True
                        # squeeze 강도: percentile이 낮을수록 강함
                        intensity = 1.0 - bb_width_pctile_arr[idx_back] / 100.0
                        squeeze_intensity = max(squeeze_intensity, intensity)
                        break
            # 현재 확장 중: BB width가 이전보다 넓어짐
            is_expanding = False
            if not np.isnan(bb_width_arr[i]) and i > 0:
                if not np.isnan(bb_width_arr[i - 1]):
                    is_expanding = bb_width_arr[i] > bb_width_arr[i - 1]
            bb_squeeze_ok = was_squeezed and is_expanding
        ...
        if (vpin_ok and btc_ok and rsi_velocity_ok and vol_ok
            ...
            # c206 신규: squeeze 강도 → TP 보너스
            squeeze_tp_extra = tp_squeeze_bonus * squeeze_intensity
...
def build_combos() -> list[dict]:
    combos = []
    for bb_p, bb_s, sq_th, sq_lb, exp_b, tp_sq in product(
        BB_PERIOD_LIST, BB_STD_LIST, SQUEEZE_TH_LIST,
        SQUEEZE_LB_LIST, EXPAND_BARS_LIST, TP_SQUEEZE_BONUS_LIST,
    ):
        combos.append({
            "bb_period": bb_p,
            "bb_std": bb_s,
...
def precompute_bb(
    ...
    """BB width + BB width percentile 사전 계산."""
    c = df["close"].values
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi c206 — BB Squeeze → Expansion 진입 타이밍 최적화 ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 45 AND trades >= 15")
    print("가설: 횡보(squeeze) 후 확장 시점에 진입 → 추세 초기 포착")
    ...
    # BB width는 파라미터 의존 → 캐시 키 = (sym, bb_period, bb_std, squeeze_lb)
    bb_cache: dict[tuple, tuple[np.ndarray, np.ndarray]] = {}
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            bb_key = (sym, combo["bb_period"], combo["bb_std"],
                      combo["squeeze_lb"])
            if bb_key not in bb_cache:
                bb_cache[bb_key] = precompute_bb(
                    df_tr, combo["bb_period"],
                    combo["bb_std"], combo["squeeze_lb"])
            bb_w, bb_wp = bb_cache[bb_key]
    ...
    hdr = (f"{'bbP':>4} {'bbS':>4} {'sqTh':>5} {'sqLB':>5} "
           f"{'expB':>5} {'tpSq':>5} | "
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['bb_period']:>4} {r['bb_std']:>4.1f} "
            f"{r['squeeze_th']:>5} {r['squeeze_lb']:>5} "
            f"{r['expand_bars']:>5} {r['tp_squeeze_bonus']:>5.1f} | "
    ...
    for r in valid:
        key = (r["bb_period"], r["bb_std"], r["squeeze_th"],
               r["squeeze_lb"], r["expand_bars"], r["tp_squeeze_bonus"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                bb_w, bb_wp = precompute_bb(
                    df_test, params["bb_period"],
                    params["bb_std"], params["squeeze_lb"])

                r = backtest(
                    df_test,
                    params["bb_period"], params["bb_std"],
                    params["squeeze_th"], params["squeeze_lb"],
                    params["expand_bars"], params["tp_squeeze_bonus"],
                    btc_c, btc_s, esp, vol_mom, atr_pctile,
                    bb_w, bb_wp)
        ...
        if oos_sharpes:
            ...
            all_pass = (all(s >= 3.0 for s in oos_sharpes)
                        and avg_oos >= 5.0)
            print(
                f"  #{rank}: bbP={params['bb_period']} "
                f"bbS={params['bb_std']:.1f} "
                f"sqTh={params['squeeze_th']} "
                f"sqLB={params['squeeze_lb']} "
                f"expB={params['expand_bars']} "
                f"tpSq={params['tp_squeeze_bonus']:.1f} | "
    ...
    for rank, params in enumerate(wf_top3, 1):
        ...
        for slip in SLIPPAGE_LEVELS:
            ...
            for sym in sym_data_ok:
                ...
                esp, vol_mom, atr_pctile = precompute_base(df_full)
                bb_w, bb_wp = precompute_bb(
                    df_full, params["bb_period"],
                    params["bb_std"], params["squeeze_lb"])
                r = backtest(
                    df_full,
                    params["bb_period"], params["bb_std"],
            ...
            print(
                f"  {slip * 100:.2f}% {sh:>+8.3f} "
                f"{pooled['wr']:>5.1%} "
                f"{pooled['avg_ret'] * 100:>+6.2f}% "
                f"{pooled['max_dd'] * 100:>+6.2f}% "
                f"{pooled['mcl']:>4} {pooled['trades']:>5}")

    # -- Phase 4: 심볼별 분해 (Top 1) --
    best = wf_sorted[0]
    print(f"\n{'=' * 80}")
    print(f"=== 심볼별 OOS 성능 분해 (Top 1: "
          f"bbP={best['bb_period']} bbS={best['bb_std']:.1f} "
          f"sqTh={best['squeeze_th']} sqLB={best['squeeze_lb']} "
          f"expB={best['expand_bars']} "
          f"tpSq={best['tp_squeeze_bonus']:.1f}) ===")
    ...
    for sym in sym_data_ok:
        ...
        for fold in WF_FOLDS:
            ...
            esp, vol_mom, atr_pctile = precompute_base(df_test)
            bb_w, bb_wp = precompute_bb(
                df_test, best["bb_period"],
                best["bb_std"], best["squeeze_lb"])
            r = backtest(
                df_test,
                best["bb_period"], best["bb_std"],
                best["squeeze_th"], best["squeeze_lb"],
                best["expand_bars"], best["tp_squeeze_bonus"],
                btc_c, btc_s, esp, vol_mom, atr_pctile,
                bb_w, bb_wp)
            sh = r["sharpe"] if not np.isnan(r["sharpe"]) else 0.0
            print(
                f"  {sym} Fold {fold['test'][0][:7]}~"
                f"{fold['test'][1][:7]}: "
                f"Sharpe={sh:+.3f}  WR={r['wr']:.1%}  "
    ...
    print(f"  c199 기준 (regime dual exit): avg_OOS=+51.425")
    print(f"  c206 최적: avg_OOS={best['avg_oos_sharpe']:+.3f}")
    delta = best["avg_oos_sharpe"] - 51.425
    print(f"  Δ vs c199: {delta:+.3f} "
          f"({'개선' if delta > 0 else '악화'})")

    # -- 최종 요약 --
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    status = ("PASS" if best["avg_oos_sharpe"] > 5.0
              and best["total_oos_trades"] >= 15 else "FAIL")
    print(f"★ OOS 최적: bbP={best['bb_period']} "
          f"bbS={best['bb_std']:.1f} "
          f"sqTh={best['squeeze_th']} sqLB={best['squeeze_lb']} "
          f"expB={best['expand_bars']} "
          f"tpSq={best['tp_squeeze_bonus']:.1f}")
    print(f"  (c199 고정: rTh={REGIME_TH} hiTP={HI_TP_BONUS} "
          f"hiTr={HI_TRAIL_RELAX} loSL={LO_SL_TIGHTEN})")
    print(f"  (c192 고정: ttA={TRAIL_TIGHTEN_AFTER} "
          f"ttF={TRAIL_TIGHTEN_FACTOR})")
    print(f"  (c190 고정: vMomLB={VOL_MOM_LB} vMomMin={VOL_MOM_MIN} "
          f"tpBonus={TP_SLOPE_BONUS})")
    print(f"  (c186 고정: body={BODY_RATIO_MIN} rsiD={RSI_DELTA_MIN} "
          f"sLB={EMA_SLOPE_LB} sPth={EMA_SLOPE_PCTILE_TH})")
    ...
    print(f"  avg OOS Sharpe: {best['avg_oos_sharpe']:+.3f} {status}")
    print(f"  train Sharpe: {best['train_sharpe']:+.3f}")
    for fi, fd in enumerate(best["fold_details"]):
        print(f"  Fold {fi + 1}: Sharpe={fd['sharpe']:+.3f}  "
