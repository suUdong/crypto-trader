...
RUNTIME_CONFIG_PATH = _project_root / "config" / "daemon.toml"
ROTATION_STATE_PATH = _project_root / "artifacts" / "alpha-rotation-state.json"
ROTATION_MIN_STREAK = 2
...
def _load_active_accum_wallets() -> list[str]:
    try:
        config = load_config(RUNTIME_CONFIG_PATH, {})
        wallets = [wallet.name for wallet in config.wallets if "accumulation" in wallet.name]
        if wallets:
            return wallets
    except Exception as exc:
        print(f"[AutoUpdate] accumulation wallet load fallback: {exc}")
    return [
...
def get_alpha_scan_results() -> tuple[str, float, dict, dict, pd.DataFrame, str]:
    """Returns scan text, threshold, signals, raw data, dataframe, and closed-bar id."""
    ...
    closed_bar = btc_df.index[-2] if len(btc_df.index) >= 2 else btc_df.index[-1]
    closed_bar_id = (
        closed_bar.isoformat()
        if hasattr(closed_bar, "isoformat")
        else str(closed_bar)
    )
...
def main() -> None:
    ...
    while True:
        try:
            ...
            top_symbols = parse_alpha_watchlist(
                scan_data,
                threshold=cal_threshold,
                max_symbols=5,
            )
            accumulation_candidates = select_accumulation_candidates(
            ...
            stealth_watch = filter_rotation_candidates(stealth_watch)
            ...
            ACCUM_WALLETS = _load_active_accum_wallets()
            rotation_source = None
            rotation_pool: list[dict] = []
            required_slots = len(ACCUM_WALLETS)

            if not ACCUM_WALLETS:
                print("[AutoUpdate] 활성 accumulation wallet 없음 — 심볼 교체 스킵")

            alpha_rotation_pool: list[dict] = []
            stealth_rotation_pool: list[dict] = []

            if ACCUM_WALLETS:
                rotation_state = load_rotation_state(ROTATION_STATE_PATH)
                rotation_state = update_rotation_state(
                    rotation_state,
                    alpha_symbols=[row["symbol"] for row in accumulation_candidates],
                    stealth_symbols=[row["symbol"] for row in stealth_watch],
                    bar_id=closed_bar_id,
                )
                save_rotation_state(ROTATION_STATE_PATH, rotation_state)

                alpha_rotation_pool = select_persistent_candidates(
                    accumulation_candidates,
                    streaks=rotation_state["alpha_streaks"],
                    min_streak=ROTATION_MIN_STREAK,
                    max_symbols=required_slots,
                )
                stealth_rotation_pool = select_persistent_candidates(
                    stealth_watch,
                    streaks=rotation_state["stealth_streaks"],
                    min_streak=ROTATION_MIN_STREAK,
                    max_symbols=required_slots,
                )

                if len(alpha_rotation_pool) >= required_slots:
                    rotation_pool = alpha_rotation_pool
                    rotation_source = "alpha"
                elif len(stealth_rotation_pool) >= required_slots:
                    rotation_pool = stealth_rotation_pool
                    rotation_source = "stealth"
                else:
                    print(
                        "[AutoUpdate] persistence gate — "
                        f"alpha={len(alpha_rotation_pool)}/{required_slots} "
                        f"stealth={len(stealth_rotation_pool)}/{required_slots} "
                        f"(min_streak={ROTATION_MIN_STREAK})"
                    )
            ...
            with watchlist_path.open("w") as f:
                json.dump({
                    ...
                    "alpha_rotation_candidates": alpha_rotation_pool,
                    "stealth_rotation_candidates": stealth_rotation_pool,
                    "rotation_candidates": rotation_pool,
                    "rotation_source": rotation_source,
                    "rotation_min_streak": ROTATION_MIN_STREAK,
                    "rotation_required_slots": required_slots,
                    "rotation_gate_passed": bool(rotation_pool),
                    "closed_bar_id": closed_bar_id,
                }, f, indent=2)
            print(
                "Watchlist saved: "
                f"alpha={[s['symbol'] for s in top_symbols]} "
                f"accumulation={[s['symbol'] for s in accumulation_candidates]} "
                f"rotation={rotation_source or 'blocked'}:"
                f"{[s['symbol'] for s in rotation_pool]}"
            )

            if rotation_pool and rotation_source and ACCUM_WALLETS:
                try:
                    sys.path.insert(0, str(_project_root / "scripts"))
                    from wallet_auto_updater import apply_symbol_rotation
                    assignments = [
                        (wallet_name, rotation_pool[i]["symbol"])
                        for i, wallet_name in enumerate(ACCUM_WALLETS)
                    ]
                    head_metric = (
