"""
vpin_multi 사이클 232 — Symbol-Adaptive Exit Scaling
- 기반: c226 OOS Sharpe +38.407 (eV=0.35 sV=0.35 xV=0.15 vpc=0 dR=0)
- 문제:
  1) TP/SL/Trail 배수가 모든 심볼 동일 → SOL(고변동)과 ETH(중변동) 특성 미반영
  2) ETH Sharpe(31.1) vs SOL(49.9) 격차 → ETH 출구 최적화 여지
  3) XRP 0거래 → 출구 완화만으로는 해결 불가, 진입 문턱 추가 탐색
- 가설:
  A) 심볼별 TP 스케일: ETH는 TP 축소(빠른 익절)→WR↑, SOL은 TP 유지/확대→avg↑
  B) 심볼별 SL 스케일: ETH는 SL 타이트→MDD↓, SOL은 SL 여유→조기손절 방지
  C) XRP 진입: VPIN 문턱 0.10-0.15 극저 + MOM 문턱 0.0003 완화 탐색
- 탐색 그리드:
  ETH_TP_SCALE: [0.7, 0.85, 1.0]    — ETH TP 배수 스케일
  SOL_TP_SCALE: [1.0, 1.15, 1.3]    — SOL TP 배수 스케일
  ETH_SL_SCALE: [0.8, 1.0]          — ETH SL 타이트닝
  SOL_SL_SCALE: [1.0, 1.2]          — SOL SL 여유
  XRP_MOM_TH: [0.0003, 0.0005, 0.0007] — XRP 모멘텀 문턱 완화
  = 3×3×2×2×3 = 108 combos
- 고정: c226 최적 진입 파라미터 전체
- 목표: avg OOS Sharpe >= 40 AND trades >= 24
- 3-fold WF + 슬리피지 스트레스
"""
...
# -- c232 탐색 그리드 --
ETH_TP_SCALE_LIST = [0.7, 0.85, 1.0]
SOL_TP_SCALE_LIST = [1.0, 1.15, 1.3]
ETH_SL_SCALE_LIST = [0.8, 1.0]
SOL_SL_SCALE_LIST = [1.0, 1.2]
XRP_MOM_TH_LIST = [0.0003, 0.0005, 0.0007]
...
def backtest(
    ...
    while i < n - 1:
        ...
        vpin_ok = (
            ...
            and mom_val >= sym_mom_th
            and RSI_FLOOR < rsi_val < RSI_CEILING_BASE
            and c[i] > ema_val
        )
        btc_ok = (
        ...
        # RSI 기반 동적 스케일링
        rsi_ratio = (RSI_CEILING_BASE - rsi_val) / (
            RSI_CEILING_BASE - RSI_FLOOR)
...
def get_sym_params(
    ...
    """Returns (vpin_th, mom_th, tp_scale, sl_scale) for symbol."""
    if symbol == "KRW-ETH":
        return (ETH_VPIN_TH, MOM_THRESH,
                combo["eth_tp_scale"], combo["eth_sl_scale"])
    elif symbol == "KRW-SOL":
        return (SOL_VPIN_TH, MOM_THRESH,
                combo["sol_tp_scale"], combo["sol_sl_scale"])
    elif symbol == "KRW-XRP":
        return (XRP_VPIN_TH, combo["xrp_mom_th"], 1.0, 1.0)
    return (VPIN_LOW_BASE, MOM_THRESH, 1.0, 1.0)


def build_combos() -> list[dict]:
    combos = []
    for ets, sts, esl, ssl, xmt in product(
        ETH_TP_SCALE_LIST, SOL_TP_SCALE_LIST,
        ETH_SL_SCALE_LIST, SOL_SL_SCALE_LIST,
        XRP_MOM_TH_LIST,
    ):
        combos.append({
            "eth_tp_scale": ets,
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi c232 — Symbol-Adaptive Exit Scaling ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 40 AND trades >= 24")
    print("가설: 심볼별 TP/SL 스케일→ETH WR↑/SOL avg↑ + XRP 모멘텀 완화→거래 확보")
    print(f"기준선: c226 OOS +38.407 / c199 OOS +51.425 (BEST)")
    print(f"c226 고정 진입: eV={ETH_VPIN_TH} sV={SOL_VPIN_TH} "
          f"xV={XRP_VPIN_TH} vpc=0 dR=0")
    ...
    print(f"탐색: ethTP={ETH_TP_SCALE_LIST} solTP={SOL_TP_SCALE_LIST}")
    print(f"      ethSL={ETH_SL_SCALE_LIST} solSL={SOL_SL_SCALE_LIST}")
    print(f"      xrpMOM={XRP_MOM_TH_LIST}")
    print("=" * 80)
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            vpin_th, mom_th, tp_sc, sl_sc = get_sym_params(sym, combo)
            r = backtest(
                df_tr, sym, vpin_th, mom_th, tp_sc, sl_sc,
    ...
    hdr = (f"{'eTP':>5} {'sTP':>5} {'eSL':>5} {'sSL':>5} {'xMOM':>7} | "
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['eth_tp_scale']:>5.2f} {r['sol_tp_scale']:>5.2f} "
            f"{r['eth_sl_scale']:>5.2f} {r['sol_sl_scale']:>5.2f} "
            f"{r['xrp_mom_th']:>7.4f} | "
    ...
    for r in valid:
        key = (r["eth_tp_scale"], r["sol_tp_scale"],
               r["eth_sl_scale"], r["sol_sl_scale"],
               r["xrp_mom_th"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                vpin_th, mom_th, tp_sc, sl_sc = get_sym_params(sym, params)
                r = backtest(
                    df_test, sym, vpin_th, mom_th, tp_sc, sl_sc,
        ...
        if oos_sharpes:
            ...
            print(
                f"  #{rank}: eTP={params['eth_tp_scale']:.2f} "
                f"sTP={params['sol_tp_scale']:.2f} "
                f"eSL={params['eth_sl_scale']:.2f} "
                f"sSL={params['sol_sl_scale']:.2f} "
                f"xMOM={params['xrp_mom_th']:.4f} | "
    ...
    print(f"파라미터: eTP={best['eth_tp_scale']:.2f} "
          f"sTP={best['sol_tp_scale']:.2f} "
          f"eSL={best['eth_sl_scale']:.2f} "
          f"sSL={best['sol_sl_scale']:.2f} "
          f"xMOM={best['xrp_mom_th']:.4f}")
        ...
        for fold in WF_FOLDS:
            for sym in sym_data_ok:
                ...
                esp, vol_mom, atr_pctile, mom_pctile = precompute_base(
                    df_test)
                bb_wp = precompute_bb(df_test)
                vpin_th, mom_th, tp_sc, sl_sc = get_sym_params(sym, best)
                r = backtest(
                    df_test, sym, vpin_th, mom_th, tp_sc, sl_sc,
                    btc_c, btc_s, esp, vol_mom, atr_pctile, bb_wp,
    ...
    print(f"  c232 최적: avg_OOS={best['avg_oos']:+.3f}")
    delta199 = best["avg_oos"] - c199_baseline
    delta226 = best["avg_oos"] - c226_baseline
    label199 = "개선" if delta199 > 0 else "악화"
    label226 = "개선" if delta226 > 0 else "악화"
    print(f"  Δ vs c199: {delta199:+.3f} ({label199})")
    print(f"  Δ vs c226: {delta226:+.3f} ({label226})")

    # -- 최종 요약 --
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: eTP={best['eth_tp_scale']:.2f} "
          f"sTP={best['sol_tp_scale']:.2f} "
          f"eSL={best['eth_sl_scale']:.2f} "
          f"sSL={best['sol_sl_scale']:.2f} "
          f"xMOM={best['xrp_mom_th']:.4f}")
