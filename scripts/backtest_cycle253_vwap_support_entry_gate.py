"""
사이클 253: c250 최적 고정 + VWAP 지지 진입 게이트 그리드
...
가설:
  - 현재 진입은 Donchian 브레이크아웃 + ADX + BTC SMA + 모멘텀 등 가격/추세 기반
  - 볼륨 가중 평균가(VWAP) 대비 가격 위치를 추가하면 허위 브레이크아웃 필터링 가능
  - 가격이 VWAP 위에서 브레이크아웃 → 볼륨 지지가 있는 진정한 돌파
  - VWAP 자체가 상승 중일 때만 진입 → 매수세 우위 확인
  - c244(vol_accel)과 다른 접근: 거래량 자체가 아닌 "거래량 가중 가격 수준" 활용
...
새 탐색 그리드 (c253):
  VWAP_LB:        [10, 20, 30]    — VWAP 계산 룩백 (봉 수)
  VWAP_MIN_DEV:   [0.0, 0.5, 1.0, 1.5]  — 가격이 VWAP 대비 최소 N% 위에 있어야 진입
  VWAP_SLOPE_LB:  [0, 3, 5]      — VWAP 상승 확인 룩백 (0=비활성)
  = 3 × 4 × 3 = 36 combos
...
# ─── c253 탐색 그리드 ─────────────────────────────────────────
VWAP_LB_LIST = [10, 20, 30]
VWAP_MIN_DEV_LIST = [0.0, 0.5, 1.0, 1.5]
VWAP_SLOPE_LB_LIST = [0, 3, 5]
...
def compute_vwap(
    ...
    for i in range(period - 1, n):
        ...
        if vol_sum > 0:
            result[i] = np.sum(tp_vol[i - period + 1:i + 1]) / vol_sum
    return result
...
def run_backtest(
    ...
    for i in range(warmup, n - 1):
        ...
        if position is not None:
            ...
            # ── c253 탐색: VWAP 지지 게이트 ────────────────────
            if not np.isnan(vwap_arr[i]) and vwap_arr[i] > 0:
                # 가격이 VWAP 대비 최소 편차 이상 위에 있어야 진입
                if vwap_min_dev > 0:
                    dev_pct = (c[i] / vwap_arr[i] - 1.0) * 100.0
                    if dev_pct < vwap_min_dev:
                        continue

                # VWAP 상승 확인
                if (vwap_slope_lb > 0 and i >= vwap_slope_lb
                        and not np.isnan(vwap_arr[i - vwap_slope_lb])):
                    if vwap_arr[i] <= vwap_arr[i - vwap_slope_lb]:
                        continue
...
def main() -> None:
    print("=" * 80)
    print("=== c253: c250최적 고정 + VWAP 지지 진입 게이트 그리드 ===")
    ...
    print(f"탐색: VWAP_LB × VWAP_MIN_DEV × VWAP_SLOPE_LB")
    print("=" * 80)
    ...
    for sym in SYMBOLS:
        ...
        # VWAP 사전 계산 (각 룩백)
        vwap_by_lb: dict[int, np.ndarray] = {}
        for lb in VWAP_LB_LIST:
            vwap_by_lb[lb] = compute_vwap(c_arr, h_arr, lo_arr, v_arr, lb)
        ...
        sym_precomp[sym] = {
            ...
            "index": df.index,
            "vwap_by_lb": vwap_by_lb,
        }

    # 그리드
    grid = list(product(
        VWAP_LB_LIST, VWAP_MIN_DEV_LIST, VWAP_SLOPE_LB_LIST,
    ))
    print(f"\n총 조합: {len(grid)}")

    # 베이스라인 (c250 최적 — VWAP 비활성)
    print("\n--- 베이스라인 (c250 최적, VWAP OFF) ---")
    base_fold_sharpes = []
    base_fold_details = []
    base_total_n = 0
    ...
    for window in WINDOWS:
        ...
        for sym in SYMBOLS:
            sp = sym_precomp[sym]
            # VWAP OFF: min_dev=0, slope_lb=0
            dummy_vwap = np.full(len(sp["c"]), np.nan)
            raw = run_backtest(
    ...
    for gi, combo in enumerate(grid):
        vwap_lb, vwap_dev, vwap_slb = combo
        ...
        for window in WINDOWS:
            ...
            for sym in SYMBOLS:
                sp = sym_precomp[sym]
                vwap_arr = sp["vwap_by_lb"][vwap_lb]
                raw = run_backtest(
                    sp["c"], sp["o"], sp["h"], sp["lo"], sp["v"],
                    sp["dc_up"], sp["dc_lo"],
                    sp["atr"], sp["adx"],
    ...
    for val in VWAP_LB_LIST:
        ...
        if sub:
            ...
            print(f"  vLB={val}: top5 avgSh={avg_sh:+.3f}  F2={avg_f2:+.3f}")

    print("\n=== VWAP Min Deviation 효과 ===")
    for val in VWAP_MIN_DEV_LIST:
        sub = [r for r in valid if r["params"][1] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  vDev={val:.1f}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")

    print("\n=== VWAP Slope Lookback 효과 ===")
    for val in VWAP_SLOPE_LB_LIST:
        sub = [r for r in valid if r["params"][2] == val]
        if sub:
            top5 = sorted(sub, key=lambda x: x["avg_sharpe"], reverse=True)[:5]
            avg_sh = np.mean([r["avg_sharpe"] for r in top5])
            avg_f2 = np.mean([r["f2_sharpe"] for r in top5])
            print(f"  vSlb={val}: top5 avgSh={avg_sh:+.3f}  "
                  f"F2={avg_f2:+.3f}")
    ...
    for r in passing[:10]:
        p = r["params"]
        print(f"  vLB={p[0]} vDev={p[1]:.1f} vSlb={p[2]} | "
    ...
    if valid:
        ...
        print(f"  c253 최적: avg_OOS={b['avg_sharpe']:+.3f} "
              f"F2={b['f2_sharpe']:+.3f} "
              f"F3={b['f3_sharpe']:+.3f} "
    ...
    if valid:
        ...
        print(f"★ OOS 최적: vLB={p[0]} vDev={p[1]:.1f} vSlb={p[2]}")
        print(f"  (c250 고정: boMgn={BREAKOUT_MARGIN} aPct={ATR_PCTILE_MIN} "
              f"vRat={VOL_RATIO_MIN} btcMM={BTC_MOM_MIN})")
