"""
사이클 131: stealth_3gate stealth_window(W) 최적화
- SMA=10, Gate4=OFF, TP=10%, SL=1.0%, MAX_HOLD=24 고정 (사이클 130 최적값)
- stealth_window W: [20, 24, 28, 32, 36, 40, 48] 탐색
  (현재 W=36은 SMA=20 시절에 최적화됨 → SMA=10 환경에서 재검증 필요)
- RS[0.5,1.0), acc>1.0 고정
...
# Fixed params (사이클 130 최적값)
SMA_N = 10       # 사이클 130에서 최적화됨
RS_LO = 0.5
RS_HI = 1.0
TP = 0.10
SL = 0.010
...
def compute_btc_signal(df4h: pd.DataFrame, dfday: pd.DataFrame, w: int) -> pd.Series:
    """Gate 1+2: BTC>SMA10(daily) + BTC stealth accumulation (4h, window=w)."""
...
def compute_alt_entry(
    df_alt: pd.DataFrame, df_btc4h: pd.DataFrame, btc_sig: pd.Series, w: int
) -> pd.Series:
    """Gate 3: alt RS∈[RS_LO,RS_HI) + alt acc>1 (window=w)."""
...
def main() -> None:
    t0 = time.time()
    print("=== stealth_3gate stealth_window(W) 최적화 (사이클 131) ===")
    print(f"SMA={SMA_N}(고정), RS=[{RS_LO},{RS_HI}), TP={TP*100:.0f}%, SL={SL*100:.1f}%, MAX_HOLD={MAX_HOLD}")
    ...
    for sym in symbols:
        try:
            ...
            if df is None or df.empty or len(df) < max(W_LIST) * 4:
                continue
            alt_raw[sym] = df
        except Exception:
            pass
    ...
    # Grid: W탐색
    print("\n[3/3] stealth_window W 탐색...")
    results = []

    for w in W_LIST:
        # Compute BTC signal for this W
        btc_sig = compute_btc_signal(df_btc4h, df_btcday, w)
        ...
        for sym, df in alt_raw.items():
            if len(df) < w * 4:
                continue
            entry = compute_alt_entry(df, df_btc4h, btc_sig, w)
            if entry.sum() < 5:
                continue
        ...
        flag = "✅" if passed else "  "
        baseline_mark = " 📌(현재)" if w == 36 else ""
        print(
            f"{flag} W={w:2d}{baseline_mark} | BTC활성={btc_active:4d} alt={len(alt_signals):3d} | "
    ...
    for r in sorted(results, key=lambda x: x.get("w2_sharpe", 0) if not np.isnan(x.get("w2_sharpe", float("nan"))) else 0, reverse=True):
        ...
        base_mark = " 📌" if r["W"] == 36 else ""
        print(
            f"{flag} W={r['W']:2d}{base_mark} | "
    ...
    out_path = _root / "scripts" / "cycle131_window_opt_results.csv"
    df_res.to_csv(out_path, index=False)
    print(f"\n결과 저장: {out_path}")

    # 결론
    baseline = next((r for r in results if r["W"] == 36), None)
    best = max(results, key=lambda x: x.get("w2_sharpe", 0) if not np.isnan(x.get("w2_sharpe", float("nan"))) else 0)
    if baseline:
        print(f"\n기준선 (W=36/SMA=10): W1={baseline['w1_sharpe']:.3f}(n={baseline['w1_n']}) W2={baseline['w2_sharpe']:.3f}(n={baseline['w2_n']})")
    print(f"최우수: W={best['W']} W2={best['w2_sharpe']:.3f}(n={best['w2_n']})")

    if baseline and best["w2_sharpe"] > baseline["w2_sharpe"] + 0.10:
        diff = best["w2_sharpe"] - baseline["w2_sharpe"]
        print(f"\n→ W={best['W']}이 현재(W=36)보다 W2 Sharpe +{diff:.3f} 우위")
        print(f"  daemon.toml stealth_window 변경 권장: 36 → {best['W']}")
    else:
        print(f"\n→ W=36이 SMA=10 환경에서도 최적 또는 동등 수준 — daemon 변경 불필요")
