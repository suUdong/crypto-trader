"""
vpin_multi 사이클 213 — Entry Quality Score → 적응형 TP/SL 스케일링
- 기반: c199 OOS Sharpe +51.425 (레짐 조건부 이중 출구) BEST
  c209 결과: squeeze soft gate + vol surge → OOS +37.473 (악화)
  → vol surge 무용(vsM=0), squeeze TP bonus(sqTP=0.5)만 소폭 도움
  ...
  BB 고정(c206): bbP=20 bbS=2.0 sqTh=20 sqLB=30 expB=2
- 문제:
  1) c199~c212 모든 오버레이가 c199 대비 악화 또는 미미
  2) 필터 추가 → 거래 수 감소 → Sharpe 불안정
  3) 진입은 binary pass/fail → 모든 진입이 동일 TP/SL 적용
- 가설:
  A) 진입 조건들을 점수화: VPIN 얼마나 낮은지, 모멘텀 얼마나 강한지,
     RSI velocity 크기, volume 크기, body ratio 크기, ATR regime 강도
  B) 높은 점수(고확신) → TP 보너스 확대 (fat tail 수익 극대화)
  C) 낮은 점수(경계선) → SL 타이트닝 (손실 제한)
  D) 최소 점수 임계값 → 너무 약한 시그널 필터링 (거래 수 vs 품질 균형)
- 점수 컴포넌트 (각 0.0~1.0):
  score_vpin: (VPIN_LOW - vpin) / VPIN_LOW  (VPIN이 낮을수록 높음)
  score_mom: min(1, (mom - MOM_THRESH) / MOM_THRESH)
  score_rsi_vel: min(1, (rsi_delta - RSI_DELTA_MIN) / RSI_DELTA_MIN)
  score_volume: min(1, (vol_ratio - VOL_MULT) / VOL_MULT)
  score_regime: regime_score (이미 0~1)
  score_body: min(1, (body - BODY_RATIO_MIN) / BODY_RATIO_MIN)
  total_score = sum of above (0~6)
- 탐색 그리드:
  SCORE_MIN: [1.5, 2.0, 2.5, 3.0, 3.5]  — 최소 진입 점수
  TP_SCORE_MULT: [0.0, 0.3, 0.5, 0.8]    — 점수당 TP ATR 보너스
  SL_SCORE_MULT: [0.0, 0.05, 0.10]        — 점수당 SL ATR 타이트닝
  TRAIL_SCORE_MULT: [0.0, 0.05, 0.10]     — 점수당 trail 타이트닝
  = 5×4×3×3 = 180 combos
...
# -- c213 탐색 그리드 --
SCORE_MIN_LIST = [1.5, 2.0, 2.5, 3.0, 3.5]
TP_SCORE_MULT_LIST = [0.0, 0.3, 0.5, 0.8]
SL_SCORE_MULT_LIST = [0.0, 0.05, 0.10]
TRAIL_SCORE_MULT_LIST = [0.0, 0.05, 0.10]
...
def backtest(
    ...
    while i < n - 1:
        ...
        # 기존 VPIN 진입 조건 (c165 고정) — binary gate
        vpin_ok = (
            vpin_val < VPIN_LOW
        ...
        # === c213: Entry Quality Score 계산 ===
        # 각 컴포넌트: 0.0~1.0 (임계값 넘은 정도를 정규화)
        score_vpin = min(1.0, max(0.0,
            (VPIN_LOW - vpin_val) / VPIN_LOW))
        score_mom = min(1.0, max(0.0,
            (mom_val - MOM_THRESH) / (MOM_THRESH + 1e-9)))
        score_rsi_vel = min(1.0, max(0.0,
            (rsi_delta - RSI_DELTA_MIN) / (RSI_DELTA_MIN + 1e-9)))
        score_volume = min(1.0, max(0.0,
            (vol_ratio - VOL_MULT) / (VOL_MULT + 1e-9)))

        # regime score (c199 고정 로직, 0~1)
        regime_score = 0.0
        ...
        score_body = min(1.0, max(0.0,
            (body_val - BODY_RATIO_MIN) / (BODY_RATIO_MIN + 1e-9)))

        # BB squeeze 보너스 (c209 검증: squeeze 시 TP 도움됨)
        squeeze_bonus = 0.0
        if not np.isnan(bb_width_pctile_arr[i]):
        ...
        total_score = (score_vpin + score_mom + score_rsi_vel
                       + score_volume + regime_score + score_body
                       + squeeze_bonus)
        ...
        # === c213: 점수 기반 TP/SL/Trail 조정 ===
        score_tp_extra = tp_score_mult * score_excess
        score_sl_tighten = sl_score_mult * score_excess
        score_trail_tighten = trail_score_mult * score_excess
        ...
        # 점수 높으면 trail 더 타이트 (수익 확보)
        base_trail_mult = max(0.05, base_trail_mult - score_trail_tighten)
...
def build_combos() -> list[dict]:
    combos = []
    for sc_min, tp_sm, sl_sm, tr_sm in product(
        SCORE_MIN_LIST, TP_SCORE_MULT_LIST,
        SL_SCORE_MULT_LIST, TRAIL_SCORE_MULT_LIST,
    ):
        # tp/sl/trail 모두 0이면 score_min만 필터 역할 → 중복 줄이기
        if tp_sm == 0 and sl_sm == 0 and tr_sm == 0 and sc_min != SCORE_MIN_LIST[0]:
            # score_min만 다르고 스케일링 0이면 c199와 동일(진입 필터만)
            # 다양한 score_min은 탐색 가치 있음 → 유지
            pass
        combos.append({
            "score_min": sc_min,
            "tp_score_mult": tp_sm,
            "sl_score_mult": sl_sm,
            "trail_score_mult": tr_sm,
        })
    return combos
...
def main() -> None:
    print("=" * 80)
    print("=== vpin_multi c213 — Entry Quality Score → 적응형 TP/SL ===")
    print(f"심볼: {', '.join(SYMBOLS)}  "
          f"목표: OOS Sharpe >= 52 AND trades >= 12")
    print("가설: 진입 조건 점수화 → 고확신=공격적TP, 저확신=타이트SL")
    ...
    print(f"점수: vpin+mom+rsi_vel+volume+regime+body+squeeze (0~6.5)")
    print("=" * 80)
    ...
    for idx, combo in enumerate(combos):
        ...
        for sym in sym_data_ok:
            ...
            r = backtest(
                df_tr,
                combo["score_min"], combo["tp_score_mult"],
                combo["sl_score_mult"], combo["trail_score_mult"],
    ...
    hdr = (f"{'scMin':>6} {'tpSM':>5} {'slSM':>5} {'trSM':>5} | "
    ...
    for r in valid[:16]:
        ...
        print(
            f"{r['score_min']:>6.1f} {r['tp_score_mult']:>5.1f} "
            f"{r['sl_score_mult']:>5.2f} {r['trail_score_mult']:>5.2f} | "
    ...
    for r in valid:
        key = (r["score_min"], r["tp_score_mult"],
               r["sl_score_mult"], r["trail_score_mult"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for rank, params in enumerate(unique_top, 1):
        ...
        for fold_i, fold in enumerate(WF_FOLDS):
            ...
            for sym in sym_data_ok:
                ...
                r = backtest(
                    df_test,
                    params["score_min"], params["tp_score_mult"],
                    params["sl_score_mult"], params["trail_score_mult"],
                    btc_c, btc_s, esp, vol_mom, atr_pctile, bb_wp)
        ...
        if oos_sharpes:
            ...
            print(
                f"  #{rank}: scMin={params['score_min']:.1f} "
                f"tpSM={params['tp_score_mult']:.1f} "
                f"slSM={params['sl_score_mult']:.2f} "
                f"trSM={params['trail_score_mult']:.2f} | "
    ...
    print(f"파라미터: scMin={best['score_min']:.1f} "
          f"tpSM={best['tp_score_mult']:.1f} "
          f"slSM={best['sl_score_mult']:.2f} "
          f"trSM={best['trail_score_mult']:.2f}")
    ...
    for slip in SLIPPAGE_LEVELS:
        ...
        for fold in WF_FOLDS:
            for sym in sym_data_ok:
                ...
                r = backtest(
                    df_test,
                    best["score_min"], best["tp_score_mult"],
                    best["sl_score_mult"], best["trail_score_mult"],
                    btc_c, btc_s, esp, vol_mom, atr_pctile, bb_wp,
                    slippage=slip)
                if r["trades"] > 0 and not np.isnan(r["sharpe"]):
    ...
    print(f"  c213 최적: avg_OOS={best['avg_oos']:+.3f}")
    ...
    print(f"\n{'=' * 80}")
    print("=== 최종 요약 ===")
    print(f"★ OOS 최적: scMin={best['score_min']:.1f} "
          f"tpSM={best['tp_score_mult']:.1f} "
          f"slSM={best['sl_score_mult']:.2f} "
          f"trSM={best['trail_score_mult']:.2f}")
    print(f"  (c199 고정: rTh={REGIME_TH} hiTP={HI_TP_BONUS} "
