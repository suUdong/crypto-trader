"""
사이클 118: SUI vpin 전략 탐색
- 목적: SUI stealth_3gate Sharpe ~4.3 한계 확인 → vpin_eth 프레임워크로 SUI 엣지 탐색
- 가설: ETH와 유사하게 SUI에서도 VPIN proxy + momentum + RSI 조합 유효 여부
- 기준선: vpin_eth daemon 파라미터 (vh=0.55, vm=0.0005, hold=18, TP=6%, SL=0.8%)
- Gate1 (BTC>SMA20): stealth_3gate에서 검증된 레짐 필터 포함 여부 테스트
- WF 창:
    W1: IS=2023-05~2024-06, OOS=2024-07~2025-06 (BULL 기간)
    W2: IS=2023-05~2025-03, OOS=2025-04~2026-04 (BULL+BEAR 혼재)
...
WINDOWS = [
    {
        "name": "W1",
        "is_start":  "2023-05-01", "is_end":  "2024-06-30",
        "oos_start": "2024-07-01", "oos_end": "2025-06-30",
    },
    {
        "name": "W2",
        "is_start":  "2023-05-01", "is_end":  "2025-03-31",
        "oos_start": "2025-04-01", "oos_end": "2026-04-04",
    },
]

# 1단계: 기준선 (vpin_eth daemon 파라미터 그대로 SUI에 적용)
BASELINE = dict(vh=0.55, vm=0.0005, hold=18, tp=0.06, sl=0.008)

# 2단계: 그리드 탐색
# SUI는 ETH보다 변동성 큼 → TP/SL 범위 확장
GRID = {
    "vh":   [0.50, 0.55, 0.60],
...
# Gate1: BTC > SMA20 여부 (True/False 두 케이스)
...
EMA_PERIOD   = 20
MOM_LOOKBACK = 8
BTC_SMA_PERIOD = 20  # Gate1 BTC SMA
...
def backtest(
    ...
    """vpin 전략 백테스트. Gate1(BTC>SMA20) 옵션."""
    c = df_sui["close"].values
    o = df_sui["open"].values
    n = len(c)
...
def run_window(w: dict, use_gate1: bool, params: dict) -> dict:
    df_sui_is  = load_historical(SYMBOL,  CTYPE, w["is_start"],  w["is_end"])
    df_sui_oos = load_historical(SYMBOL,  CTYPE, w["oos_start"], w["oos_end"])
    ...
    is_r  = backtest(df_sui_is,  df_btc_is,  **params, use_gate1=use_gate1)
    oos_r = backtest(df_sui_oos, df_btc_oos, **params, use_gate1=use_gate1)
...
def main() -> None:
    print("=" * 75)
    print("사이클 118: SUI vpin 전략 탐색")
    print(f"심볼: {SYMBOL} / 타임프레임: {CTYPE}")
    print(f"판정 기준: OOS Sharpe > {PASS_SHARPE}, WR > {PASS_WR:.0%}, trades >= {PASS_TRADES}")
    print(f"WF 창: W1 OOS=2024H2~2025H1, W2 OOS=2025H2~2026")
    print("=" * 75)

    # ── 1단계: 기준선 테스트 ───────────────────────────────────────────────────
    print("\n▶ 1단계: 기준선 테스트 (vpin_eth daemon 파라미터 SUI 적용)")
    ...
    print(f"  {'#':>4} {'Gate':>5} {'vh':>5} {'vm':>7} {'hold':>4} {'TP':>5} {'SL':>5} | "
    ...
    if two_pass:
        ...
        print(f"  ✅ SUI vpin 전략 유효! 2/2창 통과 {len(two_pass)}개")
        ...
        best1 = one_pass[0]
        print(f"  ◆ SUI vpin 1/2창 통과 — 추가 검증 필요")
        print(f"  최고 avg Sharpe: {best1['avg_sharpe']:+.3f}")
    else:
        print("  ❌ SUI vpin 기준 미달 — 전략 변경 필요")
