...
class TestWinStreakBoost(unittest.TestCase):
    def test_no_boost_under_3_wins(self) -> None:
        ...
        risk = RiskManager(
            RiskConfig(risk_per_trade_pct=0.01, stop_loss_pct=0.03, max_position_pct=1.0)
        )
    ...
    def test_boost_capped_at_1_2x(self) -> None:
        """Win-streak boost should not exceed 1.2x (tightened from 1.3x)."""
        risk = RiskManager(
            RiskConfig(risk_per_trade_pct=0.01, stop_loss_pct=0.03, max_position_pct=1.0)
        )
        base_size = risk.size_position(100_000.0, 50_000.0)
        ...
        self.assertLessEqual(ratio, 1.21)  # small float tolerance
        self.assertGreaterEqual(ratio, 1.19)
