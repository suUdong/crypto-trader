...
class ConsecutiveLossesTests(unittest.TestCase):
    def _ks(self, max_consecutive_losses: int = 3) -> KillSwitch:
        return KillSwitch(KillSwitchConfig(max_consecutive_losses=max_consecutive_losses))

    def test_two_consecutive_losses_does_not_trigger(self) -> None:
        ks = self._ks()
        for _ in range(2):
            _loss(ks)
        self.assertFalse(ks.is_triggered)
        self.assertEqual(ks.state.consecutive_losses, 2)

    def test_three_consecutive_losses_triggers(self) -> None:
        ks = self._ks()
        for _ in range(3):
            _loss(ks)
    ...
    def test_loss_win_then_three_losses_triggers(self) -> None:
        """loss -> win -> loss x 3 should trigger (counter resets on win)."""
        ks = self._ks()
        _loss(ks)
        _win(ks)
        self.assertEqual(ks.state.consecutive_losses, 0)
