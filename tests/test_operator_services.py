...
from crypto_trader.backtest.baseline import BacktestBaselineStore, build_backtest_fingerprint
...
from crypto_trader.models import BacktestBaseline, Candle, StrategyRunRecord
...
class OperatorServicesTests(unittest.TestCase):
    def test_generate_operator_artifacts_writes_drift_promotion_and_memo(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(Path(config.runtime.backtest_baseline_path).exists())
    ...
    def test_generate_operator_artifacts_reuses_matching_saved_baseline(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            base = Path(temp_dir)
            config = AppConfig(
                trading=TradingConfig(symbol="KRW-BTC", candle_count=25),
                ...
                telegram=TelegramConfig(),
                runtime=RuntimeConfig(
                    backtest_baseline_path=str(base / "baseline.json"),
                    drift_report_path=str(base / "drift.json"),
                    promotion_gate_path=str(base / "promotion.json"),
                    daily_memo_path=str(base / "memo.md"),
                    strategy_run_journal_path=str(base / "runs.jsonl"),
            ...
            class FailingMarketData(FakeMarketData):
                ...
                def get_ohlcv(self, symbol: str, interval: str, count: int) -> list[Candle]:
                    raise AssertionError(
                        "baseline should have been reused without market-data fetch"
                    )
            ...
            self.assertAlmostEqual(artifacts.backtest_baseline.total_return_pct, 0.12)
