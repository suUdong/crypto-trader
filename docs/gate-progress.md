...
- Runtime snapshot: `2026-03-26T19:11:25.316864+00:00` from `artifacts/runtime-checkpoint.json`
...
- Live portfolio evidence is still too thin for promotion. At this snapshot, total mark-to-market PnL is `-8.81 KRW (-0.0001%)`, realized PnL is `0.00 KRW`, and only `kimchi_premium` has an open position.
...
| `kimchi_premium` | 1 | 1,000,000.00 | 999,991.19 | -8.81 | -0.0009% | 0.00 | 0 | 1 | +0.47% | 0.22 | Only strategy currently carrying live exposure, so it is also the only visible drag on the portfolio. |
...
| **Portfolio** | **9** | **9,000,000.00** | **8,999,991.19** | **-8.81** | **-0.0001%** | **0.00** | **0** | **1** | — | — | Gate remains blocked by zero realized paper PnL. |
...
| --- | --- | --- | --- |
| Backtest return | `> 0%` | `+2.06%` from `artifacts/backtest-baseline.json` | PASS |
| Backtest max drawdown | `<= 20%` | `0.00%` from `artifacts/backtest-baseline.json` | PASS |
| Paper runs | `>= 5` | `20` runs from `artifacts/drift-report.json` | PASS |
| Drift status | not `out_of_sync` or `caution` | `on_track` from `artifacts/drift-report.json` | PASS |
| Latest verdict | not `pause_strategy` or `reduce_risk` | latest `strategy-runs.jsonl` record is `continue` at `2026-03-26T19:11:25.316273+00:00` | PASS |
| Paper realized PnL | `> 0%` | `0.00%` from `artifacts/drift-report.json`; runtime checkpoint also shows `0.00 KRW` realized PnL | FAIL |
